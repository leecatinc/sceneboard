...
export const GENERATED_PARENT_PATHS = [
...
const safeSegment = (value, name) => {
  if (typeof value !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/u.test(value)
    || value === '.' || value === '..') {
    throw new CertificationError('EVIDENCE_OUTPUT_OWNERSHIP_VIOLATION', `${name} is not a safe path segment`);
  }
  return value;
};
...
export const resolveOwnedChild = (parent, ...segments) => {
  ...
  const child = resolve(parent, ...segments.map((segment, index) => safeSegment(segment, `segment-${index}`)));
...
export const assertOwnedDirectory = async (path) => {
  ...
  if (await realpath(path) !== path) throw new CertificationError('EVIDENCE_OUTPUT_OWNERSHIP_VIOLATION');
...
export const ensureOwnedParent = async (workspaceRoot, relativePath) => {
  ...
  if (offset !== relativePath.split('/').join(sep)) throw new CertificationError('EVIDENCE_OUTPUT_OWNERSHIP_VIOLATION');
