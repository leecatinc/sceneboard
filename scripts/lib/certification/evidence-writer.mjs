...
const secretLikeKey = /(?:password|secret|token|credential|cookie|authorization|proof|otp|csrf|rawValue|bindValue|payloadBody)/iu;

const safeObject = (value, path = '') => {
  if (value === null || typeof value === 'boolean' || typeof value === 'number') return;
  if (typeof value === 'string') {
    if (/-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/u.test(value) || /\bsk-[A-Za-z0-9_-]{20,}\b/u.test(value)) {
...
const exclusiveJson = async (path, value, maxBytes = MAX_RECORD_BYTES) => {
  ...
  try { await writeFile(path, bytes, { flag: 'wx', mode: 0o600 }); }
    ...
    if (error?.code === 'EEXIST') throw new CertificationError('EVIDENCE_OUTPUT_OWNERSHIP_VIOLATION');
    throw error;
  }
  return sha256(bytes);
};
...
export class CertificationEvidenceWriter {
  ...
  #assertToken(ownerToken) {
    if (ownerToken !== this.ownerToken) throw new CertificationError('EVIDENCE_OUTPUT_OWNERSHIP_VIOLATION');
  }

  async writeRecord(ownerToken, row) {
    this.#assertToken(ownerToken);
    if (!/^[A-Z0-9][A-Z0-9._-]{0,127}$/u.test(row?.rowId ?? '')) throw new CertificationError('EVIDENCE_SCHEMA_INVALID');
    return exclusiveJson(resolveOwnedChild(resolve(this.attemptRoot, 'records'), `${row.rowId}.json`), row);
  }

  async writeAttachment(ownerToken, bytes, mediaType) {
    this.#assertToken(ownerToken);
    const value = Buffer.from(bytes);
    if (value.length > MAX_ATTACHMENT_BYTES) throw new CertificationError('EVIDENCE_ATTACHMENT_TOO_LARGE');
    if (/^(?:text\/|application\/(?:json|xml))/u.test(mediaType)) safeObject(value.toString('utf8'), 'attachment');
    const contentSha256 = sha256(value);
    const path = resolveOwnedChild(resolve(this.attemptRoot, 'attachments'), `${contentSha256}.bin`);
    try { await writeFile(path, value, { flag: 'wx', mode: 0o600 }); }
    catch (error) { if (error?.code !== 'EEXIST') throw error; }
    await exclusiveJson(resolveOwnedChild(resolve(this.attemptRoot, 'attachments'), `${contentSha256}.json`), {
      schemaVersion: 1, contentSha256, byteLength: value.length, mediaType,
    }).catch((error) => { if (error.code !== 'EVIDENCE_OUTPUT_OWNERSHIP_VIOLATION') throw error; });
  ...
  async finalizePhase(ownerToken, phaseId, index) {
    ...
    catch { throw new CertificationError('EVIDENCE_OUTPUT_OWNERSHIP_VIOLATION'); }
  ...
  async evidenceTreeSha256() {
    ...
    const visit = async (directory) => {
      for (const entry of await readdir(directory, { withFileTypes: true })) {
        ...
        if (entry.isSymbolicLink()) throw new CertificationError('EVIDENCE_OUTPUT_OWNERSHIP_VIOLATION');
        if (entry.isDirectory()) await visit(path);
        else if (entry.isFile() && entry.name !== 'release-index.json' && entry.name !== '.owner.json') {
          rows.push({ path: relative(this.attemptRoot, path).split('\\').join('/'), sha256: sha256(await readFile(path)) });
        }
      }
    };
  ...
  async finalizeRelease(ownerToken, index) {
    ...
    const releaseIndexSha256 = await exclusiveJson(resolve(this.attemptRoot, 'release-index.json'), release);
