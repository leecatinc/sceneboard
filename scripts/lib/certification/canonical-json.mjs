...
const CONTEXTUAL_CONTAINER_PATTERN = /(?:^|[{\s,])["']?(?:proof|generation)["']?\s*[:=]\s*([\[{])/gimu;
const SECRET_SHAPED_FRAGMENT_PATTERN = /(?:^|[\s:[{,=])["']?[A-Za-z0-9_-]{22,43}["']?(?=$|[\s,\]};])/mu;
...
export const containsSecretLikeMaterial = (text) => /-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/u.test(text)
  ...
  || /(?:^|[{\s,])["']?(?:proof|generation)["']?\s*[:=]\s*(?:["'][A-Za-z0-9_-]{22,43}["']|[A-Za-z0-9_-]{22,43})(?=$|[\s,;}])/imu.test(text)
  || containsNestedContextualSecret(text)
  || /(?:^|[\s"'(=])\/(?:[^/\s"'`]+\/)*leecat-board\/credentials\/[A-Za-z0-9_-]+(?:\/[^\s"'`]*)?/u.test(text)
  || /[A-Za-z]:\\[^\r\n"']*\\leecat-board\\credentials\\[A-Za-z0-9_-]+/u.test(text);

export const assertExactKeys = (value, expected, code = 'UNKNOWN_FIELDS') => {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) throw new CertificationError(code);
...
export const resolveInside = (root, path, code = 'PATH_ESCAPE') => {
  if (typeof path !== 'string' || path.length === 0 || isAbsolute(path) || normalize(path) !== path) {
    throw new CertificationError(code);
  }
  const absolute = resolve(root, path);
  const offset = relative(root, absolute);
  if (offset === '..' || offset.startsWith(`..${sep}`) || isAbsolute(offset)) throw new CertificationError(code);
  return absolute;
};
