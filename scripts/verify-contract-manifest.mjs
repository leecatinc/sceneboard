...
const installedSkillRoot = resolve(root, 'skills/sceanboard');
const installedSkillPathPrefix = 'skills/sceanboard/';
const utf8 = new TextDecoder('utf-8', { fatal: true });

const entryKeys = [
  'id', 'kind', 'resources', 'expectedCardinality', 'canonicalization',
  'materializationPredecessor', 'missingOrExtraResult',
];
const resourceKeys = [
  'resourceId', 'owner', 'path', 'exportName', 'exportKind', 'projectionId', 'selector',
];
const manifestKeys = [
  'schemaVersion', 'evidenceSchemaVersion', 'inventorySha256', 'manifestSchemaSha256',
  'nodeNpmPolicy', 'dependencyPolicy', 'topology', 'migrations', 'toolRegistry',
  'corpus', 'ownerPublishers', 'schemaModel', 'installedSkill', 'resources',
];
const resultKeys = [
  'key', 'owner', 'canonicalPath', 'exportName', 'exportKind', 'selectorSha256',
  'canonicalization', 'fingerprintSha256',
];
...
const terminalToolNames = [
  'board_connection_status', 'board_pair_request', 'board_pair_status',
  'board_list', 'board_get', 'board_create', 'board_archive', 'board_capabilities_get',
  'board_scene_get', 'board_scene_replace', 'board_scene_patch', 'board_scene_clear',
  'board_artifact_get', 'board_artifact_put', 'board_artifact_stop',
  'board_history_list', 'board_history_get', 'board_history_restore',
  'board_interaction_request', 'board_interaction_status', 'board_interaction_respond',
];

const fail = (code, message = code) => { throw new CertificationError(code, message); };
const equal = (left, right, code) => {
  if (canonicalJson(left) !== canonicalJson(right)) fail(code);
};
const lfText = (bytes) => {
  let value;
  try { value = utf8.decode(bytes); } catch { fail('CONTRACT_INPUT_DRIFT'); }
...
const astProjection = (text, resource) => {
  const source = ts.createSourceFile(resource.path, text, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
  if (source.parseDiagnostics.length > 0) fail('CONTRACT_SELECTOR_INVALID');
  const classMethod = /^ClassDeclaration\[name=([A-Za-z_$][\w$]*)\]\/MethodDeclaration\[name=([A-Za-z_$][\w$]*)\]$/u.exec(resource.selector);
  if (classMethod) {
    if (resource.exportKind !== 'class' || resource.exportName !== classMethod[1] || resource.projectionId === null) {
      fail('CONTRACT_SELECTOR_INVALID');
    }
    const classes = source.statements.filter((node) => ts.isClassDeclaration(node)
      && nameText(node) === classMethod[1] && isExported(node));
    if (classes.length !== 1) fail('CONTRACT_SELECTOR_INVALID');
    const methods = classes[0].members.filter((node) => ts.isMethodDeclaration(node) && nameText(node) === classMethod[2]);
    if (methods.length !== 1 || !methods[0].body) fail('CONTRACT_SELECTOR_INVALID');
    const method = methods[0];
    const signature = compactSpace(text.slice(method.getStart(source), method.body.getStart(source)));
    return {
  ...
  if (named) {
    if (resource.exportKind !== named[1] || resource.exportName !== named[2] || resource.projectionId !== null) {
      fail('CONTRACT_SELECTOR_INVALID');
    }
...
const observeResource = async (entry, resource) => {
  ...
  if (resource.selector === 'whole-file') {
    if (resource.exportName !== null || resource.exportKind !== null || resource.projectionId !== null) {
      fail('CONTRACT_SELECTOR_INVALID');
    }
    if (entry.canonicalization === 'canonical-json' && extname(resource.path) === '.json') {
      let value;
      try { value = JSON.parse(text); } catch { fail('CONTRACT_INPUT_DRIFT'); }
  ...
  if ((resource.path === 'skills/sceanboard/SKILL.md' || resource.path.startsWith(installedSkillPathPrefix))
    && containsSecretLikeMaterial(text)) {
    fail('INSTALLED_SKILL_SECRET_MATERIAL');
  }
  const selectorSha256 = sha256(resource.selector);
  const key = canonicalJson([
    canonicalPath, resource.exportName, resource.exportKind, selectorSha256, entry.canonicalization,
  ]);
  return {
...
const validateInventoryShape = (inventory) => {
  ...
  if (inventory.schemaVersion !== 1 || !Array.isArray(inventory.entries)) fail('CONTRACT_INVENTORY_SCHEMA_INVALID');
  ...
  for (const [index, entry] of inventory.entries.entries()) {
    ...
    if (typeof entry.id !== 'string' || entryIds.has(entry.id) || expectedGroups.get(entry.id) !== entry.expectedCardinality
      || !Array.isArray(entry.resources) || entry.resources.length !== entry.expectedCardinality
      || !['utf8-lf', 'canonical-json', 'typescript-ast'].includes(entry.canonicalization)
      || entry.missingOrExtraResult !== 'CONTRACT_INPUT_DRIFT') {
      fail('CONTRACT_INPUT_DRIFT');
    }
    if (entry.materializationPredecessor !== null
      && (typeof entry.materializationPredecessor !== 'string'
        || !inventory.entries.slice(0, index).some(({ id }) => id === entry.materializationPredecessor))) {
      fail('CONTRACT_PREDECESSOR_INVALID');
    }
    entryIds.add(entry.id);
    for (const resource of entry.resources) {
      assertExactKeys(resource, resourceKeys, 'CONTRACT_INVENTORY_SCHEMA_INVALID');
      if (typeof resource.resourceId !== 'string' || resourceIds.has(resource.resourceId)
        || !/^D[1-9]$/u.test(resource.owner) || typeof resource.path !== 'string'
...
const validateInstalledSkillClosure = async (inventory) => {
  ...
  const expected = entry.resources.map(({ path }) => path).sort((left, right) => left.localeCompare(right, 'en'));
  const actual = (await collectInstalledSkillPaths()).sort((left, right) => left.localeCompare(right, 'en'));
...
const validateNoOverlap = (observed) => {
  ...
  for (const siblings of byPath.values()) {
    ...
    const ranged = siblings.filter(({ range }) => range !== null).sort((a, b) => a.range[0] - b.range[0]);
...
const parsePublisher = async (path) => {
  ...
  assertExactKeys(value, [
    'schemaVersion', 'owner', 'resourcePath', 'publisherTestPath', 'contractIds', 'selectors', 'tupleListSha256',
  ], 'CONTRACT_OWNER_PUBLISHER_STALE');
  if (value.schemaVersion !== 1 || !Array.isArray(value.contractIds) || !Array.isArray(value.selectors)
...
const validatePublishers = async (observedById) => {
  const descriptors = [
    ['D2', 'leecat-board-nextjs/test/contracts/certification-handoffs/d2-board-api-tuples.v1.json'],
    ['D5', 'leecat-board-nextjs/test/contracts/certification-handoffs/d5-board-api-tuples.v1.json'],
    ['D7', 'leecat-board-nextjs/test/contracts/certification-handoffs/d7-board-api-tuples.v1.json'],
    ['D8', 'leecat-board-nextjs/test/contracts/certification-handoffs/d8-board-api-tuples.v1.json'],
    ['D3', 'leecat-board-nestjs/test/contracts/certification-handoffs/d3-application-seams.v1.json'],
  ];
  const publishers = [];
  ...
  for (const [owner, path] of descriptors) {
    ...
    const publishedInput = observedItems.find((item) => item.result.canonicalPath === path && item.selector === 'whole-file');
    const publishedTest = observedItems.find((item) => item.result.canonicalPath === publisher.value.publisherTestPath && item.selector === 'whole-file');
    if (!publishedInput || !publishedTest) fail('CONTRACT_OWNER_PUBLISHER_MISSING');
    if (publisher.value.owner !== owner || new Set(publisher.value.contractIds).size !== publisher.value.contractIds.length) {
      fail('CONTRACT_OWNER_PUBLISHER_STALE');
    }
    for (const selector of publisher.value.selectors) {
      const expectedKeys = owner === 'D3'
        ? ['projectionId', 'sourcePath', 'exportName', 'exportKind', 'memberName', 'memberKind', 'signature', 'selector', 'contractIds']
        : ['projectionId', 'exportName', 'exportKind', 'memberName', 'memberKind', 'signature', 'selector', 'contractId'];
      ...
      if (!observed || observed.result.owner !== owner || observed.result.canonicalPath !== sourcePath
        || observed.selector !== selector.selector || observed.signature !== selector.signature
        || observed.result.exportName !== selector.exportName || observed.result.exportKind !== selector.exportKind) {
        fail('CONTRACT_OWNER_PUBLISHER_STALE');
      }
    }
    publishers.push({
  ...
  const browserSource = lfText(await readFile(resolve(root, 'leecat-board-nextjs/lib/api/board-api.ts')));
  const browserAst = ts.createSourceFile('board-api.ts', browserSource, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
  const browserClass = browserAst.statements.find((node) => ts.isClassDeclaration(node) && nameText(node) === 'BoardApiClient');
  if (!browserClass) fail('CONTRACT_OWNER_PUBLISHER_STALE');
  const publicBrowserMethods = browserClass.members.filter((node) => ts.isMethodDeclaration(node)
    && !ts.isPrivateIdentifier(node.name)
    && !node.modifiers?.some((modifier) => [ts.SyntaxKind.PrivateKeyword, ts.SyntaxKind.ProtectedKeyword].includes(modifier.kind)))
    .map(nameText);
  equal([...publicBrowserMethods].sort(), [...browserNames].sort(), 'CONTRACT_OWNER_PUBLISHER_STALE');
  ...
  if (d3ByPath.size !== 9 || d3.selectors.length !== 17) fail('CONTRACT_OWNER_PUBLISHER_STALE');
  for (const [path, memberNames] of d3ByPath) {
    ...
    const declaration = ast.statements.find((node) => ts.isClassDeclaration(node) && nameText(node) === className);
    if (!declaration) fail('CONTRACT_OWNER_PUBLISHER_STALE');
    const publicMethods = declaration.members.filter((node) => ts.isMethodDeclaration(node)
      && !ts.isPrivateIdentifier(node.name)
      && !node.modifiers?.some((modifier) => [ts.SyntaxKind.PrivateKeyword, ts.SyntaxKind.ProtectedKeyword].includes(modifier.kind)))
      .map(nameText);
...
const observeToolRegistry = async () => {
  const source = lfText(await readFile(resolve(root, 'leecat-board-mcp/src/tools/register-tools.ts')));
  ...
  const registered = [...source.matchAll(/\badd\('([^']+)'/gu)].map((match) => match[1]);
  equal(core.slice(0, 3), terminalToolNames.slice(0, 3), 'TOOL_REGISTRY_DRIFT');
  equal(core, [...terminalToolNames.slice(0, 12), ...terminalToolNames.slice(15, 18)], 'TOOL_REGISTRY_DRIFT');
  equal(downstream, [...terminalToolNames.slice(12, 15), ...terminalToolNames.slice(18)], 'TOOL_REGISTRY_DRIFT');
...
const observeMigrations = async (observedById) => {
  const source = lfText(await readFile(resolve(root, 'leecat-board-nestjs/src/database/migrations/registry.ts')));
  ...
  if (versions.length !== 15 || upAssets.length !== 15 || downAssets.length !== 3) fail('MIGRATION_REGISTRY_DRIFT');
  const assets = [...observedById.values()].filter(({ resourceId }) => resourceId.startsWith('MIGRATION-ASSET-'));
  ...
  return {
    ...
    assetSetSha256: canonicalJsonSha256(assets.map(({ result }) => ({ path: result.canonicalPath, sha256: result.fingerprintSha256 }))),
  };
};
...
const observeTopology = async () => {
  const frontend = parseEnv(lfText(await readFile(resolve(root, 'leecat-board-nextjs/.env.example'))));
  const backend = parseEnv(lfText(await readFile(resolve(root, 'leecat-board-nestjs/.env.example'))));
  const runtime = parseEnv(lfText(await readFile(resolve(root, 'packages/artifact-runtime/.env.example'))));
  const topology = {
  ...
  equal([
    frontend.NEXT_PUBLIC_BOARD_API_URL, frontend.NEXT_PUBLIC_ARTIFACT_RUNTIME_ORIGIN,
    runtime.ARTIFACT_RUNTIME_APP_ORIGIN, runtime.ARTIFACT_RUNTIME_API_ORIGIN,
    runtime.ARTIFACT_RUNTIME_ORIGIN, runtime.ARTIFACT_RUNTIME_LISTEN_HOST,
  ], [
    'http://127.0.0.1:3411', 'http://127.0.0.2:3412', 'http://127.0.0.1:3410',
    'http://127.0.0.1:3411', 'http://127.0.0.2:3412', '127.0.0.2',
  ], 'RUNTIME_TOPOLOGY_DRIFT');
  equal([topology.appOrigin, topology.apiOrigin, topology.artifactRuntimeOrigin], [
    'http://127.0.0.1:3410', 'http://127.0.0.1:3411', 'http://127.0.0.2:3412',
  ], 'RUNTIME_TOPOLOGY_DRIFT');
  if (existsSync(resolve(root, 'test/certification/service-topology.v1.json'))) fail('SECOND_TOPOLOGY_AUTHORITY');
  return topology;
};

const aggregateForEntry = (observed, entryId) => canonicalJsonSha256(observed
  .filter((item) => item.entryId === entryId)
  .map(({ result }) => ({ key: result.key, owner: result.owner, sha256: result.fingerprintSha256 })));
...
export const observeContractInventory = async ({ inventoryValue, inventoryBytes } = {}) => {
  ...
  const dependencyInventory = await readJson(resolve(root, 'test/certification/dependency-inventory.v1.json'));
  const lockfileBytes = await readFile(resolve(root, 'package-lock.json'));
  const schemaProjectionIds = ['SCHEMA-PROJECTION-02', 'SCHEMA-PROJECTION-03', 'SCHEMA-PROJECTION-04', 'SCHEMA-PROJECTION-05'];
  const schemaModel = {
    contractSha256: observedById.get('SCHEMA-PROJECTION-01')?.result.fingerprintSha256,
    aggregateTestSha256: observedById.get('SCHEMA-PROJECTION-AGGREGATE-TEST')?.result.fingerprintSha256,
  ...
  const resources = observed.map(({ result }) => result).sort((left, right) => left.key.localeCompare(right.key, 'en'));
  return {
    schemaVersion: 1,
...
export const validateManifestShape = (manifest) => {
  if (manifest !== null && typeof manifest === 'object' && !Array.isArray(manifest)) {
    ...
    if ('certificationSourceCommit' in manifest || 'attemptId' in manifest || 'laneId' in manifest
      || 'observedInputHashes' in manifest || 'observedEvidenceHashes' in manifest) {
      fail('CONTRACT_RUNTIME_FIELD_MISPLACED');
    }
  }
  assertExactKeys(manifest, manifestKeys, 'CONTRACT_MANIFEST_SCHEMA_INVALID');
  if (manifest.schemaVersion !== 1 || manifest.evidenceSchemaVersion !== 1
    || !Array.isArray(manifest.resources) || !Array.isArray(manifest.ownerPublishers)) {
    fail('CONTRACT_MANIFEST_SCHEMA_INVALID');
  }
  for (const result of manifest.resources) assertExactKeys(result, resultKeys, 'CONTRACT_MANIFEST_SCHEMA_INVALID');
};

export const verifyContractManifest = async ({ manifestValue, manifestBytes, inventoryValue, inventoryBytes } = {}) => {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  try {
    if (process.argv.slice(2).some((argument) => argument !== '--observe')) fail('CONTRACT_MANIFEST_ARGUMENT_INVALID');
