...
const workspaceManifestPaths = [
  'package.json',
  'leecat-board-mcp/package.json',
  'leecat-board-nestjs/package.json',
  'leecat-board-nextjs/package.json',
...
const parseProfile = (argumentsList) => {
  const value = argumentsList.find((argument) => argument.startsWith('--profile='))?.slice('--profile='.length) ?? 'static';
  if (value !== 'static' && value !== 'clean-tree') throw new CertificationError('DEPENDENCY_INSTALL_FAILED');
  return value;
};

const licenseOf = (name, value) => {
  if (typeof value.license === 'string') return [value.license, 'lockfile'];
  if (Array.isArray(value.licenses) && value.licenses.every((entry) => typeof entry?.type === 'string')) {
...
export const verifyDependencies = async ({
  profile = 'static', lockValue, lockBytes: providedLockBytes, inventoryValue, inventoryBytes, policyValue,
} = {}) => {
  ...
  const [lockSource, inventorySource, policySource] = await Promise.all([
    lockValue ? null : readJson(lockfilePath), inventoryValue ? null : readJson(inventoryPath), policyValue ? null : readJson(policyPath),
  ]);
  ...
  const effectiveInventoryBytes = inventoryBytes ?? inventorySource?.bytes ?? Buffer.from(`${JSON.stringify(inventory)}\n`);
  assertExactKeys(inventory, [
    'schemaVersion', 'registryHost', 'lockfileVersion', 'lockfileSha256', 'packageManifestSha256', 'entries',
  ...
  if (lock.lockfileVersion !== 3 || inventory.lockfileVersion !== 3 || inventory.lockfileSha256 !== sha256(lockBytes)) {
  ...
  const npmVersion = execFileSync('npm', ['--version'], { cwd: root, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim();
  if (npmVersion !== '10.9.3' || Number(process.versions.node.split('.')[0]) < 22) {
    throw new CertificationError('NODE_NPM_VERSION_MISMATCH');
  }
  const lockWorkspacePaths = new Set(Object.keys(lock.packages)
    .filter((path) => path !== '' && !path.startsWith('node_modules/') && !path.includes('/node_modules/'))
  ...
  for (const record of inventory.packageManifestSha256) {
    ...
    if (!existsSync(resolve(root, record.path)) || sha256(readFileSync(resolve(root, record.path))) !== record.sha256) {
      throw new CertificationError('LOCKFILE_MANIFEST_MISMATCH');
    }
  }
  const external = Object.entries(lock.packages)
    .filter(([path, value]) => path.startsWith('node_modules/') && typeof value?.version === 'string')
    .sort(([left], [right]) => left.localeCompare(right, 'en'));
  if (external.length !== inventory.entries.length) throw new CertificationError('DEPENDENCY_TREE_MISMATCH');
  ...
  for (const [index, [path, value]] of external.entries()) {
    ...
    assertExactKeys(record, [
      'path', 'name', 'version', 'license', 'licenseSource', 'classification', 'resolved', 'integrity', 'optional',
    ], 'DEPENDENCY_TREE_MISMATCH');
    ...
    if (resolved.protocol !== 'https:' || resolved.hostname !== inventory.registryHost
      || resolved.username !== '' || resolved.password !== '' || value.resolved !== record.resolved) {
      throw new CertificationError(resolved.username !== '' || resolved.password !== ''
        ? 'CREDENTIAL_URL_DETECTED' : 'DEPENDENCY_REGISTRY_SOURCE_INVALID');
    }
    ...
    if (license === null || record.license !== license || record.licenseSource !== licenseSource
      || record.classification !== classificationOf(license) || !classifications.has(record.classification)) {
      throw new CertificationError('LICENSE_AMBIGUOUS');
    }
    if (record.optional !== (value.optional === true)) throw new CertificationError('DEPENDENCY_TREE_MISMATCH');
    if (!record.optional && !existsSync(resolve(root, record.path, 'package.json'))) {
      throw new CertificationError('DEPENDENCY_TREE_MISMATCH');
    }
  }
  if (profile === 'clean-tree') {
    throw new CertificationError('DEPENDENCY_INSTALL_FAILED', 'clean-tree acquisition requires an explicit isolated network gate');
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  try {
    process.stdout.write(`${JSON.stringify(await verifyDependencies({ profile: parseProfile(process.argv.slice(2)) }))}\n`);
  } catch (error) {
