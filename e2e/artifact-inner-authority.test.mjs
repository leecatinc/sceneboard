...
const makePackage = (javascript) => {
  const resources = [
    { path: 'index.html', mediaType: 'text/html', bytes: Buffer.from('<main style="width:1300.2px;height:700.1px">artifact</main>') },
...
const outerBuild = await build({
  entryPoints: [new URL('../packages/artifact-runtime/src/runner/outer.ts', import.meta.url).pathname],
  bundle: true, write: false, format: 'iife', platform: 'browser', target: ['es2022'], minify: true, legalComments: 'none',
  define: {
...
const outerDocument = '<!doctype html><html><body style="margin:0"><script src="/outer.js"></script></body></html>';
...
test('production outer and inner keep hostile authored code outside the private authority lane', { timeout: 20_000 }, async (context) => {
  const browser = await chromium.launch({ headless: true });
  context.after(() => browser.close());
  const browserContext = await browser.newContext();
  await browserContext.addInitScript(({ outerOrigin }) => {
    if (location.origin === outerOrigin) Object.defineProperty(Performance.prototype, 'now', { configurable: true, value: () => 1_000 });
  ...
  await browserContext.route('**/*', async (route) => {
    ...
    if (url.origin === OUTER_ORIGIN && url.pathname === '/runner') return route.fulfill({ contentType: 'text/html', body: outerDocument });
    if (url.origin === OUTER_ORIGIN && url.pathname === '/outer.js') return route.fulfill({ contentType: 'application/javascript', body: outerSource });
    return route.abort('blockedbyclient');
  });

  const page = await browserContext.newPage();
  await page.goto(`${APP_ORIGIN}/`, { waitUntil: 'load' });
  await page.waitForFunction(() => document.body.dataset.ready === 'true', undefined, { timeout: 5_000 }).catch(async (error) => {
    console.error('authority harness state', await page.evaluate(() => ({ state: window.__stackState, ready: document.body.dataset.ready })), page.frames().map((frame) => frame.url()));
    throw error;
  });
  ...
  assert.deepEqual(await innerFrame.evaluate(() => ({
  ...
  assert.ok(box);
  const viewport = await innerFrame.evaluate(() => ({ width: window.innerWidth, height: window.innerHeight }));
  ...
  await page.waitForFunction(() => window.__stackState.navigationValues.some((message) => message.type === 'artifact.navigation.pan.end'));
  ...
  const navigationAfterMiddlePan = await page.evaluate(() => window.__stackState.navigationValues.length);
  ...
  assert.equal(await page.evaluate(() => window.__stackState.navigationValues.length), navigationAfterMiddlePan);
  await innerFrame.evaluate(() => globalThis.__restoreNumericPrimitives());
  await page.mouse.move(box.x + viewport.width / 3, box.y + viewport.height / 3);
  await page.mouse.down({ button: 'middle' });
  await page.waitForFunction(() => window.__stackState.navigationValues.filter((message) => message.type === 'artifact.navigation.pan.start').length === 2);
  await page.evaluate(() => window.__setNavigation(false));
  await page.waitForFunction(() => window.__stackState.navigationValues.some((message) => message.type === 'artifact.navigation.pan.cancel'));
  await page.mouse.up({ button: 'middle' });
  assert.equal((await page.evaluate(() => window.__stackState.navigationValues.filter((message) => message.type === 'artifact.navigation.pan.cancel').length)), 1);
  ...
  assert.deepEqual(await innerFrame.evaluate(() => ({ ...globalThis.__authorityProbe, hostMessages: [...globalThis.__authorityProbe.hostMessages] })), {
    data: false,
    ports: false,
    keys: false,
    ownKeys: false,
    ...
    hostBinaries: [],
    publicCalls: 4,
    publicKeys: ['changeSelection', 'onHostMessage', 'requestCapability', 'requestResize', 'userAction'],
  ...
  await page.waitForFunction(() => window.__stackState.messageValues.some((message) => message.type === 'artifact.resize.request' && message.value.source === 'observer'));
  assert.deepEqual(await page.evaluate(() => window.__stackState.messageValues.filter((message) => message.type === 'artifact.resize.request' && message.value.source === 'observer')), [{
    type: 'artifact.resize.request', value: { width: 1_301, height: 701, source: 'observer' },
  }]);
  ...
  assert.deepEqual(await page.evaluate(() => window.__stackState.receivedBinaries), [[1, 3, 5, 7]]);
  assert.equal(await innerFrame.evaluate(() => globalThis.__authorityProbe.downloadDetached), true);
  ...
  assert.deepEqual(await innerFrame.evaluate(() => ({ messages: globalThis.__authorityProbe.hostMessages, binaries: globalThis.__authorityProbe.hostBinaries })), {
    messages: ['host.capability.result'], binaries: [[2, 4, 6, 8]],
  });
  assert.equal(await page.evaluate(() => window.__stackState.sentBinaryDetached), true);

  const acceptedBeforeFlood = await page.evaluate(() => window.__stackState.messages.filter((type) => type === 'artifact.selection.change').length);
  ...
  const terminal = await page.evaluate(() => ({ ...window.__stackState, messages: [...window.__stackState.messages] }));
  assert.equal(terminal.protocolErrors, 1);
  assert.deepEqual(terminal.protocolErrorCodes, ['rate']);
  const acceptedLargeSelections = await page.evaluate((baseline) => window.__stackState.messages.filter((type) => type === 'artifact.selection.change').length - baseline, acceptedBeforeFlood);
