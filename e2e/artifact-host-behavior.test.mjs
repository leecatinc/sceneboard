...
const runtimePublic = new URL('../packages/artifact-runtime/dist/public/', import.meta.url).pathname;
...
const uint16 = (value) => { const bytes = Buffer.alloc(2); bytes.writeUInt16BE(value); return bytes; };
const uint32 = (value) => { const bytes = Buffer.alloc(4); bytes.writeUInt32BE(value); return bytes; };
const makePackage = () => {
  const resources = [
    { path: 'index.html', mediaType: 'text/html', bytes: Buffer.from('<main id="production-artifact" style="width:1200px;height:675px">production artifact</main>') },
    { path: 'main.js', mediaType: 'application/javascript', bytes: Buffer.from("globalThis.__productionAuthoredScript = true; globalThis.__productionTrustedWheels = 0; window.addEventListener('wheel', (event) => { if (event.isTrusted) globalThis.__productionTrustedWheels += 1; }); window.SceneBoardArtifact.requestResize(1600, 900);") },
  ].map((resource) => ({ ...resource, sha256: sha256(resource.bytes), byteLength: resource.bytes.byteLength }));
  const manifest = {
    protocolVersion: 1,
    type: 'artifact.manifest',
    artifact,
    entryPath: 'index.html',
    resources: resources.map(({ path, mediaType, sha256: digest, byteLength }) => ({ path, mediaType, sha256: digest, byteLength })),
    requestedCapabilities: [],
  };
  const manifestBytes = Buffer.from(canonicalJson(manifest));
  const parts = [Buffer.from([76, 67, 65, 82, 84, 86, 49, 0]), uint32(manifestBytes.byteLength), manifestBytes, uint16(resources.length)];
...
execFileSync('npm', ['run', 'build:runtime', '--workspace', '@leecat-board/artifact-runtime'], { cwd: new URL('..', import.meta.url).pathname, stdio: 'pipe' });
...
const contentType = (path) => extname(path) === '.js' ? 'application/javascript' : extname(path) === '.json' ? 'application/json' : 'text/html';
const parseTransform = (value) => {
  const match = /translate3d\(([-\d.]+)px,\s*([-\d.]+)px,\s*0(?:px)?\)\s*scale\(([-\d.]+)\)/u.exec(value ?? '');
  assert.ok(match, `unexpected transform: ${String(value)}`);
  return { x: Number(match[1]), y: Number(match[2]), scale: Number(match[3]) };
};

test('production ArtifactHost owns runtime, registry, controls, transitions, and terminal cleanup', { timeout: 30_000 }, async (context) => {
  const browser = await chromium.launch({ headless: true });
  context.after(() => browser.close());
  const browserContext = await browser.newContext();
  await browserContext.route('**/*', async (route) => {
    const url = new URL(route.request().url());
    if (url.origin === APP_ORIGIN && url.pathname === '/') return route.fulfill({ contentType: 'text/html', body: document });
    if (url.origin === APP_ORIGIN && url.pathname === '/fixture.js') return route.fulfill({ contentType: 'application/javascript', body: fixtureSource });
    if (url.origin === APP_ORIGIN && url.pathname === '/bridge-fixture.js') return route.fulfill({ contentType: 'application/javascript', body: bridgeFixtureSource });
  ...
  await page.waitForFunction(() => document.querySelector('.artifact-host')?.classList.contains('artifact-active'), undefined, { timeout: 10_000 });
  ...
  await page.waitForFunction(() => window.__artifactHostHarness.snapshot().resizeEvents.some((event) => event.width === 1_600 && event.height === 900));
  await page.evaluate(() => { window.__initialArtifactFrame = document.querySelector('.artifact-runtime-frame'); });
  ...
  assert.ok(initial.resizeEvents.some((event) => event.width === 1_600 && event.height === 900 && event.source === 'explicit'));

  const box = await innerFrame.locator('body').boundingBox();
  assert.ok(box);
  await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2);
  await page.mouse.wheel(0, -120);
  await page.waitForFunction(() => window.__artifactHostHarness.snapshot().navigationEvents.length === 1);
  ...
  assert.deepEqual(zoomed.navigationEvents, [{ version: 0, intent: { type: 'artifact.navigation.wheel', xMillionth: 500_000, yMillionth: 500_000, deltaY: -120 } }]);
  ...
  await page.mouse.move(panBox.x + panBox.width / 2 + 40, panBox.y + panBox.height / 2 + 25, { steps: 1 });
  await page.mouse.up({ button: 'middle' });
  await page.waitForFunction(() => window.__artifactHostHarness.snapshot().navigationEvents.some((event) => event.intent.type === 'artifact.navigation.pan.end'), undefined, { timeout: 5_000 });
  ...
  await page.evaluate(() => {
    ...
    container.scrollLeft = 73; container.scrollTop = 41;
  ...
  assert.equal(await page.evaluate(() => document.querySelector('.artifact-runtime-frame') === window.__initialArtifactFrame), true);
  await page.mouse.wheel(0, -120);
  await page.waitForTimeout(75);
  assert.equal((await page.evaluate(() => window.__artifactHostHarness.snapshot())).navigationEvents.filter((event) => event.intent.type === 'artifact.navigation.wheel').length, 1);
  ...
  assert.equal(await page.evaluate(() => document.querySelector('.artifact-runtime-frame') === window.__initialArtifactFrame), true);
  await page.mouse.wheel(0, -120);
  await page.waitForFunction(() => window.__artifactHostHarness.snapshot().navigationEvents.filter((event) => event.intent.type === 'artifact.navigation.wheel').length === 2);
  assert.equal((await page.evaluate(() => window.__artifactHostHarness.snapshot())).navigationEvents.filter((event) => event.intent.type === 'artifact.navigation.wheel').at(-1).version, 1);
  const centeredTransform = actualAgain.transform;
  await page.evaluate(() => { window.__artifactHostHarness.mode('fit-height'); window.__artifactHostHarness.mode('actual'); });
  assert.equal((await page.evaluate(() => window.__artifactHostHarness.snapshot())).transform, centeredTransform);

  const beforeResize = await page.evaluate(() => { window.__holdFrames = true; return { ...window.__resourceProbe, held: window.__resourceProbe.held.size }; });
  const resizeCountBeforeLatest = await page.evaluate(() => window.__artifactHostHarness.snapshot().resizeEvents.length);
  await innerFrame.evaluate(() => window.SceneBoardArtifact.requestResize(1_600, 900));
  await page.waitForFunction((count) => window.__artifactHostHarness.snapshot().resizeEvents.length === count + 1, resizeCountBeforeLatest);
  assert.deepEqual((await page.evaluate(() => window.__artifactHostHarness.snapshot().resizeEvents.at(-1))), {
    version: 1, width: 1_600, height: 900, source: 'explicit',
  });
  await page.waitForFunction((count) => window.__resourceProbe.rafScheduled > count, beforeResize.rafScheduled);
  const queued = await page.evaluate(() => ({ ...window.__resourceProbe, held: window.__resourceProbe.held.size }));
  assert.equal(queued.held, 1);
  await page.evaluate(() => window.__artifactHostHarness.stop());
  await page.waitForFunction(() => document.querySelector('.artifact-host')?.classList.contains('artifact-stopped'));
  await page.waitForTimeout(75);
  const terminal = await page.evaluate(() => ({ snapshot: window.__artifactHostHarness.snapshot(), resources: { ...window.__resourceProbe, held: window.__resourceProbe.held.size } }));
  ...
  assert.ok(terminal.snapshot.resizeEvents.some((event) => event.width === 1_600 && event.height === 900));
  ...
  assert.ok(terminal.resources.events.indexOf('host.dispose') < terminal.resources.events.indexOf('port.close'));
  assert.ok(terminal.resources.events.indexOf('port.close') < terminal.resources.events.indexOf('frame.remove'));
...
test('active ArtifactHost unmount unregisters and disposes its runtime', { timeout: 30_000 }, async (context) => {
  const browser = await chromium.launch({ headless: true });
  context.after(() => browser.close());
  const browserContext = await browser.newContext();
  await browserContext.route('**/*', async (route) => {
    ...
    if (url.origin === RUNTIME_ORIGIN) {
      const relative = url.pathname === '/runner' ? 'runner.html' : url.pathname.replace(/^\//u, '');
      return route.fulfill({ contentType: contentType(relative), body: readFileSync(join(runtimePublic, relative)) });
    }
    return route.abort('blockedbyclient');
  });
  const page = await browserContext.newPage();
...
test('production bridge batches its activation seed and keeps later resize callback-only', { timeout: 30_000 }, async (context) => {
  const browser = await chromium.launch({ headless: true });
  context.after(() => browser.close());
  const browserContext = await browser.newContext();
  ...
  await browserContext.route('**/*', async (route) => {
    const url = new URL(route.request().url());
    if (url.origin === APP_ORIGIN && url.pathname === '/') return route.fulfill({ contentType: 'text/html', body: bridgeDocument });
    if (url.origin === APP_ORIGIN && url.pathname === '/bridge-fixture.js') return route.fulfill({ contentType: 'application/javascript', body: bridgeFixtureSource });
    if (url.origin === RUNTIME_ORIGIN) {
      const relative = url.pathname === '/runner' ? 'runner.html' : url.pathname.replace(/^\//u, '');
      return route.fulfill({ contentType: contentType(relative), body: readFileSync(join(runtimePublic, relative)) });
  ...
  const page = await browserContext.newPage();
  await page.goto(`${APP_ORIGIN}/`, { waitUntil: 'load' });
  await page.waitForFunction(() => window.__artifactBridgeHarness?.snapshot().phase === 'active', undefined, { timeout: 10_000 });
  ...
  assert.ok(active.timeline.some((entry) => entry.phase === 'active' && entry.contentSize?.width === 1_200 && entry.contentSize?.height === 675));
  assert.ok(active.resizeEvents.some((entry) => entry.phase !== 'active' && entry.value.source === 'explicit'));
  ...
  await page.waitForFunction((priorRenderCount) => window.__artifactBridgeHarness.snapshot().renderCount > priorRenderCount, renderCount, { timeout: 5_000 });
  ...
  await page.waitForFunction(() => window.__artifactBridgeHarness.snapshot().resizeEvents.some((entry) => entry.value.width === 1_700));
  const frameBox = await innerFrame.locator('body').boundingBox();
  assert.ok(frameBox);
  await innerFrame.locator('body').hover({ position: { x: 100, y: 100 } });
  await page.mouse.wheel(0, -120);
  await innerFrame.waitForFunction(() => globalThis.__productionTrustedWheels === 1, undefined, { timeout: 5_000 });
  await page.waitForFunction(() => window.__artifactBridgeHarness.snapshot().navigationEvents.length === 1, undefined, { timeout: 5_000 });
  ...
  assert.deepEqual(afterResize.navigationEvents.map((entry) => entry.version), [1]);
  assert.equal(await page.evaluate(() => window.__artifactBridgeHarness.frameStable()), true);
  ...
  assert.deepEqual((await page.evaluate(() => window.__artifactBridgeHarness.snapshot())).contentSize, null);
});
