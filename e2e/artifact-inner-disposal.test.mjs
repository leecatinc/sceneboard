...
const openHarness = async (context, documentBody) => {
  ...
  await browserContext.route('**/*', async (route) => {
    ...
    if (url.origin === OUTER_ORIGIN)
      return route.fulfill({ contentType: 'text/html', body: outerDocument });
    if (url.origin === INNER_ORIGIN && url.pathname === '/inner')
      return route.fulfill({ contentType: 'text/html', body: documentBody });
    if (url.origin === INNER_ORIGIN && url.pathname === '/inner.js')
      return route.fulfill({ contentType: 'application/javascript', body: innerSource });
...
test(
  'inner disposal wins a deferred Mermaid continuation',
  { timeout: 15_000 },
  async (context) => {
...
test(
  'inner disposal wins a deferred authored script load',
  { timeout: 15_000 },
  async (context) => {
    const harness = await openHarness(
      context,
...
test(
  'inner disposal cancels and guards the first deferred measurement frame',
  { timeout: 15_000 },
  async (context) => {
    const harness = await openHarness(
      context,
      innerDocument({
        resources: { css: null, javascript: null, diagram: false },
        prelude: manualFramePrelude,
      }),
    );
    await harness.page.waitForFunction(() => window.__messages.includes('artifact.ready'));
    assert.deepEqual(
      await harness.inner.evaluate(() => window.__frameHarness.queued.map((entry) => entry.id)),
      [1],
    );
    await harness.outer.evaluate(() => window.__disposeInner());
    await harness.page.waitForFunction(() => window.__messages.includes('peer.disposed'));
    ...
    assert.deepEqual(await harness.page.evaluate(() => window.__messages), [
      'artifact.ready',
      'peer.disposed',
    ]);
  },
...
test(
  'inner disposal cancels and guards the second deferred measurement frame',
  { timeout: 15_000 },
  async (context) => {
    const harness = await openHarness(
      context,
