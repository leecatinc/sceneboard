...
const packageBytes = Uint8Array.from(atob(window.__artifactFixture.packageBase64), (value) =>
  value.charCodeAt(0),
);
...
function Fixture() {
  ...
  const bridge = useArtifactBridgeV1({
    ...
    load,
    viewMode,
    onResizeRequest: (value) =>
      resizeEvents.push({ version: callbackVersion, phase: bridge.phase, renderCount, value }),
...
window.__artifactBridgeHarness = Object.freeze({
  ...
  callbackVersion(version) {
    if (updateCallbackVersion === null)
      throw new TypeError('artifact callback updater is unavailable');
  ...
  rememberFrame() {
    rememberedFrame = document.querySelector('iframe');
  },
  frameStable() {
    return rememberedFrame !== null && rememberedFrame === document.querySelector('iframe');
  },
});
