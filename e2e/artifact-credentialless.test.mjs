...
const runnerHeaders = {
  ...
  'Permissions-Policy': 'accelerometer=(), autoplay=(), camera=(), clipboard-read=(), clipboard-write=(), display-capture=(), fullscreen=(), geolocation=(), gyroscope=(), microphone=(), payment=(), publickey-credentials-get=(), storage-access=(), usb=(), web-share=()',
};
...
test('dedicated local runtime cookie host sends no credentials and remains credentialless and opaque', { timeout: 15_000 }, async (context) => {
  const browser = await chromium.launch({ headless: true });
  context.after(() => browser.close());
  const browserContext = await browser.newContext();
  const runtimeRequests = [];
  await browserContext.addCookies([
    { name: 'lcb_session', value: 'session-canary', url: APP_ORIGIN, httpOnly: true, sameSite: 'Lax' },
  ...
  assert.deepEqual((await browserContext.cookies(APP_ORIGIN)).map(({ name, value }) => ({ name, value })).sort((left, right) => left.name.localeCompare(right.name)), [
