...
if (
  process.env.CONFIRM_ISOLATED_PAIRING_CALIBRATION !==
  'I_CONFIRM_THIS_IS_AN_ISOLATED_DISPOSABLE_SCHEMA'
) {
  throw new TypeError(
    'pairing calibration requires explicit isolated disposable schema confirmation',
  );
}
const modulePath = process.env.PAIRING_CALIBRATION_HARNESS_MODULE;
if (modulePath === undefined || modulePath.length === 0)
  throw new TypeError('PAIRING_CALIBRATION_HARNESS_MODULE is required');
const budgetEncoded = process.env.PAIRING_CLAIM_CAPACITY_BUDGET_JSON;
if (budgetEncoded === undefined)
  throw new TypeError('PAIRING_CLAIM_CAPACITY_BUDGET_JSON is required');
...
if (typeof imported.createPairingClaimCalibrationHarness !== 'function') {
  throw new TypeError(
    'pairing calibration harness module has no createPairingClaimCalibrationHarness export',
  );
}
