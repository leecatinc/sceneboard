...
import type { EmailVerificationConfirmation, EmailVerificationRequest } from './auth.dto.js';
...
export class EmailVerificationService {
  ...
  async confirm(
    input: EmailVerificationConfirmation,
    now: number,
  ...
  assertTicket(emailNormalized: string, ticket: string, now: number): void {
    ...
    if (!this.crypto.constantTimeEqual(provided, expected))
      throw new AppError('AUTH_EMAIL_VERIFICATION_REQUIRED');
  }
  ...
  private codeHash(emailNormalized: string, code: string): string {
    return encodeBase64Url(
      this.crypto.hmac('email-verification-code/v1', `${emailNormalized}\u0000${code}`),
    );
  }
  ...
  private ticketSignature(emailNormalized: string, expiresAt: number, nonce: string): Buffer {
    return this.crypto.hmac(
      ...
      `${emailNormalized}\u0000${expiresAt}\u0000${nonce}`,
    );
  }
  ...
  private async cancelFailedDelivery(
    ...
    try {
      await this.redis.evaluate(
        CANCEL_VERIFICATION_LUA,
        [keys.verification, keys.cooldown],
        [codeHash],
      );
    } catch {
      // Preserve the original Gmail delivery failure without exposing cleanup details.
    }
  }
}
...
const parseIntegerPair = (value: unknown): readonly [number, number] => {
  if (!Array.isArray(value) || value.length !== 2)
    throw new TypeError('Redis verification result is invalid');
