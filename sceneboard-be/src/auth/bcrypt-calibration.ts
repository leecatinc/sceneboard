...
export const summarizeLatency = (samples: readonly number[]): LatencySummary => {
  ...
  const percentile = (ratio: number): number =>
    values[Math.max(0, Math.ceil(values.length * ratio) - 1)]!;
  const median =
    values.length % 2 === 0
...
): Promise<{ value: Value; durationMs: number }> => {
...
const measureCapacity = async (
  ...
  await runBounded(samplesPerKind, concurrency, async () => {
    if (!(await adapter.compare(CALIBRATION_PASSWORD, referenceHash)))
      throw new Error('bcrypt calibration compare failed');
  });
  ...
  return {
    ...
    compareThroughputPerSecond: rounded(
      samplesPerKind / Math.max(compareDurationMs / 1_000, 0.000_001),
    ),
...
const validateBudget = (budget: BcryptCapacityBudget): void => {
  if (!DEFAULT_CONCURRENCY.includes(budget.maxConcurrency))
    throw new TypeError('capacity maxConcurrency must be 1, 4, or 8');
  for (const [key, value] of Object.entries(budget)) {
    if (key === 'maxConcurrency') continue;
    if (!Number.isFinite(value) || value <= 0)
      throw new TypeError(`capacity budget ${key} must be positive`);
  }
};
...
const capacityPasses = (
  ...
  const measurement = measurements.find(
    (candidate) => candidate.concurrency === budget.maxConcurrency,
  );
  if (measurement === undefined) return false;
  return (
    measurement.hashThroughputPerSecond >= budget.minHashThroughputPerSecond &&
...
export const calibrateBcrypt = async (
  ...
  if (
    costs.length === 0 ||
    costs.some((cost) => !Number.isInteger(cost) || cost < 10 || cost > 14)
  ) {
    throw new TypeError('bcrypt calibration costs must be integers from 10 through 14');
  }
  if (!Number.isInteger(warmups) || warmups < 3)
    throw new TypeError('bcrypt calibration requires at least three warmups per cost');
  if (!Number.isInteger(samples) || samples < 20)
    throw new TypeError('bcrypt calibration requires at least twenty samples per kind');
  ...
  for (const cost of costs) {
    ...
    for (let index = 0; index < warmups; index += 1) {
      ...
      if (!(await adapter.compare(CALIBRATION_PASSWORD, referenceHash)))
        throw new Error('bcrypt calibration warmup failed');
    }
    ...
    const latencyGatePassed =
      hash.medianMs >= latencyGate.medianMinMs &&
    ...
    results.push({
      ...
      capacity,
      capacityGatePassed:
        budget === null || !latencyGatePassed ? null : capacityPasses(capacity, budget),
    });
  }
  const selected = results.find(
    (result) => result.latencyGatePassed && result.capacityGatePassed === true,
  );
  return {
    schemaVersion: 1,
