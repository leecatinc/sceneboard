...
export class SessionsRepository implements SessionWriterPort, SessionPersistence {
  ...
  async insert(
    ...
    input: Parameters<SessionWriterPort['insert']>[1],
  ): Promise<void> {
      ...
      await this.mysql.withConnection((connection) =>
        ...
        `,
          ...
          const [pairingRows] = await connection.execute<
            Array<RowDataPacket & { id: string; grantId: string }>
          >(
            `
        ...
        `,
          ...
          const revokeReason =
            reason === 'logout' ? 1 : reason === 'reuse' ? 3 : reason === 'disabled' ? 4 : 5;
          await connection.execute(
            `
        ...
        `,
          ...
          `,
            ...
            if (isDuplicateKey(error))
              throw new SessionCollisionError('session replacement collided');
            throw error;
          }
          await this.audit.writeMandatory(
            { connection },
...
const mysqlTimestampToIso = (value: string): string =>
  new Date(mysqlTimestampToMillis(value)).toISOString();
