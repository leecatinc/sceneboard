...
export class CsrfService {
  ...
  private verifySignatureOnly(
    token: string,
    now: number,
  ): { kind: 'a' | 's'; binding: string } | null {
    ...
    try {
      if (
        kind === 'a'
          ? binding !== '-'
          : decodeBase64UrlStrict(binding, { exactBytes: 16 }).byteLength !== 16
      )
        return null;
  ...
  authGeneration(
    ...
    if (csrfToken === null)
      throw new TypeError('csrf token is required for a live auth generation');
  ...
  private familyBinding(familyPublicId: string): string {
    return encodeBase64Url(
      this.crypto.hmac('csrf-family-binding/v1', familyPublicId).subarray(0, 16),
    );
  }

  private issue(kind: 'a' | 's', binding: string, expiresAt: number): IssuedCsrfToken {
    if (!Number.isSafeInteger(expiresAt) || String(expiresAt).length !== 13)
      throw new TypeError('CSRF expiry must be a 13-digit epoch millisecond value');
