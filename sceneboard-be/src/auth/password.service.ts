...
export class PasswordService {
  constructor(
    ...
    if (!Number.isInteger(cost) || cost < 10 || cost > 14)
      throw new TypeError('bcrypt cost must be between 10 and 14');
    if (bcrypt.getRounds(BCRYPT_DUMMY_HASH) !== cost)
      throw new TypeError('bcrypt dummy hash rounds do not match configured cost');
  }
  ...
  validate(password: string): void {
    ...
    if (
      ...
      bytes > 72 ||
      password.includes('\0') ||
      LONE_SURROGATE_PATTERN.test(password)
    ) {
  ...
  async padFailure(startedAt: number, signal?: AbortSignal): Promise<void> {
    ...
    if (remaining > 0)
      await sleep(remaining, undefined, signal === undefined ? undefined : { signal });
  }
}
