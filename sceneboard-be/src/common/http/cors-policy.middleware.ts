...
const ROUTES: readonly RouteProfileV1[] = [
  ...
  {
    pattern: /^\/api\/v1\/pairings\/[^/]+$/u,
    methods: ['GET', 'DELETE'],
    headers: DEFAULT_HEADERS,
  },
  {
    pattern: /^\/api\/v1\/pairings\/[^/]+\/(?:decision|redeem)$/u,
    methods: ['POST'],
    headers: DEFAULT_HEADERS,
  },
  {
    pattern: /^\/api\/v1\/pairings\/[^/]+\/client-status$/u,
    methods: ['GET'],
    headers: DEFAULT_HEADERS,
  },
  ...
  {
    pattern: /^\/api\/v1\/boards\/[^/]+\/(?:capabilities|revisions)$/u,
    methods: ['GET'],
    headers: DEFAULT_HEADERS,
  },
  {
  ...
  {
    pattern: /^\/api\/v1\/boards\/[^/]+\/revisions\/[^/]+$/u,
    methods: ['GET'],
    headers: DEFAULT_HEADERS,
  },
  {
    pattern: /^\/api\/v1\/boards\/[^/]+\/revisions\/[^/]+\/restore$/u,
    methods: ['POST'],
    headers: DEFAULT_HEADERS,
  },
  {
    pattern: /^\/api\/v1\/boards\/[^/]+\/artifacts$/u,
    methods: ['POST'],
    headers: DEFAULT_HEADERS,
  },
  {
    pattern: /^\/api\/v1\/boards\/[^/]+\/artifacts\/[^/]+\/versions\/[^/]+(?:\/package)?$/u,
    methods: ['GET'],
    headers: DEFAULT_HEADERS,
  },
  {
    pattern:
      /^\/api\/v1\/boards\/[^/]+\/artifacts\/[^/]+\/versions\/[^/]+\/capability-requests\/network-fetch$/u,
    methods: ['POST'],
    headers: DEFAULT_HEADERS,
  },
  {
    pattern: /^\/api\/v1\/boards\/[^/]+\/interactions\/[^/]+$/u,
    methods: ['GET'],
    headers: DEFAULT_HEADERS,
  },
  {
    pattern: /^\/api\/v1\/boards\/[^/]+\/interactions\/[^/]+\/(?:cancel|supersede)$/u,
    methods: ['POST'],
    headers: DEFAULT_HEADERS,
  },
];
...
export const evaluateCorsPreflightV1 = (
  ...
  if (
    ...
    !profile.methods.includes(requestedMethod)
  ) {
...
export class CorsPolicyMiddleware implements NestMiddleware {
  ...
  use(request: CorsRequestV1, response: CorsResponseV1, next: () => void): void {
    ...
    const decision =
      origin === this.environment.browserOrigin
    ...
    if (decision.headers.length > 0)
      response.setHeader('Access-Control-Allow-Headers', decision.headers.join(', '));
