...
export const SECURITY_RETENTION_TARGETS: readonly RetentionTarget[] = [
  {
    name: 'pairing-created-expiry',
    indexName: 'ix_pairing_state_code_expiry',
    oldestColumn: 'code_expires_at',
    batchLimit: 500,
    selectSql:
      'SELECT CAST(id AS CHAR) AS id FROM pairing_requests FORCE INDEX (ix_pairing_state_code_expiry) WHERE state = 1 AND code_expires_at <= ? ORDER BY code_expires_at, id LIMIT ?',
    cutoff: atNow,
    mutation: 'pairing-expire',
  },
  {
    name: 'pairing-pending-expiry',
    indexName: 'ix_pairing_state_decision_expiry',
    oldestColumn: 'decision_expires_at',
    batchLimit: 500,
    selectSql:
      'SELECT CAST(id AS CHAR) AS id FROM pairing_requests FORCE INDEX (ix_pairing_state_decision_expiry) WHERE state = 2 AND decision_expires_at <= ? ORDER BY decision_expires_at, id LIMIT ?',
    cutoff: atNow,
    mutation: 'pairing-expire',
  },
  {
    name: 'pairing-approved-expiry',
    indexName: 'ix_pairing_state_redeem_expiry',
    oldestColumn: 'redeem_expires_at',
    batchLimit: 500,
    selectSql:
      'SELECT CAST(id AS CHAR) AS id FROM pairing_requests FORCE INDEX (ix_pairing_state_redeem_expiry) WHERE state = 3 AND redeem_expires_at <= ? ORDER BY redeem_expires_at, id LIMIT ?',
    cutoff: atNow,
    mutation: 'pairing-expire',
  },
  {
    name: 'pairing-terminal-delete',
    indexName: 'ix_pairing_state_updated',
    oldestColumn: 'updated_at',
    batchLimit: 500,
    selectSql:
      'SELECT CAST(id AS CHAR) AS id FROM pairing_requests FORCE INDEX (ix_pairing_state_updated) WHERE state IN (4,5,6,7,8) AND updated_at <= ? ORDER BY state, updated_at, id LIMIT ?',
    cutoff: (now) => daysBefore(now, 30),
    mutation: 'delete-pairing',
  },
  {
    name: 'grant-active-expiry',
    indexName: 'ix_grants_status_expiry',
    oldestColumn: 'expires_at',
    batchLimit: 500,
    selectSql:
      'SELECT CAST(id AS CHAR) AS id FROM mcp_grants FORCE INDEX (ix_grants_status_expiry) WHERE status = 2 AND expires_at <= ? ORDER BY status, expires_at, id LIMIT ?',
    cutoff: atNow,
    mutation: 'grant-expire',
  },
  {
    name: 'credential-rotated-delete',
    indexName: 'ix_grant_credentials_status_revoked',
    oldestColumn: 'revoked_at',
    batchLimit: 500,
    selectSql:
      'SELECT CAST(id AS CHAR) AS id FROM mcp_grant_credentials FORCE INDEX (ix_grant_credentials_status_revoked) WHERE status = 2 AND revoked_at <= ? ORDER BY status, revoked_at, id LIMIT ?',
    cutoff: (now) => daysBefore(now, 365),
    mutation: 'delete-credential',
  },
  {
    name: 'credential-revoked-delete',
    indexName: 'ix_grant_credentials_status_revoked',
    oldestColumn: 'revoked_at',
    batchLimit: 500,
    selectSql:
      'SELECT CAST(id AS CHAR) AS id FROM mcp_grant_credentials FORCE INDEX (ix_grant_credentials_status_revoked) WHERE status = 3 AND revoked_at <= ? ORDER BY status, revoked_at, id LIMIT ?',
    cutoff: (now) => daysBefore(now, 365),
    mutation: 'delete-credential',
  },
  {
    name: 'grant-revoked-delete',
    indexName: 'ix_grants_status_revoked',
    oldestColumn: 'revoked_at',
    batchLimit: 500,
    selectSql:
      'SELECT CAST(id AS CHAR) AS id FROM mcp_grants FORCE INDEX (ix_grants_status_revoked) WHERE status = 3 AND revoked_at <= ? ORDER BY status, revoked_at, id LIMIT ?',
    cutoff: (now) => daysBefore(now, 365),
    mutation: 'delete-grant',
  },
  {
    name: 'grant-expired-delete',
    indexName: 'ix_grants_status_expiry',
    oldestColumn: 'expires_at',
    batchLimit: 500,
    selectSql:
      'SELECT CAST(id AS CHAR) AS id FROM mcp_grants FORCE INDEX (ix_grants_status_expiry) WHERE status = 4 AND expires_at <= ? ORDER BY status, expires_at, id LIMIT ?',
    cutoff: (now) => daysBefore(now, 365),
    mutation: 'delete-grant',
  },
  {
    name: 'session-expired-delete',
    indexName: 'ix_sessions_status_absolute',
    oldestColumn: 'absolute_expires_at',
    batchLimit: 500,
    ...
    cutoff: (now) => daysBefore(now, 180),
    mutation: 'delete-session',
  },
  {
    name: 'session-revoked-delete',
    indexName: 'ix_sessions_status_revoked',
    oldestColumn: 'revoked_at',
    batchLimit: 500,
  ...
  {
    name: 'session-rotated-delete',
    indexName: 'ix_sessions_status_rotated',
    oldestColumn: 'rotated_at',
    batchLimit: 500,
  ...
  {
    name: 'security-audit-delete',
    indexName: 'ix_audit_occurred',
    oldestColumn: 'occurred_at',
    batchLimit: 500,
    selectSql:
      'SELECT CAST(id AS CHAR) AS id FROM security_audit_events FORCE INDEX (ix_audit_occurred) WHERE occurred_at <= ? ORDER BY occurred_at, id LIMIT ?',
    cutoff: (now) => daysBefore(now, 365),
...
export class SecurityRetentionService {
  ...
  async execute(
    mode: SecurityRetentionMode,
    now: number = Date.now(),
  ): Promise<SecurityRetentionReport> {
    ...
    if (mode === 'status')
      return this.mysql.withConnection(async (connection) => {
        ...
        for (const target of SECURITY_RETENTION_TARGETS) {
          const [rows] = await connection.execute<StatusRow[]>(statusSqlFor(target), [
            target.cutoff(now),
          ]);
          const dueCount = Number(rows[0]?.dueCount ?? 0);
          if (!Number.isSafeInteger(dueCount) || dueCount < 0)
            throw new TypeError('retention due count is invalid');
    ...
    return this.mysql.withConnection(async (connection) => {
      ...
      if (!isMysqlLockAcquired(lockRows[0]?.acquired)) {
        return {
          ...this.emptyReport(mode, startedAt),
          outcome: 'overlap',
      ...
      try {
        ...
        for (const target of SECURITY_RETENTION_TARGETS) {
          while (report.selectedRows < MAX_ROWS && elapsed(startedAt) < MAX_DURATION_MS) {
            ...
            report.mutatedRows += await withTransaction(
              connection,
              'READ COMMITTED',
              (transaction) =>
                this.mutateBatch(
  ...
  private async expirePairing(
    ...
    const [familyLinks] = await connection.execute<
      Array<RowDataPacket & { familyPublicId: string }>
    >(
      `
    ...
    await connection.execute(
      'SELECT id FROM auth_sessions WHERE family_public_id = ? ORDER BY id FOR UPDATE',
      [family],
    );
    ...
    if (row.grantDatabaseId !== null) {
      const [grantRows] = await connection.execute<
        Array<RowDataPacket & { publicId: string; status: number }>
      >('SELECT public_id AS publicId, status FROM mcp_grants WHERE id = ? FOR UPDATE', [
        row.grantDatabaseId,
      ]);
      const grant = grantRows[0];
      if (grant === undefined || grant.status !== 1)
      ...
      await connection.execute(
        'UPDATE mcp_grant_credentials SET status = 3, revoked_at = ? WHERE grant_id = ? AND status = 1',
        [new Date(now), row.grantDatabaseId],
      );
      const [grantUpdated] = await connection.execute<ResultSetHeader>(
        'UPDATE mcp_grants SET status = 4, revoked_at = ?, revoke_reason = 3, updated_at = ? WHERE id = ? AND status = 1',
        [new Date(now), new Date(now), row.grantDatabaseId],
      );
      if (grantUpdated.affectedRows !== 1)
        throw new Error('pending grant expiry lost its compare-and-set');
      await this.audit.writeMandatory(
        { connection },
        {
          event: 'grant_expire',
  ...
  private async expireGrant(connection: PoolConnection, id: string, now: number): Promise<number> {
    const [familyLinks] = await connection.execute<
      Array<RowDataPacket & { familyPublicId: string | null }>
    >(
      `
      SELECT s.family_public_id AS familyPublicId
    ...
    const row = rows[0];
    if (row === undefined || row.sourceFamilyPublicId !== (family ?? null) || row.due !== 1)
      return 0;
    await connection.execute(
      'SELECT id FROM mcp_grant_credentials WHERE grant_id = ? ORDER BY id FOR UPDATE',
      [id],
    ...
    const [updated] = await connection.execute<ResultSetHeader>(
      'UPDATE mcp_grants SET status = 4, revoked_at = ?, revoke_reason = 3, updated_at = ? WHERE id = ? AND status = 2',
...
const deleteStatementFor = (
  ...
  switch (target.name) {
    ...
    case 'credential-rotated-delete':
      return {
        statement: `DELETE FROM mcp_grant_credentials WHERE id IN (${placeholders}) AND status = ? AND revoked_at <= ?`,
        state: 2,
      };
    ...
    case 'grant-revoked-delete':
      return {
        statement: `DELETE FROM mcp_grants WHERE id IN (${placeholders}) AND status = ? AND revoked_at <= ?`,
        state: 3,
      };
    case 'grant-expired-delete':
      return {
        statement: `DELETE FROM mcp_grants WHERE id IN (${placeholders}) AND status = ? AND expires_at <= ?`,
        state: 4,
      };
    ...
    case 'security-audit-delete':
      return {
        statement: `DELETE FROM security_audit_events WHERE id IN (${placeholders}) AND occurred_at <= ?`,
        state: null,
      };
    default:
