...
const exactLength = (value: unknown, length: number): boolean =>
  Buffer.isBuffer(value) && value.byteLength === length;
...
const numericBinds = (cursor: string | null): readonly unknown[] => [
  safeUnsigned(cursor ?? '0'),
  0,
];
...
const pageSql = (
  ...
  joins: string,
  cursorColumn: string,
): Pick<ProbeDefinition, 'ascendingSql' | 'lowestSql' | 'highestSql'> => ({
...
const definitions: readonly ProbeDefinition[] = [
  {
    ...
    cursorOf: (row) =>
      JSON.stringify([safeUnsigned(row.grantId, false), String(row.boardPublicId)]),
    cursorBinds: (cursor) => {
      if (cursor === null) return ['0', '0', '', 0];
      try {
        const parsed = JSON.parse(cursor) as unknown;
        if (!Array.isArray(parsed) || parsed.length !== 2 || !parsedBoardId(parsed[1]))
          throw new TypeError();
    ...
    validate: (row) => {
      if (
        !safeUnsigned(row.grantId, false) ||
  ...
  },
  {
    ...
    validate: (row, input) => {
      ...
      if (
        row.fingerprintBytes !== fingerprint.byteLength ||
        !sameDigest(fingerprint, row.fingerprintSha256)
      ) {
        throw new PersistenceProbeFailure('ROW_MAPPING', false);
      }
      if (row.statusCode === 'P') {
...
const estimatedBytes = (rows: readonly ProbeRow[]): number =>
  rows.reduce(
    (total, row) =>
      total +
      Object.values(row).reduce<number>((rowTotal, value) => {
...
export const createMysqlPersistenceCertificationProbes = (
  mysql: MysqlService,
): readonly PersistenceCertificationProbeV1[] =>
  definitions.map((definition) => new MysqlPersistenceCertificationProbe(mysql, definition));
