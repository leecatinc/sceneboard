...
export class RedisPresenceRepository implements BrowserPresenceStatusReaderV1 {
  ...
  async open(input: {
    ...
    if (!Array.isArray(result) || Number(result[0]) < 1) {
      throw new AppError('RATE_LIMITED', { retryAfterSeconds: 35 });
    }
  ...
  async touch(handle: RedisPresenceHandleV1): Promise<boolean> {
    const now = Date.now();
    const value = await this.redis.evaluate(
      TOUCH_SCRIPT,
      [
  ...
  async aggregate(boardId: BoardId): Promise<PresenceAggregateV1> {
    const result = await this.redis.evaluate(
      AGGREGATE_SCRIPT,
      [this.keyspace.presenceIndexKey(boardId), this.keyspace.presenceVersionKey(boardId)],
      [String(Date.now())],
    );
    if (!Array.isArray(result) || result.length > 4_001)
      throw new Error('invalid Redis presence aggregate');
    const version = safeVersion(result[0]);
    ...
    for (const value of result.slice(1)) {
      ...
      if (
        !/^[A-Za-z0-9_-]{22}$/u.test(connectionId ?? '') ||
        (principalKind !== 'user' &&
          principalKind !== 'mcp_client' &&
          principalKind !== 'service') ||
        !/^[A-Za-z0-9_-]{1,128}$/u.test(principalId ?? '') ||
        (state !== 'online' && state !== 'away') ||
        !/^(?:0|[1-9][0-9]*)$/u.test(lastSeenSource ?? '')
      )
        throw new Error('invalid Redis presence record');
      ...
      if (current === undefined) {
        ...
        strongest.set(key, {
          ...
          lastSeenAt:
            current.lastSeenAt > summary.lastSeenAt ? current.lastSeenAt : summary.lastSeenAt,
        });
      }
    }
    const presence = [...strongest.entries()]
