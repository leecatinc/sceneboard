...
import { SceneParserV1, type SceneV1 } from '@sceneboard/board-schema';
...
const decompress = (input: Buffer): Promise<Buffer> =>
  new Promise((resolve, reject) => {
...
export class SceneCheckpointCodec {
  async encode(input: unknown): Promise<EncodedSceneCheckpointV1> {
    ...
    if (canonicalPayload.byteLength < 1 || canonicalPayload.byteLength > MAX_CANONICAL_BYTES)
  ...
  async decode(
    input: StoredSceneCheckpointV1,
  ): Promise<{ scene: SceneV1; canonicalBytes: Buffer }> {
    if (
      input.schemaVersion !== '1.0.0' ||
      input.codec !== 'B' ||
      input.canonicalBytes < 1 ||
      input.canonicalBytes > MAX_CANONICAL_BYTES ||
      input.storedBytes < 1 ||
      input.storedBytes > MAX_STORED_BYTES ||
