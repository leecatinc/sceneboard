...
type LockedRuntimeRow = StoredVersionRow;
...
const artifactNotFound = (artifact: ArtifactReferenceV1): BoardContractError =>
  new BoardContractError({
    protocolVersion: 1,
    type: 'board.error',
...
const nextUsage = (
  ...
  const manifestBytes =
    parseUnsigned(row.manifestCanonicalBytes) + BigInt(publication.manifestBytes.byteLength);
  const resourceBytes =
    parseUnsigned(row.resourceBytes) +
    BigInt(publication.resources.reduce((total, item) => total + item.byteLength, 0));
  if (artifacts > BigInt(BOARD_LIMITS_V1.maxBoardArtifacts))
    throw limitFailure('maxBoardArtifacts', artifacts);
  if (versions > BigInt(BOARD_LIMITS_V1.maxBoardArtifactVersions))
    throw limitFailure('maxBoardArtifactVersions', versions);
  ...
  private certify(
    ...
    const resources = resourceRows.map((resourceRow, index): PreparedArtifactResourceV1 => {
      ...
      if (
        ...
        resourceRow.resourceOrdinal !== index + 1 ||
      ...
      if (
        ...
        (bytes.byteLength !== descriptor.byteLength ||
          !equalDigest(digest(bytes), resourceRow.resourceSha256))
      )
        throw internalFailure();
