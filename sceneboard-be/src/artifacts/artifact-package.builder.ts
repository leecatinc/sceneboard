...
export const encodeArtifactPackageV1 = (
  ...
  for (let index = 0; index < resources.length; index += 1) {
    ...
    if (item === undefined || path === undefined || path.byteLength > 65_535)
      throw new RangeError('artifact package path');
...
export class ArtifactPackageBuilderV1 {
  build(input: {
    ...
    const resources = [
      ...
      ...(input.source.javascript === null
        ? []
        : [resource('main.js', 'text/javascript', input.source.javascript)]),
    ];
