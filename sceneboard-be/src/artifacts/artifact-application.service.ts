...
export class ArtifactApplicationService extends ArtifactApplicationPortV1 {
  ...
  async publish(input: Parameters<ArtifactApplicationPortV1['publish']>[0]) {
    let stage = 'normalize';
    const publication = this.normalizer.normalize({
    ...
    return this.accessPolicy
      .withAuthorizedBoardTransaction(
        {
          principal: input.principal,
        ...
        async (connection, context) => {
          for (const capability of publication.manifest.requestedCapabilities) {
            if (
              !context.artifactCapabilityPolicy.allowedArtifactRequestCapabilities.includes(
                capability,
              )
            ) {
          ...
          stage = 'admission';
          const admission = await this.mutations.begin(
            connection,
            context,
            request.data.value,
            prepared,
  ...
  async stop(input: Parameters<ArtifactApplicationPortV1['stop']>[0]) {
    ...
    return this.accessPolicy.withAuthorizedBoardTransaction(
      ...
      async (connection, context) => {
        ...
        const stored = await this.artifacts.lockRuntime(
          connection,
          head.boardPk,
          request.command.artifact,
        );
        if (stored.runtime.status !== 'ready' && stored.runtime.status !== 'stopped')
        ...
        if (stored.runtime.status === 'ready') {
          const sequence = await this.mutations.allocateSequence(connection, head, prepared);
          await this.artifacts.markStopped(
            connection,
            stored.versionPk,
            sequence,
