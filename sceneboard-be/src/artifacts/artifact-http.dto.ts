...
): BoardArtifactPutSourceParseResultV1 => {
  const error = invalidBoardPayload(issue);
  error.details = { path, issue };
  return { ok: false, error };
};

const globalId = (value: unknown, _path: string): string | null => {
...
export const BoardArtifactPutSourceV1Parser = Object.freeze({
  parse(input: unknown): BoardArtifactPutSourceParseResultV1 {
    if (input === null || typeof input !== 'object' || Array.isArray(input))
      return fail('artifact_source_object');
    ...
    if (expectedRevisionId === null)
      return fail('artifact_source_revision_id', ['expectedRevisionId']);
    if (
    ...
    const artifactId =
      source.artifactId === null ? null : globalId(source.artifactId, 'artifactId');
    if (source.artifactId !== null && artifactId === null)
      return fail('artifact_source_artifact_id', ['artifactId']);
    for (const key of ['html', 'css', 'javascript'] as const) {
      const value = source[key];
      if (
        key === 'html' ? typeof value !== 'string' : value !== null && typeof value !== 'string'
      ) {
        return fail('artifact_source_text_type', [key]);
      }
      if (typeof value === 'string' && hasLoneSurrogate(value))
        return fail('artifact_source_lone_surrogate', [key]);
    }
    ...
    if (
      requestedCapabilities.length > ARTIFACT_REQUEST_CAPABILITIES_V1.length ||
    ...
    for (let index = 1; index < requestedCapabilities.length; index += 1) {
      if (
        (requestedCapabilities[index - 1] as string) >= (requestedCapabilities[index] as string)
      ) {
