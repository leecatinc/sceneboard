...
const invalidCursor = (): BoardContractError =>
  new BoardContractError(invalidBoardPayload('invalid SSE resume cursor'));
...
export class SseCursorCodec {
  ...
  decode(cursor: string): SseResumeCursorPayloadV1 {
    ...
    const canonical = canonicalizeJsonV1(validated);
    if (!canonical.ok || !equalBytes(payloadBytes, canonical.data.canonicalBytes))
      throw invalidCursor();
    return validated;
  }
  ...
  #validatePayload(value: unknown): SseResumeCursorPayloadV1 {
    ...
    if (
      keys.join(',') !== 'b,e,k,s,t,v' ||
      source.v !== 1 ||
      (source.k !== 'event' && source.k !== 'snapshot') ||
      !Number.isSafeInteger(source.s) ||
      Number(source.s) < 1 ||
      typeof source.e !== 'string' ||
      !GlobalIdStringParserV1.parse(source.e).ok ||
      typeof source.t !== 'string' ||
      !TIMESTAMP_PATTERN.test(source.t) ||
      !Number.isFinite(Date.parse(source.t)) ||
      new Date(Date.parse(source.t)).toISOString() !== source.t
    ) {
      throw invalidCursor();
    }
