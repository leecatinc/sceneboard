...
import {
  BoardSseService,
  type BoardSseRequestLifecycleV1,
  type BoardSseResponseV1,
} from './board-sse.service.js';
import {
  RequireBrowserBoardStream,
  type BrowserBoardStreamRequestV1,
} from './stream-admission.guard.js';

interface BoardStreamHttpRequest
  extends BoardPrincipalRequest, BrowserBoardStreamRequestV1, BoardSseRequestLifecycleV1 {}
...
export class BoardSseController {
  ...
  async events(
    ...
    if (admission === undefined || principal?.kind !== 'user')
      throw new Error('browser stream admission invariant failed');
