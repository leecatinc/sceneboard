...
import type { BoardId, EventId, TabId } from '@sceneboard/board-schema';

const SALT = Buffer.from('leecat-board/stream/v1', 'ascii');

const derive = (material: Buffer, info: string): Buffer =>
  Buffer.from(hkdfSync('sha256', material, SALT, Buffer.from(info, 'ascii'), 32));
...
export class RedisStreamKeyspace {
  ...
  constructor(
    material: Buffer,
    private readonly prefix = 'sceneboard:',
  ) {
    if (material.byteLength !== 32)
      throw new TypeError('stream key material must be exactly 32 bytes');
    if (prefix !== 'sceneboard:') throw new TypeError('stream Redis prefix must be sceneboard:');
  ...
  principalFingerprint(ownerUserPk: bigint): string {
    if (ownerUserPk < 1n || ownerUserPk > 0xffff_ffff_ffff_ffffn)
      throw new TypeError('ownerUserPk is outside uint64');
    const bytes = Buffer.allocUnsafe(8);
    bytes.writeBigUInt64BE(ownerUserPk);
    return fingerprint(
      this.#principalKey,
      Buffer.concat([Buffer.from('owner-user-pk\0', 'ascii'), bytes]),
    );
  }
