...
const validateRow = (row: IntegrityProbeRowV1): void => {
  ...
  if (
    createdEvent.sequence !== stored.createdEventSequence ||
  ...
  if (
    stored.createdEventSequence === stored.stateEventSequence &&
  ...
  if (stored.interaction.state === 'superseded') {
    if (
      row.successorHitlPk === null ||
      row.successorCreatedAt === null ||
