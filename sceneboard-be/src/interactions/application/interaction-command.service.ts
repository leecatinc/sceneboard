...
const assertActor = (
  ...
  if (
    !parsed.ok ||
    parsed.data.value.actor.principalKind !== context.actor.principalKind ||
...
export class InteractionCommandService extends HitlMutationApplicationPortV1 {
  ...
  async apply(input: {
    ...
    if (
      input.request.command.type !== 'hitl.request' &&
      input.request.command.type !== 'hitl.respond'
    ) {
      throw internalHitlFailure();
    }
    const now = this.clock();
    if (!Number.isFinite(now.valueOf())) throw internalHitlFailure();
    const prepared = prepareControlMutationV1({
      principal: input.principal,
      request: input.request,
      now,
    });
    const envelope = { ...input.request, actor: input.principal.actor } as
      | HitlRequestMutationEnvelopeV1
      | HitlRespondMutationEnvelopeV1;
      ...
      async (connection, context) => {
        if (envelope.command.type === 'hitl.request') {
          return this.requestPrepared(
            connection,
            context,
            envelope as HitlRequestMutationEnvelopeV1,
            prepared,
          );
        }
        return this.respondPrepared(
          connection,
          context,
          envelope as HitlRespondMutationEnvelopeV1,
          prepared,
        );
      },
    );
  ...
  private async requestPrepared(
    ...
    try {
      ...
      if (isDuplicateInteractionIdError(error))
        throw hitlIdConflict(envelope.command.hitlRequestId);
      throw error;
    }
    ...
    const completed = await this.mutations.complete(
      ...
      head,
      prepared,
      result({ request, prepared, interaction }),
    );
  ...
  private async respondPrepared(
    ...
    const stored = await this.interactions.lockByBoardPk(
      connection,
      head.boardPk,
      envelope.command.hitlRequestId,
    );
