    ...
    const outcome = await this.accessPolicy.withAuthorizedBoardTransaction(
      ...
      async (connection, context): Promise<LifecycleOutcome> => {
        ...
        if (updatedMs >= expiresMs) {
          ...
          return {
            error: hitlExpired(interaction.hitlRequestId, interaction.expiresAt as TimestampV1),
          };
        }
        let successor: StoredInteractionV1 | null = null;
        if (action === 'supersede') {
          const successorId = (input.request as HitlSupersedeAdapterRequestV1)
          ...
          if (
            successor.interaction.state !== 'open' ||
        ...
        if (action === 'cancel') {
          await this.interactions.cancel(connection, {
            ...
            context,
            sequence,
            updatedAtSql: formatMysqlTimestampUtc(new Date(updatedMs)),
          });
        } else {
