...
test('uses one atomic Lua call with an HMAC-only namespaced identity', async () => {
  ...
  const service = new RateLimitService(redis, crypto, 'sceneboard:');
  ...
  assert.match(captured?.key ?? '', /^sceneboard:rate:v1:login-ip:[A-Za-z0-9_-]{43}$/);
...
test('maps over-limit TTL to an integer Retry-After and Redis loss to fail-closed 503', async () => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof AppError && error.code === 'RATE_LIMITED' && error.retryAfterSeconds === 2,
  );
