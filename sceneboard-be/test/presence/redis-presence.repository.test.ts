...
import type { BoardId, TabId } from '@sceneboard/board-schema';
...
test('open, touch, and compare-delete close use bounded TTL scripts and version hints', async () => {
  ...
  const repository = new RedisPresenceRepository(redis as never, keyspace, {
    randomBase64Url: () => 'BBBBBBBBBBBBBBBBBBBBBB',
  } as never);
  ...
  assert.equal(
    evaluations[0]?.keys.some((key) => key.includes(BOARD_ID)),
    false,
  );
  assert.equal(publications.length, 1);
  assert.equal(
    parseBoardEventHintV1(publications[0]!.message, keyspace.boardFingerprint(BOARD_ID))?.kind,
    'presence',
  );
...
test('connection caps reject with the public rate-limit contract', async () => {
  const repository = new RedisPresenceRepository(
    {
      evaluate: async () => [-1, 0],
      publish: async () => 0,
    } as never,
    new RedisStreamKeyspace(Buffer.alloc(32, 19)),
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof AppError && error.code === 'RATE_LIMITED' && error.retryAfterSeconds === 35,
  );
});
...
test('aggregation is unique, online-dominant, latest-seen, and deterministically sorted', async () => {
  ...
  assert.deepEqual(
    aggregate.presence.map((item) => [item.principal.principalId, item.state, item.lastSeenAt]),
    [
