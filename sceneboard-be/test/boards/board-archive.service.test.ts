...
import {
  BoardArchiveService,
  type BoardArchiveRequestV1,
} from '../../src/boards/board-archive.service.js';
...
const request = (requestId: string): BoardArchiveRequestV1 => {
  const parsed = BoardOperationRequestParserV1.parse({
    ...
    requestId,
    type: 'board.archive',
    idempotencyKey: 'archive-board-key-0001',
    boardId,
    confirm: true,
  });
  assert.equal(parsed.ok, true);
  if (!parsed.ok || parsed.data.value.type !== 'board.archive')
    ...
    async execute(sql: string, binds: unknown[] = []): Promise<[unknown, unknown]> {
      ...
      if (normalized.startsWith('INSERT INTO board_idempotency_records')) {
        ...
        stored = {
          ...
          fingerprintSha256: binds[7],
          actorGrantId: binds[8],
          actorScopesSha256: binds[10],
          resultPayload: null,
...
test('archives without changing the non-null head and replays without a second write or event', async () => {
  const value = setup();
  const first = await value.service.archive({
    principal: principal(),
    request: request('request_archive_1'),
  });
  ...
  const replay = await value.service.archive({
    principal: principal(),
    request: request('request_archive_2'),
  });
  assert.equal(replay.replayed, true);
  assert.deepEqual(replay.result, first.result);
  assert.equal(value.archiveWrites(), 1);
  assert.equal(
    value.calls.some((call) => call.includes('board_event_outbox')),
    false,
  );
});
