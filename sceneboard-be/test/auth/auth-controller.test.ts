...
const issue: IssuedAuthSession = {
  response: {
    user: {
      userId: 'user_1' as IssuedAuthSession['response']['user']['userId'],
...
const unusedSessionResolver: SessionControllerService = {
  ...
  async renew() {
    throw new Error('unexpected session renewal');
  },
};
...
const unusedEmailVerification = {
  async request() {
    throw new Error('unexpected email verification request');
  },
  async confirm() {
    throw new Error('unexpected email verification confirmation');
  },
  assertTicket() {},
...
const unusedPasswordChanges = {
  async change() {
    throw new Error('unexpected password change');
  },
...
test('anonymous CSRF bootstrap emits only a CSRF cookie and generation proof', () => {
  ...
  const controller = new AuthController(
    ...
    unusedLogout,
    unusedEmailVerification,
    unusedPasswordChanges,
  );
...
test('anonymous CSRF bootstrap rejects any existing session cookie without mutation', () => {
  ...
  assert.throws(
    () =>
      controller.csrf({ cookies: { lcb_test_session: 'present' } }, response, 1_800_000_000_000),
    (error) => error instanceof AppError && error.code === 'AUTH_SESSION_PRESENT',
  );
...
test('password change uses the authenticated session without mutating current cookies', async () => {
  ...
  await controller.changePassword(
    ...
    { headers: {}, cookies: {}, authSession: activeSession },
    response,
    1_800_000_000_000,
  );
...
test('session resolver clears cookies only for committed or safely unresolvable terminals', async () => {
  ...
  await assert.rejects(() =>
    controller.session({ cookies: { lcb_test_session: 'token' } }, response, 1_800_000_000_000),
  );
...
test('session renewal clears cookies only after a committed terminal result', async () => {
  ...
  const unavailableController = new AuthController(
    services,
    unavailable,
    new CsrfService(crypto),
    new CookieService('test'),
...
test('logout clears cookies after success and after an already-terminal audit failure only', async () => {
  ...
  const terminalAuditFailure: LogoutControllerService = {
    async logout() {
      throw new LogoutClearableError('SERVICE_UNAVAILABLE');
    },
  };
  const terminalController = new AuthController(
    services,
    unusedSessionResolver,
    new CsrfService(crypto),
    new CookieService('test'),
  ...
  await assert.rejects(() =>
    terminalController.logout({}, { cookies: {}, headers: {} }, terminal.response),
  );
  ...
  const activeController = new AuthController(
    services,
    unusedSessionResolver,
    new CsrfService(crypto),
    new CookieService('test'),
  ...
  await assert.rejects(() =>
    activeController.logout({}, { cookies: {}, headers: {} }, active.response),
  );
