...
test('stores only an email fingerprint and code HMAC before sending a six-digit Gmail code', async () => {
  ...
  const service = new EmailVerificationService(redis, crypto, mailer, 'sceneboard:');
  ...
  assert.equal(
    redis.calls[0]?.keys.some((value) => value.includes('user@example.dev')),
    false,
  );
...
test('issues an email-bound short-lived ticket after one successful code claim', async () => {
  ...
  const service = new EmailVerificationService(redis, crypto, new CapturingMailer(), 'sceneboard:');
  ...
  assert.match(
    confirmed.verificationTicket,
    /^v1\.[0-9]{13}\.[A-Za-z0-9_-]{22}\.[A-Za-z0-9_-]{43}$/,
  );
  assert.equal(confirmed.expiresAt, new Date(now + 15 * 60 * 1_000).toISOString());
  assert.doesNotThrow(() =>
    service.assertTicket('user@example.dev', confirmed.verificationTicket, now),
  );
  assert.throws(
...
test('returns the Redis cooldown as a retryable rate limit', async () => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof AppError && error.code === 'RATE_LIMITED' && error.retryAfterSeconds === 62,
  );
});
