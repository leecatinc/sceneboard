...
test('grant cursor binds one canonical keyset tuple to its authenticated owner', () => {
  ...
  assert.deepEqual(cursors.parse('user_1', cursor), {
    createdAt: '2027-01-15T08:00:00.000Z',
    id: '42',
  });
  assert.throws(
    () => cursors.parse('user_2', cursor),
    (error) => error instanceof AppError && error.code === 'INVALID_PAYLOAD',
  );
  assert.throws(
    () => cursors.parse('user_1', `${cursor}A`),
    (error) => error instanceof AppError && error.code === 'INVALID_PAYLOAD',
  );
});
...
test('grant list query accepts only opaque cursor and canonical decimal limit', () => {
  ...
  assert.deepEqual(parseGrantListQuery({ cursor: 'opaque', limit: '100' }), {
    cursor: 'opaque',
    limit: 100,
  });
