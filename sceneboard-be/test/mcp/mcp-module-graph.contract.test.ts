...
test('MCP connection route is mounted once with one no-body profile and shared modules', async () => {
  ...
  const moduleSource = await readFile(
    new URL('../../src/mcp/mcp.module.ts', import.meta.url),
    'utf8',
  );
  ...
  const profiles = RAW_BODY_PROFILES.filter(
    (profile) => profile.pathTemplate === '/api/v1/mcp/connection',
  );
  assert.deepEqual(profiles, [
    { kind: 'd1-no-body', method: 'GET', pathTemplate: '/api/v1/mcp/connection' },
  ]);
});

test('connection controller uses the shared board principal and request correlation guards', async () => {
  const source = await readFile(
    new URL('../../src/mcp/mcp-connection.controller.ts', import.meta.url),
    'utf8',
  );
