...
const tableNames = (asset: string): string[] => {
  ...
  return [...source.matchAll(/CREATE TABLE(?: IF NOT EXISTS)?\s+([a-z][a-z0-9_]*)\s*\(/gu)].map(
    (match) => match[1] as string,
  );
};

export const readOwnerProjection = (owner: OwnerProjectionV1['owner']): OwnerProjectionV1 =>
  JSON.parse(
    read(
      `test/contracts/schema-projections/${owner.toLowerCase()}.expected-schema.v1.json`,
    ).toString('utf8'),
...
export const assertOwnerProjection = (owner: OwnerProjectionV1['owner']): OwnerProjectionV1 => {
  ...
  for (const entry of projection.registryEntries) {
    ...
    assert.deepEqual(
      ...
      entry.order === 1 ? [] : [MIGRATION_REGISTRY[entry.order - 2]?.version],
    );
  }
