...
test('D3 publishes the exact segmented application, snapshot, and outbox seams', () => {
  ...
  assert.equal(publisher.resourcePath, 'sceneboard-be/src');
  assert.equal(
    publisher.publisherTestPath,
    'sceneboard-be/test/contracts/d3-application-seams.contract.test.ts',
  );
  ...
  for (const selector of publisher.selectors) {
    ...
    assert.ok(selector.sourcePath.startsWith('sceneboard-be/src/'));
