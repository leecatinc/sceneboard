...
const assertionIds = [
  ...
  ...Array.from({ length: 5 }, (_, index) => `OPS-CORRELATION-${String(index + 1).padStart(2, '0')}`),
  ...Array.from({ length: 12 }, (_, index) => `OPS-REDACTION-${String(index + 1).padStart(2, '0')}`),
];
...
test('operations assertion map has 37 unique live-required rows and owner health thresholds', async () => {
  ...
  const health = await readFile(new URL('../../leecat-board-nestjs/src/sse/board-stream-health.service.ts', import.meta.url), 'utf8');
...
test('certification metric names and low-cardinality labels are closed', () => {
  const metrics = {
    leecat_board_certification_phase_total: ['phase', 'result', 'owner'],
    leecat_board_certification_phase_duration_seconds: ['phase', 'profile'],
    leecat_board_certification_cleanup_total: ['resource', 'result'],
    leecat_board_certification_gate_total: ['gate', 'result', 'owner'],
  };
  assert.equal(Object.keys(metrics).length, 4);
  assert.equal(Object.values(metrics).flat().some((label) => /id|token|board/iu.test(label)), false);
});
