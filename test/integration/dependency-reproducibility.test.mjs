...
import { verifyDependencies } from '../../scripts/verify-dependency-reproducibility.mjs';

const clone = (value) => JSON.parse(JSON.stringify(value));
const lockBytes = await readFile(new URL('../../package-lock.json', import.meta.url));
const lock = JSON.parse(lockBytes.toString('utf8'));
const inventory = JSON.parse(await readFile(
  new URL('../certification/dependency-inventory.v1.json', import.meta.url),
  'utf8',
));
const fixtureRoot = new URL('../certification/fixtures/dependency-reproducibility/', import.meta.url);
...
test('committed public-registry dependency inventory matches the hydrated static tree', async () => {
  ...
  assert.equal(result.dependencyCount, 418);
...
test('dependency mutation fixtures preserve exact fail-closed reasons', async (context) => {
  const names = [
    'registry-host-drift.v1.json', 'credential-url.v1.json', 'integrity-drift.v1.json',
    'manifest-lock-mismatch.v1.json', 'installed-tree-drift.v1.json',
  ];
  for (const name of names) await context.test(name, async () => {
    ...
    if (fixture.mutation === 'inventory-first-integrity') changed.entries[0].integrity = 'sha512-AAAA';
...
test('license ambiguity and clean-tree acquisition cannot degrade to skipped', async () => {
  ...
  await assert.rejects(
    () => verifyDependencies({
      profile: 'static', lockValue: lock, lockBytes, inventoryValue: ambiguous,
