...
test('multi-client live harness consumes the sole snapshot/replay/cursor/reducer path', async () => {
  ...
  const admission = await readFile(
    new URL('sceneboard-be/src/sse/stream-admission.guard.ts', root),
    'utf8',
  );
  const cut = await readFile(
    new URL('sceneboard-be/src/sse/board-stream-cut.service.ts', root),
    'utf8',
  );
  const client = await readFile(
    new URL('packages/board-sdk/src/sse/board-stream-client.ts', root),
    'utf8',
  );
  const reconciler = await readFile(
    new URL('packages/board-sdk/src/events/event-reconciler.ts', root),
    'utf8',
  );
