...
test('forward-only D3/D7/D8 migrations expose no automatic destructive rollback', async () => {
  const registry = await readFile(new URL('leecat-board-nestjs/src/database/migrations/registry.ts', root), 'utf8');
  const forwardOnly = [...registry.matchAll(/version: '([^']+)'[\s\S]*?upAsset: '([^']+)'[\s\S]*?reversible: false,[\s\S]*?downAsset: null/gu)];
  assert.equal(forwardOnly.length, 12);
  for (const [, , asset] of forwardOnly) {
    const sql = await readFile(new URL(`leecat-board-nestjs/src/database/migrations/sql/${asset}`, root), 'utf8');
...
test('restore is quarantine-only and production targeting is rejected', () => {
  assert.deepEqual(parseCertificationArguments(['--phase=restore', '--profile=quarantine']), {
    phase: 'restore', profile: 'quarantine',
  });
  assert.throws(
