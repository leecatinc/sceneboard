...
import { canonicalJson, containsSecretLikeMaterial } from '../../scripts/lib/certification/canonical-json.mjs';
import {
...
const rejectsCode = async (operation, code) => assert.rejects(operation, (error) => error?.code === code);
...
test('golden manifest is canonical, stable, and exactly reproducible from all 399 closed inputs', async () => {
  const golden = await readFixture('golden.v1.json');
  assert.equal(golden.bytes.toString('utf8'), `${canonicalJson(golden.value)}\n`);
  const result = await verifyContractManifest({ manifestValue: golden.value, manifestBytes: golden.bytes });
  assert.equal(result.status, 'PASS');
  assert.equal(result.ownerCount, 9);
  assert.equal(result.resourceCount, 399);
  assert.equal(result.migrationCount, 15);
  assert.equal(result.sqlAssetCount, 18);
  assert.equal(result.finalToolCount, 21);
});
...
test('manifest rejects unknown top-level fields before observing source', () => {
  assert.throws(
    () => validateManifestShape({ schemaVersion: 1, inventorySha256: '0'.repeat(64), inferredBaseline: true }),
...
test('inventory rejects aliases, owner swaps, whole-file overlap, and publisher omission', async (context) => {
  ...
  await context.test('second resource alias', async () => {
    ...
    await rejectsCode(
      () => observeContractInventory({ inventoryValue: changed, inventoryBytes: mutationBytes(changed) }),
  ...
  await context.test('D8 browser owner swap', async () => {
    ...
    group.resources.find(({ resourceId }) => resourceId === 'D8-HITL-MUTATION-REQUEST-SELECTOR').owner = 'D7';
    await rejectsCode(
      () => observeContractInventory({ inventoryValue: changed, inventoryBytes: mutationBytes(changed) }),
  ...
  await context.test('whole-file and member projection overlap', async () => {
    ...
    const target = group.resources.find(({ resourceId }) => resourceId === 'D3-OUTBOX-LOAD-PENDING');
  ...
  await context.test('explicit D8 publisher source omission', async () => {
    const changed = clone(inventory);
    const group = changed.entries.find(({ id }) => id === 'D5-BROWSER-API-SEAMS');
    const targetIndex = group.resources.findIndex(({ resourceId }) => resourceId === 'D8-HITL-LIFECYCLE-SUPERSEDE-SELECTOR');
    group.resources[targetIndex] = {
      resourceId: 'D8-HITL-LIFECYCLE-SUPERSEDE-SELECTOR',
      owner: 'D8',
      path: 'leecat-board-nextjs/test/contracts/certification-publisher.test-helper.ts',
      exportName: null,
      exportKind: null,
      projectionId: null,
      selector: 'whole-file',
...
test('all committed contract-inventory mutation fixtures are consumed with exact reasons', async (context) => {
  ...
  const names = [
    'duplicate-alias.v1.json', 'owner-swap.v1.json', 'whole-file-projection-overlap.v1.json',
    'overlapping-projections.v1.json', 'missing-owner-publisher.v1.json',
    'publisher-source-omission.v1.json', 'browser-adapter-owner-swap.v1.json',
    'browser-adapter-selector-overlap.v1.json', 'browser-adapter-d8-publisher-omission.v1.json',
    'browser-adapter-d8-member-signature-drift.v1.json',
  ];
  for (const name of names) await context.test(name, async () => {
    ...
    if (fixture.mutation === 'whole-file-d3-outbox') {
      Object.assign(d3.resources.find(({ resourceId }) => resourceId === 'D3-OUTBOX-LOAD-PENDING'), {
        exportName: null, exportKind: null, projectionId: null, selector: 'whole-file',
      });
    }
    if (fixture.mutation === 'duplicate-d3-selector') {
      const source = d3.resources.find(({ resourceId }) => resourceId === 'D3-OUTBOX-LIST-PENDING');
      const target = d3.resources.find(({ resourceId }) => resourceId === 'D3-OUTBOX-LOAD-PENDING');
      target.exportName = source.exportName;
      target.exportKind = source.exportKind;
      target.selector = source.selector;
    }
    if (fixture.mutation === 'omit-d2-publisher-input' || fixture.mutation === 'omit-d8-publisher-input') {
      const owner = fixture.mutation === 'omit-d2-publisher-input' ? 'D2' : 'D8';
      const target = publishers.resources.find(({ resourceId }) => resourceId === `${owner}-BROWSER-PUBLISHER`);
      target.path = 'leecat-board-nextjs/test/contracts/certification-publisher.test-helper.ts';
    }
    if (fixture.mutation === 'omit-d8-source-selector') {
      const target = browser.resources.find(({ resourceId }) => resourceId === 'D8-HITL-LIFECYCLE-SUPERSEDE-SELECTOR');
    ...
    if (fixture.mutation === 'swap-d8-selector-owner') {
      browser.resources.find(({ resourceId }) => resourceId === 'D8-HITL-MUTATION-REQUEST-SELECTOR').owner = 'D7';
    }
    if (fixture.mutation === 'duplicate-d8-selector') {
      const source = browser.resources.find(({ resourceId }) => resourceId === 'D8-HITL-LIFECYCLE-CANCEL-SELECTOR');
      const target = browser.resources.find(({ resourceId }) => resourceId === 'D8-HITL-LIFECYCLE-SUPERSEDE-SELECTOR');
      target.exportName = source.exportName;
      target.exportKind = source.exportKind;
      target.selector = source.selector;
    ...
    if (fixture.mutation === 'replace-d8-selector-with-private-method') {
      const target = browser.resources.find(({ resourceId }) => resourceId === 'D8-HITL-MUTATION-REQUEST-SELECTOR');
      target.selector = 'ClassDeclaration[name=BoardApiClient]/MethodDeclaration[name=emptyMutation]';
    }
    await rejectsCode(
      () => verifyContractManifest({
