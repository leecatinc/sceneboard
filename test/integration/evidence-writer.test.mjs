...
test('evidence writer is append-only, token-bound, bounded, and non-self-referential', async (context) => {
  ...
  await assert.rejects(() => writer.writeRecord(writer.ownerToken, record), (error) => error?.code === 'EVIDENCE_OUTPUT_OWNERSHIP_VIOLATION');
  await assert.rejects(() => writer.writeRecord('wrong-token', { ...record, rowId: 'STATIC-002' }), (error) => error?.code === 'EVIDENCE_OUTPUT_OWNERSHIP_VIOLATION');
  const attachment = await writer.writeAttachment(writer.ownerToken, Buffer.from('safe static summary'), 'text/plain');
...
test('safe command policy rejects npx, shell syntax, production targets, and destructive commands', () => {
  ...
  assert.deepEqual(assertSafeCommand({ command: 'npm', args: ['run', 'verify:contracts'], workspaceRoot: root }), {
    command: 'npm', args: ['run', 'verify:contracts'],
  });
  for (const input of [
