...
import {
  ...
  validatePairInput,
  writeCredential,
} from '../../skills/sceanboard/scripts/sceneboard-api-core.mjs';
...
const CAPABILITIES = {
  ...
  supported: {
    nodeTypes: ['layout.split', 'layout.grid', 'layout.tabs', 'layout.canvas', 'content.markdown', 'content.code', 'content.table', 'content.chart', 'content.map', 'content.drawing', 'content.status', 'content.image', 'content.progress', 'content.hitl', 'content.artifact'],
    commandTypes: ['scene.replace', 'scene.clear', 'scene.restore', 'hitl.request', 'hitl.respond', 'artifact.publish', 'artifact.stop'],
    operationTypes: ['board.list', 'board.get', 'board.create', 'board.archive', 'capabilities.get', 'history.list', 'history.get', 'artifact.get', 'hitl.read'],
    eventTypes: ['board.snapshot', 'board.revision.created', 'hitl.updated', 'artifact.status.changed', 'presence.updated', 'stream.resync.required', 'stream.heartbeat', 'stream.error'],
    hitlKinds: ['info', 'choice', 'form', 'confirmation'],
    artifactRequestCapabilities: ['clipboard.write', 'download', 'fullscreen', 'network.fetch'],
  },
  limits: {
    maxEnvelopeBytes: 1_048_576, maxSceneBytes: 786_432, maxSceneDepth: 12, maxSceneNodes: 500,
    maxJsonDepth: 64, maxJsonContainerEntries: 10_000, maxSplitChildren: 12, maxGridColumns: 24,
    maxGridRows: 100, maxGridItems: 200, maxTabs: 20, maxCanvasItems: 200, maxCanvasExtent: 100_000,
    maxTitleChars: 200, maxImageAltChars: 500, maxMarkdownChars: 100_000, maxCodeChars: 200_000,
    maxTableColumns: 50, maxTableRows: 500, maxTableCells: 10_000, maxChartSeries: 32,
    maxChartPoints: 10_000, maxMapFeatures: 5_000, maxDrawingElements: 5_000, maxArtifactResources: 128,
    maxArtifactResourceBytes: 5_242_880, maxArtifactTotalBytes: 10_485_760, maxBoardArtifacts: 100,
    maxBoardArtifactVersions: 1_000, maxBoardArtifactResourceRows: 10_000, maxBoardArtifactChargedBytes: 536_870_912,
    maxHitlOptions: 50, maxHitlFields: 50, maxHitlTextChars: 60_000, maxHitlResponseBytes: 65_536,
    maxPageSize: 100, maxPageCursorChars: 512, maxHitlWaitMs: 30_000,
  },
...
const boardSummary = (revisionId = 'revision_2', revisionNumber = 2) => ({
  boardId: 'board_1', title: 'QA', createdAt: TIME, updatedAt: TIME, archivedAt: null,
...
const boardSnapshot = (revisionId = 'revision_2', revisionNumber = 2) => ({
  ...
  boardId: 'board_1',
  revision: {
    revisionId, revisionNumber, createdAt: TIME, previousRevisionId: revisionNumber === 1 ? null : 'revision_1',
    originType: revisionNumber === 1 ? 'board.create' : 'scene.replace', sourceRevisionId: null,
...
const openHitl = () => ({
  ...
  definition: { kind: 'info', title: 'Continue?', body: 'Review the result.', acknowledgeLabel: 'OK' },
  state: 'open',
  createdAt: TIME,
...
const response = (value, { requestId = null, status = 200, connection = false } = {}) => new Response(
...
const operationEnvelope = (requestId, type, data, { replayed = false, mutationBoardId = null } = {}) => ({
  protocolVersion: 1,
  type: 'board.http.success',
  requestId,
  result: mutationBoardId === null ? {
    protocolVersion: 1,
...
const configured = async (prefix) => {
  ...
  const env = { ...process.env, XDG_STATE_HOME: join(root, 'state'), SCENEBOARD_PROFILE: `${prefix}profile` };
...
test('pair input uses the server grant scope catalog order', () => {
  ...
  assert.throws(() => validatePairInput({
    ...input,
...
test('board-scoped connection accepts the closed capabilities contract and rejects missing cache headers', async () => {
  ...
  const fetchImpl = async (url, options) => response(connection(selectedBoard), {
    requestId: options.headers['X-Request-Id'],
    connection: true,
  });
  const result = await invokeProtected('board_connection_status', { boardId: 'board_1' }, { cwd: root, env, fetchImpl });
  ...
  assert.equal((await invokeProtected('board_connection_status', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => response(prerelease, {
      requestId: options.headers['X-Request-Id'], connection: true,
    }),
  })).result.connection.versions.mcpServer, '2.0.0-beta.1+build.7');
  const malformedVersion = connection(selectedBoard);
  malformedVersion.versions.mcpServer = '01.2.3';
  await assert.rejects(invokeProtected('board_connection_status', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => response(malformedVersion, {
      requestId: options.headers['X-Request-Id'], connection: true,
    }),
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  await assert.rejects(
    invokeProtected('board_connection_status', { boardId: 'board_1' }, {
      cwd: root,
      env,
      fetchImpl: async (url, options) => response(connection(selectedBoard), { requestId: options.headers['X-Request-Id'] }),
    }),
    { code: 'BOARD_API_RESPONSE_INVALID' },
  );
  ...
  await assert.rejects(invokeProtected('board_connection_status', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => response(connection(incomplete), {
      requestId: options.headers['X-Request-Id'], connection: true,
    }),
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  ...
  await assert.rejects(invokeProtected('board_connection_status', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => response(connection(extra), {
      requestId: options.headers['X-Request-Id'], connection: true,
    }),
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  ...
  await assert.rejects(invokeProtected('board_connection_status', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => response(secretClient, {
      requestId: options.headers['X-Request-Id'], connection: true,
    }),
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  ...
  await assert.rejects(invokeProtected('board_connection_status', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => response(connection(secretTitle), {
      requestId: options.headers['X-Request-Id'], connection: true,
    }),
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  ...
  await assert.rejects(invokeProtected('board_connection_status', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => response(controlClient, {
      requestId: options.headers['X-Request-Id'], connection: true,
    }),
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
});
...
test('connection and error projections reject non-string global and local identifiers', async () => {
  ...
  for (const invalidId of [null, 7, [], {}]) {
    ...
    await assert.rejects(invokeProtected('board_connection_status', { boardId: null }, {
      cwd: root,
      env,
      fetchImpl: async (url, options) => response(invalidConnection, {
        requestId: options.headers['X-Request-Id'], connection: true,
      }),
    }), { code: 'BOARD_API_RESPONSE_INVALID' });
    ...
    await assert.rejects(requestJson({
      ...
      path: '/board', requestId, expectedStatus: [200], allowedErrorCodes: ['DUPLICATE_NODE_ID'],
      fetchImpl: async () => new Response(JSON.stringify({ error: {
        protocolVersion: 1, type: 'board.error', code: 'DUPLICATE_NODE_ID', message: 'Duplicate',
        category: 'validation', retryable: false, httpStatusHint: 422,
        details: { nodeId: invalidId, firstPath: ['root'], duplicatePath: ['root', 1] },
      } }), { status: 422, headers: { 'content-type': 'application/json; charset=utf-8', 'x-request-id': requestId } }),
    }), { code: 'BOARD_API_RESPONSE_INVALID' });
  }
});
...
test('scene patch reports revision conflict before applying operations', async () => {
  ...
  await assert.rejects(invokeProtected('board_scene_patch', {
    boardId: 'board_1',
    expectedRevisionId: 'revision_1',
    idempotencyKey: 'qa-key-1234567890',
    operations: [{ type: 'replace_root', root: null }],
  }, { cwd: root, env, fetchImpl }), (error) => {
    assert.equal(error.code, 'REVISION_CONFLICT');
    assert.deepEqual(error.details, {
      boardId: 'board_1', expectedRevisionId: 'revision_1', actualRevisionId: 'revision_2',
      actualRevisionNumber: 2, recovery: 'fetch_latest_then_retry',
    });
    return true;
  });
...
test('protected results enforce request, status, and board correlation', async () => {
  ...
  const board = await invokeProtected('board_get', { boardId: 'board_1' }, {
    ...
    fetchImpl: async (url, options) => {
      const requestId = options.headers['X-Request-Id'];
      return response(operationEnvelope(requestId, 'board.get', { board: boardSummary(), snapshot: boardSnapshot() }), { requestId });
    },
  });
  assert.equal(board.result.result.board.boardId, 'board_1');
  const shortTextBoard = boardSummary();
  shortTextBoard.title = 'Line one\nLine two';
  const shortTextResult = await invokeProtected('board_get', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => {
      const requestId = options.headers['X-Request-Id'];
      return response(operationEnvelope(requestId, 'board.get', { board: shortTextBoard, snapshot: boardSnapshot() }), { requestId });
    },
  });
  assert.equal(shortTextResult.result.result.board.title, 'Line one\nLine two');
  await assert.rejects(invokeProtected('board_create', { title: 'QA', idempotencyKey: 'qa-key-1234567890' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => {
      const requestId = options.headers['X-Request-Id'];
      return response(operationEnvelope(requestId, 'board.create', { board: { boardId: 'board_1' } }), { requestId, status: 201 });
    },
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  await assert.rejects(invokeProtected('board_scene_clear', {
    boardId: 'board_1', expectedRevisionId: 'revision_1', idempotencyKey: 'qa-key-1234567890',
  }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => {
      const requestId = options.headers['X-Request-Id'];
      return response(operationEnvelope(requestId, 'scene.clear', {}, { mutationBoardId: 'board_2' }), { requestId });
    },
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  const crossBoard = boardSnapshot();
  crossBoard.boardId = 'board_2';
  await assert.rejects(invokeProtected('board_get', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => {
      const requestId = options.headers['X-Request-Id'];
      return response(operationEnvelope(requestId, 'board.get', { board: boardSummary(), snapshot: crossBoard }), { requestId });
    },
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  const secretBearing = boardSnapshot();
  secretBearing.scene.root = { id: 'note', type: 'content.markdown', markdown: `prefix ${TOKEN} suffix` };
  await assert.rejects(invokeProtected('board_get', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => {
      const requestId = options.headers['X-Request-Id'];
      return response(operationEnvelope(requestId, 'board.get', { board: boardSummary(), snapshot: secretBearing }), { requestId });
    },
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
});
...
test('HITL projections are exact, correlated, chronological, and context-secret-free', async () => {
  ...
  const invoke = (hitl) => invokeProtected('board_get', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => {
      ...
      snapshot.hitl = [hitl];
      const requestId = options.headers['X-Request-Id'];
      return response(operationEnvelope(requestId, 'board.get', { board: boardSummary(), snapshot }), { requestId });
    },
  });
  ...
  unknownChoice.definition = {
    kind: 'choice', title: 'Choose', multiple: false, minSelections: 1, maxSelections: 1,
  ...
  contextualSecret.definition = {
    kind: 'form', title: 'Form', submitLabel: 'Submit',
    fields: [{ id: 'proof', type: 'text', label: 'Value', required: true, defaultValue: null, minLength: 1, maxLength: 100 }],
  };
  ...
  sensitiveSelect.definition.fields[0] = {
    id: 'token', type: 'select', label: 'Token', required: false,
    defaultValue: 'g'.repeat(22), options: [{ id: 'g'.repeat(22), label: 'Ordinary option' }],
  };
  ...
  assert.equal((await invoke(ordinaryPasswordField)).result.result.snapshot.hitl[0].response.values.password, 'ordinary answer');

  const tableSnapshot = boardSnapshot();
  tableSnapshot.scene.root = {
    id: 'table', type: 'content.table',
    columns: [{ key: 'token', label: 'Design token', valueType: 'string' }],
    rows: [{ id: 'row', cells: { token: 'primary-color' } }],
  };
  const table = await invokeProtected('board_get', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => {
      const requestId = options.headers['X-Request-Id'];
      return response(operationEnvelope(requestId, 'board.get', { board: boardSummary(), snapshot: tableSnapshot }), { requestId });
    },
  });
  ...
  nestedScene.scene.root = {
    id: 'map', type: 'content.map', viewport: { longitude: 0, latitude: 0, zoom: 1 },
    data: {
      type: 'FeatureCollection',
      features: [{
        type: 'Feature',
        properties: { password: { value: 'ordinary answer' }, token: [{ value: 'primary-color' }] },
        geometry: { type: 'Point', coordinates: [0, 0] },
      }],
    },
  };
  const invokeScene = () => invokeProtected('board_get', { boardId: 'board_1' }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => {
      const requestId = options.headers['X-Request-Id'];
      return response(operationEnvelope(requestId, 'board.get', {
        board: boardSummary(), snapshot: nestedScene,
      }), { requestId });
    },
  });
  assert.equal((await invokeScene()).result.result.snapshot.scene.root.data.features[0]
    .properties.password.value, 'ordinary answer');
...
test('pairing parsers enforce exact finite-state and grant contracts', () => {
  const claim = parsePairingClaim({ pairingId: 'pairing_1', state: 'pending', decisionExpiresAt: TIME, pollAfterSeconds: 2 });
  ...
  }, claim.pairingId).retryAfterSeconds, 5);
  assert.throws(() => parsePairingStatus({
    pairingId: claim.pairingId,
    state: 'pending',
    retryAfterSeconds: 0,
    decisionExpiresAt: TIME,
    redeemExpiresAt: null,
  }, claim.pairingId), { code: 'BOARD_API_RESPONSE_INVALID' });
  ...
  for (const mutate of [
    ...
    (value) => { value.grant.revokedAt = TIME; },
    (value) => { value.grant.scopes = []; },
    (value) => { value.grant.client.clientName = TOKEN; },
  ]) {
...
test('strict stdin and failure projection reject duplicate JSON and redact secrets', () => {
  assert.throws(() => parseApiInputBytes(Buffer.from('{"boardId":"a","boardId":"b"}')), { code: 'INVALID_PAYLOAD' });
  const failure = safeFailure(new SceneBoardApiError('INTERNAL_ERROR', 'synthetic', {
    details: { authorization: `Bearer ${TOKEN}`, path: '/workspace/private/example', alternatePath: '/opt/private/example' },
  }), 'board_list');
...
test('board errors are operation-allowlisted, exactly projected, and secret-free', async () => {
  const requestId = 'request_1234567890';
  const invalid = (error) => new Response(JSON.stringify({ error }), {
  ...
  const capabilityDenied = {
    protocolVersion: 1, type: 'board.error', code: 'CAPABILITY_DENIED', message: 'Denied', category: 'auth',
    retryable: false, httpStatusHint: 403, details: { capability: 'artifact.publish' },
  };
  await assert.rejects(requestJson({
    config: { baseUrl: 'http://127.0.0.1:3411', timeoutMs: 1_000 }, path: '/board', requestId,
    expectedStatus: [200], allowedErrorCodes: ['INVALID_PAYLOAD', 'BOARD_NOT_FOUND'],
    fetchImpl: async () => invalid(capabilityDenied),
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  const malformed = {
    protocolVersion: 1, type: 'board.error', code: 'INVALID_PAYLOAD', message: 'Invalid', category: 'not-validation',
    retryable: false, httpStatusHint: 400, details: { path: '/private/example', issue: `prefix ${TOKEN} suffix` },
  };
  await assert.rejects(requestJson({
    config: { baseUrl: 'http://127.0.0.1:3411', timeoutMs: 1_000 }, path: '/board', requestId,
    expectedStatus: [200], allowedErrorCodes: ['INVALID_PAYLOAD'], fetchImpl: async () => invalid(malformed),
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  const secretPath = {
    protocolVersion: 1, type: 'board.error', code: 'INVALID_PAYLOAD', message: 'Invalid', category: 'validation',
    retryable: false, httpStatusHint: 400, details: { path: [TOKEN], issue: 'invalid field' },
  };
  await assert.rejects(requestJson({
    config: { baseUrl: 'http://127.0.0.1:3411', timeoutMs: 1_000 }, path: '/board', requestId,
    expectedStatus: [200], allowedErrorCodes: ['INVALID_PAYLOAD'], fetchImpl: async () => invalid(secretPath),
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  const longUnknown = {
    protocolVersion: 1, type: 'board.error', code: 'UNKNOWN_NODE_TYPE', message: 'Unknown node type', category: 'validation',
    retryable: false, httpStatusHint: 422, details: { path: Array.from({ length: 65 }, (_, index) => index), receivedType: 'x'.repeat(201) },
  };
  await assert.rejects(requestJson({
    config: { baseUrl: 'http://127.0.0.1:3411', timeoutMs: 1_000 }, path: '/board', requestId,
    expectedStatus: [200], allowedErrorCodes: ['UNKNOWN_NODE_TYPE'], fetchImpl: async () => invalid(longUnknown),
  }), { code: 'UNKNOWN_NODE_TYPE' });
  const largeProtocolMajor = {
    protocolVersion: 1, type: 'board.error', code: 'PROTOCOL_VERSION_MISMATCH', message: 'Protocol mismatch', category: 'protocol',
    retryable: false, httpStatusHint: 409,
    details: { reason: 'major', supportedMajor: 1, receivedMajor: 9_007_199_254_740_992, field: 'protocolVersion' },
  };
  await assert.rejects(requestJson({
    config: { baseUrl: 'http://127.0.0.1:3411', timeoutMs: 1_000 }, path: '/board', requestId,
    expectedStatus: [200], allowedErrorCodes: ['PROTOCOL_VERSION_MISMATCH'], fetchImpl: async () => invalid(largeProtocolMajor),
  }), { code: 'PROTOCOL_VERSION_MISMATCH' });
  const { root, env } = await configured('operation-errors-');
  const errorResponse = async (url, options) => new Response(JSON.stringify({ error: capabilityDenied }), {
    status: 403,
    headers: { 'content-type': 'application/json; charset=utf-8', 'x-request-id': options.headers['X-Request-Id'] },
  });
  await assert.rejects(invokeProtected('board_get', { boardId: 'board_1' }, {
    cwd: root, env, fetchImpl: errorResponse,
  }), { code: 'BOARD_API_RESPONSE_INVALID' });
  await assert.rejects(invokeProtected('board_artifact_put', {
    boardId: 'board_1', expectedRevisionId: 'revision_1', idempotencyKey: 'qa-key-1234567890',
    artifactId: null, html: '<main>QA</main>', css: null, javascript: null, requestedCapabilities: [],
  }, { cwd: root, env, fetchImpl: errorResponse }), { code: 'CAPABILITY_DENIED' });
});
...
test('credential invalidation lock contention preserves the auth result and truthful token presence', async () => {
  ...
  const unauthenticated = async (url, options) => new Response(JSON.stringify({ error: {
    protocolVersion: 1, type: 'board.error', code: 'UNAUTHENTICATED', message: 'Unauthenticated',
    category: 'auth', retryable: false, httpStatusHint: 401, details: null,
  } }), { status: 401, headers: {
    'content-type': 'application/json; charset=utf-8', 'x-request-id': options.headers['X-Request-Id'],
    'cache-control': 'no-store, private', pragma: 'no-cache', vary: 'Origin, Cookie, Authorization',
  } });
  await assert.rejects(invokeProtected('board_list', { cursor: null, limit: 10, includeArchived: false }, {
    cwd: root, env, fetchImpl: unauthenticated,
  }), { code: 'UNAUTHENTICATED' });
  const status = await invokeProtected('board_connection_status', { boardId: null }, {
    cwd: root, env, fetchImpl: unauthenticated,
  });
...
test('retry delay that exceeds the remaining deadline is reported as timeout', async () => {
  ...
  await assert.rejects(requestJson({
    config: { baseUrl: 'http://127.0.0.1:3411', timeoutMs: 1_000 },
    path: '/busy', requestId, expectedStatus: [200], allowedErrorCodes: ['SERVICE_UNAVAILABLE'], retryKind: 'read',
    fetchImpl: async () => new Response(JSON.stringify({ error: {
      protocolVersion: 1, type: 'board.error', code: 'SERVICE_UNAVAILABLE', message: 'Busy',
      category: 'availability', retryable: true, httpStatusHint: 503, details: { retryAfterSeconds: 2 },
    } }), { status: 503, headers: {
      'content-type': 'application/json; charset=utf-8', 'x-request-id': requestId, 'retry-after': '2',
    } }),
  }), { code: 'BOARD_API_TIMEOUT' });
...
test('scene patch shares one monotonic timeout across head fetch and mutation', async () => {
  ...
  const startedAt = Date.now();
  await assert.rejects(invokeProtected('board_scene_patch', {
    boardId: 'board_1', expectedRevisionId: 'revision_2', idempotencyKey: 'qa-key-1234567890',
    operations: [{ type: 'replace_root', root: null }],
  }, {
    cwd: root,
    env,
    fetchImpl: async (url, options) => {
      ...
      return new Promise((resolve, reject) => {
        const keeper = setTimeout(() => reject(new Error('operation deadline did not abort')), 2_000);
...
test('credential state repairs directory mode and serializes generation invalidation', async () => {
  ...
  const env = { ...process.env, XDG_STATE_HOME: join(root, 'state'), SCENEBOARD_PROFILE: 'credential_race' };
  const config = await resolveApiConfig({ cwd: root, env });
...
test('patch geometry validates every supplied numeric field', () => {
  const scene = {
    ...
    root: {
      ...
      columns: 2,
      rows: 2,
      children: [{ node: { id: 'child', type: 'content.status', tone: 'info', text: 'QA' }, column: 1, row: 1, columnSpan: 1, rowSpan: 1 }],
    },
  };
  assert.throws(() => applyScenePatch(scene, [{
    type: 'set_grid_placement', gridNodeId: 'grid', childNodeId: 'child', column: 1, row: '2', columnSpan: 1, rowSpan: 1,
  }]), { code: 'INVALID_PAYLOAD' });
});
...
test('one-process pairing keeps proof private and proves the stored credential', async () => {
  ...
  const server = createServer(async (request, reply) => {
    ...
    const send = (statusCode, value, kind) => {
      ...
      if (request.headers['x-request-id'] !== undefined) reply.setHeader('x-request-id', request.headers['x-request-id']);
    ...
    if (url.pathname === '/api/v1/pairings/claim') {
      ...
      send(202, { pairingId: 'pairing_1', state: 'pending', decisionExpiresAt: '2026-07-17T12:05:00.000Z', pollAfterSeconds: 2 }, 'claim');
      return;
    }
    if (url.pathname.endsWith('/client-status')) {
      proof = String(request.headers.authorization).replace('PairingProof ', '');
      send(200, {
        pairingId: 'pairing_1', state: 'approved', retryAfterSeconds: null,
        decisionExpiresAt: '2026-07-17T12:05:00.000Z', redeemExpiresAt: '2026-07-17T12:07:00.000Z',
      }, 'status');
      return;
    }
    if (url.pathname.endsWith('/redeem')) {
      send(200, {
        ...
        grant: {
          grantId: 'grant_1',
          client: { clientId: 'client_1', clientName: 'SceneBoard QA', installationFingerprint: 'abcdefghijklmnop' },
          scopes: ['board.read'], lifecyclePermissions: [], boardIds: ['board_1'], lifetime: 'persistent', status: 'active',
          createdAt: TIME, activatedAt: TIME, lastUsedAt: null, expiresAt: '2026-08-17T12:00:00.000Z', revokedAt: null,
        },
      }, 'redeem');
      return;
    }
  ...
  const child = spawn(process.execPath, [join(process.cwd(), 'skills/sceanboard/scripts/sceneboard-api.mjs'), 'pair'], {
    cwd: root,
    env: {
      ...process.env,
  ...
  assert.equal(createHash('sha256').update(Buffer.from(proof, 'base64url')).digest('base64url'), challenge);
  const credentialPath = join(stateRoot, 'leecat-board', 'credentials', 'qa_pair', 'credential.json');
  assert.equal((await stat(credentialPath)).mode & 0o777, 0o600);
  assert.equal(JSON.parse(await readFile(credentialPath, 'utf8')).accessToken, TOKEN);
  assert.deepEqual(output.trim().split('\n').map(JSON.parse).map((event) => event.event), ['claimed', 'status', 'redeemed']);
});
...
test('pairing rejects contradictory authorization before credential persistence', async () => {
  ...
  const server = createServer(async (request, reply) => {
    ...
    if (url.pathname.endsWith('/claim')) {
      send(202, { pairingId: 'pairing_mismatch', state: 'pending', decisionExpiresAt, pollAfterSeconds: 2 }, 'claim');
    } else if (url.pathname.endsWith('/client-status')) {
      send(200, { pairingId: 'pairing_mismatch', state: 'approved', retryAfterSeconds: null,
        decisionExpiresAt, redeemExpiresAt }, 'status');
    } else if (url.pathname.endsWith('/redeem')) {
      send(200, { tokenType: 'Bearer', accessToken: TOKEN, grant: {
        grantId: 'grant_1',
        client: { clientId: 'client_1', clientName: 'SceneBoard QA', installationFingerprint: 'abcdefghijklmnop' },
        scopes: ['board.read'], lifecyclePermissions: [], boardIds: ['board_1'], lifetime: 'persistent', status: 'active',
        createdAt: TIME, activatedAt: TIME, lastUsedAt: null, expiresAt: '2026-08-17T12:00:00.000Z', revokedAt: null,
...
test('ambiguous redeem retry returns one owner-recovery failure and never redeems a third time', async () => {
  ...
  const server = createServer(async (request, reply) => {
    ...
    if (url.pathname.endsWith('/claim')) {
      send(202, { pairingId: 'pairing_ambiguous', state: 'pending', decisionExpiresAt: expiresAt, pollAfterSeconds: 2 }, 'claim');
      return;
    }
    if (url.pathname.endsWith('/client-status')) {
      statusCalls += 1;
      send(200, { pairingId: 'pairing_ambiguous', state: 'approved', retryAfterSeconds: null,
        decisionExpiresAt: expiresAt, redeemExpiresAt: expiresAt }, 'status');
      return;
    }
    if (url.pathname.endsWith('/redeem')) {
      redeemCalls += 1;
