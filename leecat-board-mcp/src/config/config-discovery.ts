...
const assertSafeFile = async (path: string, source: SafeConfigSourceV1, effectiveUserId?: number): Promise<void> => {
  let status;
  try {
  ...
  if (effectiveUserId !== undefined && status.uid !== effectiveUserId) throw new BoardConfigError(source, null);
};
...
export const discoverBoardConfigV1 = async (options: BoardConfigDiscoveryOptionsV1): Promise<LoadedBoardConfigV1> => {
  ...
  if (userPath !== null && await existing(userPath)) return loadFile(userPath, 'user_config_file', effectiveUserId);
  ...
  const config = parseBoardConfigV1({
    ...
    accessTokenRef: options.env.BOARD_ACCESS_TOKEN_REF ?? 'env://LEECAT_BOARD_ACCESS_TOKEN',
    authScheme: 'bearer',
