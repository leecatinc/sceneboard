...
export type SecretReferenceResolutionV1 =
  | { kind: 'environment'; variable: 'LEECAT_BOARD_ACCESS_TOKEN' }
...
export const resolveSecretReferenceV1 = (
  ...
  if (config.accessTokenRef === 'env://LEECAT_BOARD_ACCESS_TOKEN') {
    return { kind: 'environment', variable: 'LEECAT_BOARD_ACCESS_TOKEN' };
  }
  if (env.LEECAT_BOARD_ACCESS_TOKEN !== undefined && env.LEECAT_BOARD_ACCESS_TOKEN !== '') {
