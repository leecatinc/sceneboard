...
export type BoardConfigFileV1 = {
  version: 1;
  baseUrl: string;
  accessTokenRef: 'env://LEECAT_BOARD_ACCESS_TOKEN' | `store://${string}`;
  authScheme: 'bearer';
  timeoutMs: number;
  profile: string;
};
...
const canonicalBaseUrl = (value: unknown, source: SafeConfigSourceV1): string => {
  if (typeof value !== 'string' || value.length === 0 || value.length > 2_048
    || /[<>${}\s]/.test(value) || /example\.(?:com|org|net)$/i.test(value)) {
...
export const parseBoardConfigV1 = (value: unknown, source: SafeConfigSourceV1): BoardConfigFileV1 => {
  ...
  if (!Number.isSafeInteger(value.timeoutMs) || Number(value.timeoutMs) < 1_000 || Number(value.timeoutMs) > 120_000) {
  ...
  if (rawAccessTokenRef !== 'env://LEECAT_BOARD_ACCESS_TOKEN' && rawAccessTokenRef !== `store://${value.profile}`) {
    throw new BoardConfigError(source, 'accessTokenRef');
  }
  const accessTokenRef: BoardConfigFileV1['accessTokenRef'] = rawAccessTokenRef === 'env://LEECAT_BOARD_ACCESS_TOKEN'
    ? rawAccessTokenRef
    : `store://${value.profile}`;
  return {
    version: 1,
