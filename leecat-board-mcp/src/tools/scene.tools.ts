...
import { applySceneTransformV1, type SceneTransformOperationV1 } from '@leecat-board/board-sdk/scene-transform';

import { ProtectedBoardGatewayV1 } from './protected-board.gateway.js';
import { createRequestIdV1, GlobalIdSchemaV1, IdempotencyKeySchemaV1, SceneTransformOperationSchemaV1 } from './tool-schemas.js';
import { notConnectedV1, sdkToolResultV1, toolFailureV1, validationFailureV1 } from './tool-result.js';

export const SceneGetInputSchemaV1 = z.object({ boardId: GlobalIdSchemaV1, revisionId: GlobalIdSchemaV1.nullable() }).strict();
export const SceneReplaceInputSchemaV1 = z.object({ boardId: GlobalIdSchemaV1, expectedRevisionId: GlobalIdSchemaV1, idempotencyKey: IdempotencyKeySchemaV1, scene: z.unknown() }).strict();
export const ScenePatchInputSchemaV1 = z.object({ boardId: GlobalIdSchemaV1, expectedRevisionId: GlobalIdSchemaV1, idempotencyKey: IdempotencyKeySchemaV1, operations: z.array(SceneTransformOperationSchemaV1).min(1).max(1_000) }).strict();
export const SceneClearInputSchemaV1 = z.object({ boardId: GlobalIdSchemaV1, expectedRevisionId: GlobalIdSchemaV1, idempotencyKey: IdempotencyKeySchemaV1 }).strict();
...
export class SceneToolHandlersV1 {
  ...
  async get(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    if (parsed.data.revisionId === null) {
      ...
      return result.connected
        ...
        : toolFailureV1('board_scene_get', requestId, 'mcp', notConnectedV1() as unknown as Record<string, unknown>);
    }
    const result = await this.gateway.call((client) => client.getHistory({
      protocolVersion: 1,
    ...
    return result.connected
      ? sdkToolResultV1('board_scene_get', requestId, result.value, result.value.ok ? { type: 'history', history: result.value.metadata.history } : null)
      : toolFailureV1('board_scene_get', requestId, 'mcp', notConnectedV1() as unknown as Record<string, unknown>);
  }
  ...
  async replace(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    if (!scene.ok) return toolFailureV1('board_scene_replace', requestId, 'board', scene.error as unknown as Record<string, unknown>);
    ...
    return result.connected
      ...
      : toolFailureV1('board_scene_replace', requestId, 'mcp', notConnectedV1() as unknown as Record<string, unknown>);
  }
  ...
  async patch(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    const head = await this.gateway.call((client) => client.getBoard({
      protocolVersion: 1,
      requestId: requestId as RequestId,
      type: 'board.get',
      boardId: parsed.data.boardId as BoardId,
    }, signal));
    if (!head.connected) return toolFailureV1('board_scene_patch', requestId, 'mcp', notConnectedV1() as unknown as Record<string, unknown>);
    ...
    const transformed = applySceneTransformV1(snapshot.snapshot.scene, parsed.data.operations as SceneTransformOperationV1[]);
    if (!transformed.ok) return toolFailureV1('board_scene_patch', requestId, 'board', transformed.error as unknown as Record<string, unknown>);
    const result = await this.gateway.call((client) => client.mutateBoard({
      protocolVersion: 1,
      requestId: requestId as RequestId,
      boardId: parsed.data.boardId as BoardId,
    ...
    return result.connected
      ? sdkToolResultV1('board_scene_patch', requestId, result.value, { type: 'scene-transform', transformedFromRevisionId: snapshot.snapshot.revision.revisionId })
      : toolFailureV1('board_scene_patch', requestId, 'mcp', notConnectedV1() as unknown as Record<string, unknown>);
  }
  ...
  async clear(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    return result.connected
      ...
      : toolFailureV1('board_scene_clear', requestId, 'mcp', notConnectedV1() as unknown as Record<string, unknown>);
  }
}
