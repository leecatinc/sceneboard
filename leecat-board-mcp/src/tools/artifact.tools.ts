...
const SourceTextSchemaV1 = z.string().refine(
  (value) => !/[\uD800-\uDFFF]/u.test(value),
  'lone surrogate is not allowed',
);
...
export const ArtifactGetInputSchemaV1 = z.object({
  boardId: GlobalIdSchemaV1,
  artifactId: GlobalIdSchemaV1,
  versionId: GlobalIdSchemaV1,
}).strict();

export const ArtifactPutInputSchemaV1 = z.object({
  boardId: GlobalIdSchemaV1,
...
export const ArtifactStopInputSchemaV1 = z.object({
  boardId: GlobalIdSchemaV1,
  expectedRevisionId: GlobalIdSchemaV1,
  idempotencyKey: IdempotencyKeySchemaV1,
  artifactId: GlobalIdSchemaV1,
...
export class ArtifactToolHandlersV1 {
  ...
  async get(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    const result = await this.gateway.call((client) => client.getArtifact({
      protocolVersion: 1,
    ...
    return result.connected
      ...
      : toolFailureV1('board_artifact_get', requestId, 'mcp', notConnectedV1() as unknown as Record<string, unknown>);
  }
  ...
  async put(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    const result = await this.gateway.call((client) => client.putArtifact(
    ...
    return result.connected
      ...
      : toolFailureV1('board_artifact_put', requestId, 'mcp', notConnectedV1() as unknown as Record<string, unknown>);
  }
  ...
  async stop(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    return result.connected
      ...
      : toolFailureV1('board_artifact_stop', requestId, 'mcp', notConnectedV1() as unknown as Record<string, unknown>);
  }
}
