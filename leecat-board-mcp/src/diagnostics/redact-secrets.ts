const SECRET_KEY = /(authorization|token|proof|challenge|code|password|cookie|secret|path|generation)/i;
...
export const redactSecretsV1 = (value: unknown, depth = 0): unknown => {
  ...
  if (typeof value === 'string') {
    return value.replace(TOKEN_PATTERN, '[REDACTED]').replace(PAIRING_PROOF_PATTERN, 'PairingProof [REDACTED]');
  }
