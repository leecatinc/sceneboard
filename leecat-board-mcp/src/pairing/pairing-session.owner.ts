...
import { ProfileLeaseErrorV1, type ProfileStateLeaseV1 } from '../credentials/profile-state.lease.js';
import {
  PairingHttpClientV1,
  type PairingClaimResponseV1,
  type PairingClientStatusV1,
  type PairingHttpResultV1,
...
export interface PairingCoordinatorPortV1 {
  request(input: PairRequestInputV1, signal?: AbortSignal): Promise<PairingCoordinatorResultV1<BoardPairRequestResultV1>>;
  status(pairingId: string, waitTimeoutMs: number, signal?: AbortSignal): Promise<PairingCoordinatorResultV1<BoardPairStatusResultV1>>;
  close(): Promise<void>;
}
...
export type AuthorizedConnectionProbeV1 = (accessToken: string, signal?: AbortSignal) => Promise<boolean>;
...
export class PairingSessionOwnerV1 implements PairingCoordinatorPortV1 {
  ...
  async request(input: PairRequestInputV1, signal?: AbortSignal): Promise<PairingCoordinatorResultV1<BoardPairRequestResultV1>> {
    if (this.session !== null) {
      return { ok: false, source: 'local', error: { code: 'PROFILE_BUSY', reason: 'active_owner' } };
    }
    if (!await this.leases.verify()) return { ok: false, source: 'local', error: { code: 'PAIRING_SINK_UNAVAILABLE' } };
    try {
    ...
    try {
      ...
      return { ok: false, source: 'local', error: { code: 'PROFILE_BUSY', reason: 'liveness_unknown' } };
    }
    try {
  ...
  async status(
    ...
    while (true) {
      ...
      if (!result.ok) {
        if (result.source === 'pairing' && result.error.code === 'PAIRING_PROOF_INVALID') await this.teardown(session);
        return result;
      }
