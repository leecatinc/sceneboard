...
export type ConnectionHttpLocalErrorV1 =
  ...
  | { code: 'RESPONSE_INVALID'; retryable: false; reason: 'status' | 'content_type' | 'utf8' | 'json' | 'duplicate_member' | 'schema' | 'correlation' | 'body_too_large' };
...
export type ConnectionHttpClientOptionsV1 = {
  ...
  fetch: typeof fetch;
  timeoutMs: number;
  logger: { log(event: { route: string; attempt: number; durationMs: number; requestId: string; resultCode: string }): void };
};
...
const exactCatalogSubset = <Value extends string>(value: unknown, catalog: readonly Value[], minimum: number): Value[] | null => {
...
const parseConnection = (value: unknown, requestId: string, boardId: string | null): SafeAuthorizedConnectionV1 | null => {
  ...
  const grant = exactRecord(root.grant, [
    'grantId', 'client', 'scopes', 'lifecyclePermissions', 'boardIds', 'lifetime', 'status', 'activatedAt', 'expiresAt',
  ]);
  const versions = exactRecord(root.versions, ['mcpServer', 'boardProtocol', 'api']);
  if (principal === null || grant === null || versions === null || principal.principalKind !== 'mcp_client'
    || versions.mcpServer !== '0.0.0' || versions.boardProtocol !== '1.0.0' || versions.api !== 'v1') return null;
  ...
  if (!parsedPrincipalId.ok || !parsedPrincipalGrantId.ok || !parsedGrantId.ok || client === null || clientId === null
    || parsedPrincipalGrantId.data.value !== parsedGrantId.data.value || parsedPrincipalId.data.value !== clientId
    || typeof client.clientName !== 'string' || [...client.clientName].length < 1 || [...client.clientName].length > 100
    || /[\u0000-\u001f\u007f-\u009f\uD800-\uDFFF]/u.test(client.clientName)
    || typeof client.installationFingerprint !== 'string' || !/^[A-Za-z0-9_-]{16}$/.test(client.installationFingerprint)) return null;
  ...
  if (root.selectedBoard !== null) {
    ...
    if (selected === null || !['online', 'offline', 'unknown'].includes(String(selected.browserPresence))) return null;
    ...
    if (!boardResult.ok || boardResult.data.value.result.type !== 'board.list' || !capabilities.ok) return null;
    const board = boardResult.data.value.result.boards[0];
    if (board === undefined || board.boardId !== boardId || !parsedBoardIds.includes(board.boardId)) return null;
    selectedBoard = {
      board,
...
const local = (error: ConnectionHttpLocalErrorV1): ConnectionHttpResultV1 => ({ ok: false, source: 'local', error });
...
export class ConnectionHttpClientV1 {
  ...
  async get(
    ...
    if (!TOKEN_PATTERN.test(accessToken)) return local({ code: 'TRANSPORT_ERROR', retryable: true, phase: 'connect' });
    ...
    try {
      ...
      if (timeoutSignal.aborted) return local({ code: 'TIMEOUT', retryable: true, timeoutMs: this.options.timeoutMs });
    ...
    if (response.headers.get('cache-control') !== 'no-store, private' || response.headers.get('pragma') !== 'no-cache'
    ...
    if (bytes === 'body_too_large') return local({ code: 'RESPONSE_INVALID', retryable: false, reason: 'body_too_large' });
    if (bytes === 'response') {
      if (outerSignal?.aborted) return local({ code: 'CANCELLED', retryable: false });
      if (timeoutSignal.aborted) return local({ code: 'TIMEOUT', retryable: true, timeoutMs: this.options.timeoutMs });
    ...
    if (!parsed.ok) return local({ code: 'RESPONSE_INVALID', retryable: false, reason: parsed.reason });
    if (response.status === 200) {
      const value = parseConnection(parsed.value, requestId, boardId);
      if (value === null) return local({ code: 'RESPONSE_INVALID', retryable: false, reason: 'schema' });
      this.options.logger.log({ route: '/api/v1/mcp/connection', attempt: 1, durationMs: performance.now() - startedAt, requestId, resultCode: 'connected' });
    ...
    this.options.logger.log({ route: '/api/v1/mcp/connection', attempt: 1, durationMs: performance.now() - startedAt, requestId, resultCode: error.data.value.code });
