...
const root = resolve(workspaceRoot, 'skills/sceanboard');
...
test('downloadable sceanboard skill matches the exact terminal tool and authority contracts', async () => {
  const skill = await readFile(join(root, 'SKILL.md'), 'utf8');
  const references = Object.fromEntries(await Promise.all(referenceNames.map(async (name) => (
    [name, await readFile(join(root, 'references', name), 'utf8')] as const
  ))));
  const combined = `${skill}\n${Object.values(references).join('\n')}`;

  for (const name of BOARD_TOOL_NAMES_V1) assert.equal(combined.includes(`\`${name}\``), true, name);
  ...
  assert.match(skill, /board_interaction_cancel.*do not exist|board_interaction_cancel.*does not exist/u);
  ...
  assert.match(references['scene-contract.md'] ?? '', /packages\/board-sdk\/scene-transform|board_scene_patch/u);
  ...
  assert.doesNotMatch(combined, /server confirms exactly one active writable board|the only active board/u);
  assert.doesNotMatch(combined, /Redis owns[^\n]*pairing state|Redis[^\n]*pairing TTL/u);
  assert.doesNotMatch(combined, /Use transient patches|Default `checkpoint`|common fields \+ optional `message`/u);
  assert.doesNotMatch(combined, /shared DTOs, scene\/layout\/block schemas|Keep a stable `blockId`/u);
});
