...
test('approval redeems once, atomically commits the token, probes it, and returns only a safe grant', async () => {
  ...
  const leaseProvider = new ProfileLeaseProviderV1(new LinuxProfileLeaseHelperAdapterV1(helper, digest));
  const requests: Request[] = [];
  const queue = [
    new Response(JSON.stringify({ pairingId: 'pairing_1', state: 'pending', decisionExpiresAt: '2026-07-16T17:00:00.000Z', pollAfterSeconds: 2 }), { status: 202, headers: responseHeaders() }),
    new Response(JSON.stringify({ pairingId: 'pairing_1', state: 'approved', retryAfterSeconds: null, decisionExpiresAt: '2026-07-16T17:00:00.000Z', redeemExpiresAt: '2026-07-16T17:15:00.000Z' }), { status: 200, headers: responseHeaders('Authorization') }),
  ...
  const owner = new PairingSessionOwnerV1(
    ...
    leaseProvider,
    (proof) => new PairingHttpClientV1({
      baseUrl: 'http://127.0.0.1:3001', timeoutMs: 30_000, proofHeaderProvider: proof,
  ...
  assert.deepEqual(requested.ok && requested.value, {
    pairingId: 'pairing_1', state: 'pending', decisionExpiresAt: '2026-07-16T17:00:00.000Z', pollAfterSeconds: 2, hasToken: false,
  });
