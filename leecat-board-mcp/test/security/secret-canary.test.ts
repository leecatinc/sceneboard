...
test('safe diagnostics redact credentials, pairing proof, code, generation, and store paths', () => {
  let output = '';
  const logger = new SafeStderrLoggerV1((line) => { output += line; });
  logger.log({
...
test('MCP source has no direct database, Redis, Nest application, or deep backend import', async () => {
  ...
  const gateway = await readFile(new URL('../../src/tools/protected-board.gateway.ts', import.meta.url), 'utf8');
  const source = `${server}\n${gateway}`;
  for (const forbidden of ['mysql2', 'ioredis', "from 'redis'", 'leecat-board-nestjs', '../leecat-board-nestjs']) {
