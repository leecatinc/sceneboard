...
test('config discovery selects explicit, environment, nearest, user, then environment fallback', async () => {
  ...
  const processResult = await discoverBoardConfigV1({
    argv: [`--config=${explicit}`], cwd: nested, env: { BOARD_CONFIG: fromEnv, XDG_CONFIG_HOME: userRoot },
  });
  assert.equal(processResult.source, 'process_option');

  const envResult = await discoverBoardConfigV1({ argv: [], cwd: nested, env: { BOARD_CONFIG: fromEnv } });
  assert.equal(envResult.source, 'board_config_env');

  const nearestResult = await discoverBoardConfigV1({ argv: [], cwd: nested, env: { XDG_CONFIG_HOME: userRoot } });
  assert.equal(nearestResult.source, 'nearest_board_file');

  const userResult = await discoverBoardConfigV1({ argv: [], cwd: root, env: { XDG_CONFIG_HOME: userRoot } });
  assert.equal(userResult.source, 'user_config_file');

  const fallbackRoot = await makeRoot();
  const fallback = await discoverBoardConfigV1({ argv: [], cwd: fallbackRoot, env: { BOARD_API_URL: 'https://sceneboard.dev' } });
  assert.equal(fallback.source, 'environment');
  assert.equal(fallback.path, null);
  assert.equal(fallback.config.accessTokenRef, 'env://LEECAT_BOARD_ACCESS_TOKEN');
...
test('environment fallback rejects mismatched store profiles and malformed timeouts', async () => {
  const root = await makeRoot();
  await assert.rejects(() => discoverBoardConfigV1({
    argv: [],
    cwd: root,
    env: {
      BOARD_API_URL: 'https://sceneboard.dev',
...
test('selected invalid candidates fail closed without falling through', async () => {
  ...
  await assert.rejects(
    () => discoverBoardConfigV1({
      argv: [`--config=${invalid}`], cwd: root, env: { BOARD_API_URL: 'https://sceneboard.dev' },
    }),
...
test('config files reject symlinks and group or other writable modes', async () => {
  ...
  await assert.rejects(() => discoverBoardConfigV1({ argv: [`--config=${link}`], cwd: root, env: {} }), BoardConfigError);

  await chmod(target, 0o622);
  await assert.rejects(() => discoverBoardConfigV1({ argv: [`--config=${target}`], cwd: root, env: {} }), BoardConfigError);
});
