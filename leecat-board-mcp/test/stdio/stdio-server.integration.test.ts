...
test('real stdio process publishes the authenticated terminal 21-tool surface and shuts down without secret output', async () => {
  ...
  const backend = createServer((request, response) => {
    ...
    requests.push({ authorization: request.headers.authorization, boardId: url.searchParams.get('boardId') });
  ...
  const transport = new StdioClientTransport({
    ...
    args: [join(root, 'leecat-board-mcp', 'src', 'index.ts')],
    cwd: root,
    env: {
      HOME: home,
  ...
  try {
    ...
    const structured = status.structuredContent as { result: { state: string; connection: { selectedBoard: unknown } } };
  ...
  assert.deepEqual(requests.map((request) => request.boardId), [null, null]);
