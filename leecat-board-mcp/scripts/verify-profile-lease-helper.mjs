...
if (!status.isFile() || status.isSymbolicLink() || (status.mode & 0o777) !== 0o500 || status.uid !== process.geteuid?.()) {
  throw new Error('profile lease helper permissions are invalid');
}
if (!digestStatus.isFile() || digestStatus.isSymbolicLink()
    || digestStatus.uid !== process.geteuid?.()) throw new Error('profile lease helper digest file is invalid');
