...
export const readProjectRootServer = async ({ projectRoot, environment = process.env, read = readFile }) => {
  ...
  if (Buffer.byteLength(text, 'utf8') > 1024 * 1024) throw new TypeError('invalid_sceneboard_project_config_size');
  const document = parseJson(text, 'project');
  if (!isRecord(document) || !isRecord(document.mcpServers)) throw new TypeError('invalid_sceneboard_project_config');
...
export const readCodexServer = ({ projectRoot, environment = process.env, run = spawnSync }) => {
  ...
  if (!isRecord(document) || document.name !== SERVER_NAME) throw new TypeError('invalid_sceneboard_codex_config');
...
export const resolveSceneBoardServer = async ({
  ...
  if (environment.SCENEBOARD_CONFIG_DEPTH === '1') throw new TypeError('recursive_sceneboard_launcher');
  const inheritedProjectRoot = environment.SCENEBOARD_PROJECT_ROOT
  ...
  return productionServer({
    pluginRoot,
    productionApiUrl: productionApiUrl ?? environment.SCENEBOARD_PRODUCTION_API_URL ?? PRODUCTION_API_URL,
  });
};
