...
const successResponse = (result: BoardOperationResultV1, metadata: unknown = { history: null }): Response => new Response(
  JSON.stringify({
    protocolVersion: 1,
...
test('sends a closed Bearer GET with exact query fields and parses the shared success envelope', async () => {
  ...
  assert.deepEqual(logs.map(({ route, attempt, resultCode }) => ({ route, attempt, resultCode })), [{
    route: '/api/v1/boards',
    attempt: 1,
...
test('artifact put preserves the finalized eight-key D7 source body and request correlation', async () => {
  ...
  const fetchValue: typeof fetch = async (input, init) => {
    ...
    const body = JSON.parse(new TextDecoder().decode(init?.body as ArrayBuffer)) as Record<string, unknown>;
...
test('rejects response body overflow from Content-Length before parsing', async () => {
  const request = operationRequest('operation-request-board-list.v1.json', 'board.list');
  const fetchValue: typeof fetch = async () => new Response('{}', {
    status: 200,
    headers: {
      'Content-Type': 'application/json; charset=utf-8',
...
test('transport-neutral parser rejects duplicate outer members and request correlation drift', () => {
  ...
  const duplicate = encoder.encode(`{"protocolVersion":1,"protocolVersion":1,"type":"board.http.success","requestId":"request_1","result":{},"metadata":{"history":null}}`);
  assert.deepEqual(parseBoardHttpResultV1(duplicate, {
    status: 200,
    requestId,
  ...
  assert.deepEqual(parseBoardHttpResultV1(drifted, {
    status: 200,
    requestId,
    resultType: 'board.list',
...
test('history success requires metadata aligned to the exact ordered revision entries', () => {
  ...
  history.entries[0] = { ...history.entries[0], revisionId: 'revision_other' as typeof history.entries[number]['revisionId'] };
  const reversed = encoder.encode(JSON.stringify({
    protocolVersion: 1,
    type: 'board.http.success',
    requestId: result.requestId,
    result,
  ...
  assert.deepEqual(parseBoardHttpResultV1(reversed, {
    status: 200,
    requestId: result.requestId,
    resultType: 'history.list',
...
test('artifact success correlates the exact immutable artifact/version pair', () => {
  ...
  assert.equal(parseBoardHttpResultV1(bytes, {
    status: 200,
    requestId: result.requestId,
  ...
  assert.deepEqual(parseBoardHttpResultV1(bytes, {
    status: 200,
    requestId: result.requestId,
    resultType: 'artifact.get',
