...
const artifactEventParsed = BoardEventEnvelopeParserV1.parse(
  JSON.parse(
    readFileSync(
      new URL(
        '../../../board-schema/test/fixtures/valid/event-artifact-status-changed.v1.json',
        import.meta.url,
      ),
      'utf8',
    ),
...
if (parsedArtifactEvent.data.type !== 'artifact.status.changed')
  throw new TypeError('artifact event fixture is invalid');
const artifactEvent = { ...parsedArtifactEvent, data: parsedArtifactEvent.data };

const hitlEventParsed = BoardEventEnvelopeParserV1.parse(
  JSON.parse(
    readFileSync(
      new URL(
        '../../../board-schema/test/fixtures/valid/event-hitl-updated.v1.json',
        import.meta.url,
      ),
      'utf8',
    ),
...
if (!hitlEventParsed.ok || hitlEventParsed.data.value.data.type !== 'hitl.updated')
  throw new TypeError('HITL event fixture is invalid');
...
if (!answeredHitlParsed.ok || answeredHitlParsed.data.value.data.type !== 'hitl.updated')
  throw new TypeError('answered HITL event fixture is invalid');
const answeredHitlEvent = {
  ...answeredHitlParsed.data.value,
  data: answeredHitlParsed.data.value.data,
};
...
if (!secondHitlParsed.ok || secondHitlParsed.data.value.data.type !== 'hitl.updated')
  throw new TypeError('second HITL event fixture is invalid');
...
if (
  !answeredSecondHitlParsed.ok ||
  answeredSecondHitlParsed.data.value.data.type !== 'hitl.updated'
)
  throw new TypeError('answered second HITL event fixture is invalid');
const answeredSecondHitlEvent = {
  ...answeredSecondHitlParsed.data.value,
  data: answeredSecondHitlParsed.data.value.data,
};
...
if (!wrongBoardHitlParsed.ok || wrongBoardHitlParsed.data.value.data.type !== 'hitl.updated')
  throw new TypeError('wrong-board HITL event fixture is invalid');
const wrongBoardHitlEvent = {
  ...wrongBoardHitlParsed.data.value,
  data: wrongBoardHitlParsed.data.value.data,
};
...
test('durable HITL appends and updates only the exact identity in a populated historical state', () => {
  ...
  assert.deepEqual(
    state.liveSnapshot.hitl.map((item) => item.hitlRequestId),
    ['hitl_1', 'hitl_2'],
  );
...
test('wrong-board durable HITL fails closed without changing the input state', () => {
  ...
  assert.throws(
    () => applyDurableEventV1(state, wrongBoardHitlEvent),
    /durable event targets another board/u,
  );
