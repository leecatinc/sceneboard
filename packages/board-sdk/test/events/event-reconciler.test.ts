...
test('gates snapshot, durable, and presence cursor advancement on matching commits', () => {
  ...
  const reconciler = createBoardEventReconcilerV1({
    boardId: BOARD_ID,
  ...
  assert.deepEqual(reconciler.evaluate(inputFor(durable, 'cursor_2')), {
    kind: 'protocol_failure',
    reason: 'control_sequence',
    clearCursor: false,
  });
  assert.throws(() => reconciler.commit(999, { kind: 'effect_applied' }), TypeError);
  assert.deepEqual(reconciler.commit(pendingSnapshot.acceptanceId, { kind: 'effect_applied' }), {
    kind: 'continue',
  ...
  assert.deepEqual(reconciler.commit(pendingDurable.acceptanceId, { kind: 'effect_applied' }), {
    kind: 'continue',
  ...
  assert.deepEqual(reconciler.commit(pendingPresence.acceptanceId, { kind: 'effect_applied' }), {
    kind: 'continue',
    lastAppliedSequence: 2,
    cursor: 'cursor_2',
  });
...
test('retains exact duplicates and clears unsafe ordering outcomes', () => {
  ...
  assert.deepEqual(reconciler.evaluate(inputFor(durable, 'cursor_replayed')), {
    kind: 'duplicate',
    sequence: 10,
    retainedCursor: 'cursor_10',
  });
  const conflicting = withSequence(durable, 10, 'event_conflict');
  assert.deepEqual(reconciler.evaluate(inputFor(conflicting, 'cursor_10')), {
    kind: 'resync_required',
    reason: 'conflicting_duplicate',
    clearCursor: true,
  });
  const gap = withSequence(readEvent('event-hitl-updated.v1.json'), 12);
  assert.deepEqual(reconciler.evaluate(inputFor(gap, 'cursor_12')), {
    kind: 'resync_required',
    reason: 'sequence_gap',
    clearCursor: true,
  });
  const stale = withSequence(readEvent('event-hitl-updated.v1.json'), 8);
  assert.deepEqual(reconciler.evaluate(inputFor(stale, 'cursor_9')), {
    kind: 'resync_required',
...
test('rebases revision hints through an authoritative HTTP snapshot admission', () => {
  ...
  assert.deepEqual(reconciler.evaluate(inputFor(below, 'cursor_3')), {
    kind: 'protocol_failure',
    reason: 'snapshot_below_minimum',
    clearCursor: false,
  });
});
...
test('separates heartbeat, server resync, stream error, and rejection outcomes', () => {
  ...
  assert.deepEqual(reconciler.evaluate(inputFor(error, null)), {
    kind: 'stream_error',
  ...
  assert.deepEqual(reconciler.evaluate(inputFor(resync, null)), {
    kind: 'resync_required',
    reason: 'gap',
    clearCursor: true,
  });
