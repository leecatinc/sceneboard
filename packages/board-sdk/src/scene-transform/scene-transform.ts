...
const isBoardError = (value: BoardNodeV1 | BoardErrorV1 | Record<string, unknown>): value is BoardErrorV1 => (
  value.type === 'board.error'
);
...
const isSafeIndex = (value: unknown): value is number => Number.isSafeInteger(value) && Number(value) >= 0;
...
const isLocalId = (value: unknown): value is string => typeof value === 'string' && LOCAL_ID_PATTERN.test(value);
...
const validateNode = (value: unknown, operationIndex: number, field: string): BoardNodeV1 | BoardErrorV1 => {
...
const prefixError = (error: BoardErrorV1, operationIndex: number, field: string): BoardErrorV1 => {
  if (error.code === 'INVALID_PAYLOAD' || error.code === 'INVALID_LAYOUT' || error.code === 'LIMIT_EXCEEDED') {
    return {
      ...error,
      details: {
        ...error.details,
...
const placementEntry = (
  ...
  if (parent.type === 'layout.grid') {
    ...
    if (!hasExactKeys(placement, keys)
      || !isSafeIndex(placement.column) || placement.column < 1
      || !isSafeIndex(placement.row) || placement.row < 1
      || !isSafeIndex(placement.columnSpan) || placement.columnSpan < 1
      || !isSafeIndex(placement.rowSpan) || placement.rowSpan < 1) {
  ...
  if (parent.type === 'layout.tabs') {
    if (!hasExactKeys(placement, ['parentType', 'tabId', 'label'])
      || !isLocalId(placement.tabId) || !isShortText(placement.label)) {
  ...
  if (!hasExactKeys(placement, keys)
    || !isFiniteNonNegative(placement.x) || !isFiniteNonNegative(placement.y)
    || !isFinitePositive(placement.width) || !isFinitePositive(placement.height)
...
const applyOperation = (
  ...
  if (operation.type === 'replace_root') {
    if (!hasExactKeys(operation, ['type', 'root'])) return invalid(operationIndex, 'type', 'unknown operation field');
  ...
  if (operation.type === 'replace_node') {
    ...
    if (location.parent === null) scene.root = replacement;
      ...
      const entries = layoutEntries(location.parent);
      if (entries === null || location.index === null) return invalidLayout(operationIndex, 'nodeId', 'reference');
  ...
  if (operation.type === 'insert_child') {
    if (!hasExactKeys(operation, ['type', 'parentNodeId', 'index', 'node', 'placement'])
      || !isLocalId(operation.parentNodeId) || !isSafeIndex(operation.index)) {
      return invalid(operationIndex, 'index', 'invalid insert_child operation');
    }
    const parentLocation = index.get(operation.parentNodeId);
    if (parentLocation === undefined) return invalid(operationIndex, 'parentNodeId', 'parent was not found');
    const entries = layoutEntries(parentLocation.node);
    if (entries === null || operation.index > entries.length) return invalidLayout(operationIndex, 'index', 'bounds');
    const node = validateNode(operation.node, operationIndex, 'node');
    if (isBoardError(node)) return node;
    if (index.has(node.id)) return invalid(operationIndex, 'node', 'node ID already exists');
    if (!isLayoutNode(parentLocation.node)) return invalidLayout(operationIndex, 'parentNodeId', 'reference');
    const entry = placementEntry(parentLocation.node, structuredClone(node), operation.placement, operationIndex);
  ...
  if (operation.type === 'move_child') {
    const keys = ['type', 'sourceParentNodeId', 'destinationParentNodeId', 'nodeId', 'destinationIndex', 'placement'];
    ...
    const sourceIndex = source.type.startsWith('layout.') ? directChildIndex(source as LayoutNode, operation.nodeId) : -1;
    ...
    while (ancestor !== undefined) {
      if (ancestor.node.id === operation.nodeId) return invalidLayout(operationIndex, 'destinationParentNodeId', 'reference');
    ...
    if (operation.destinationIndex > maximumDestination) return invalidLayout(operationIndex, 'destinationIndex', 'bounds');
  ...
  if (operation.type === 'set_split_weight') {
    ...
    if (!hasExactKeys(operation, keys) || !isLocalId(operation.splitNodeId)
      || !isLocalId(operation.childNodeId) || !isFinitePositive(operation.weight)) {
      return invalid(operationIndex, 'weight', 'invalid split weight operation');
    }
    const node = index.get(operation.splitNodeId)?.node;
    if (node?.type !== 'layout.split') return invalidLayout(operationIndex, 'splitNodeId', 'reference');
    const childIndex = directChildIndex(node, operation.childNodeId);
    if (childIndex < 0 || node.children[childIndex] === undefined) return invalidLayout(operationIndex, 'childNodeId', 'reference');
  ...
  if (operation.type === 'set_grid_placement') {
    ...
    if (!hasExactKeys(operation, keys) || !isLocalId(operation.gridNodeId) || !isLocalId(operation.childNodeId)
      || !isSafeIndex(operation.column) || operation.column < 1
      || !isSafeIndex(operation.row) || operation.row < 1
      || !isSafeIndex(operation.columnSpan) || operation.columnSpan < 1
      || !isSafeIndex(operation.rowSpan) || operation.rowSpan < 1) {
      return invalid(operationIndex, 'column', 'invalid grid placement operation');
    }
    const node = index.get(operation.gridNodeId)?.node;
    if (node?.type !== 'layout.grid') return invalidLayout(operationIndex, 'gridNodeId', 'reference');
    const childIndex = directChildIndex(node, operation.childNodeId);
    const child = node.children[childIndex];
    if (childIndex < 0 || child === undefined) return invalidLayout(operationIndex, 'childNodeId', 'reference');
    Object.assign(child, {
  ...
  if (operation.type === 'set_canvas_rect') {
    ...
    if (!hasExactKeys(operation, keys) || !isLocalId(operation.canvasNodeId) || !isLocalId(operation.childNodeId)
      || !isFiniteNonNegative(operation.x) || !isFiniteNonNegative(operation.y)
      || !isFinitePositive(operation.width) || !isFinitePositive(operation.height)
    ...
    if (node?.type !== 'layout.canvas') return invalidLayout(operationIndex, 'canvasNodeId', 'reference');
    const childIndex = directChildIndex(node, operation.childNodeId);
    const child = node.children[childIndex];
    if (childIndex < 0 || child === undefined) return invalidLayout(operationIndex, 'childNodeId', 'reference');
    Object.assign(child, {
  ...
  if (operation.type === 'set_active_tab') {
    if (!hasExactKeys(operation, ['type', 'tabsNodeId', 'tabId'])
      || !isLocalId(operation.tabsNodeId) || !isLocalId(operation.tabId)) {
  ...
  if (operation.type === 'upsert_drawing_element') {
    if (!hasExactKeys(operation, ['type', 'drawingNodeId', 'element']) || !isLocalId(operation.drawingNodeId)) {
    ...
    if (node?.type !== 'content.drawing' || !isRecord(elementCandidate) || !isLocalId(elementCandidate.id)) {
  ...
  if (operation.type === 'remove_drawing_element') {
    if (!hasExactKeys(operation, ['type', 'drawingNodeId', 'elementId'])
      || !isLocalId(operation.drawingNodeId) || !isLocalId(operation.elementId)) {
      return invalid(operationIndex, 'elementId', 'invalid drawing element removal');
    }
    const node = index.get(operation.drawingNodeId)?.node;
    if (node?.type !== 'content.drawing') return invalidLayout(operationIndex, 'drawingNodeId', 'reference');
    const elementIndex = node.elements.findIndex((element) => element.id === operation.elementId);
    if (elementIndex < 0) return invalid(operationIndex, 'elementId', 'drawing element was not found');
