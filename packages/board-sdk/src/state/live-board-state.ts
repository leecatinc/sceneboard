...
export const applyDurableEventV1 = (
  ...
  if (envelope.boardId !== state.liveSnapshot.boardId) throw new TypeError('durable event targets another board');
...
export const enterLatestV1 = (
  ...
  if (snapshot.boardId !== state.liveSnapshot.boardId) throw new TypeError('latest snapshot targets another board');
  return {
    ...state,
    liveSnapshot: snapshot,
    mode: { kind: 'live' },
