...
const validateOptions = (options: BoardStreamClientOptionsV1): void => {
  ...
  if (!TAB_ID_PATTERN.test(options.tabId))
    throw new TypeError('tabId must be 22-character unpadded base64url');
  ...
  if (
    !Number.isSafeInteger(options.minimumSnapshotSequence) ||
    options.minimumSnapshotSequence < 0
  ) {
    throw new TypeError('minimumSnapshotSequence must be a non-negative safe integer');
  }
  if (typeof options.dispatch?.open !== 'function')
    throw new TypeError('dispatch.open is required');
  ...
  if (!(options.routeSignal instanceof AbortSignal))
    throw new TypeError('routeSignal must be an AbortSignal');
};
...
export const createBoardStreamClientV1 = (
  ...
  const consumeResponse = async (
    ...
    if (
      response.headers.get('content-type')?.toLowerCase() !== 'text/event-stream; charset=utf-8' ||
      response.body === null
    ) {
      ...
      while (true) {
        ...
        for (const record of records) {
          ...
          if (evaluation.kind === 'pending_effect') {
            try {
              if (evaluation.effect.kind === 'replace_snapshot') {
                ...
                if (commit.kind !== 'continue')
                  throw new TypeError('snapshot commit requested an invalid restart');
                cursor = commit.cursor;
                lastAppliedSequence = commit.lastAppliedSequence;
              } else if (evaluation.effect.kind === 'refresh_revision_snapshot') {
                const admission = await options.callbacks.refreshRevisionSnapshot(
                  evaluation.effect.event,
                );
                const commit = reconciler.commit(evaluation.acceptanceId, admission);
                if (commit.kind !== 'restart_without_cursor')
                  throw new TypeError('revision commit did not request restart');
                cursor = null;
                ...
                if (commit.kind !== 'continue')
                  throw new TypeError('durable commit requested an invalid restart');
                cursor = commit.cursor;
                lastAppliedSequence = commit.lastAppliedSequence;
              } else {
                ...
                if (commit.kind !== 'continue')
                  throw new TypeError('presence commit requested an invalid restart');
              }
            } catch {
              try {
              ...
              const callback =
                evaluation.effect.kind === 'replace_snapshot'
                  ? 'snapshot'
            ...
            if (!(await emitState({ state: 'live', lastAppliedSequence })))
              return callbackFailure('state');
            silenceBase = performance.now();
            continue;
          }
          ...
          return {
            kind: 'terminal',
            failure: { kind: 'protocol', sourceStatus: 200, error: null },
          };
        }
    ...
    while (stopReason === null) {
      ...
      if (dispatchResult.kind === 'consumed') outcome = dispatchResult.value;
        ...
        if (cursor !== null && !invalidCursorResetUsed) {
          ...
          outcome = {
            kind: 'terminal',
            failure: { kind: 'invalid_request', sourceStatus: 400, error: dispatchResult.error },
          };
      } else if (dispatchResult.sourceStatus === 403) {
        outcome = {
          kind: 'terminal',
          failure: { kind: 'forbidden', sourceStatus: 403, error: dispatchResult.error },
        };
        ...
        outcome = {
          kind: 'terminal',
          failure: { kind: 'internal_error', sourceStatus: 500, error: dispatchResult.error },
        };
      }
      ...
      if (
        outcome.reason === 'revision_rebase' ||
        outcome.reason === 'server_resync' ||
        outcome.reason === 'invalid_cursor_reset'
      ) {
