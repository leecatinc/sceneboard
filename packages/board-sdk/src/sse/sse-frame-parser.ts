...
const parseRecord = (
  ...
  for (const line of lines) {
    ...
    if (line[0] === 0x3a) {
      if (eventSeen || idSeen || dataSeen)
        throw new SseProtocolErrorV1('comments and fields cannot be mixed');
      try {
    ...
    if (colon <= 0)
      throw new SseProtocolErrorV1('SSE field line requires a non-empty name and colon');
    ...
    if (field === 'event') {
      ...
      if (!equalAscii(value, 'board.event.v1'))
        throw new SseProtocolErrorV1('unknown SSE event name');
      eventSeen = true;
      continue;
    }
    if (field === 'id') {
      if (
        ...
        idSeen ||
        dataSeen ||
        line.byteLength > MAX_ID_LINE_BYTES ||
        value.byteLength < 1
      ) {
...
const createSseFrameParser = (documentSchemaVersion: 1 | 2): SseFrameParserV1 => {
  ...
  const push = (chunk: Uint8Array): ParsedSseRecordV1[] => {
    ...
    for (let index = 0; index < combined.byteLength; index += 1) {
      ...
      if (delimiter !== null && delimiter !== style)
        throw new SseProtocolErrorV1('mixed SSE line delimiters');
      delimiter = style;
