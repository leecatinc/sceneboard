...
export type BoardSdkHttpLocalErrorV1 =
  ...
  | { code: 'RESPONSE_INVALID'; retryable: false; reason: BoardHttpResultParseFailureReasonV1 | 'status' | 'content_type' | 'body_too_large' | 'correlation' };
...
const localFailure = <K extends string>(error: BoardSdkHttpLocalErrorV1): Extract<BoardSdkHttpResultV1<K>, { ok: false }> => ({
  ok: false,
  error,
});
...
const validateBaseUrl = (value: string): string => {
  ...
  if (url.origin !== value || url.pathname !== '/' || url.username !== '' || url.password !== ''
    || url.search !== '' || url.hash !== '' || (url.protocol !== 'https:' && !(url.protocol === 'http:' && loopback))) {
...
export class BoardSdkHttpClient {
  ...
  constructor(options: BoardSdkHttpClientOptionsV1) {
    ...
    if (typeof options.bearerTokenProvider !== 'function') throw new TypeError('bearerTokenProvider is required');
    if (!Number.isSafeInteger(options.timeoutPolicy?.timeoutMs) || options.timeoutPolicy.timeoutMs < 1) {
  ...
  listBoards(request: OperationRequest<'board.list'>, signal?: AbortSignal): Promise<BoardSdkHttpResultV1<'board.list'>> {
    ...
    return this.#call({
      method: 'GET', routeTemplate: '/api/v1/boards', path: '/api/v1/boards', query, body: null,
      requestId: parsed.requestId, resultType: 'board.list', retryKind: 'read',
    }, signal);
  }

  getBoard(request: OperationRequest<'board.get'>, signal?: AbortSignal): Promise<BoardSdkHttpResultV1<'board.get'>> {
    const parsed = this.#operation(request, 'board.get');
    return this.#operationGet(parsed, `/api/v1/boards/${parsed.boardId}`, '/api/v1/boards/:boardId', signal);
  }

  createBoard(request: OperationRequest<'board.create'>, signal?: AbortSignal): Promise<BoardSdkHttpResultV1<'board.create'>> {
    const parsed = this.#operation(request, 'board.create');
    return this.#call({
      method: 'POST', routeTemplate: '/api/v1/boards', path: '/api/v1/boards', body: this.#operationBytes(parsed),
      requestId: parsed.requestId, resultType: 'board.create', retryKind: 'mutation',
    }, signal);
  }

  archiveBoard(request: OperationRequest<'board.archive'>, signal?: AbortSignal): Promise<BoardSdkHttpResultV1<'board.archive'>> {
    const parsed = this.#operation(request, 'board.archive');
    return this.#call({
      method: 'POST', routeTemplate: '/api/v1/boards/:boardId/archive',
      path: `/api/v1/boards/${parsed.boardId}/archive`, body: this.#operationBytes(parsed),
      requestId: parsed.requestId, resultType: 'board.archive', boardId: parsed.boardId, retryKind: 'mutation',
    }, signal);
  }

  getCapabilities(request: OperationRequest<'capabilities.get'>, signal?: AbortSignal): Promise<BoardSdkHttpResultV1<'capabilities.get'>> {
    const parsed = this.#operation(request, 'capabilities.get');
    return this.#operationGet(parsed, `/api/v1/boards/${parsed.boardId}/capabilities`, '/api/v1/boards/:boardId/capabilities', signal);
  }
  ...
  mutateBoard<K extends MutationRequestV1['command']['type']>(
    ...
    if (!parsed.ok || parsed.data.value.command.type !== request.command.type) throw new TypeError('invalid D1 mutation request');
    return this.#call({
      method: 'POST', routeTemplate: '/api/v1/boards/:boardId/mutations',
      path: `/api/v1/boards/${parsed.data.value.boardId}/mutations`, body: parsed.data.canonicalBytes,
      requestId: parsed.data.value.requestId, resultType: request.command.type,
      boardId: parsed.data.value.boardId, retryKind: 'mutation',
    }, signal) as Promise<BoardSdkHttpResultV1<K>>;
  }

  restoreRevision(
    request: MutationRequestV1 & { command: Extract<MutationRequestV1['command'], { type: 'scene.restore' }> },
    signal?: AbortSignal,
  ): Promise<BoardSdkHttpResultV1<'scene.restore'>> {
    const parsed = MutationRequestParserV1.parse(request);
    if (!parsed.ok || parsed.data.value.command.type !== 'scene.restore') throw new TypeError('invalid scene.restore request');
    ...
    return this.#call({
      method: 'POST', routeTemplate: '/api/v1/boards/:boardId/revisions/:revisionId/restore',
      path: `/api/v1/boards/${parsed.data.value.boardId}/revisions/${parsed.data.value.command.sourceRevisionId}/restore`,
      body, requestId: parsed.data.value.requestId, resultType: 'scene.restore',
      boardId: parsed.data.value.boardId, revisionId: parsed.data.value.command.sourceRevisionId,
      retryKind: 'mutation',
    }, signal);
  }

  listHistory(request: OperationRequest<'history.list'>, signal?: AbortSignal): Promise<BoardSdkHttpResultV1<'history.list'>> {
    ...
    if (parsed.cursor !== null) query.set('cursor', parsed.cursor);
    return this.#call({
      method: 'GET', routeTemplate: '/api/v1/boards/:boardId/revisions',
      path: `/api/v1/boards/${parsed.boardId}/revisions`, query, body: null,
      requestId: parsed.requestId, resultType: 'history.list', boardId: parsed.boardId, retryKind: 'read',
    }, signal);
  }

  getHistory(request: OperationRequest<'history.get'>, signal?: AbortSignal): Promise<BoardSdkHttpResultV1<'history.get'>> {
    const parsed = this.#operation(request, 'history.get');
    return this.#call({
      method: 'GET', routeTemplate: '/api/v1/boards/:boardId/revisions/:revisionId',
      path: `/api/v1/boards/${parsed.boardId}/revisions/${parsed.revisionId}`,
      query: new URLSearchParams({ requestId: parsed.requestId }), body: null,
      requestId: parsed.requestId, resultType: 'history.get', boardId: parsed.boardId,
      revisionId: parsed.revisionId, retryKind: 'read',
    }, signal);
  }

  getArtifact(request: OperationRequest<'artifact.get'>, signal?: AbortSignal): Promise<BoardSdkHttpResultV1<'artifact.get'>> {
    const parsed = this.#operation(request, 'artifact.get');
    return this.#call({
      method: 'GET', routeTemplate: '/api/v1/boards/:boardId/artifacts/:artifactId/versions/:versionId',
      path: `/api/v1/boards/${parsed.boardId}/artifacts/${parsed.artifact.artifactId}/versions/${parsed.artifact.versionId}`,
      query: new URLSearchParams({ requestId: parsed.requestId }), body: null,
      requestId: parsed.requestId, resultType: 'artifact.get', boardId: parsed.boardId,
      artifact: parsed.artifact, retryKind: 'read',
    }, signal);
  }
  ...
  putArtifact(
    ...
    return this.#call({
      method: 'POST', routeTemplate: '/api/v1/boards/:boardId/artifacts',
      path: `/api/v1/boards/${source.boardId}/artifacts`, body,
      requestId, resultType: 'artifact.publish', boardId: source.boardId, retryKind: 'mutation',
    }, signal);
  }

  getInteraction(request: OperationRequest<'hitl.read'>, signal?: AbortSignal): Promise<BoardSdkHttpResultV1<'hitl.read'>> {
    ...
    return this.#call({
      method: 'GET', routeTemplate: '/api/v1/boards/:boardId/interactions/:hitlRequestId',
      path: `/api/v1/boards/${parsed.boardId}/interactions/${parsed.hitlRequestId}`,
      query, body: null, requestId: parsed.requestId, resultType: 'hitl.read',
      boardId: parsed.boardId, retryKind: 'read',
    }, signal);
  }

  #operation<K extends BoardOperationRequestV1['type']>(request: OperationRequest<K>, kind: K): OperationRequest<K> {
    const parsed = BoardOperationRequestParserV1.parse(request);
    if (!parsed.ok || parsed.data.value.type !== kind) throw new TypeError(`invalid ${kind} request`);
  ...
  #operationGet<K extends 'board.get' | 'capabilities.get'>(
    ...
    return this.#call({
      method: 'GET', routeTemplate, path,
      query: new URLSearchParams({ requestId: request.requestId }), body: null,
      requestId: request.requestId, resultType: request.type, boardId: request.boardId, retryKind: 'read',
    }, signal);
  }

  async #call<K extends string>(spec: CallSpec<K>, outerSignal?: AbortSignal): Promise<BoardSdkHttpResultV1<K>> {
    ...
    try {
      for (let attempt = 1; attempt <= maximumAttempts; attempt += 1) {
        ...
        try {
          ...
          if (!await sleepWithinDeadlineV1(delay, deadline.remainingMs, deadline.signal)) return result;
          continue;
        } finally {
          token = '';
        }
        ...
        if (response.headers.get('content-type')?.toLowerCase() !== 'application/json; charset=utf-8') {
          await response.body?.cancel().catch(() => undefined);
          return localFailure({ code: 'RESPONSE_INVALID', retryable: false, reason: 'content_type' });
        }
        ...
        if (bytes === 'body_too_large') return localFailure({ code: 'RESPONSE_INVALID', retryable: false, reason: 'body_too_large' });
        if (bytes === 'response') {
          const result = deadline.signal.aborted
            ? deadline.timedOut()
              ? localFailure<K>({ code: 'TIMEOUT', retryable: true, timeoutMs: this.#timeoutMs })
        ...
        if (response.headers.get('x-request-id') !== spec.requestId) {
          return localFailure({ code: 'RESPONSE_INVALID', retryable: false, reason: 'correlation' });
        }
        const parsed = parseBoardHttpResultV1(bytes, {
          status: response.status,
          requestId: spec.requestId,
          resultType: spec.resultType as BoardOperationResultDataV1['type'] | MutationResultV1['result']['type'],
        ...
        if (!parsed.ok) return localFailure({ code: 'RESPONSE_INVALID', retryable: false, reason: parsed.reason });
        if (!parsed.value.ok) {
