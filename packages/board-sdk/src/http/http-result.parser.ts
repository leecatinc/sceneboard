...
const parseHistory = (value: unknown): HistoryAdapterMetadataV1 | null => {
  if (!isRecord(value) || !hasExactKeys(value, ['protocolVersion', 'type', 'entries', 'navigation'])
    || value.protocolVersion !== 1 || value.type !== 'history.adapter-metadata'
    || !Array.isArray(value.entries) || value.entries.length > 100) return null;
  const entries: HistoryAdapterMetadataV1['entries'] = [];
  for (const entry of value.entries) {
    if (!isRecord(entry) || !hasExactKeys(entry, ['revisionId', 'label']) || !validLabel(entry.label)) return null;
  ...
  if (value.navigation !== null) {
    ...
    if (!isRecord(candidate)
      || !hasExactKeys(candidate, ['revisionId', 'previousRevisionId', 'nextRevisionId', 'latestRevisionId'])) return null;
    const revisionId = parseRevisionId(candidate.revisionId);
    const latestRevisionId = parseRevisionId(candidate.latestRevisionId);
    const previousRevisionId = candidate.previousRevisionId === null ? null : parseRevisionId(candidate.previousRevisionId);
    const nextRevisionId = candidate.nextRevisionId === null ? null : parseRevisionId(candidate.nextRevisionId);
    if (revisionId === null || latestRevisionId === null
      || (candidate.previousRevisionId !== null && previousRevisionId === null)
      || (candidate.nextRevisionId !== null && nextRevisionId === null)) return null;
...
const historyCorrelates = (
  ...
  if (result.result.type === 'history.list') {
    return metadata !== null
      ...
      && result.result.entries.every((entry: HistoryEntryV1, index: number) => (
...
const pathCorrelates = (
  ...
  if (expectation.boardId !== undefined) {
    ...
    if ((result.result.type === 'board.get' || result.result.type === 'board.create')
      && result.result.board.boardId !== expectation.boardId) return false;
    if (result.result.type === 'board.archive' && result.result.board.boardId !== expectation.boardId) return false;
    if (result.result.type === 'history.get' && result.result.snapshot.boardId !== expectation.boardId) return false;
  }
  if (expectation.artifact !== undefined) {
    if (result.result.type !== 'artifact.get'
      || result.result.manifest.artifact.artifactId !== expectation.artifact.artifactId
      || result.result.manifest.artifact.versionId !== expectation.artifact.versionId) return false;
  }
...
export const parseBoardHttpResultV1 = (
  ...
  if (hasExactKeys(decoded, ['error'])) {
    ...
    if (expectation.status !== error.data.value.httpStatusHint) return { ok: false, reason: 'correlation' };
    return { ok: true, value: { ok: false, error: error.data.value } };
  }

  if (!hasExactKeys(decoded, ['protocolVersion', 'type', 'requestId', 'result', 'metadata'])
    || decoded.protocolVersion !== 1 || decoded.type !== 'board.http.success'
    || decoded.requestId !== expectation.requestId || !isRecord(decoded.metadata)
    || !hasExactKeys(decoded.metadata, ['history'])) return { ok: false, reason: 'schema' };
