...
export type BoardNodeV1 =
  | (NodeBaseV1 & {
      ...
      gap: number;
      children: Array<{ node: BoardNodeV1; weight: number }>;
    })
  | (NodeBaseV1 & {
      type: 'layout.grid';
      columns: number;
  ...
  | (NodeBaseV1 & {
      type: 'layout.tabs';
      activeTabId: TabId;
      tabs: Array<{ tabId: TabId; label: ShortText; node: BoardNodeV1 }>;
    })
  | (NodeBaseV1 & {
      type: 'layout.canvas';
      width: number;
  ...
  | (NodeBaseV1 & {
      ...
      rows: Array<{ id: LocalFieldId; cells: Record<string, string | number | boolean | null> }>;
    })
  | (NodeBaseV1 & {
      type: 'content.chart';
      chartType: 'line' | 'bar' | 'area' | 'pie' | 'scatter';
      xAxis: { valueType: 'category' | 'number' | 'datetime'; label?: ShortText };
      yAxis: { label?: ShortText; min?: number; max?: number };
      series: Array<{
        id: LocalFieldId;
        label: ShortText;
...
const childItems = (
  ...
  if (
    ...
    return ((node.children as Array<{ node: UnknownNode }> | undefined) ?? []).map(
      (item, index) => ({
        node: item.node,
        path: [...path, 'children', index, 'node'],
        depth: depth + 1,
      }),
    );
  }
  if (node.type === 'layout.tabs') {
    return ((node.tabs as Array<{ node: UnknownNode }> | undefined) ?? []).map((item, index) => ({
      node: item.node,
      path: [...path, 'tabs', index, 'node'],
      depth: depth + 1,
    }));
  }
  return [];
...
const addLayoutIssue = (context: z.RefinementCtx, path: Array<string | number>, message: string) =>
  context.addIssue({ code: z.ZodIssueCode.custom, path, message: `[INVALID_LAYOUT] ${message}` });

const validateNodeRelationsV1 = (
  root: UnknownNode | null,
  ...
  if (nodes.length > MAX_SCENE_NODES)
    context.addIssue({
      ...
      path: [...prefix, 'root'],
      message: '[LIMIT:maxSceneNodes] scene node count exceeded',
    });
  const ids = new Map<string, Array<string | number>>();
  for (const item of nodes) {
    const path = [...prefix, ...item.path];
    if (item.depth > MAX_SCENE_DEPTH)
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path,
        message: '[LIMIT:maxSceneDepth] scene depth exceeded',
      });
    ...
    if (first)
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: [...path, 'id'],
        message: `[DUPLICATE_NODE_ID:${item.node.id}:${JSON.stringify(first)}] duplicate node ID`,
      });
    ...
    if (item.node.type === 'layout.tabs') {
      ...
      if (new Set(tabs.map((tab) => tab.tabId)).size !== tabs.length)
        addLayoutIssue(context, [...path, 'tabs'], 'tab IDs must be unique');
      if (!tabs.some((tab) => tab.tabId === item.node.activeTabId))
        addLayoutIssue(context, [...path, 'activeTabId'], 'active tab is missing');
    } else if (item.node.type === 'layout.grid') {
      const placements = item.node.children as Array<{
        column: number;
        row: number;
        columnSpan: number;
        rowSpan: number;
      ...
      placements.forEach((placement, index) => {
        if (
          placement.column + placement.columnSpan - 1 > (item.node.columns as number) ||
          placement.row + placement.rowSpan - 1 > (item.node.rows as number)
        )
          addLayoutIssue(context, [...path, 'children', index], 'grid item is outside tracks');
        for (let row = placement.row; row < placement.row + placement.rowSpan; row += 1)
          for (
            ...
            if (occupied.has(key))
              addLayoutIssue(context, [...path, 'children', index], 'grid items overlap');
            occupied.add(key);
          }
      });
      ...
      (
        item.node.children as Array<{ x: number; y: number; width: number; height: number }>
      ).forEach((placement, index) => {
        if (placement.x + placement.width > width || placement.y + placement.height > height)
          addLayoutIssue(context, [...path, 'children', index], 'canvas item is outside bounds');
      });
      ...
      if (new Set(keys).size !== keys.length)
        addLayoutIssue(context, [...path, 'columns'], 'table column keys must be unique');
      if (new Set(rows.map((row) => row.id)).size !== rows.length)
        addLayoutIssue(context, [...path, 'rows'], 'table row IDs must be unique');
      if (columns.length * rows.length > MAX_TABLE_CELLS)
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: [...path, 'rows'],
          message: '[LIMIT:maxTableCells] table cell count exceeded',
        });
            ...
            column.valueType === 'string'
              ...
              : column.valueType === 'number'
                ...
                : column.valueType === 'boolean'
                  ...
                  : typeof value === 'string' && isTimestampCellV1(value);
          if (!valid)
            addLayoutIssue(
              context,
              [...path, 'rows', rowIndex, 'cells', column.key],
              'table cell type does not match column',
            );
        });
      });
      ...
      const chart = item.node as UnknownNode & {
        ...
        series: Array<{ points: Array<{ x: string | number; y: number | null }> }>;
      };
      if (
        ...
        addLayoutIssue(context, [...path, 'yAxis', 'max'], 'axis bounds are invalid');
      const totalPoints = chart.series.reduce((total, series) => total + series.points.length, 0);
      if (totalPoints > MAX_CHART_POINTS)
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: [...path, 'series'],
          message: '[LIMIT:maxChartPoints] chart point count exceeded',
        });
      if (
        chart.chartType === 'pie' &&
        (chart.series.length !== 1 ||
          chart.xAxis.valueType !== 'category' ||
          chart.series[0]?.points.some((point) => point.y === null || point.y < 0))
      )
        addLayoutIssue(context, [...path, 'series'], 'pie chart requirements are invalid');
      if (chart.chartType === 'scatter' && chart.xAxis.valueType !== 'number')
        addLayoutIssue(context, [...path, 'xAxis', 'valueType'], 'scatter x axis must be numeric');
      chart.series.forEach((series, seriesIndex) =>
        series.points.forEach((point, pointIndex) => {
          const validX =
            chart.xAxis.valueType === 'number'
              ? typeof point.x === 'number'
              : chart.xAxis.valueType === 'datetime'
                ? typeof point.x === 'string' && isTimestampCellV1(point.x)
                : typeof point.x === 'string';
          if (!validX)
            addLayoutIssue(
              context,
              [...path, 'series', seriesIndex, 'points', pointIndex, 'x'],
              'chart x value does not match axis',
            );
        }),
      );
    } else if (item.node.type === 'content.map') {
      const features = (
        item.node.data as { features: Array<{ geometry: { type: string; coordinates: unknown } }> }
      ).features;
      if (features.length > MAX_MAP_FEATURES)
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: [...path, 'data', 'features'],
          message: '[LIMIT:maxMapFeatures] map feature count exceeded',
        });
          ...
          (feature.geometry.coordinates as Array<Array<[number, number]>>).forEach(
            (ring, ringIndex) => {
              ...
              if (!first || !last || first[0] !== last[0] || first[1] !== last[1])
                addLayoutIssue(
                  context,
                  [...path, 'data', 'features', featureIndex, 'geometry', 'coordinates', ringIndex],
      ...
      if (
        ...
        addLayoutIssue(context, [...path, 'value'], 'progress value does not match state');
    }
  }
};

export const BoardNodeSchemaV1 = BoardNodeSchemaInternalV1.superRefine((node, context) =>
  validateNodeRelationsV1(node, context, []),
).describe('BoardNodeV1') as unknown as z.ZodType<BoardNodeV1>;

export const SceneSchemaV1 = z
  .object({
    protocolVersion: z.literal(1),
    type: z.literal('scene'),
    root: BoardNodeSchemaInternalV1.nullable(),
  })
  .strict()
  .superRefine((scene, context) =>
    validateNodeRelationsV1(scene.root, context),
  ) as unknown as z.ZodType<SceneV1>;
