...
const validateJsonValueV1 = (input: unknown): KernelResultV1<JsonValue> => {
  ...
  while (stack.length > 0) {
    ...
    if (typeof value === 'number') {
      if (!Number.isFinite(value))
        return {
          ok: false,
          issue: { kind: 'invalid_json', path, message: 'number must be finite' },
        };
      continue;
    }
    if (typeof value === 'string') {
      if (hasLoneSurrogateV1(value))
        return {
          ok: false,
          issue: { kind: 'invalid_json', path, message: 'lone surrogate is not allowed' },
        };
      continue;
    }
    if (typeof value !== 'object')
      return { ok: false, issue: { kind: 'invalid_json', path, message: 'value is not JSON' } };
    if (active.has(value))
      return {
        ok: false,
        issue: { kind: 'invalid_json', path, message: 'cyclic values are not allowed' },
      };
    if (Object.getOwnPropertySymbols(value).length > 0) {
      return {
        ok: false,
        issue: { kind: 'invalid_json', path, message: 'symbol keys are not allowed' },
      };
    }
    if (!Array.isArray(value) && !isPlainObject(value)) {
      return {
        ok: false,
        issue: { kind: 'invalid_json', path, message: 'object must have a plain prototype' },
      };
    }
    ...
    if (ownStringKeys.length !== keys.length) {
      return {
        ok: false,
        issue: { kind: 'invalid_json', path, message: 'non-enumerable properties are not allowed' },
      };
    }
    if (keys.length > MAX_JSON_CONTAINER_ENTRIES) {
      return {
        ok: false,
        issue: {
          kind: 'json_container_entries',
          path,
          message: 'JSON container entries exceeded',
          actual: keys.length,
          maximum: MAX_JSON_CONTAINER_ENTRIES,
        },
      };
    }
    if (Array.isArray(value) && keys.length !== value.length) {
      return {
        ok: false,
        issue: { kind: 'invalid_json', path, message: 'sparse arrays are not allowed' },
      };
    }
      ...
      if (hasLoneSurrogateV1(key)) {
        return {
          ...
          issue: {
            kind: 'invalid_json',
            path: [...path, key],
            message: 'lone surrogate key is not allowed',
          },
        };
      }
    }
  ...
  return {
    ok: true,
    value: input as JsonValue,
    canonicalBytes: canonicalBytesV1(input as JsonValue),
  };
};

const canonicalBytesV1 = (input: JsonValue): Uint8Array => {
  const output: string[] = [];
  type CanonicalTask = { kind: 'value'; value: JsonValue } | { kind: 'token'; value: string };
...
const runBytesKernel = (
  ...
  if (hasDuplicateObjectKeysV1(source)) {
    return {
      ok: false,
      issue: { kind: 'invalid_json', path: [], message: 'duplicate object member' },
    };
  }
  let input: unknown;
  try {
...
): KernelResultV1<z.output<Schema>> => {
