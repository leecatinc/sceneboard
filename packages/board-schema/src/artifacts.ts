...
import { ARTIFACT_REQUEST_CAPABILITIES_V1, BOARD_ERROR_CODES_V1 } from './catalogs.js';
import {
  ArtifactIdSchemaV1,
...
export const ArtifactPathSchemaV1 = z
  .string()
  .refine(isNormalizedArtifactPathV1, 'artifact path must be normalized');
...
export const ArtifactManifestSchemaV1 = z
  .object({
    ...
    requestedCapabilities: z
      .array(ArtifactRequestCapabilitySchemaV1)
  ...
  .superRefine((manifest, context) => {
    ...
    for (let index = 0; index < manifest.resources.length; index += 1) {
      ...
      if (paths.has(resource.path))
        context.addIssue({
          ...
          message: 'duplicate artifact resource path',
        });
    ...
    if (!paths.has(manifest.entryPath))
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['entryPath'],
        message: 'entryPath must reference a resource',
      });
    if (totalBytes > MAX_ARTIFACT_TOTAL_BYTES)
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['resources'],
        message: '[LIMIT:maxArtifactTotalBytes] artifact total bytes exceeded',
      });
    for (let index = 0; index < manifest.requestedCapabilities.length; index += 1) {
      if (
        index > 0 &&
        (manifest.requestedCapabilities[index - 1] ?? '') >=
          (manifest.requestedCapabilities[index] ?? '')
      ) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['requestedCapabilities'],
          message: 'requestedCapabilities must be sorted and unique',
        });
        break;
      }
    }
...
export const ArtifactRuntimeSummarySchemaV1 = z
  ...
  .superRefine((summary, context) => {
    ...
    if (requiresFailure !== (summary.failure !== null))
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['failure'],
