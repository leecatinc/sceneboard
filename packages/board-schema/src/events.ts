...
import { BoardSnapshotSchema, BoardSnapshotSchemaV1 } from './snapshots.js';
...
const BoardEventDataSchemaV2 = z.discriminatedUnion('type', [
...
type BoardEventValidationInput = {
  boardId: string;
  sequence: number;
  occurredAt: string;
  revisionId: string | null;
  data: z.infer<typeof BoardEventDataSchemaV2>;
};

const validateBoardEventEnvelope = (event: BoardEventValidationInput, context: z.RefinementCtx) => {
...
const createBoardEventEnvelopeSchema = <DataSchema extends z.ZodTypeAny>(dataSchema: DataSchema) =>
  z
    .object({
    ...
    .superRefine((event, context) =>
      validateBoardEventEnvelope(event as BoardEventValidationInput, context),
    );

export const BoardEventEnvelopeSchemaV1 = createBoardEventEnvelopeSchema(BoardEventDataSchemaV1);
export const BoardEventEnvelopeSchemaV2 = createBoardEventEnvelopeSchema(BoardEventDataSchemaV2);
...
export type BoardEventEnvelopeV2 = z.infer<typeof BoardEventEnvelopeSchemaV2>;
export type BoardEventDataV2 = BoardEventEnvelopeV2['data'];
