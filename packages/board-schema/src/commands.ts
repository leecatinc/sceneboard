...
const SceneReplaceCommandSchemaV1 = z.object({ type: z.literal('scene.replace'), scene: SceneSchemaV1 }).strict();
const SceneClearCommandSchemaV1 = z.object({ type: z.literal('scene.clear') }).strict();
const SceneRestoreCommandSchemaV1 = z.object({ type: z.literal('scene.restore'), sourceRevisionId: RevisionIdSchemaV1 }).strict();
const HitlRequestCommandSchemaV1 = z.object({ type: z.literal('hitl.request'), hitlRequestId: HitlRequestIdSchemaV1, request: HitlRequestDefinitionSchemaV1 }).strict();
const HitlRespondCommandSchemaV1 = z.object({ type: z.literal('hitl.respond'), hitlRequestId: HitlRequestIdSchemaV1, response: HitlResponseSchemaV1 }).strict();
const ArtifactPublishCommandSchemaV1 = z.object({ type: z.literal('artifact.publish'), manifest: ArtifactManifestSchemaV1 }).strict();
const ArtifactStopCommandSchemaV1 = z.object({ type: z.literal('artifact.stop'), artifact: ArtifactReferenceSchemaV1, reason: ShortTextSchemaV1.optional() }).strict();

export const BoardMutationCommandSchemaV1 = z.discriminatedUnion('type', [SceneReplaceCommandSchemaV1, SceneClearCommandSchemaV1, SceneRestoreCommandSchemaV1, HitlRequestCommandSchemaV1, HitlRespondCommandSchemaV1, ArtifactPublishCommandSchemaV1, ArtifactStopCommandSchemaV1]);
...
export const MutationEnvelopeSchemaV1 = z.object({ ...MutationRequestShapeV1, actor: ActorContextSchemaV1 }).strict();
...
const MutationResultDataSchemaV1 = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('scene.restore'), sourceRevisionId: RevisionIdSchemaV1, revision: RevisionSummarySchemaV1 }).strict(),
...
export const MutationResultSchemaV1 = z
  .object({ protocolVersion: z.literal(1), type: z.literal('mutation.result'), requestId: RequestIdSchemaV1, boardId: BoardIdSchemaV1, replayed: z.boolean(), eventIds: z.array(EventIdSchemaV1), result: MutationResultDataSchemaV1 })
  .strict()
  .superRefine((result, context) => {
    if (new Set(result.eventIds).size !== result.eventIds.length) context.addIssue({ code: z.ZodIssueCode.custom, path: ['eventIds'], message: 'event IDs must be unique' });
  });
