...
const HitlContentSchemaV1 = ContentTextSchemaV1.refine(
  (value) => scalarLengthV1(value) <= MAX_MARKDOWN_CHARS,
  '[LIMIT:maxMarkdownChars] HITL body is too long',
);
const HitlTextValueSchemaV1 = ContentTextSchemaV1.refine(
...
const FormRequestSchemaV1 = z
  ...
  .strict();
const ConfirmationRequestSchemaV1 = z
  .object({
...
export const HitlRequestDefinitionSchemaV1 = z
  ...
  .superRefine((definition, context) => {
    if (definition.kind === 'choice') {
      ...
      if (
        !definition.multiple &&
        (definition.minSelections !== 1 || definition.maxSelections !== 1)
      )
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['minSelections'],
      ...
      if (
        definition.minSelections > definition.maxSelections ||
      ...
      definition.fields.forEach((field, index) => {
        ...
        if (field.type === 'text') {
          ...
          if (
            ...
            ((field.min !== null && field.defaultValue < field.min) ||
              (field.max !== null && field.defaultValue > field.max))
          )
            context.addIssue({
              code: z.ZodIssueCode.custom,
              path: ['fields', index, 'defaultValue'],
          ...
          if (
            field.defaultValue !== null &&
            !field.options.some((option) => option.id === field.defaultValue)
          )
            context.addIssue({
              code: z.ZodIssueCode.custom,
              path: ['fields', index, 'defaultValue'],
...
const InfoResponseSchemaV1 = z
  .object({ kind: z.literal('info'), acknowledged: z.literal(true) })
  .strict();
const ChoiceResponseSchemaV1 = z
  .object({
    kind: z.literal('choice'),
    selectedOptionIds: z.array(LocalFieldIdSchemaV1).min(1).max(MAX_HITL_OPTIONS),
  })
  .strict();
const FormResponseSchemaV1 = z
  .object({
    kind: z.literal('form'),
    values: z.record(
      LocalFieldIdSchemaV1,
  ...
  .strict();
const ConfirmationResponseSchemaV1 = z
  .object({ kind: z.literal('confirmation'), confirmed: z.boolean() })
  .strict();
...
const validateResponseAgainstDefinition = (
  ...
  if (definition.kind === 'choice' && response.kind === 'choice') {
    ...
    if (
      response.selectedOptionIds.length < definition.minSelections ||
      response.selectedOptionIds.length > definition.maxSelections
    )
  ...
  if (definition.kind === 'form' && response.kind === 'form') {
    ...
    if (
      keys.length !== definition.fields.length ||
      definition.fields.some((field) => !Object.hasOwn(response.values, field.id))
    )
...
export const HitlRequestSuccessSchemaV1 = HitlInteractionSchemaV1.refine(
  (interaction) => interaction.state === 'open',
  { path: ['state'], message: 'request success must be open' },
);
export const HitlRespondSuccessSchemaV1 = HitlInteractionSchemaV1.refine(
