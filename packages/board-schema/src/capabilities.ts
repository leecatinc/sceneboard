...
const exactCatalog = <T extends readonly [string, ...string[]]>(catalog: T) =>
  z.array(z.enum(catalog)).length(catalog.length).superRefine((values, context) => {
    if (values.some((value, index) => value !== catalog[index])) context.addIssue({ code: z.ZodIssueCode.custom, message: 'catalog order must match protocol' });
  });
const sortedSubset = <T extends readonly [string, ...string[]]>(catalog: T) => z.array(z.enum(catalog)).superRefine((values, context) => {
  for (let index = 1; index < values.length; index += 1) if ((values[index - 1] ?? '') >= (values[index] ?? '')) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: 'capabilities must be sorted and unique' });
    break;
  }
});

const BoardLimitsSchemaV1 = z.object(Object.fromEntries(Object.entries(BOARD_LIMITS_V1).map(([key, value]) => [key, z.literal(value)])) as { [K in keyof typeof BOARD_LIMITS_V1]: z.ZodLiteral<(typeof BOARD_LIMITS_V1)[K]> }).strict();
...
export const BoardCapabilitiesSchemaV1 = z
  .object({
    ...
    supported: z.object({ nodeTypes: exactCatalog(NODE_TYPES_V1), commandTypes: exactCatalog(BOARD_MUTATION_COMMAND_TYPES_V1), operationTypes: exactCatalog(BOARD_OPERATION_TYPES_V1), eventTypes: exactCatalog(BOARD_EVENT_TYPES_V1), hitlKinds: exactCatalog(HITL_KINDS_V1), artifactRequestCapabilities: exactCatalog(ARTIFACT_REQUEST_CAPABILITIES_V1) }).strict(),
