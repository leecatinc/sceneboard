...
const nestedScene = (depth: number) => {
  ...
  for (let level = depth - 1; level >= 1; level -= 1) {
    root = {
      id: `node${level}`,
      type: 'layout.tabs',
      activeTabId: `tab${level}`,
      tabs: [{ tabId: `tab${level}`, label: `Tab ${level}`, node: root }],
    };
  }
...
test('rejects unknown types after earlier scene limits', () => {
  const unknown = {
    protocolVersion: 1,
    type: 'scene',
    root: { id: 'root', type: 'content.unknown' },
  };
...
test('enforces duplicate IDs and layout correlations', async () => {
  for (const path of [
    'invalid/scene-duplicate-node-id.v1.json',
    'invalid/scene-grid-overlap.v1.json',
    'invalid/scene-tabs-missing-active.v1.json',
  ]) {
...
test('guards standalone nodes with the recursive scene policy', async () => {
  ...
  assert.equal(result.ok, true);
  const extra = BoardNodeParserV1.parse({
    id: 'node',
...
test('uses one discriminant-indexed node dispatch and one lazy recursion edge', async () => {
  const source = await import('node:fs/promises').then(({ readFile }) =>
    readFile(new URL('../src/scene.ts', import.meta.url), 'utf8'),
  );
  ...
  const makeScene = (count: number) => ({
    ...
    root: {
      ...
      children: Array.from({ length: count }, (_, index) => ({
        node: { id: `node${index}`, type: 'content.markdown', markdown: 'ok' },
        weight: 1,
      })),
    },
  });
...
test('selects scene bytes and typed row limits before an unknown sibling type', () => {
  ...
  const table = {
    id: 'table',
    type: 'content.table',
    columns: [{ key: 'value', label: 'Value', valueType: 'string' }],
    rows: Array.from({ length: 501 }, (_, index) => ({
      id: `row${index}`,
      cells: { value: 'ok' },
    })),
  };
  const rowFault = SceneParserV1.parse({
    protocolVersion: 1,
    type: 'scene',
    root: {
      id: 'root',
  ...
  const largeChildren = Array.from({ length: 4 }, (_, index) => ({
    node: {
      id: `code${index}`,
      type: 'content.code',
      language: 'txt',
      code: 'a'.repeat(200_000),
      showLineNumbers: false,
      wrap: false,
    },
    weight: 1,
  }));
  const byteFault = SceneParserV1.parse({
    protocolVersion: 1,
    type: 'scene',
    root: {
      id: 'large',
      ...
      direction: 'horizontal',
      gap: 0,
      children: [{ node: unknown, weight: 1 }, ...largeChildren],
    },
  });
...
test('keeps maximum scene depth reachable through every required scene-bearing carrier', async () => {
  ...
  const operation = (await loadFixture('valid/operation-result-board-get.v1.json')) as Record<
    string,
    unknown
  >;
  const event = (await loadFixture('valid/event-board-snapshot.v1.json')) as Record<
    string,
    unknown
  >;
  const mutation = (await loadFixture('valid/mutation-request-scene-replace.v1.json')) as Record<
    string,
    unknown
  >;
  ...
  const carriers = [
    ...
    [
      MutationRequestParserV1,
      { ...mutation, command: { type: 'scene.replace', scene: atLimit } },
      { ...mutation, command: { type: 'scene.replace', scene: overLimit } },
    ],
    ...
    [
      BoardOperationResultParserV1,
      {
        ...
        result: {
          ...
          snapshot: {
            ...((operation.result as Record<string, unknown>).snapshot as object),
            scene: atLimit,
          },
        },
      },
    ...
    [
      BoardEventEnvelopeParserV1,
      {
        ...
        data: {
          ...
          snapshot: {
            ...((event.data as Record<string, unknown>).snapshot as object),
            scene: atLimit,
          },
        },
      },
...
test('keeps the canonical scene byte ceiling reachable through required wrappers', async () => {
  ...
  const lastCode = (
    (overLimit.root as Record<string, unknown>).children as Array<{ node: { code: string } }>
  ).at(-1)?.node;
  assert.ok(lastCode);
  lastCode.code += 'a';
  ...
  for (const [parser, valid, invalid] of carriers) {
    ...
    if (!result.ok) {
      ...
      assert.deepEqual(result.error.details, {
        scope: 'scene',
        actualBytes: BOARD_LIMITS_V1.maxSceneBytes + 1,
        maximumBytes: BOARD_LIMITS_V1.maxSceneBytes,
      });
    }
  }
});
...
test('enforces every aggregate scene collection at its advertised boundary', async (context) => {
  const cases = [
    {
      ...
      make: (count: number) => ({
        ...
        gap: 0,
        children: Array.from({ length: count }, (_, index) => ({
          node: markdownNode(`node${index}`),
          column: (index % 24) + 1,
          row: Math.floor(index / 24) + 1,
          columnSpan: 1,
          rowSpan: 1,
        })),
      }),
        ...
        rows: [
          {
            ...
            cells: Object.fromEntries(
              Array.from({ length: count }, (_, index) => [`column${index}`, 'ok']),
            ),
          },
        ],
      }),
  ...
  for (const item of cases) {
    await context.test(item.name, () => {
      assert.equal(
        SceneParserV1.parse({ protocolVersion: 1, type: 'scene', root: item.make(item.maximum) })
          .ok,
        true,
      );
      const result = SceneParserV1.parse({
        protocolVersion: 1,
        type: 'scene',
        root: item.make(item.maximum + 1),
      });
      assert.equal(result.ok, false);
      if (!result.ok) {
...
test('proves Unicode scalar counting for BMP, astral, and combining text', () => {
  for (const character of ['a', '𐀀', 'é']) {
    ...
    const accepted = SceneParserV1.parse({
      protocolVersion: 1,
      type: 'scene',
      root: { id: 'root', type: 'content.markdown', markdown: atLimit },
    });
    assert.equal(accepted.ok, true);
    const over = SceneParserV1.parse({
      protocolVersion: 1,
      type: 'scene',
      root: { id: 'root', type: 'content.markdown', markdown: `${atLimit}${character}` },
    });
    assert.equal(over.ok, false);
    if (!over.ok) assert.equal(over.error.code, 'LIMIT_EXCEEDED');
  }
...
test('returns earlier byte and domain limits before unknown node dispatch in standalone and wrapped scenes', async () => {
  ...
  ((byteScene.root as Record<string, unknown>).children as unknown[]).push({
    node: { id: 'unknown', type: 'content.unknown' },
    weight: 1,
  });
  const mutation = (await loadFixture('valid/mutation-request-scene-replace.v1.json')) as Record<
    string,
  ...
  for (const [parser, input] of [
    ...
    [
      MutationRequestParserV1,
      { ...mutation, command: { type: 'scene.replace', scene: byteScene } },
    ],
  ] as const) {
  ...
  const domainScene = {
    ...
    root: {
      ...
      gap: 0,
      children: [
        { node: { id: 'unknown', type: 'content.unknown' }, weight: 1 },
        { node: table, weight: 1 },
      ],
    },
  };
  ...
  if (!standalone.ok)
    assert.deepEqual(standalone.error.details, {
      limit: 'maxTableRows',
      actual: 501,
      maximum: 500,
      path: ['root', 'children', 1, 'node', 'rows'],
    });
  const wrapped = MutationRequestParserV1.parse({
    ...mutation,
    command: { type: 'scene.replace', scene: domainScene },
  });
  assert.equal(wrapped.ok, false);
  if (!wrapped.ok)
    assert.deepEqual(wrapped.error.details, {
      limit: 'maxTableRows',
      actual: 501,
      maximum: 500,
      path: ['command', 'scene', 'root', 'children', 1, 'node', 'rows'],
    });
});
...
test('keeps remaining numeric, text, and cumulative scene limits reachable', async (context) => {
  const expectBoundary = async (
    ...
    context.test(name, () => {
      assert.equal(
        SceneParserV1.parse({ protocolVersion: 1, type: 'scene', root: validRoot }).ok,
        true,
      );
  ...
  await expectBoundary(
    ...
    'maxGridColumns',
    {
      ...
      rows: 1,
      gap: 0,
      children: [{ node: markdownNode('child'), column: 1, row: 1, columnSpan: 1, rowSpan: 1 }],
    },
    {
      id: 'root',
      type: 'layout.grid',
  ...
  await expectBoundary(
    ...
    'maxCanvasExtent',
    {
      ...
      width: 100_000,
      height: 100_000,
      children: [{ node: markdownNode('child'), x: 0, y: 0, width: 1, height: 1, zIndex: 0 }],
    },
    {
      id: 'root',
      type: 'layout.canvas',
  ...
  const table = (rowCount: number) => ({
    ...
    type: 'content.table',
    columns,
    rows: Array.from({ length: rowCount }, (_, index) => ({ id: `row${index}`, cells })),
  });
  ...
  if (!tableOver.ok)
    assert.deepEqual(tableOver.error.details, {
      limit: 'maxTableCells',
      actual: 10_050,
      maximum: 10_000,
      path: ['root', 'rows'],
  ...
  const chart = (secondSeriesCount: number) => ({
    id: 'root',
    type: 'content.chart',
    chartType: 'line',
    xAxis: { valueType: 'number' },
    ...
    series: [
      {
        id: 'series0',
        label: 'Series 0',
        points: Array.from({ length: 5_000 }, (_, index) => ({ x: index, y: index })),
      },
      {
        id: 'series1',
        label: 'Series 1',
        points: Array.from({ length: secondSeriesCount }, (_, index) => ({ x: index, y: index })),
      },
    ],
  });
