...
const mergePatch = (value: unknown, patch: unknown): unknown => {
  ...
  const output = value !== null && typeof value === 'object' && !Array.isArray(value)
...
const classifyMutationReuse = (original: MutationEnvelopeV1, attempted: MutationEnvelopeV1): string | null => {
  if (original.actor.grantId !== attempted.actor.grantId) return 'grant_changed';
  if (canonicalText(original.actor.scopes) !== canonicalText(attempted.actor.scopes)) return 'scopes_changed';
  if (original.expectedRevisionId !== attempted.expectedRevisionId) return 'expected_revision_changed';
  if (canonicalText(original.command) !== canonicalText(attempted.command)) return 'payload_changed';
  return null;
};
...
const classifyLifecycleReuse = (
  ...
  if (originalRequest.type === 'board.create' && attemptedRequest.type === 'board.create' && originalRequest.title !== attemptedRequest.title) return 'title_changed';
  return null;
};

const classifyHitlRequestOutcome = (input: JsonRecord): string => {
  if (input.sameKey === true) return input.sameFingerprint === true ? 'stored_replay' : 'IDEMPOTENCY_KEY_REUSED';
...
const classifyHitlRespondOutcome = (input: JsonRecord): string => {
  ...
  if (state === 'answered' || state === 'superseded' || state === 'cancelled') return 'HITL_RESPONSE_CONFLICT';
...
const evaluateScenario = async (fixture: ScenarioDocument): Promise<void> => {
  ...
  switch (fixture.scenario) {
    ...
    case 'idempotency-changed-payload-conflict': {
      ...
      if (expected.attemptedOperationType !== undefined) assert.equal(attempted.command.type, expected.attemptedOperationType);
      break;
    }
    case 'idempotency-scopes-reordered-replay': {
      ...
      const second = { ...base, requestId: String(expected.retryRequestId), actor: normalized[1].data.value } as MutationEnvelopeV1;
      assert.equal(mutationFingerprintText(first), mutationFingerprintText(second));
      assert.equal(buildMutationFingerprintV1({ ...base, actor: candidates[0] } as MutationEnvelopeV1).ok, false);
      break;
    }
    ...
    case 'board-create-initial-empty-head': {
      const result = BoardOperationResultParserV1.parse(await loadFixture(String(fixture.resultFixture)));
      assert.equal(result.ok, true);
    ...
    case 'board-get-initial-snapshot': {
      const created = BoardOperationResultParserV1.parse(await loadFixture(String(fixture.createResultFixture)));
      const fetched = BoardOperationResultParserV1.parse(await loadFixture(String(fixture.getResultFixture)));
      assert.equal(created.ok && fetched.ok, true);
      if (!created.ok || !fetched.ok || created.data.value.result.type !== 'board.create' || fetched.data.value.result.type !== 'board.get') break;
      assert.equal(created.data.value.result.snapshot.revision.revisionId, fetched.data.value.result.snapshot.revision.revisionId);
      assert.equal(fetched.data.value.result.snapshot.scene.root, null);
      break;
    }
    case 'first-scene-replace-from-created-head': {
      const created = BoardOperationResultParserV1.parse(await loadFixture(String(fixture.createResultFixture)));
      const request = MutationRequestParserV1.parse(await loadFixture(String(fixture.requestFixture)));
      ...
      if (!created.ok || !request.ok || !result.ok || created.data.value.result.type !== 'board.create' || result.data.value.result.type !== 'scene.replace') break;
      assert.equal(request.data.value.expectedRevisionId, created.data.value.result.snapshot.revision.revisionId);
    ...
    case 'scene-clear-after-create': {
      ...
      if (!created.ok || !result.ok || created.data.value.result.type !== 'board.create' || result.data.value.result.type !== 'scene.clear') break;
      assert.equal(result.data.value.result.revision.revisionNumber, expected.revisionNumber);
      assert.equal(result.data.value.result.revision.revisionNumber > created.data.value.result.snapshot.revision.revisionNumber, true);
      break;
    }
    case 'hitl-placement-reconnect-correlation': {
      const snapshot = asRecord(await loadFixture(String(fixture.snapshotFixture)));
      const event = BoardEventEnvelopeParserV1.parse(await loadFixture(String(fixture.eventFixture)));
    ...
    case 'board-archive-idempotency-replay': {
      const original = await loadFixture(String(fixture.originalFixture)) as BoardLifecycleIdempotencyEnvelopeV1;
      const attempted = mergePatch(original, fixture.attemptedPatch) as BoardLifecycleIdempotencyEnvelopeV1;
    ...
    case 'snapshot-event-reconnect-watermark': {
      const result = BoardEventEnvelopeParserV1.parse(await loadFixture(String(fixture.eventFixture)));
      assert.equal(result.ok, true);
      if (!result.ok) break;
      ...
      for (const sequence of fixture.duplicateSequences as number[]) assert.equal(sequence <= event.sequence, true);
      assert.equal(expected.firstAcceptedSequence, event.sequence + 1);
      break;
    }
    case 'hitl-terminal-chronology-boundaries': {
      for (const path of fixture.validFixtures as string[]) assert.equal(HitlInteractionParserV1.parse(await loadFixture(path)).ok, true, path);
      for (const path of fixture.invalidFixtures as string[]) assert.equal(HitlInteractionParserV1.parse(await loadFixture(path)).ok, false, path);
      for (const item of fixture.readCases as JsonRecord[]) {
        const changed = Date.parse(String(item.returnedStateUpdatedAt)) > Date.parse(String(item.afterStateUpdatedAt));
    ...
    case 'hitl-request-result-outcomes': {
      ...
      if (!request.ok || !result.ok || request.data.value.command.type !== 'hitl.request' || result.data.value.result.type !== 'hitl.request') break;
      assert.equal(request.data.value.command.hitlRequestId, result.data.value.result.hitl.hitlRequestId);
      assert.equal(canonicalText(request.data.value.command.request), canonicalText(result.data.value.result.hitl.definition));
      assert.equal(result.data.value.result.hitl.state, 'open');
      for (const item of fixture.outcomeCases as JsonRecord[]) assert.equal(classifyHitlRequestOutcome(item), item.outcome);
      break;
    }
    case 'hitl-respond-result-outcomes': {
      ...
      const result = MutationResultParserV1.parse(await loadFixture(String(fixture.resultFixture)));
      assert.equal(request.ok && result.ok, true);
      if (!request.ok || !result.ok || request.data.value.command.type !== 'hitl.respond' || result.data.value.result.type !== 'hitl.respond') break;
      assert.equal(request.data.value.command.hitlRequestId, result.data.value.result.hitl.hitlRequestId);
      assert.equal(canonicalText(request.data.value.command.response), canonicalText(result.data.value.result.hitl.response));
      assert.equal(result.data.value.result.hitl.state, 'answered');
      for (const item of fixture.outcomeCases as JsonRecord[]) assert.equal(classifyHitlRespondOutcome(item), item.outcome);
      break;
    }
    default:
...
test('accepts exact v1 and rejects added fields and both major mismatches', async () => {
  ...
  assert.equal(SceneParserV1.parse(await loadFixture('invalid/protocol-v1-added-field.v1.json')).ok, false);
