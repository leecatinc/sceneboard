...
const canonicalParser: BoardContractParserV1<unknown> = {
  ...
  parseBytes: (bytes) => {
    try {
      return canonicalizeJsonV1(JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes)) as unknown);
    } catch {
...
test('parses every valid fixture through its declared public parser', async () => {
  for (const entry of FIXTURE_CATALOG) {
    ...
    if (decoded.ok && encoded.ok) assert.deepEqual(encoded.data.canonicalBytes, decoded.data.canonicalBytes, entry.path);
  }
});
...
test('rejects every invalid fixture with exact catalog metadata', async () => {
  for (const entry of FIXTURE_CATALOG) {
    ...
    const actualPath = result.error.details !== null && 'path' in result.error.details ? result.error.details.path : null;
    ...
    if (!encoded.ok) assert.deepEqual(encoded.error, result.error, `${entry.path} decoded/bytes equivalence`);
  }
});
...
test('canonicalizes scalar-key order, negative zero, and Unicode without normalization', () => {
  ...
  if (result.ok) assert.equal(new TextDecoder().decode(result.data.canonicalBytes), '{"a":",","":"é","𐀀":0}');
});
...
test('fails safely for non-JSON values and hostile nesting', () => {
  ...
  cyclic.self = cyclic;
  values.push(cyclic);
  for (const value of values) assert.doesNotThrow(() => assert.equal(canonicalizeJsonV1(value).ok, false));
});
...
test('applies raw framing checks before decoded parsing', () => {
  ...
  const duplicate = SceneParserV1.parseBytes(new TextEncoder().encode('{"protocolVersion":1,"type":"scene","root":null,"root":null}'));
...
test('keeps the generic JSON fan-out ceiling reachable and rejects one over', () => {
  const atLimit = Object.fromEntries(Array.from({ length: 10_000 }, (_, index) => [`key${index}`, index]));
  assert.equal(canonicalizeJsonV1(atLimit).ok, true);
  const overLimit = { ...atLimit, key10000: 10_000 };
  const result = canonicalizeJsonV1(overLimit);
  assert.equal(result.ok, false);
  if (!result.ok) assert.deepEqual(result.error.details, { limit: 'maxJsonContainerEntries', actual: 10_001, maximum: 10_000, path: [] });
});
...
test('all public parsers return errors as values for unsafe input and malformed bytes', () => {
  ...
  for (const [name, parser] of Object.entries(parsers)) {
    ...
    assert.doesNotThrow(() => assert.equal(parser.parseBytes(malformed).ok, false), `${name} bytes`);
  }
});
...
test('documents the intentional raw-only framing divergence', async () => {
  ...
  assert.equal(SceneParserV1.parseBytes(new TextEncoder().encode('{"protocolVersion":1,"type":"scene","root":null,"root":null}')).ok, false);
