...
test('keeps the exact twenty-four schema source owners', async () => {
...
test('prevents reverse shared-package and application imports', async () => {
  ...
  for (const file of files) {
    ...
    assert.doesNotMatch(source, /@leecat-board\/(board-sdk|board-ui|artifact-runtime)/, file.pathname);
    assert.doesNotMatch(source, /leecat-board-(nextjs|nestjs|mcp)/, file.pathname);
    if (!file.pathname.endsWith('/parsers.ts') && !file.pathname.endsWith('/index.ts')) assert.doesNotMatch(source, /['"]\.\/parsers\.js['"]|['"]\.\/index\.js['"]/, file.pathname);
  }
});
...
test('keeps the runtime value-import graph acyclic', async () => {
  const files = await collect(sourceRoot);
  const byName = new Map(files.map((file) => [file.pathname.replace(sourceRoot.pathname, '').replace(/\.ts$/, ''), file]));
  const graph = new Map<string, string[]>();
  for (const [name, file] of byName) {
    const source = await readFile(file, 'utf8');
    const imports = [...source.matchAll(/^import(?!\s+type\b)[\s\S]*?from\s+['"](\.\.?\/[^'"]+)\.js['"];?$/gm)]
      .map((match) => match[1])
...
test('keeps shared primitive and revision runtime ownership in identifiers', async () => {
  ...
  for (const [symbol, consumers] of expectedConsumers) {
    ...
    for (const file of files) {
      ...
      assert.equal(new RegExp(`(?:const|let|var)\\s+${symbol}\\s*=`).test(source), false, `${name} duplicates ${symbol}`);
    }
...
test('uses discriminant-indexed dispatch for every exact tagged catalog', async () => {
  const expectations = new Map<string, Array<[string, number]>>([
    ...
    ['hitl', [["z.discriminatedUnion('type'", 1], [".discriminatedUnion('kind'", 2]]],
    ['commands', [["z.discriminatedUnion('type'", 2]]],
    ['operations', [["z.discriminatedUnion('type'", 2]]],
    ['events', [["z.discriminatedUnion('type'", 1]]],
    ['errors', [["z.discriminatedUnion('code'", 1], ["z.discriminatedUnion('scope'", 1]]],
  ]);
  const files = await collect(sourceRoot);
  ...
  for (const [file, patterns] of expectations) {
    ...
    for (const [pattern, minimum] of patterns) assert.equal(source.split(pattern).length - 1 >= minimum, true, `${file}: ${pattern}`);
  }
});
