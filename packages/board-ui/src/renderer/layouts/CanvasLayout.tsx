  ...
  return (
    <section className="scene-layout scene-canvas" aria-label={node.title ?? 'Canvas'}>
      <div className="scene-canvas-controls" aria-label="Canvas zoom controls">
        ...
        <button type="button" onClick={() => setZoom((value) => Math.min(2, value + 0.25))} aria-label="Zoom in">+</button>
      ...
      <details className="scene-canvas-list"><summary>Readable canvas contents</summary><ol>{node.children.map((child) => <li key={child.node.id}>{child.node.title ?? child.node.type}</li>)}</ol></details>
    </section>
  );
};
