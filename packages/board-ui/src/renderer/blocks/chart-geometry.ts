...
const chartXValues = (node: ChartNodeV1): Array<string | number> =>
  node.series.flatMap((series) => series.points.map((point) => point.x));
...
const yDomain = (node: ChartNodeV1): [number, number] | null => {
  ...
  if (node.yAxis.min !== undefined && node.yAxis.max !== undefined && minimum === maximum)
    return null;
  if (minimum === maximum) {
  ...
  if (node.yAxis.min !== undefined && maximum <= minimum)
    maximum = minimum + Math.max(1, Math.abs(minimum) * 0.01);
  if (node.yAxis.max !== undefined && minimum >= maximum)
    minimum = maximum - Math.max(1, Math.abs(maximum) * 0.01);
  return [minimum, maximum];
};

export const buildChartGeometryV1 = (node: ChartNodeV1): ChartGeometryV1 => {
  const categoryDomain =
    node.xAxis.valueType === 'category' ? [...new Set(chartXValues(node).map(String))] : [];
  const domain = yDomain(node);
  if (domain === null)
    return { tableOnly: true, points: [], categoryDomain, yMinimum: null, yMaximum: null };
  const [minimum, maximum] = domain;
  const points = node.series.flatMap((series) =>
  ...
  const safe = points.every(
    (point) => Number.isFinite(point.x) && (point.y === null || Number.isFinite(point.y)),
  );
  return {
    tableOnly: !safe,
    points: safe ? points : [],
