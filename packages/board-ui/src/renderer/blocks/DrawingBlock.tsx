...
import type { DrawingNodeV1, DrawingStyleV1 } from '@leecat-board/board-schema';
...
function isResettable(current: ArtifactViewTransformV1, centered: ArtifactViewTransformV1) {
  return Math.abs(current.scale - 1) > 0.000_001
    || Math.abs(current.x - centered.x) > 0.5
    || Math.abs(current.y - centered.y) > 0.5;
}

function InteractiveDrawingBlock({ node, controller }: { node: DrawingNodeV1; controller: DrawingViewControllerV1 }) {
  ...
  const emitState = useCallback((scale: number | null, canReset: boolean) => {
  ...
  const applyLayout = useCallback((resetActual = false) => {
    ...
    if (viewport === null || stage === null || stage === undefined || plane === null || plane === undefined) return;
    ...
    if (mode === 'actual') {
      centeredRef.current = centerArtifactViewV1({ availableWidth, availableHeight, contentWidth, contentHeight });
      const next = resetActual ? centeredRef.current : transformRef.current;
      const stageSize = sizeArtifactStageV1({ mode, availableWidth, availableHeight, contentWidth, contentHeight, scale: next.scale });
    ...
    const next = fitArtifactViewV1({ mode, availableWidth, availableHeight, contentWidth, contentHeight });
    const stageSize = sizeArtifactStageV1({ mode, availableWidth, availableHeight, contentWidth, contentHeight, scale: next.scale });
    stage.style.width = `${stageSize.width}px`;
    stage.style.height = `${stageSize.height}px`;
    writeTransform(next);
  ...
  const applyWheel = useCallback((event: WheelEvent) => {
  ...
  useEffect(() => {
    ...
    applyLayout(true);
    const observer = new ResizeObserver(() => applyLayout(latestControllerRef.current.mode === 'actual' && !isResettable(transformRef.current, centeredRef.current)));
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const next = panArtifactViewV1(transformRef.current, event.clientX - panPointRef.current.x, event.clientY - panPointRef.current.y);
    panPointRef.current = { x: event.clientX, y: event.clientY };
    writeTransform(next);
    emitState(next.scale, isResettable(next, centeredRef.current));
  };
  ...
  return (
    <figure className="scene-block scene-drawing-block">
      ...
      <div
        ...
        onPointerUp={(event) => { if (event.button === 1) stopPanning(event); }}
        onPointerCancel={stopPanning}
        onLostPointerCapture={stopPanning}
      >
        <div className="scene-drawing-stage">
          <div className="scene-drawing-transform"><DrawingSvg node={node} /></div>
        </div>
      </div>
      <p className="visually-hidden">Drawing with {node.elements.length} typed elements.</p>
    </figure>
