...
export const MapBlock: RendererComponentV1<'content.map'> = ({ node }) => (
  <figure className="scene-block scene-map-block">
    <figcaption>
      {node.title ?? 'Map'} <span className="scene-badge">Offline · no tiles</span>
    </figcaption>
      ...
      {node.data.features.map((feature, index) => {
        ...
        if (feature.geometry.type === 'Point') {
          ...
          return point.visible ? (
            <circle key={key} cx={point.x} cy={point.y} r="2.5" className="scene-map-mark" />
          ) : null;
        }
        if (feature.geometry.type === 'LineString') {
          const points = feature.geometry.coordinates.map((position) =>
            projectMapPositionV1(position, node.viewport),
          );
          return points.some((point) => point.visible) ? (
            <polyline
              key={key}
              points={points.map((point) => `${point.x},${point.y}`).join(' ')}
              className="scene-map-line"
            />
          ) : null;
        }
        return (
    ...
    <ol className="scene-feature-list">
      {node.data.features.map((feature, index) => (
        <li key={String(feature.id ?? index)}>
          {safeFeatureName(feature.properties, index)} · {feature.geometry.type}
        </li>
      ))}
    </ol>
  </figure>
