...
export const tokenizeSafeMarkdownV1 = (markdown: string): SafeMarkdownTokenV1[] => {
  ...
  for (const line of lines) {
    ...
    if (/^\s*(?:---+|\*\*\*+)\s*$/.test(line)) tokens.push({ type: 'separator' });
      ...
      if (heading !== null) tokens.push({ type: 'heading', level: heading[1]?.length as 1 | 2 | 3, text: heading[2] ?? '' });
      else if (unordered !== null) tokens.push({ type: 'unordered-item', text: unordered[1] ?? '' });
  ...
  if (code !== null) tokens.push({ type: 'code', language: code.language, text: code.lines.join('\n') });
  return tokens;
};
