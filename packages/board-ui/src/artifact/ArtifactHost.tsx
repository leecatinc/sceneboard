...
import { NodeIdParserV1 } from '@leecat-board/board-schema';
...
function AdmittedArtifactHost(input: ArtifactHostInputV1) {
  ...
  const emitViewState = useCallback((phase: 'register' | 'interaction' | 'unregister', scale: number) => {
  ...
  const applyLayout = useCallback((resetActual = false) => {
    const container = containerElementRef.current;
    ...
    if (container === null || stage === null || stage === undefined || transformPlane === null || transformPlane === undefined || frame === null || frame === undefined) return;
    ...
    if (mode === 'actual') {
      const next = resetActual || !hasInteractedRef.current
        ? centerArtifactViewV1({ availableWidth, availableHeight, contentWidth: size.width, contentHeight: size.height })
        : transformRef.current;
      const stageSize = sizeArtifactStageV1({ mode, availableWidth, availableHeight, contentWidth: size.width, contentHeight: size.height, scale: next.scale });
      stage.style.width = `${stageSize.width}px`;
      stage.style.height = `${stageSize.height}px`;
      writeTransform(next);
      return;
    }
    const next = fitArtifactViewV1({ mode, availableWidth, availableHeight, contentWidth: size.width, contentHeight: size.height });
    const stageSize = sizeArtifactStageV1({ mode, availableWidth, availableHeight, contentWidth: size.width, contentHeight: size.height, scale: next.scale });
    stage.style.width = `${stageSize.width}px`;
    stage.style.height = `${stageSize.height}px`;
    writeTransform(next);
  ...
  const applyResizeRequest = useCallback((request: ArtifactResizeRequestV1) => {
    ...
    sizeFrameRef.current = requestAnimationFrame(() => {
      ...
      applyLayout(!hasInteractedRef.current && (latestInputRef.current.viewMode ?? 'fit-height') === 'actual');
    });
  }, [applyLayout]);

  const onResizeRequest = useCallback((request: ArtifactResizeRequestV1) => {
    applyResizeRequest(request);
    try { latestInputRef.current.onResizeRequest?.(request); } catch { return; }
  }, [applyResizeRequest]);

  const applyNavigationIntent = useCallback((intent: ArtifactNavigationIntentV1) => {
    ...
    if (intent.type === 'artifact.navigation.wheel') {
      ...
      const next = zoomArtifactViewV1({ transform: transformRef.current, pointerX: anchor.x, pointerY: anchor.y, deltaY: intent.deltaY });
      writeTransform(next);
    ...
    if (pan.shouldMove && (intent.type === 'artifact.navigation.pan.move' || intent.type === 'artifact.navigation.pan.end')) {
      const next = panArtifactViewByInnerDeltaV1(transformRef.current, intent.deltaX, intent.deltaY);
      writeTransform(next);
  ...
  const onNavigationIntent = useCallback((intent: ArtifactNavigationIntentV1) => {
    applyNavigationIntent(intent);
    try { latestInputRef.current.onNavigationIntent?.(intent); } catch { return; }
  ...
  const bridge = useArtifactBridgeV1({ ...input, viewMode: input.viewMode ?? 'fit-height', onNavigationIntent, onResizeRequest });
  ...
  useEffect(() => {
    if (bridge.phase === 'stopped' || bridge.phase === 'blocked' || bridge.phase === 'failed' || bridge.phase === 'unsupported') resetCanvasState();
  }, [bridge.phase, resetCanvasState]);

  useEffect(() => {
    const isActiveActual = bridge.phase === 'active' && (input.viewMode ?? 'fit-height') === 'actual';
  ...
  useEffect(() => {
    const reset = advanceArtifactResetEpochV1(appliedResetEpochRef.current, input);
    if (!reset.advanced || bridge.phase !== 'active' || (input.viewMode ?? 'fit-height') !== 'actual') return;
    appliedResetEpochRef.current = reset.epoch;
    hasInteractedRef.current = false;
    applyLayout(true);
    emitViewState('interaction', 1);
  }, [applyLayout, bridge.phase, emitViewState, input.hostInstanceId, input.incarnationKey, input.resetCommand, input.viewMode]);
...
export function ArtifactHost(input: ArtifactHostInputV1) {
  ...
  const hasValidIdentity = NodeIdParserV1.parse(input.hostInstanceId).ok && input.incarnationKey === expectedIncarnation;
  if (!hasValidIdentity) {
    return (
