...
export const centerArtifactViewV1 = (input: {
  ...
  if (
    ![input.availableWidth, input.availableHeight, input.contentWidth, input.contentHeight].every(
      finitePositive,
    )
  )
...
export const zoomArtifactViewV1 = (input: {
  ...
  if (
    !finiteTransform(input.transform) ||
...
export const panArtifactViewV1 = (
  ...
  finiteTransform(transform) && Number.isFinite(deltaX) && Number.isFinite(deltaY)
...
export const mapArtifactAnchorV1 = (input: {
  ...
  if (
    ...
    input.xMillionth < 0 ||
    input.xMillionth > 1_000_000 ||
    input.yMillionth < 0 ||
    input.yMillionth > 1_000_000 ||
    input.frameWidth <= 0 ||
    input.frameHeight <= 0
  )
    return null;
