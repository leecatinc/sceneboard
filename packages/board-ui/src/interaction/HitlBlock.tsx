...
import type { BoardId, HitlInteractionV1, RevisionId } from '@sceneboard/board-schema';
...
export function HitlBlock({
  ...
  useEffect(
    () => () => {
      ...
      const stableParent = element.parentElement?.closest<HTMLElement>(
        '.scene-block, .scene-layout, .scene-root',
      );
  ...
  return (
    ...
    >
      {interaction.state === 'open' ? (
        <>
          <div className="hitl-heading-row">
      ...
      {submission.kind !== 'idle' && (
        <div
          className={`hitl-submission hitl-submission-${submission.kind}`}
          role={
            submission.kind === 'failed' || submission.kind === 'reconciliation_failed'
              ? 'alert'
              : 'status'
          }
        >
          ...
          {submission.kind === 'recording_unknown' && (
            <div className="hitl-actions">
              <button
                type="button"
                onClick={() => void controller.retry(interaction.hitlRequestId)}
              >
                Retry recording
              </button>
              {controller.canCopy(interaction.hitlRequestId) && (
                <button
                  type="button"
                  onClick={() => void controller.copy(interaction.hitlRequestId)}
                >
                  Copy reconciliation handoff
                </button>
              )}
            </div>
          )}
      ...
      {submission.kind === 'recording_unknown' && (
        <p className={copy.kind === 'failed' ? 'hitl-copy-failed' : ''} role="status">
          {copy.message}
        </p>
      )}
      <div className="hitl-live-update" aria-live="polite" aria-atomic="true">
        {liveAnnouncement !== '' && (
          <>
            ...
            <button type="button" onClick={() => terminalHeading.current?.focus()}>
              Review interaction
            </button>
          </>
        )}
