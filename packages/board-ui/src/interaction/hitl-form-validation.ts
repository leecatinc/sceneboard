...
const validateField = (field: HitlFieldV1, value: HitlFormValueV1 | undefined): string | null => {
  ...
  if (field.type === 'boolean')
    return typeof value === 'boolean' ? null : `${field.label} must be checked or cleared.`;
...
export const validateHitlFormV1 = (
  ...
  | { ok: true; response: Extract<HitlResponseV1, { kind: 'form' }> }
  | { ok: false; errors: HitlFormErrorsV1 } => {
  ...
  const response = {
    ...
    values: Object.fromEntries(
      definition.fields.map((field) => [field.id, values[field.id] ?? null]),
    ) as Record<LocalFieldId, HitlFormValueV1>,
  };
