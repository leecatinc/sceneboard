...
import { SafeMarkdownContent } from '../renderer/blocks/SafeMarkdownContent.js';
import {
...
const SECRET_WARNING =
  'Do not enter passwords, API keys, private keys, recovery codes, or authentication secrets.';
...
const intent = (props: OpenProps, response: HitlResponseV1) => ({
...
const SecretWarning = ({ id }: { id: string }) => (
  <p id={id} className="hitl-secret-warning" role="note">
    {SECRET_WARNING}
  </p>
);
...
function InfoResponse(props: OpenProps) {
  ...
  return (
    <div className="hitl-controls">
      <div className="hitl-action-footer">
        <button
          type="button"
          disabled={props.controller.isSubmitting(props.interaction.hitlRequestId)}
          onClick={() =>
            void props.controller.submit(intent(props, { kind: 'info', acknowledged: true }))
          }
        >
          {definition.acknowledgeLabel}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="hitl-controls">
      ...
      <p id={`${props.idPrefix}-choice-help`} className="scene-help" aria-live="polite">
        {selected.length} selected · minimum {definition.minSelections}, maximum{' '}
      ...
      <div className="hitl-action-footer">
        <button
          ...
          onClick={() =>
            void props.controller.submit(
              intent(props, { kind: 'choice', selectedOptionIds: selected as never }),
            )
          }
        >
          Submit response
...
function VerifiedFormResponse(
  props: OpenProps & {
    definition: Extract<HitlInteractionV1['definition'], { kind: 'form' }>;
  },
) {
  const [values, setValues] = useState<Record<string, HitlFormValueV1>>(() =>
    initialHitlFormValuesV1(props.definition),
  );
  ...
  const submit = (event: FormEvent<HTMLFormElement>): void => {
    ...
    if (!checked.ok) {
      ...
      const first = props.definition.fields.find((field) =>
        Object.hasOwn(checked.errors, field.id),
      );
  ...
  return (
    <form className="hitl-controls hitl-form" noValidate onSubmit={submit}>
      ...
      {props.definition.fields.map((field) => {
        ...
        const describedBy =
          errors[field.id] === undefined
            ? `${props.idPrefix}-secret-warning`
            : `${props.idPrefix}-secret-warning ${errorId}`;
        const common = {
        ...
        return (
          <div key={field.id} className="hitl-field">
            {field.type === 'boolean' ? (
              <label className="hitl-boolean" htmlFor={common.id}>
                <input
                  {...common}
                  ref={(element) => {
                    controls.current[field.id] = element;
                  }}
                  type="checkbox"
                  checked={value === true}
            ...
            {field.type === 'number' && (
              <input
                ...
                onChange={(event) => {
                  ...
                  setValue(
                    field.id,
                    event.currentTarget.value === '' || !Number.isFinite(number) ? null : number,
                  );
                }}
              />
            )}
            ...
            {errors[field.id] !== undefined && (
              <p id={errorId} className="hitl-field-error">
                {errors[field.id]}
              </p>
            )}
          </div>
      ...
      <p className="hitl-validation-summary" role="alert">
        {announcement}
      </p>
      <div className="hitl-action-footer">
        <button
...
function ConfirmationResponse(props: OpenProps) {
  ...
  return (
    <div
      className={`hitl-controls hitl-confirmation ${definition.impact === 'destructive' ? 'hitl-destructive' : ''}`}
    >
      {definition.impact === 'destructive' && (
        <p className="hitl-impact-warning">
          <span aria-hidden="true">⚠</span> This action may be irreversible. Nothing happens unless
          you explicitly confirm.
        </p>
      )}
      <SecretWarning id={`${props.idPrefix}-secret-warning`} />
      <div
        className="hitl-actions hitl-action-footer"
        aria-describedby={`${props.idPrefix}-secret-warning`}
      >
        <button type="button" disabled={submitting} onClick={() => answer(false)}>
          {definition.cancelLabel}
        </button>
        {definition.impact === 'standard' && (
          <button type="button" disabled={submitting} onClick={() => answer(true)}>
            {definition.confirmLabel}
          </button>
        )}
        {definition.impact === 'destructive' && !reviewed && (
          <button type="button" disabled={submitting} onClick={() => setReviewed(true)}>
            Review impact
          </button>
        )}
        {definition.impact === 'destructive' && reviewed && (
...
export function HitlOpenContent(props: OpenProps) {
  const definition = props.interaction.definition;
  return (
    <div className="hitl-open-content">
      <p className="hitl-kind-label">
        {definition.kind === 'info'
          ? 'Information'
      ...
      {'body' in definition && definition.body !== undefined && (
        <SafeMarkdownContent className="hitl-body hitl-rich-text" markdown={definition.body} />
      )}
