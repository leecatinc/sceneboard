...
const STATE_COPY = {
  ...
  superseded: { title: 'Replaced by a newer request', detail: 'This request can no longer be answered.' },
...
export const HitlTerminalSummary = forwardRef<HTMLHeadingElement, {
  ...
  return (
    <div className={`hitl-terminal hitl-terminal-${interaction.state}`}>
      ...
      {interaction.state === 'answered' && interaction.response !== null && (
        <pre className="hitl-response" aria-label="Recorded response">{JSON.stringify(interaction.response, null, 2)}</pre>
      )}
      {controller.canCopy(interaction.hitlRequestId) && (
        <div className="hitl-copy-row">
          <button type="button" onClick={() => void controller.copy(interaction.hitlRequestId)}>Copy authoritative state</button>
          <span role="status" className={copyState.kind === 'failed' ? 'hitl-copy-failed' : ''}>{copyState.message}</span>
        </div>
      )}
    </div>
  );
