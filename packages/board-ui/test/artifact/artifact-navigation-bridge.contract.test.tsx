...
import { ArtifactBridgeEndpointV1, type ArtifactNavigationIntentV1 } from '@leecat-board/artifact-runtime/bridge';
...
test('actual mode dispatches a navigation message received over a real channel exactly once', async () => {
  ...
  const intent = { type: 'artifact.navigation.wheel', xMillionth: 500_000, yMillionth: 250_000, deltaY: -120 } as const;
  ...
  const settled = new Promise<void>((resolve, reject) => {
    channel.port2.once('message', (raw) => {
      try {
        ...
        assert.equal(dispatchArtifactNavigationIntentV1('actual', message, (value) => received.push(value)), true);
        resolve();
...
test('fit modes suppress navigation without invoking the callback', () => {
  ...
  let calls = 0;
  assert.equal(dispatchArtifactNavigationIntentV1('fit-height', intent, () => { calls += 1; }), false);
  assert.equal(dispatchArtifactNavigationIntentV1('fit-width', intent, () => { calls += 1; }), false);
