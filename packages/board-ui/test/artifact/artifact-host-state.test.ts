...
import type { ArtifactResizeRequestV1 } from '@leecat-board/artifact-runtime/bridge';
import {
...
const resize = (width: number, height: number, source: 'observer' | 'explicit'): ArtifactResizeRequestV1 => ({ width, height, source });

const resetInput = (epoch: number, hostInstanceId = 'node_a', incarnationKey = 'route:node_a:artifact:version'): ArtifactHostInputV1 => ({
...
test('resize admission rejects invalid bounds and equal sizes remain a no-op', () => {
  ...
  assert.equal(admitArtifactResizeRequestV1(queue, resize(16_385, 600, 'explicit')).accepted, false);
  assert.equal(changesArtifactSizeV1({ width: 1200, height: 675 }, resize(1200, 675, 'explicit')), false);
  assert.equal(changesArtifactSizeV1({ width: 1200, height: 675 }, resize(1201, 675, 'explicit')), true);
});
...
test('reset high-water advances only for newer exact positive safe epochs', () => {
  ...
  assert.deepEqual(advanceArtifactResetEpochV1(Number.MAX_SAFE_INTEGER - 1, resetInput(Number.MAX_SAFE_INTEGER)), {
...
test('pan cancel clears host panning without requesting a transform mutation', () => {
  assert.deepEqual(applyArtifactPanIntentV1(true, { type: 'artifact.navigation.pan.cancel', pointerId: 7 }), {
    panning: false,
    shouldMove: false,
  });
});
