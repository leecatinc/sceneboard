...
const interaction = (
  ...
  state: HitlInteractionV1['state'] = 'open',
): HitlInteractionV1 => {
...
const controller = (
  mode: HitlInteractionControllerV1['mode'] = 'live',
...
test('destructive confirmation has non-approval first and no one-step positive action', () => {
  ...
  assert.match(
    html,
    /This action may be irreversible\. Nothing happens unless you explicitly confirm\./u,
  );
  assert.ok(html.indexOf('Keep data') < html.indexOf('Review impact'));
  assert.doesNotMatch(html, /Delete permanently/u);
});

test('long HITL guidance renders as structured safe content beneath a stable heading', () => {
  const current = snapshot();
  const hitl = interaction({
    kind: 'choice',
    title: 'Choose the safest release path',
    body: [
      '## Why this decision matters',
      '',
      '**Contract D1** requires the payment boundary to remain closed until inventory is known.',
      '',
      '- The customer must not be charged for unavailable inventory.',
      '- The existing checkout session remains unchanged.',
      '',
      '> This decision changes review priority only.',
    ].join('\n'),
    multiple: false,
    minSelections: 1,
    maxSelections: 1,
    options: [
      {
        id: 'payment_boundary' as never,
        label: 'Verify the payment boundary first',
        description: 'Confirm that unavailable inventory stops checkout before payment.',
      },
    ],
  });
  const html = renderToStaticMarkup(
  ...
  assert.match(html, /class="hitl-heading-row"/u);
  assert.match(html, /class="hitl-body hitl-rich-text"/u);
  assert.match(html, /<h4>Why this decision matters<\/h4>/u);
  assert.match(html, /<strong>Contract D1<\/strong>/u);
  assert.match(html, /<blockquote>This decision changes review priority only\.<\/blockquote>/u);
  assert.match(html, /The customer must not be charged for unavailable inventory\./u);
});
...
test('form validation returns exact typed values and rejects required null', () => {
  ...
  if (valid.ok)
    assert.deepEqual(valid.response, {
      kind: 'form',
      values: { name: 'Lee', count: 2, ready: false },
    });
});
