...
const entries = [
  { logicalName: 'mermaid', path: mermaidPath, sha256: mermaidHash, byteLength: mermaidBytes.byteLength, mediaType },
  { logicalName: 'outer', path: outerPath, sha256: outerHash, byteLength: outerBytes.byteLength, mediaType },
].sort((left, right) => left.path.localeCompare(right.path));
const template = await readFile(resolve(source, 'runner.html'), 'utf8');
if ((template.match(/__OUTER_ASSET_PATH__/g) ?? []).length !== 1) throw new Error('runner template outer placeholder is invalid');
...
await Promise.all([
  ...
  writeFile(resolve(publicDirectory, 'fixed-assets.v1.json'), `${JSON.stringify(entries, null, 2)}\n`, { encoding: 'utf8', flag: 'wx' }),
]);
