...
export const canonicalOriginV1 = (input: string, label = 'origin'): URL => {
  if (typeof input !== 'string' || input.length === 0 || input !== input.trim() || /[^\x20-\x7e]/u.test(input)) {
...
const evidenceV2 = (input: unknown): AuthArtifactOriginEvidenceV2 => {
  ...
  if (value.schemaVersion !== ARTIFACT_ORIGIN_EVIDENCE_SCHEMA_V2
  ...
  for (const key of ['generatedAt', 'expiresAt', 'frontendOrigin', 'apiOrigin', 'runtimeOrigin'] as const) {
...
export const assertArtifactRuntimeTopologyV1 = (
  ...
  if (input.appEnv === 'development' || input.appEnv === 'test') {
    if (frontend.origin !== 'http://127.0.0.1:3410'
    ...
    if (frontend.protocol !== 'https:' || api.protocol !== 'https:' || runtime.protocol !== 'https:') {
  ...
  if (evidence.appEnv !== input.appEnv
