...
export const postArtifactBridgeMessageV1 = (
  ...
  if (binary === undefined) {
    ...
    if (envelope.message !== message || envelope.message.type !== expectedType) throw new TypeError('artifact bridge send was mutated');
    postMessage(envelope, []);
    return;
  }
  const envelope = endpoint.send(message, { messagePorts: 0, arrayBufferBytes: [binary.byteLength] });
  if (envelope.message !== message || envelope.message.type !== expectedType) throw new TypeError('artifact bridge send was mutated');
