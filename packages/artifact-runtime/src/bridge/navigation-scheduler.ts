...
type NavigationMessageV1 = Extract<
  ArtifactBridgeMessageV1,
  { type: `artifact.navigation.${string}` }
>;
...
export class ArtifactNavigationSchedulerV1 {
  ...
  move(pointerId: number, deltaX: number, deltaY: number): void {
    const pan = this.#pan;
    if (!this.#enabled || pan === null || pan.pointerId !== pointerId || pan.terminal !== null)
      return;
    ...
    if (pan !== null) {
      if (!pan.startSent) {
        ...
        return {
          ...
          xMillionth: pan.xMillionth,
          yMillionth: pan.yMillionth,
        };
      }
      ...
      if (deltaX !== 0 || deltaY !== 0)
        return { type: 'artifact.navigation.pan.move', pointerId: pan.pointerId, deltaX, deltaY };
    }
  ...
  #hasPending(): boolean {
    const pan = this.#pan;
    if (pan !== null)
      return !pan.startSent || pan.terminal !== null || pan.deltaX !== 0 || pan.deltaY !== 0;
  ...
  #readTime(): number {
    ...
    if (!nativeNumberIsFinite(value) || value < 0)
      throw new TypeError('navigation clock is invalid');
  ...
  #boundedTotal(left: number, right: number): number {
    return nativeMathMax(
      -nativeNumberMaxSafeInteger,
      nativeMathMin(nativeNumberMaxSafeInteger, left + right),
    );
  }
