...
export type ArtifactBridgeMessageV1 =
  ...
  | { type: 'host.package.start'; transferId: Base64Url22; totalBytes: number; chunkBytes: 262_144; chunkCount: number; manifestSha256: string; packageSha256: string }
  | { type: 'host.package.chunk'; transferId: Base64Url22; index: number; offset: number; byteLength: number }
  | { type: 'runner.package.ack'; transferId: Base64Url22; index: number; receivedBytes: number }
  | { type: 'host.package.end'; transferId: Base64Url22; chunkCount: number; totalBytes: number; packageSha256: string }
  ...
  | { type: 'host.inner.init'; policyEpoch: Base64Url22; requestedCapabilities: ArtifactRequestCapabilityV1[] }
  | { type: 'artifact.ready' }
  | { type: 'host.theme'; value: { colorScheme: 'light' | 'dark'; foreground: string; background: string; accent: string; muted: string } }
  ...
  | { type: 'host.capability.result'; requestId: Base64Url22; capability: ArtifactRequestCapabilityV1; ok: boolean; result?: Record<string, unknown>; error?: 'not_requested' | 'policy_denied' | 'activation_required' | 'activation_expired' | 'invalid_request' | 'revoked' | 'timeout' | 'unavailable' }
  | { type: 'host.dispose'; reason: 'unmount' | 'route_change' | 'logout' | 'policy_change' | 'status_change' | 'user_stop' | 'timeout' | 'protocol_error' }
  | { type: 'peer.disposed' }
  | { type: 'protocol.error'; code: 'schema' | 'source' | 'origin' | 'pair' | 'session' | 'sequence' | 'state' | 'size' | 'rate' | 'transfer' | 'timeout'; correlationId: Base64Url22 };
...
const ENVELOPE_KEYS = ['protocolVersion', 'type', 'channelId', 'sessionId', 'artifact', 'sequence', 'message'] as const;

const MESSAGE_KEYS: Readonly<Record<ArtifactBridgeMessageV1['type'], readonly string[]>> = Object.freeze({
  'host.bootstrap': ['type', 'appOrigin', 'runtimeOrigin', 'policyEpoch'],
  'runner.ready': ['type', 'supportedProtocolVersions'],
  'host.package.start': ['type', 'transferId', 'totalBytes', 'chunkBytes', 'chunkCount', 'manifestSha256', 'packageSha256'],
...
const exactKeys = (input: Record<string, unknown>, allowed: readonly string[], optional: readonly string[] = []): boolean => {
...
const safePlainData = (input: unknown): void => {
  ...
  while (stack.length > 0) {
    ...
    if (Array.isArray(value)) {
      ...
      if (prototype !== Object.prototype && prototype !== null) throw new TypeError('bridge objects must be plain');
...
const capabilities = (value: unknown): ArtifactRequestCapabilityV1[] => {
  ...
  for (let index = 0; index < result.length; index += 1) {
    if (!ARTIFACT_REQUEST_CAPABILITIES_V1.includes(result[index] as ArtifactRequestCapabilityV1)
...
const ids = (value: unknown): string[] => {
  ...
  for (let index = 0; index < result.length; index += 1) {
    ...
    if (typeof item !== 'string' || !GlobalIdStringParserV1.parse(item).ok
      || (index > 0 && (strings[index - 1] ?? '') >= item)) throw new TypeError('selection must be sorted and unique');
    strings.push(item);
  }
  return strings;
};
...
const validateMessage = (message: Record<string, unknown>): ArtifactBridgeMessageV1 => {
  ...
  if (!exactKeys(message, MESSAGE_KEYS[type], optional)) throw new TypeError('bridge message keys are invalid');
  switch (type) {
    case 'host.bootstrap':
      canonicalOrigin(message.appOrigin); canonicalOrigin(message.runtimeOrigin); id22(message.policyEpoch, 'policy epoch');
      break;
    case 'runner.ready':
      if (!Array.isArray(message.supportedProtocolVersions) || message.supportedProtocolVersions.length !== 1 || message.supportedProtocolVersions[0] !== 1) throw new TypeError('supported versions are invalid');
      break;
    case 'host.package.start':
      id22(message.transferId, 'transfer ID'); integer(message.totalBytes, 1, 10_747_904, 'package bytes');
      if (message.chunkBytes !== 262_144) throw new TypeError('package chunk size is invalid');
      integer(message.chunkCount, 1, 65_535, 'package chunk count'); digest(message.manifestSha256, 'manifest digest'); digest(message.packageSha256, 'package digest');
      break;
    case 'host.package.chunk':
      id22(message.transferId, 'transfer ID'); integer(message.index, 0, 65_534, 'chunk index'); integer(message.offset, 0, 10_747_903, 'chunk offset'); integer(message.byteLength, 1, 262_144, 'chunk length');
      break;
    case 'runner.package.ack':
      id22(message.transferId, 'transfer ID'); integer(message.index, 0, 65_534, 'chunk index'); integer(message.receivedBytes, 1, 10_747_904, 'received bytes');
      break;
    case 'host.package.end':
      id22(message.transferId, 'transfer ID'); integer(message.chunkCount, 1, 65_535, 'chunk count'); integer(message.totalBytes, 1, 10_747_904, 'package bytes'); digest(message.packageSha256, 'package digest');
      break;
    case 'runner.package.ready':
      id22(message.transferId, 'transfer ID'); digest(message.packageSha256, 'package digest');
      break;
    case 'host.watchdog.ping':
    case 'runner.watchdog.pong':
      id22(message.watchdogId, 'watchdog ID');
      if (typeof message.sentAtMonotonicMs !== 'number' || !Number.isFinite(message.sentAtMonotonicMs) || message.sentAtMonotonicMs < 0) throw new TypeError('watchdog time is invalid');
      break;
    case 'host.inner.init':
      id22(message.policyEpoch, 'policy epoch'); capabilities(message.requestedCapabilities);
      break;
    case 'artifact.ready':
    case 'peer.disposed':
      break;
    case 'host.theme': {
      ...
      if (!exactKeys(value, ['colorScheme', 'foreground', 'background', 'accent', 'muted'])
        || (value.colorScheme !== 'light' && value.colorScheme !== 'dark')
        || [value.foreground, value.background, value.accent, value.muted].some((item) => typeof item !== 'string' || !COLOR.test(item))) throw new TypeError('theme is invalid');
      break;
    }
    case 'host.data':
      if (typeof message.revisionId !== 'string' || !GlobalIdStringParserV1.parse(message.revisionId).ok
        || typeof message.projectionId !== 'string' || !GlobalIdStringParserV1.parse(message.projectionId).ok) throw new TypeError('host data IDs are invalid');
      break;
    case 'host.viewport':
    ...
    case 'artifact.capability.request':
      id22(message.requestId, 'request ID');
      if (!ARTIFACT_REQUEST_CAPABILITIES_V1.includes(message.capability as ArtifactRequestCapabilityV1)) throw new TypeError('capability is invalid');
      record(message.payload, 'capability payload');
      break;
    case 'host.capability.result':
      id22(message.requestId, 'request ID');
      if (!ARTIFACT_REQUEST_CAPABILITIES_V1.includes(message.capability as ArtifactRequestCapabilityV1) || typeof message.ok !== 'boolean') throw new TypeError('capability result is invalid');
      if (message.ok ? !Object.hasOwn(message, 'result') || Object.hasOwn(message, 'error') : !Object.hasOwn(message, 'error') || Object.hasOwn(message, 'result')) throw new TypeError('capability result branch is invalid');
      break;
    case 'host.dispose':
      if (!['unmount', 'route_change', 'logout', 'policy_change', 'status_change', 'user_stop', 'timeout', 'protocol_error'].includes(String(message.reason))) throw new TypeError('dispose reason is invalid');
      break;
    case 'protocol.error':
      id22(message.correlationId, 'correlation ID');
      if (!['schema', 'source', 'origin', 'pair', 'session', 'sequence', 'state', 'size', 'rate', 'transfer', 'timeout'].includes(String(message.code))) throw new TypeError('protocol error code is invalid');
      break;
  }
  return message as ArtifactBridgeMessageV1;
};

const validateTransfers = (message: ArtifactBridgeMessageV1, transfers: ArtifactBridgeTransfersV1): void => {
  if (!Number.isInteger(transfers.messagePorts) || transfers.messagePorts < 0
    || transfers.arrayBufferBytes.some((value) => !Number.isInteger(value) || value < 0)) throw new TypeError('bridge transfer metadata is invalid');
  ...
  if (transfers.messagePorts !== portCount
...
export const parseArtifactBridgeEnvelopeV1 = (
  ...
  if (!exactKeys(source, ENVELOPE_KEYS)
    || source.protocolVersion !== 1
    || source.type !== 'artifact.bridge') throw new TypeError('bridge envelope keys are invalid');
  ...
  if (controlBytes > CONTROL_CAPS[message.type]) throw new TypeError('bridge control message exceeds its cap');
  return {
    envelope: {
      protocolVersion: 1,
      type: 'artifact.bridge',
...
export const artifactBridgeControlCapV1 = (type: ArtifactBridgeMessageV1['type']): number => CONTROL_CAPS[type];
