...
export const isChargedAuthoredMessageV1 = (
  message: Pick<ArtifactBridgeMessageV1, 'type'>,
): boolean => {
...
export class ArtifactRateBudgetV1 {
  ...
  admit(byteLength: number): boolean {
    ...
    const nextCount = Math.min(
      this.#input.countBurst,
      this.#countTokens + (elapsed * this.#input.countRate) / 1000,
    );
    const nextBytes = Math.min(
      this.#input.byteBurst,
      this.#byteTokens + (elapsed * this.#input.byteRate) / 1000,
    );
    this.#lastTime = now;
