...
export const measureArtifactContentSizeV1 = (
  ...
  const right = Math.max(...candidates.map((candidate) => Math.max(
    candidate.right - origin.left,
    candidate.left - origin.left + candidate.scrollWidth,
  )));
  const bottom = Math.max(...candidates.map((candidate) => Math.max(
    candidate.bottom - origin.top,
    candidate.top - origin.top + candidate.scrollHeight,
  )));
  return {
