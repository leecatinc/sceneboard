...
export type ArtifactHostLifecycleStateV1 =
  | 'idle'
  | 'outer_bootstrap'
  | 'runner_ready'
  | 'package_transfer'
  ...
  | 'stopped'
  | 'failed';
export type ArtifactRunnerLifecycleStateV1 =
  | 'bootstrap_wait'
  | 'ready'
  | 'receiving'
  | 'certified'
...
export class ArtifactHostStateMachineV1 {
  ...
  get state(): ArtifactHostLifecycleStateV1 {
    return this.#state;
  }
  ...
  advance(
    ...
    const next: Partial<
      Record<
        ArtifactHostLifecycleStateV1,
        Partial<Record<typeof event, ArtifactHostLifecycleStateV1>>
      >
    > = {
    ...
    if (result === undefined)
      throw new TypeError(`illegal host lifecycle edge: ${this.#state} -> ${event}`);
    this.#state = result;
    return result;
  }
}
...
export class ArtifactRunnerStateMachineV1 {
  ...
  get state(): ArtifactRunnerLifecycleStateV1 {
    return this.#state;
  }
  ...
  receive(type: ArtifactBridgeMessageV1['type']): ArtifactRunnerLifecycleStateV1 {
    ...
    if (
      ...
      ['ready', 'receiving', 'certified', 'inner_handshake', 'active'].includes(this.#state)
    )
      return this.#state;
    const next: Partial<
      Record<
        ArtifactRunnerLifecycleStateV1,
        Partial<Record<ArtifactBridgeMessageV1['type'], ArtifactRunnerLifecycleStateV1>>
      >
    > = {
    ...
    if (result === undefined)
      throw new TypeError(`illegal runner lifecycle message: ${this.#state} -> ${type}`);
    this.#state = result;
    return result;
  }
}
