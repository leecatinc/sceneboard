...
export class ArtifactNavigationAdmissionV1 {
  ...
  setEnabled(
    enabled: boolean,
  ): Extract<ArtifactNavigationIntentV1, { type: 'artifact.navigation.pan.cancel' }> | null {
    this.#armed = enabled;
    return null;
  }
  ...
  admit(message: ArtifactBridgeMessageV1, active: boolean): message is ArtifactNavigationIntentV1 {
    ...
    if (!this.#armed) {
      if (
        ...
        message.pointerId !== this.#pointerId
      )
        return false;
      this.#pointerId = null;
      return true;
    ...
    if (message.type === 'artifact.navigation.pan.move')
      return this.#pointerId === message.pointerId;
    if (
