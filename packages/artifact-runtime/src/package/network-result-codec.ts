...
export const decodeArtifactNetworkResultV1 = (input: ArrayBuffer | Uint8Array): ArtifactNetworkResultV1 => {
  const bytes = input instanceof Uint8Array ? input : new Uint8Array(input);
  if (bytes.byteLength < 18 || bytes.byteLength > 1_048_576 + 64) throw new TypeError('network result framing is invalid');
  for (let index = 0; index < NETWORK_MAGIC.length; index += 1) {
    if (bytes[index] !== NETWORK_MAGIC[index]) throw new TypeError('network result magic is invalid');
  }
  ...
  try {
    mediaType = new TextDecoder('utf-8', { fatal: true }).decode(bytes.subarray(14, bodyLengthOffset));
  } catch {
