...
export const decodeArtifactPackageV1 = async (
  ...
  if (manifestLength === 0 || manifestLength > MAX_FRAMING_BYTES) throw new TypeError('artifact manifest length is invalid');
  ...
  if (resourceCount !== manifest.data.value.resources.length) throw new TypeError('artifact resource count mismatch');
  ...
  for (let index = 0; index < resourceCount; index += 1) {
    ...
    if (resourceBytes > BOARD_LIMITS_V1.maxArtifactTotalBytes
      || seen.has(path)
