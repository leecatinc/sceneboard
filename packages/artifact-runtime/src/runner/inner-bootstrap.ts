...
declare global {
  interface Window {
    ...
    mermaid?: {
      initialize(input: { securityLevel: 'strict'; startOnLoad: false }): void;
      run(input: { querySelector: string }): Promise<void>;
    };
  }
}
...
if (
  (resources.css !== null && typeof resources.css !== 'string') ||
  (resources.javascript !== null && typeof resources.javascript !== 'string') ||
  typeof resources.diagram !== 'boolean'
)
  throw new TypeError('artifact resources are invalid');
...
const nativePortOnMessageSetter = Object.getOwnPropertyDescriptor(
...
const nativeMessageEventDataGetter = Object.getOwnPropertyDescriptor(
  MessageEvent.prototype,
  'data',
)?.get;
const nativeMessageEventPortsGetter = Object.getOwnPropertyDescriptor(
  MessageEvent.prototype,
  'ports',
)?.get;
...
const nativeEventCancelableGetter = Object.getOwnPropertyDescriptor(
  Event.prototype,
  'cancelable',
)?.get;
...
const nativeMouseEventButtonsGetter = Object.getOwnPropertyDescriptor(
  MouseEvent.prototype,
  'buttons',
)?.get;
const nativeMouseEventClientXGetter = Object.getOwnPropertyDescriptor(
  MouseEvent.prototype,
  'clientX',
)?.get;
const nativeMouseEventClientYGetter = Object.getOwnPropertyDescriptor(
  MouseEvent.prototype,
  'clientY',
)?.get;
const nativePointerEventPointerIdGetter = Object.getOwnPropertyDescriptor(
  PointerEvent.prototype,
  'pointerId',
)?.get;
const nativeWheelEventDeltaYGetter = Object.getOwnPropertyDescriptor(
  WheelEvent.prototype,
  'deltaY',
)?.get;
const nativeWheelEventDeltaModeGetter = Object.getOwnPropertyDescriptor(
  WheelEvent.prototype,
  'deltaMode',
)?.get;
const nativeDocumentHiddenGetter = Object.getOwnPropertyDescriptor(
  Document.prototype,
  'hidden',
)?.get;
...
if (
  nativeEventCancelableGetter === undefined ||
  ...
  nativeWindowInnerHeightGetter === undefined
)
  throw new TypeError('trusted navigation primitives are unavailable');
let port: MessagePort | null = null;
let outboundIdentity: Readonly<
  Pick<ArtifactBridgeEnvelopeV1, 'channelId' | 'sessionId' | 'artifact'>
> | null = null;
...
const send = (message: ArtifactBridgeMessageV1, binary?: ArrayBuffer): void => {
  ...
  nativeReflectApply(nativePortPostMessage, activePort, [
    outgoing,
...
const navigation = new ArtifactNavigationSchedulerV1({
  ...
  schedule: (callback, delayMs) =>
    nativeReflectApply(nativeSetTimeout, nativeGlobalThis, [callback, delayMs]),
  cancel: (handle) =>
    nativeReflectApply(nativeClearTimeout, nativeGlobalThis, [
...
const pointerAnchor = (
  clientX: number,
  clientY: number,
): { xMillionth: number; yMillionth: number } => ({
  ...artifactPointerAnchorV1(
    clientX,
    clientY,
...
const eventCancelable = (event: Event): boolean =>
  nativeReflectApply(nativeEventCancelableGetter, event, []) as boolean;
const preventEventDefault = (event: Event): void =>
  nativeReflectApply(nativeEventPreventDefault, event, []);
const mouseButton = (event: MouseEvent): number =>
  nativeReflectApply(nativeMouseEventButtonGetter, event, []) as number;
const mouseButtons = (event: MouseEvent): number =>
  nativeReflectApply(nativeMouseEventButtonsGetter, event, []) as number;
const mouseClientX = (event: MouseEvent): number =>
  nativeReflectApply(nativeMouseEventClientXGetter, event, []) as number;
const mouseClientY = (event: MouseEvent): number =>
  nativeReflectApply(nativeMouseEventClientYGetter, event, []) as number;
const pointerId = (event: PointerEvent): number =>
  nativeReflectApply(nativePointerEventPointerIdGetter, event, []) as number;
...
window.addEventListener(
  ...
  (event) => {
    ...
    navigation.wheel({
      ...
      ...pointerAnchor(mouseClientX(event), mouseClientY(event)),
      deltaY,
    });
  },
...
window.addEventListener(
  ...
  (event) => {
    ...
    if (
      ...
      !event.isTrusted ||
      mouseButton(event) !== 1 ||
      (mouseButtons(event) & 4) === 0 ||
      activePointer !== null ||
      navigation.hasPan ||
      !nativeNumberIsInteger(id) ||
      id < 0 ||
      id > 2_147_483_647
    )
      return;
    ...
    try {
      nativeReflectApply(nativeElementSetPointerCapture, nativeDocumentElement, [id]);
    } catch {
      activePointer = null;
      return;
    }
...
window.addEventListener(
  'pointercancel',
  (event) => {
    if (event.isTrusted && activePointer?.id === pointerId(event)) cancelPan();
  },
  { capture: true },
);
nativeDocumentElement.addEventListener(
  'lostpointercapture',
  (event) => {
    if (event.isTrusted && activePointer?.id === pointerId(event)) cancelPan();
  },
...
window.addEventListener('blur', (event) => {
  if (event.isTrusted) cancelPan();
});
document.addEventListener('visibilitychange', () => {
  if (nativeReflectApply(nativeDocumentHiddenGetter, document, [])) cancelPan();
});
...
window.SceneBoardArtifact = Object.freeze({
  onHostMessage(
    ...
    if (typeof listener !== 'function' || disposed)
      throw new TypeError('artifact host listener is invalid');
  ...
  requestCapability(
    ...
    if ((capability === 'download') !== (binary !== undefined))
      throw new TypeError('download capability binary is invalid');
    ...
    () => {
      ...
      secondMeasureFrame = nativeReflectApply(nativeRequestAnimationFrame, nativeGlobalThis, [
        () => {
          ...
          const measured = measureArtifactContentSizeV1(
            origin,
...
window.addEventListener(
  ...
  (event: MessageEvent<unknown>) => {
    if (
      nativeMessageEventDataGetter === undefined ||
      nativeMessageEventPortsGetter === undefined ||
      nativeArrayBufferByteLengthGetter === undefined
    )
      return;
    ...
    const parsed = parseArtifactBridgeEnvelopeV1(eventData, {
      messagePorts: 1,
      arrayBufferBytes: [],
    });
    ...
    if (port === null) return;
    if (nativePortOnMessageSetter === undefined)
      throw new TypeError('artifact port listener is unavailable');
    nativeReflectApply(nativePortOnMessageSetter, port, [
      (innerEvent: MessageEvent<ArtifactBridgeEnvelopeV1 | BinaryCarrier>) => {
        const identity = outboundIdentity;
        ...
        if (innerEventPorts.length !== 0)
          throw new TypeError('artifact host message ports are invalid');
        if (
          innerEventData !== null &&
          typeof innerEventData === 'object' &&
          !nativeArrayIsArray(innerEventData) &&
          nativeObjectKeys(innerEventData).length === 2 &&
          nativeObjectHasOwn(innerEventData, 'envelope') &&
          nativeObjectHasOwn(innerEventData, 'binary')
        ) {
          const carrier = innerEventData as BinaryCarrier;
          let binaryByteLength: number;
          try {
            binaryByteLength = nativeReflectApply(
              nativeArrayBufferByteLengthGetter,
              carrier.binary,
              [],
            );
          } catch {
        ...
        if (raw === null || typeof raw !== 'object' || nativeArrayIsArray(raw))
          throw new TypeError('artifact bridge envelope is invalid');
        ...
        if (incoming.type === 'host.watchdog.ping') {
          send({
            ...
            watchdogId: incoming.watchdogId,
            sentAtMonotonicMs: incoming.sentAtMonotonicMs,
          });
          return;
        }
