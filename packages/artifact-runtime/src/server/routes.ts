...
export type FixedAssetEntryV1 = {
  logicalName: 'outer' | 'mermaid' | 'three';
  path: string;
  sha256: string;
  byteLength: number;
...
const HASHED_ASSET = /^\/assets\/(outer|mermaid|three)\.([0-9a-f]{64})\.js$/u;
...
const safeAssetPath = (publicDirectory: string, path: string): string => {
  ...
  if (!absolute.startsWith(`${resolve(publicDirectory)}${sep}`))
    throw new TypeError('fixed asset path escapes public directory');
  return absolute;
};

export const loadArtifactRuntimeAssetsV1 = async (
  publicDirectory: string,
): Promise<ArtifactRuntimeAssetsV1> => {
  ...
  if (!Array.isArray(input) || input.length !== 3)
    throw new TypeError('fixed asset manifest must contain outer, Mermaid, and Three.js');
  const entries = new Map<string, FixedAssetEntryV1 & { bytes: Uint8Array }>();
  let priorPath = '';
  for (const item of input) {
    if (item === null || typeof item !== 'object' || Array.isArray(item))
      throw new TypeError('fixed asset manifest entry is invalid');
    const value = item as Record<string, unknown>;
    const keys = Object.keys(value);
    if (
      keys.length !== MANIFEST_KEYS.length ||
      MANIFEST_KEYS.some((key) => !Object.hasOwn(value, key)) ||
      keys.some((key) => !MANIFEST_KEYS.includes(key as (typeof MANIFEST_KEYS)[number]))
    )
      throw new TypeError('fixed asset manifest entry keys are invalid');
    if (
      (value.logicalName !== 'outer' &&
        value.logicalName !== 'mermaid' &&
        value.logicalName !== 'three') ||
      ...
      entries.has(value.path)
    )
      throw new TypeError('fixed asset manifest entry is invalid or unsorted');
    const match = HASHED_ASSET.exec(value.path);
    if (match?.[1] !== value.logicalName || match[2] !== value.sha256)
      throw new TypeError('fixed asset path does not match its identity');
    const bytes = await readFile(safeAssetPath(publicDirectory, value.path));
    const actual = createHash('sha256').update(bytes).digest('hex');
    if (bytes.byteLength !== value.byteLength || actual !== value.sha256)
      throw new TypeError('fixed asset digest or length mismatch');
    entries.set(value.path, { ...(value as FixedAssetEntryV1), bytes });
    priorPath = value.path;
  }
  if (
    ![...entries.values()].some((entry) => entry.logicalName === 'outer') ||
    ![...entries.values()].some((entry) => entry.logicalName === 'mermaid') ||
    ![...entries.values()].some((entry) => entry.logicalName === 'three')
  )
    throw new TypeError('fixed asset manifest is incomplete');
...
export const routeArtifactRuntimeRequestV1 = (input: {
  ...
  if (input.method !== 'GET') {
    return safeResponse(
      405,
      Object.freeze({ ...buildHealthHeadersV1(), Allow: 'GET' }),
  ...
  if (input.path === '/runner') {
    return safeResponse(
      200,
      buildRunnerHeadersV1({
        appOrigin: input.topology.frontendOrigin,
        runtimeOrigin: input.topology.runtimeOrigin,
      }),
