...
const string = (environment: NodeJS.ProcessEnv, key: string): string => {
  ...
  if (value === undefined || value.length === 0 || value !== value.trim()) throw new TypeError(`${key} is required`);
  return value;
};
...
export const loadArtifactRuntimeConfigV1 = async (
  ...
  if (!['development', 'test', 'staging', 'production'].includes(appEnv)) throw new TypeError('APP_ENV is invalid');
  const evidencePath = resolve(string(environment, 'ARTIFACT_RUNTIME_EVIDENCE_FILE'));
  const frontendInputPath = resolve(string(environment, 'ARTIFACT_RUNTIME_FRONTEND_RESOLVED_INPUT_FILE'));
  const backendInputPath = resolve(string(environment, 'ARTIFACT_RUNTIME_BACKEND_RESOLVED_INPUT_FILE'));
  const runtimeInputPath = resolve(string(environment, 'ARTIFACT_RUNTIME_RESOLVED_INPUT_FILE'));
  const [evidenceBytes, frontendInputBytes, backendInputBytes, runtimeInputBytes] = await Promise.all([
  ...
  return { ...topology, publicDirectory: resolve(string(environment, 'ARTIFACT_RUNTIME_PUBLIC_DIR')) };
};
