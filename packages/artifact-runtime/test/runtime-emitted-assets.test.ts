...
const buildAndRead = (): { path: string; sha256: string; source: string } => {
  ...
  const manifest = JSON.parse(readFileSync(resolve(packageRoot, 'dist/public/fixed-assets.v1.json'), 'utf8')) as Array<{ logicalName: string; path: string; sha256: string }>;
  ...
  return { path: outer.path, sha256: createHash('sha256').update(bytes).digest('hex'), source: bytes.toString('utf8') };
};
...
test('deterministic emitted runner contains trusted navigation and one-shot sizing', () => {
  ...
  assert.deepEqual({ path: second.path, sha256: second.sha256 }, { path: first.path, sha256: first.sha256 });
