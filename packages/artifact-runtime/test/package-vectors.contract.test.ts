...
import { ArtifactManifestParserV1 } from '@leecat-board/board-schema';
...
const packageFixture = (): Uint8Array => {
  ...
  const manifest = ArtifactManifestParserV1.parse({
    ...
    resources: [{ path: 'index.html', mediaType: 'text/html', sha256: hex(resource), byteLength: resource.byteLength }],
    requestedCapabilities: [],
  });
  assert.equal(manifest.ok, true);
  if (!manifest.ok) throw new TypeError('fixture manifest failed');
  const path = artifactBytes('index.html');
  const total = 8 + 4 + manifest.data.canonicalBytes.byteLength + 2 + 2 + path.byteLength + 4 + resource.byteLength;
  const bytes = new Uint8Array(total);
  const view = new DataView(bytes.buffer);
  bytes.set(artifactBytes('LCARTV1\0'), 0);
  let offset = 8;
  view.setUint32(offset, manifest.data.canonicalBytes.byteLength, false); offset += 4;
  bytes.set(manifest.data.canonicalBytes, offset); offset += manifest.data.canonicalBytes.byteLength;
  view.setUint16(offset, 1, false); offset += 2;
  view.setUint16(offset, path.byteLength, false); offset += 2;
  bytes.set(path, offset); offset += path.byteLength;
  view.setUint32(offset, resource.byteLength, false); offset += 4;
