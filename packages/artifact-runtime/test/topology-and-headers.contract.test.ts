...
test('runner headers are exact and omit credential, CORS, and origin-agent-cluster fields', () => {
  ...
  assert.match(
    ...
    /script-src http:\/\/127\.0\.0\.2:3412 data: blob:/u,
  );
...
test('runtime routes enforce Host, GET, and the fixed allowlist', () => {
  ...
  const request = (method: string, path: string, host = '127.0.0.2:3412') =>
