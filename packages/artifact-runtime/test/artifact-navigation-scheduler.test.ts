...
type NavigationMessage = Extract<
...
test('navigation scheduler omits zero wheel sums and emits bounded residual chunks', () => {
  ...
  assert.deepEqual(
    emitted.map((message) =>
      message.type === 'artifact.navigation.wheel' ? message.deltaY : null,
    ),
    [4_096, 4_096, 808],
  );
});
...
test('navigation disable drops unopened pan and closes a visible pan once', () => {
  ...
  visible.flush();
  assert.deepEqual(
    visible.emitted.map((message) => message.type),
    ['artifact.navigation.pan.start', 'artifact.navigation.pan.cancel'],
  );
});
