...
const bridgeEndpoint = () => new ArtifactBridgeEndpointV1({ channelId: ID, sessionId: ID, artifact: ARTIFACT });
...
const receiveBinaryOverChannel = async (
  ...
  const settled = new Promise<unknown>((resolve) => {
    channel.port1.onmessage = (event) => {
      try {
        const carrier = event.data as ArtifactBridgeEnvelopeV1 | { envelope: ArtifactBridgeEnvelopeV1; binaries: ArrayBuffer[] };
...
const receiveBinaryInsteadOfPortOverChannel = async (envelope: ArtifactBridgeEnvelopeV1): Promise<unknown> => {
  const channel = new MessageChannel();
  const receiver = bridgeEndpoint();
  const settled = new Promise<unknown>((resolve) => {
    channel.port1.onmessage = (event) => {
      try {
        ...
        receiver.receive(carrier.envelope, { messagePorts: event.ports.length, arrayBufferBytes: [carrier.binary.byteLength] });
        resolve(null);
      } catch (error) {
        resolve(error);
      } finally {
...
const receivePortsOverChannel = async (envelope: ArtifactBridgeEnvelopeV1, portCount: number): Promise<unknown> => {
  const channel = new MessageChannel();
  const receiver = bridgeEndpoint();
  ...
  channel.port2.postMessage(envelope, extras.map((extra) => extra.port1));
  return settled;
};
...
test('navigation admission enforces armed active and exact pointer lifecycle', () => {
  ...
  const start = { type: 'artifact.navigation.pan.start', pointerId: 7, xMillionth: 1, yMillionth: 2 } as const;
  ...
  assert.equal(admission.admit({ type: 'artifact.navigation.pan.move', pointerId: 8, deltaX: 1, deltaY: 1 }, true), false);
  assert.equal(admission.admit({ type: 'artifact.navigation.wheel', xMillionth: 1, yMillionth: 2, deltaY: 1 }, true), false);
  assert.equal(admission.setEnabled(false), null);
  assert.equal(admission.admit({ type: 'artifact.navigation.pan.move', pointerId: 7, deltaX: 1, deltaY: 1 }, true), false);
  assert.equal(admission.admit({ type: 'artifact.navigation.pan.cancel', pointerId: 7 }, true), true);
  assert.equal(admission.admit({ type: 'artifact.navigation.pan.end', pointerId: 7, deltaX: 0, deltaY: 0 }, true), false);
  assert.equal(admission.setEnabled(false), null);
});
  ...
  const result = new Promise<{ type: string; bytes: number[] }>((resolve, reject) => {
    channel.port1.onmessage = (event) => {
      try {
        ...
        const parsed = receiver.receive(carrier.envelope, { messagePorts: event.ports.length, arrayBufferBytes: [carrier.binary.byteLength] });
...
test('every binary transfer branch rejects missing, extra, and wrong byte lengths through parser and real channels', async () => {
  const branches = [
    ...
    },
    {
      ...
      message: { type: 'artifact.capability.request', requestId: ID, capability: 'download', payload: { byteLength: 4 } },
    },
    {
      name: 'network result',
      message: { type: 'host.capability.result', requestId: ID, capability: 'network.fetch', ok: true, result: { byteLength: 4 } },
    },
  ] as const satisfies readonly { name: string; message: ArtifactBridgeMessageV1 }[];

  for (const branch of branches) {
    const envelope = bridgeEndpoint().send(branch.message, { messagePorts: 0, arrayBufferBytes: [4] });
    for (const bytes of [[], [4, 1], [3], [5]] as const) {
      assert.throws(() => bridgeEndpoint().receive(envelope, { messagePorts: 0, arrayBufferBytes: bytes }), /transfer/u, `${branch.name}: ${bytes.join(',')}`);
    }
    assert.throws(() => bridgeEndpoint().receive(envelope, { messagePorts: 1, arrayBufferBytes: [] }), /transfer/u, `${branch.name}: wrong parser transfer kind`);
    assert.match(String(await receiveBinaryOverChannel(envelope, null)), /transfer/u, `${branch.name}: missing real binary`);
    assert.match(String(await receiveBinaryOverChannel(envelope, [4, 1])), /transfer/u, `${branch.name}: extra real binary`);
    assert.match(String(await receiveBinaryOverChannel(envelope, [5])), /transfer/u, `${branch.name}: wrong real binary`);
    assert.match(String(await receiveBinaryOverChannel(envelope, null, true)), /transfer/u, `${branch.name}: wrong real transfer kind`);
    assert.equal(await receiveBinaryOverChannel(envelope, [4]), null, `${branch.name}: exact real binary`);
  }
});

test('every port transfer branch rejects missing and extra ports through parser and real channels', async () => {
  const branches = [
    { type: 'host.bootstrap', appOrigin: 'https://sceneboard.dev', runtimeOrigin: 'https://artifact.sceneboard.dev', policyEpoch: ID },
  ...
  for (const message of branches) {
    ...
    for (const ports of [0, 2]) {
      assert.throws(() => bridgeEndpoint().receive(envelope, { messagePorts: ports, arrayBufferBytes: [] }), /transfer/u, `${message.type}: ${ports}`);
      assert.match(String(await receivePortsOverChannel(envelope, ports)), /transfer/u, `${message.type}: ${ports} real ports`);
    }
    assert.throws(() => bridgeEndpoint().receive(envelope, { messagePorts: 0, arrayBufferBytes: [4] }), /transfer/u, `${message.type}: wrong parser transfer kind`);
    assert.match(String(await receiveBinaryInsteadOfPortOverChannel(envelope)), /transfer/u, `${message.type}: wrong real transfer kind`);
    assert.equal(await receivePortsOverChannel(envelope, 1), null, `${message.type}: exact real port`);
  }
});
