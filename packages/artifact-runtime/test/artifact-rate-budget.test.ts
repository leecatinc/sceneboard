...
test('classifies exactly the five authored budget message types', () => {
  const charged = ['artifact.ready', 'artifact.resize.request', 'artifact.selection.change', 'artifact.user-action', 'artifact.capability.request'] as const;
  const everyType = [
    'host.bootstrap',
    'runner.ready',
    'host.package.start',
  ...
  for (const type of everyType) assert.equal(isChargedAuthoredMessageV1({ type }), chargedSet.has(type), type);
});

test('admits exact count and byte boundaries atomically', () => {
  let now = 0;
  const budget = new ArtifactRateBudgetV1({ countRate: 1, countBurst: 2, byteRate: 10, byteBurst: 20, now: () => now });
...
test('does not spend either bucket when one charge is unavailable', () => {
  let now = 0;
  const budget = new ArtifactRateBudgetV1({ countRate: 0, countBurst: 1, byteRate: 0, byteBurst: 5, now: () => now });
...
test('rejects invalid charge and clock values', () => {
  const budget = new ArtifactRateBudgetV1({ countRate: 1, countBurst: 1, byteRate: 1, byteBurst: 1, now: () => 0 });
  assert.equal(budget.admit(-1), false);
  assert.equal(budget.admit(0.5), false);
  assert.throws(() => new ArtifactRateBudgetV1({ countRate: 1, countBurst: 1, byteRate: 1, byteBurst: 1, now: () => Number.NaN }), /clock/u);
});

test('rejects a backward clock without minting duplicate refill', () => {
  let now = 1_000;
  const budget = new ArtifactRateBudgetV1({ countRate: 1, countBurst: 1, byteRate: 10, byteBurst: 10, now: () => now });
  assert.equal(budget.admit(10), true);
  now = 500;
