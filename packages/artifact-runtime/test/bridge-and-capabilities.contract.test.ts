...
test('bridge parser enforces exact schema and transfer list', () => {
  assert.equal(parseArtifactBridgeEnvelopeV1(bootstrap, { messagePorts: 1, arrayBufferBytes: [] }).envelope.sequence, 1);
  assert.throws(() => parseArtifactBridgeEnvelopeV1({ ...bootstrap, unknown: true }, { messagePorts: 1, arrayBufferBytes: [] }), /keys/u);
...
test('bridge parser closes navigation and sourced resize shapes', () => {
  ...
  assert.equal(sender.send({ type: 'host.navigation.set', enabled: true }).message.type, 'host.navigation.set');
  assert.equal(sender.send({ type: 'artifact.navigation.wheel', xMillionth: 500_000, yMillionth: 0, deltaY: 0.25 }).message.type, 'artifact.navigation.wheel');
  assert.equal(sender.send({ type: 'artifact.resize.request', value: { width: 1_200, height: 675, source: 'observer' } }).message.type, 'artifact.resize.request');
  assert.throws(() => sender.send({ type: 'artifact.navigation.wheel', xMillionth: 0, yMillionth: 0, deltaY: 0 }), /wheel/u);
  assert.throws(() => sender.send({ type: 'artifact.resize.request', value: { width: 1_200, height: 675 } } as never), /keys/u);
});
...
test('bridge endpoint rejects replay, gaps, and pair drift', () => {
  ...
  assert.throws(() => receiver.receive(bootstrap, { messagePorts: 1, arrayBufferBytes: [] }), /sequence/u);
  const sender = new ArtifactBridgeEndpointV1({ channelId: ID, sessionId: ID, artifact });
...
test('bridge endpoint rejects same-realm validation-time message mutation', () => {
  ...
  Reflect.ownKeys = (value: object): (string | symbol)[] => {
    if (!mutated && value !== null && typeof value === 'object' && 'type' in value && value.type === 'artifact.bridge') {
      (value as { message: unknown }).message = { type: 'artifact.navigation.wheel', xMillionth: 1, yMillionth: 2, deltaY: 3 };
      mutated = true;
    }
    return nativeOwnKeys(value);
  };
  try {
    assert.throws(() => sender.send({ type: 'artifact.resize.request', value: { width: 1_200, height: 675, source: 'explicit' } }), /mutated/u);
  } finally {
...
test('capabilities are the default-denied policy intersection', () => {
  assert.deepEqual(decideArtifactCapabilityV1({
    capability: 'clipboard.write', manifestRequested: [], currentlyAllowed: ['clipboard.write'], policyEpochMatches: true,
  }), { ok: false, error: 'not_requested' });
  assert.deepEqual(decideArtifactCapabilityV1({
    capability: 'network.fetch', manifestRequested: ['network.fetch'], currentlyAllowed: ['network.fetch'], policyEpochMatches: true,
  }), { ok: false, error: 'unavailable' });
  assert.deepEqual(decideArtifactCapabilityV1({
    capability: 'fullscreen', manifestRequested: ['fullscreen'], currentlyAllowed: ['fullscreen'], policyEpochMatches: true,
