...
export function StatusRail({ snapshot, presence, onStopRendering }: { snapshot: BoardSnapshotV1; presence: readonly PresenceSummaryV1[]; onStopRendering: () => void }) {
  const { t } = useI18n();
  return (
    <aside className="status-rail" aria-label={t('board.status')}>
      ...
      {snapshot.artifacts.length > 0 && <button type="button" className="artifact-stop-sidebar" onClick={onStopRendering}>{t('board.stopRendering')}</button>}
    </aside>
  );
}
