...
export function AuthenticatedRoute({ children }: { children: ReactNode }) {
  ...
  if (admission === 'checking') return <section className="route-state" role="status"><span className="spinner" />{t('auth.verify')}</section>;
  if (admission === 'retry') return <section className="route-state" role="alert"><h2>{t('auth.verificationPaused')}</h2><p>{t('auth.verificationHelp')}</p><button className="button" onClick={() => void reconcile()}>{t('auth.retryVerification')}</button></section>;
  return children;
}
