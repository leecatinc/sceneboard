...
export function UserMenu() {
  ...
  return (
    <div className="user-menu" ref={menuRef}>
      ...
      >
        <span className="user-avatar" aria-hidden="true">{email.slice(0, 1).toUpperCase() || 'S'}</span>
      ...
      {isOpen && (
        <div className="user-menu-popover" role="menu" aria-label={t('user.accountMenu')}>
          ...
          <button type="button" role="menuitem" onClick={() => openDialog('settings')}>{t('nav.settings')}</button>
          <button type="button" role="menuitem" onClick={() => openDialog('password')}>{t('user.changePassword')}</button>
          <button type="button" role="menuitem" className="user-menu-signout" onClick={() => void signOut()}>{t('user.signOut')}</button>
          {error && <p className="error user-menu-error" role="alert">{error}</p>}
        </div>
      )}
      <AccountModal
