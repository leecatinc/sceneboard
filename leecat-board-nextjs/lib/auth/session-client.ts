  ...
  private async emailVerificationRequest<Value>(
    ...
    try {
      ...
      return {
        ...
        code: parsed.error.code,
        retryAfterSeconds: retryAfter !== null && /^[1-9][0-9]*$/.test(retryAfter) ? Number(retryAfter) : null,
      };
    } catch {
      return { kind: 'corrupt_response' };
    }
...
export type EmailVerificationClientResult<Value> = CoordinatorResult<Value>
  | { kind: 'api_error'; status: number; code: string; retryAfterSeconds: number | null }
  | { kind: 'corrupt_response' };

export type PasswordChangeClientResult = CoordinatorResult<null>
