...
} from '@leecat-board/board-sdk/client';
import {
...
} from '@leecat-board/board-sdk/state';
...
const localError = (result: Exclude<ApiResult<unknown>, { kind: 'ok' }>): SafeBoardUiErrorV1 => {
  ...
  if (result.kind === 'corrupt_response') return { kind: 'corrupt', message: 'The board response could not be verified.', retryable: false };
  if (result.kind === 'unsupported_browser') return { kind: 'unknown', message: 'This browser cannot protect the SceneBoard session.', retryable: false };
  if (result.kind === 'api_error' && result.status === 404) return { kind: 'not_found', message: 'This board was not found.', retryable: false };
  return { kind: 'offline', message: 'The latest board view could not be reached.', retryable: true };
};
...
export function useBoardSession(boardIdValue: string) {
  ...
  const startStream = useCallback(async (snapshot: BoardSnapshotV1) => {
    ...
    const client = createBoardStreamClientV1({
      ...
      callbacks: {
        replaceSnapshot(next) {
          ...
          setState((current) => current === null ? createLiveBoardStateV1(next) : replaceLiveSnapshotV1(current, next));
        },
        async refreshRevisionSnapshot() {
          if (!routeEpoch.current.isCurrent(epoch)) throw new TypeError('stale route callback');
          const result = await api.getBoard(snapshot.boardId);
          if (!routeEpoch.current.isCurrent(epoch) || result.kind !== 'ok') throw new TypeError('revision refresh was not admitted');
          setState((current) => current === null ? createLiveBoardStateV1(result.value.snapshot) : replaceLiveSnapshotV1(current, result.value.snapshot));
          return { kind: 'authoritative_revision_snapshot', lastEventSequence: result.value.snapshot.lastEventSequence };
        },
        ...
        replacePresence(presence) {
          if (!routeEpoch.current.isCurrent(epoch)) throw new TypeError('stale route callback');
          setState((current) => current === null ? current : replacePresenceV1(current, presence));
        },
        onState(connection) {
          if (!routeEpoch.current.isCurrent(epoch)) throw new TypeError('stale route callback');
          setState((current) => current === null ? current : replaceConnectionStateV1(current, connection));
        },
      },
    });
    stream.current = client;
    const visibility = () => client.setPresenceState(document.visibilityState === 'visible' ? 'online' : 'away');
    document.addEventListener('visibilitychange', visibility);
    void client.start().finally(() => document.removeEventListener('visibilitychange', visibility));
  }, [api, apiOrigin]);
  ...
  const navigateHistory = useCallback(async (revisionId: RevisionId) => {
    ...
    if (result.kind !== 'ok') {
      ...
      setState((current) => current === null ? current : failHistoryNavigationV1(current, nextError));
      return;
    }
    try {
      const navigation = correlateHistoryNavigationV1(result.value.entry, result.value.metadata);
      setState((current) => current === null ? current : enterHistoryV1(current, result.value.snapshot, navigation));
    } catch {
      const nextError: SafeBoardUiErrorV1 = { kind: 'corrupt', message: 'History navigation could not be verified.', retryable: false };
      setState((current) => current === null ? current : failHistoryNavigationV1(current, nextError));
    }
  }, [api, boardIdValue]);
  ...
  const previous = useCallback(() => {
    ...
    const revisionId = state.mode.kind === 'history'
  ...
  const latest = useCallback(async () => {
    ...
    setState((current) => current === null ? createLiveBoardStateV1(result.value.snapshot) : enterLatestV1(current, result.value.snapshot));
    setTitle(result.value.board.title);
    await startStream(result.value.snapshot);
  }, [api, boardIdValue, startStream, state?.mode.kind]);

  const rename = useCallback(async (nextTitle: string): Promise<boolean> => {
