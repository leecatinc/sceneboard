...
import type {
  ...
  HitlSubmitIntentV1,
} from '@leecat-board/board-ui/interaction';
...
const canonicalResponse = (response: HitlResponseV1): { response: HitlResponseV1; canonical: string } => {
  const parsed = HitlResponseParserV1.parse(response);
  if (!parsed.ok) throw new TypeError('invalid HITL response');
  return { response: parsed.data.value, canonical: new TextDecoder().decode(parsed.data.canonicalBytes) };
};

const definitionSummary = (interaction: HitlInteractionV1): string => {
  const body = 'body' in interaction.definition && interaction.definition.body !== undefined
    ? interaction.definition.body.replace(/\s+/gu, ' ').trim()
    : '';
  const source = body === '' ? interaction.definition.title : `${interaction.definition.title} — ${body}`;
...
export const buildHitlClipboardPayloadV1 = (input: {
  ...
  if (interaction.state === 'answered' && interaction.response !== null && interaction.answeredAt !== null) {
  ...
  if (response === undefined
    || (interaction.definition.kind === 'confirmation'
      ...
      && response.confirmed)) return null;
...
const safeFailureMessage = (result: Exclude<ApiResult<unknown>, { kind: 'ok' }>): string => {
  if (result.kind === 'board_error') {
    ...
    if (result.error.code === 'REVISION_CONFLICT') return 'The board changed. Return to the latest view and try again.';
    if (result.error.code === 'INVALID_PAYLOAD') return 'Check the response and try again.';
  }
  if (result.kind === 'unsupported_browser') return 'This browser cannot protect the SceneBoard session.';
...
export function useHitlInteractionController(input: {
  ...
  const writeClipboard = useCallback(async (
    ...
    try {
      if (typeof navigator === 'undefined' || navigator.clipboard === undefined) throw new TypeError('clipboard unavailable');
      await navigator.clipboard.writeText(payload);
      if (expectedEpoch !== epoch.current || expectedStateUpdatedAt !== interactionRef.current.stateUpdatedAt) return;
      setCopyState({ kind: 'copied', message: 'Copied.' });
    } catch {
      if (expectedEpoch !== epoch.current || expectedStateUpdatedAt !== interactionRef.current.stateUpdatedAt) return;
      setCopyState({ kind: 'failed', message: 'Copy failed. Use the copy button after granting clipboard access.' });
    }
  }, []);

  const authoritativeRead = useCallback(async (capturedEpoch: number, baseline: HitlInteractionV1) => {
  ...
  const execute = useCallback(async (response: PendingResponseV1): Promise<void> => {
    ...
    if (mustRead) {
      ...
      if (unknown && current !== null && current.state === 'open'
        && current.stateUpdatedAt === baseline.stateUpdatedAt) {
        setSubmission({
          kind: 'recording_unknown',
          message: 'The response may or may not have been recorded. The authoritative state is still open.',
          canRetry: true,
        });
      ...
      setSubmission(unknown
        ? { kind: 'reconciliation_failed', message: 'Recording is unknown and the authoritative state could not be verified. Refresh before taking further action.', canRetry: false }
        : { kind: 'failed', message: 'The interaction changed before this response was accepted.', canRetry: false });
      active.current = null;
      return;
    }
    pending.current = null;
  ...
  const submit = useCallback(async (intent: HitlSubmitIntentV1): Promise<void> => {
    if (input.mode !== 'live'
      ...
      || interactionRef.current.state !== 'open') return;
    let parsed: ReturnType<typeof canonicalResponse>;
    try {
      parsed = canonicalResponse(intent.response);
    } catch {
      setSubmission({ kind: 'failed', message: 'The response could not be verified.', canRetry: false });
      return;
    }
  ...
  const payload = useCallback((): string | null => input.mode === 'live'
    ...
    : null, [input.boardId, input.mode, submission.kind]);

  const controller: HitlInteractionControllerV1 = {
    mode: input.mode,
    isSubmitting: (hitlRequestId) => hitlRequestId === interaction.hitlRequestId && submission.kind === 'submitting',
    submit,
    submissionState: (hitlRequestId) => hitlRequestId === interaction.hitlRequestId ? submission : IDLE_SUBMISSION,
    retry: async (hitlRequestId) => { if (hitlRequestId === interaction.hitlRequestId) await retry(); },
    ...
    copyState: (hitlRequestId) => hitlRequestId === interaction.hitlRequestId ? copyState : IDLE_COPY,
  };
