...
export const parseAuthSessionSnapshot = (value: unknown): AuthSessionSnapshot => {
  if (!isObject(value) || !exactKeys(value, ['user', 'session', 'csrfToken'])) throw new TypeError('invalid auth session response');
  if (!isObject(value.user) || !exactKeys(value.user, ['userId', 'email', 'createdAt'])) throw new TypeError('invalid auth user response');
  if (!isObject(value.session) || !exactKeys(value.session, ['sessionId', 'idleExpiresAt', 'absoluteExpiresAt'])) throw new TypeError('invalid session response');
  if (
...
export const parsePublicApiError = (value: unknown): PublicApiError => {
  if (!isObject(value) || !exactKeys(value, ['error']) || !isObject(value.error)) throw new TypeError('invalid API error');
  if (typeof value.error.code !== 'string' || typeof value.error.message !== 'string') throw new TypeError('invalid API error');
