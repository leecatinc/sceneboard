...
test('serialized browser adapter union contains one exact disjoint four-owner set', () => {
  ...
  const names = publishers.flatMap(({ selectors }) => selectors.map(({ memberName }) => memberName));
  ...
  assert.equal(new Set(publishers.map(({ tupleListSha256 }) => tupleListSha256)).size, publishers.length);
