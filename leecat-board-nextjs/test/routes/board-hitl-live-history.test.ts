...
const fixture = (name: string): HitlInteractionV1 => {
  const value = JSON.parse(readFileSync(new URL(`../../../packages/board-schema/test/fixtures/valid/${name}`, import.meta.url), 'utf8')) as unknown;
...
test('clipboard formats always carry exact board, interaction, and typed response identity', () => {
  const answered = buildHitlClipboardPayloadV1({ boardId, interaction: fixture('hitl-interaction-answered.v1.json') });
  ...
  const expired = buildHitlClipboardPayloadV1({ boardId, interaction: fixture('hitl-interaction-expired.v1.json') });
...
test('destructive positive confirmation has no recording-unknown clipboard path', () => {
  const parsed = HitlInteractionParserV1.parse({
    ...
    definition: {
      kind: 'confirmation', title: 'Delete', body: 'Delete permanently?', impact: 'destructive',
  ...
  assert.equal(buildHitlClipboardPayloadV1({ boardId, interaction: parsed.data.value, recordingUnknownResponse: positive }), null);
  assert.match(buildHitlClipboardPayloadV1({ boardId, interaction: parsed.data.value, recordingUnknownResponse: negative }) ?? '', /RECORDING UNKNOWN/u);
});
