  ...
  return (
    <>
        ...
        {visibleGrants.map((grant) => (
          <article className="item" key={grant.grantId}>
            ...
            <div className="actions">
              {grant.status === 'active' && <button className="button secondary" disabled={busyGrantId !== null} onClick={() => onRotate(grant.grantId)}>{t('ai.rotate')}</button>}
              <button className="button danger" disabled={busyGrantId !== null} onClick={() => {
                setDisconnectFailed(false);
                setDisconnecting(grant);
              }}>{t('ai.disconnect')}</button>
            </div>
          </article>
        ))}
      </div>
      <ConfirmationDialog
        ...
        description={t('ai.disconnectDescription', { client: disconnecting?.client.clientName ?? '' })}
