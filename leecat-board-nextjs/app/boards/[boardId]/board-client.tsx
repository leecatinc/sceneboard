...
export function BoardClient({ boardId }: { boardId: string }) {
  ...
  return (
    <section className={`board-workspace ${state.mode.kind === 'history' ? 'is-history' : ''}`}>
      <BoardTopBar title={session.title} state={state} liveUpdated={session.liveUpdated} pairingControl={<BoardPairingControl api={api} boardId={boardId} boardTitle={session.title} />} onRename={session.rename} onPrevious={session.previous} onNext={session.next} onLatest={() => void session.latest()} />
      ...
      <div className="board-surface">
        ...
        <StatusRail snapshot={visibleSnapshot} presence={state.presence} />
      </div>
    </section>
  );
}
