...
const readSafeFile = async (path) => {
  ...
  try {
    ...
    if (error instanceof WorkflowSpecError || error instanceof WorkflowSpecIoError) throw error;
    ioFail('INPUT_READ_FAILED', '/input');
  } finally {
...
const writeAtomic = async (path, value, input) => {
  ...
  try {
    ...
    const current = await statusOrNull(absolute).catch(() => ioFail('OUTPUT_CHANGED', '/output'));
    if (
...
const main = async () => {
  ...
  if (command !== 'validate' && command !== 'canonicalize') ioFail('USAGE_ERROR', '');
  if (
