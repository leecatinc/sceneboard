...
import { createSceneRecipeCompiler, SCENE_RECIPE_BLOCK_KINDS_INTERNAL } from './scene-recipe-compiler-internal.mjs';
...
const failPlain = (error) => {
  if (!error) return;
  throw new SceneRecipeError(error.code === 'LIMIT_EXCEEDED' ? 'LIMIT_EXCEEDED' : 'INVALID_JSON', error.path);
};
  ...
  while (index < text.length) {
    ...
    if (char === '"') {
      ...
      if (expectingKey && stack.at(-1)?.type === 'object') {
        let key;
        try { key = JSON.parse(raw); } catch { return true; }
    ...
    if (char === '[') { stack.push({ type: 'array' }); expectingKey = false; index += 1; continue; }
    if (char === '}' || char === ']') { stack.pop(); expectingKey = stack.at(-1)?.type === 'object' && false; index += 1; continue; }
    if (char === ',') { expectingKey = stack.at(-1)?.type === 'object'; index += 1; continue; }
    index += 1;
  }
  return false;
};
...
export const slugifySceneRecipeIdentity = (value) => {
  ...
  const slug = value.normalize('NFKD').replace(/[\p{Mn}\p{Mc}\p{Me}]/gu, '').toLowerCase()
...
export const deriveSceneRecipeNodeId = ({ path, nodeKind, key }) => {
  if (!Array.isArray(path) || typeof nodeKind !== 'string' || (key !== undefined && typeof key !== 'string')) throw new SceneRecipeError('INVALID_VALUE', []);
  const digest = sha256(`recipe-v1\n${canonicalPath(path)}\n${nodeKind}\n${key ?? ''}`).slice(0, 12);
  return `n_${slugifySceneRecipeIdentity(key ?? nodeKind)}_${digest}`;
};

const compiler = createSceneRecipeCompiler({ digest: sha256, slugify: slugifySceneRecipeIdentity, canonicalPath, ErrorType: SceneRecipeError });
...
export const parseSceneRecipeJson = (bytes) => {
  ...
  if (buffer.byteLength > LIMITS_V1.maxEnvelopeBytes) throw new SceneRecipeError('PAYLOAD_TOO_LARGE', []);
  let text;
  try { text = new TextDecoder('utf-8', { fatal: true }).decode(buffer); } catch { throw new SceneRecipeError('INVALID_JSON', []); }
  if (hasDuplicateMembers(text)) throw new SceneRecipeError('INVALID_JSON', []);
  let parsed;
  try { parsed = JSON.parse(text); } catch { throw new SceneRecipeError('INVALID_JSON', []); }
