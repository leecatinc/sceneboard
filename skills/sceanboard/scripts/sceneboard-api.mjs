...
const readStdinJson = async () => {
  ...
  for await (const chunk of process.stdin) {
    total += chunk.length;
    if (total > 1_048_576) {
      throw new SceneBoardApiError('INVALID_PAYLOAD', 'SceneBoard API fallback input is too large', { exitCode: 2 });
    }
    chunks.push(chunk);
  }
  const bytes = Buffer.concat(chunks);
  if (bytes.length === 0) throw new SceneBoardApiError('INVALID_PAYLOAD', 'SceneBoard API fallback requires JSON on stdin', { exitCode: 2 });
...
const pair = async () => {
  ...
  try {
    ...
    let claim;
    try {
      claim = parsePairingClaim(await requestJson({
        config,
      ...
      if (error instanceof SceneBoardApiError && ['BOARD_API_TIMEOUT', 'BOARD_API_TRANSPORT_ERROR'].includes(error.code)) {
        throw new SceneBoardApiError('BOARD_API_PAIRING_OUTCOME_UNKNOWN', 'SceneBoard pairing claim outcome is unknown', {
          details: { phase: 'claim', recovery: 'owner_cancel_or_wait_then_create_new_code' },
        });
      }
      throw error;
    }
    write({ ok: true, transport: 'api', operation: 'pair', event: 'claimed', pairingId: claim.pairingId, state: claim.state, decisionExpiresAt: claim.decisionExpiresAt });
    let status = claim;
    const decisionDeadline = Date.parse(claim.decisionExpiresAt);
    let deadlinePollAttempted = false;
    while (status.state === 'pending') {
      const delaySeconds = status.retryAfterSeconds === undefined ? claim.pollAfterSeconds : status.retryAfterSeconds;
      const remaining = Math.max(0, decisionDeadline - Date.now());
      if (remaining === 0) {
        if (deadlinePollAttempted) {
          throw new SceneBoardApiError('BOARD_API_TIMEOUT', 'SceneBoard pairing status did not reach a terminal state', {
            retryable: true,
          });
        }
        deadlinePollAttempted = true;
      }
      await new Promise((resolve) => setTimeout(resolve, Math.min(delaySeconds * 1_000, remaining)));
      status = parsePairingStatus(await requestJson({
        config,
    ...
    let redeemed;
    const ambiguousTransport = (error) => error instanceof SceneBoardApiError
      && ['BOARD_API_TIMEOUT', 'BOARD_API_TRANSPORT_ERROR'].includes(error.code);
    const unknownRedeemOutcome = (state = null) => new SceneBoardApiError(
      'BOARD_API_PAIRING_OUTCOME_UNKNOWN',
      'SceneBoard pairing redeem outcome is unknown',
      { details: { phase: 'redeem', state, recovery: 'owner_rotate_or_revoke_and_repair' } },
    );
    try {
      ...
      if (ambiguousTransport(error)) {
        let resolved;
        try {
          resolved = parsePairingStatus(await requestJson({
            config,
            path: `/api/v1/pairings/${encodeURIComponent(claim.pairingId)}/client-status`,
            authorization: `PairingProof ${proof.value}`,
            expectedStatus: [200],
    ...
    try {
      ...
      const connection = await requestJson({
        ...
        requestId,
        expectedStatus: [200],
        allowedErrorCodes: ['INVALID_PAYLOAD', 'UNAUTHENTICATED', 'FORBIDDEN', 'BOARD_NOT_FOUND', 'RATE_LIMITED', 'SERVICE_UNAVAILABLE', 'INTERNAL_ERROR'],
...
const main = async () => {
  ...
  if (command !== 'invoke' || !OPERATIONS.includes(operation) || extra.length > 0) {
    throw new SceneBoardApiError('INVALID_PAYLOAD', 'Usage: describe | pair | invoke <board_operation>', { exitCode: 2 });
  }
...
try {
  ...
  const operation = OPERATIONS.includes(candidate) || candidate === 'pair' || candidate === 'describe' ? candidate : null;
