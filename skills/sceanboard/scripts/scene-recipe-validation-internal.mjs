...
export const validatePlainJson = (input) => {
  ...
  while (stack.length > 0) {
    ...
    if (depth > LIMITS_V1.maxJsonDepth) return { code: 'LIMIT_EXCEEDED', path, limit: 'maxJsonDepth' };
    ...
    if (keys.length > LIMITS_V1.maxJsonContainerEntries) return { code: 'LIMIT_EXCEEDED', path, limit: 'maxJsonContainerEntries' };
    if (Array.isArray(value) && keys.length !== value.length) return { code: 'INVALID_JSON', path };
    const descriptors = Object.getOwnPropertyDescriptors(value);
    for (const key of keys) {
      const descriptor = descriptors[key];
      if (!descriptor || descriptor.get || descriptor.set) return { code: 'INVALID_JSON', path: [...path, key] };
    ...
    for (let index = keys.length - 1; index >= 0; index -= 1) {
      ...
      stack.push({ value: value[key], path: [...path, Array.isArray(value) ? Number(key) : key], depth: depth + 1 });
    }
  }
  return null;
};
...
export const serializeCanonicalJson = (value) => {
  ...
  while (stack.length > 0) {
    ...
    else if (typeof current === 'number') output.push(Object.is(current, -0) ? '0' : JSON.stringify(current));
...
export const canonicalizeJsonValue = (value) => {
  ...
  if (typeof value === 'object') {
    const sorted = {};
    for (const key of Object.keys(value).sort(compareScalars)) sorted[key] = canonicalizeJsonValue(value[key]);
    return sorted;
  }
  return value;
};
...
export const isIdempotencyKey = (value) =>
  typeof value === 'string' && value.length >= 16 && value.length <= 128 && IDEMPOTENCY_KEY.test(value);

export const hasNoLoneSurrogate = (value) => typeof value === 'string' && !hasLoneSurrogate(value);

export const isShortText = (value) =>
  typeof value === 'string' && !hasLoneSurrogate(value) && scalarLength(value) >= 1 && scalarLength(value) <= LIMITS_V1.maxTitleChars;

export const isContentText = (value) =>
  typeof value === 'string' && !hasLoneSurrogate(value) && scalarLength(value) <= LIMITS_V1.maxCodeChars;

export const isMarkdownText = (value) =>
  typeof value === 'string' && !hasLoneSurrogate(value) && scalarLength(value) <= LIMITS_V1.maxMarkdownChars;

export const isCodeText = (value) => isContentText(value);

export const isImageAltText = (value) =>
  typeof value === 'string' && !hasLoneSurrogate(value) && scalarLength(value) <= LIMITS_V1.maxImageAltChars;
...
const isPosition = (value) =>
  Array.isArray(value) &&
  value.length === 2 &&
  isFiniteNumber(value[0]) && value[0] >= -180 && value[0] <= 180 &&
  isFiniteNumber(value[1]) && value[1] >= -90 && value[1] <= 90;
...
const validateSplitStructure = (node, path, collected) => {
  const closed = checkClosed(node, new Set(['id', 'title', 'type', 'direction', 'gap', 'children']), path);
  if (closed) return closed;
  if (node.direction !== 'horizontal' && node.direction !== 'vertical') return { path: [...path, 'direction'] };
  if (!isFiniteNumber(node.gap) || node.gap < 0 || node.gap > LIMITS_V1.maxCanvasExtent) return { path: [...path, 'gap'] };
  if (!Array.isArray(node.children) || node.children.length < 2 || node.children.length > LIMITS_V1.maxSplitChildren) return { path: [...path, 'children'] };
  for (let index = 0; index < node.children.length; index += 1) {
    ...
    if (!isFiniteNumber(child.weight) || child.weight <= 0) return { path: [...childPath, 'weight'] };
...
const validateGridStructure = (node, path, collected) => {
  const closed = checkClosed(node, new Set(['id', 'title', 'type', 'columns', 'rows', 'gap', 'children']), path);
  if (closed) return closed;
  if (!isInteger(node.columns) || node.columns < 1 || node.columns > LIMITS_V1.maxGridColumns) return { path: [...path, 'columns'] };
  if (!isInteger(node.rows) || node.rows < 1 || node.rows > LIMITS_V1.maxGridRows) return { path: [...path, 'rows'] };
  if (!isFiniteNumber(node.gap) || node.gap < 0 || node.gap > LIMITS_V1.maxCanvasExtent) return { path: [...path, 'gap'] };
  if (!Array.isArray(node.children) || node.children.length < 1 || node.children.length > LIMITS_V1.maxGridItems) return { path: [...path, 'children'] };
  for (let index = 0; index < node.children.length; index += 1) {
    const child = node.children[index];
    const childPath = [...path, 'children', index];
    if (!isPlainObject(child)) return { path: childPath };
    const childClosed = checkClosed(child, new Set(['node', 'column', 'row', 'columnSpan', 'rowSpan']), childPath);
    if (childClosed) return childClosed;
    ...
    if (!isInteger(child.columnSpan) || child.columnSpan < 1) return { path: [...childPath, 'columnSpan'] };
...
const validateTabsStructure = (node, path, collected) => {
  ...
  if (!Array.isArray(node.tabs) || node.tabs.length < 1 || node.tabs.length > LIMITS_V1.maxTabs) return { path: [...path, 'tabs'] };
...
const validateCanvasStructure = (node, path, collected) => {
  const closed = checkClosed(node, new Set(['id', 'title', 'type', 'width', 'height', 'children']), path);
  if (closed) return closed;
  if (!isFiniteNumber(node.width) || node.width <= 0 || node.width > LIMITS_V1.maxCanvasExtent) return { path: [...path, 'width'] };
  if (!isFiniteNumber(node.height) || node.height <= 0 || node.height > LIMITS_V1.maxCanvasExtent) return { path: [...path, 'height'] };
  if (!Array.isArray(node.children) || node.children.length < 1 || node.children.length > LIMITS_V1.maxCanvasItems) return { path: [...path, 'children'] };
  for (let index = 0; index < node.children.length; index += 1) {
    const child = node.children[index];
    const childPath = [...path, 'children', index];
    if (!isPlainObject(child)) return { path: childPath };
    const childClosed = checkClosed(child, new Set(['node', 'x', 'y', 'width', 'height', 'zIndex']), childPath);
    if (childClosed) return childClosed;
    ...
    if (!isFiniteNumber(child.height) || child.height <= 0) return { path: [...childPath, 'height'] };
...
const validateCodeStructure = (node, path) => {
  const closed = checkClosed(node, new Set(['id', 'title', 'type', 'language', 'code', 'showLineNumbers', 'wrap']), path);
  if (closed) return closed;
...
const validateTableColumn = (column, path) => {
  ...
  if (column.valueType !== 'string' && column.valueType !== 'number' && column.valueType !== 'boolean' && column.valueType !== 'datetime') return { path: [...path, 'valueType'] };
  return null;
};
...
const validateTableStructure = (node, path) => {
  ...
  if (closed) return closed;
  if (!Array.isArray(node.columns) || node.columns.length < 1 || node.columns.length > LIMITS_V1.maxTableColumns) return { path: [...path, 'columns'] };
  ...
  if (!Array.isArray(node.rows) || node.rows.length > LIMITS_V1.maxTableRows) return { path: [...path, 'rows'] };
...
const validateChartStructure = (node, path) => {
  const closed = checkClosed(node, new Set(['id', 'title', 'type', 'chartType', 'xAxis', 'yAxis', 'series']), path);
  if (closed) return closed;
  if (node.chartType !== 'line' && node.chartType !== 'bar' && node.chartType !== 'area' && node.chartType !== 'pie' && node.chartType !== 'scatter') return { path: [...path, 'chartType'] };
  ...
  if (node.xAxis.valueType !== 'category' && node.xAxis.valueType !== 'number' && node.xAxis.valueType !== 'datetime') return { path: [...path, 'xAxis', 'valueType'] };
  if (node.xAxis.label !== undefined && !isShortText(node.xAxis.label)) return { path: [...path, 'xAxis', 'label'] };
  ...
  if (node.yAxis.label !== undefined && !isShortText(node.yAxis.label)) return { path: [...path, 'yAxis', 'label'] };
  if (node.yAxis.min !== undefined && !isFiniteNumber(node.yAxis.min)) return { path: [...path, 'yAxis', 'min'] };
  if (node.yAxis.max !== undefined && !isFiniteNumber(node.yAxis.max)) return { path: [...path, 'yAxis', 'max'] };
  if (!Array.isArray(node.series) || node.series.length < 1 || node.series.length > LIMITS_V1.maxChartSeries) return { path: [...path, 'series'] };
  for (let index = 0; index < node.series.length; index += 1) {
    ...
    if (!Array.isArray(series.points) || series.points.length > LIMITS_V1.maxChartPoints) return { path: [...seriesPath, 'points'] };
    for (let pointIndex = 0; pointIndex < series.points.length; pointIndex += 1) {
      ...
      if (!(typeof point.x === 'string' || isFiniteNumber(point.x))) return { path: [...pointPath, 'x'] };
...
const validateMapStructure = (node, path) => {
  ...
  const viewportClosed = checkClosed(node.viewport, new Set(['longitude', 'latitude', 'zoom']), [...path, 'viewport']);
  if (viewportClosed) return viewportClosed;
  if (!isFiniteNumber(node.viewport.longitude) || node.viewport.longitude < -180 || node.viewport.longitude > 180) return { path: [...path, 'viewport', 'longitude'] };
  if (!isFiniteNumber(node.viewport.latitude) || node.viewport.latitude < -90 || node.viewport.latitude > 90) return { path: [...path, 'viewport', 'latitude'] };
  if (!isFiniteNumber(node.viewport.zoom) || node.viewport.zoom < 0 || node.viewport.zoom > 24) return { path: [...path, 'viewport', 'zoom'] };
  ...
  if (!Array.isArray(node.data.features) || node.data.features.length > LIMITS_V1.maxMapFeatures) return { path: [...path, 'data', 'features'] };
  for (let index = 0; index < node.data.features.length; index += 1) {
    ...
    const featureClosed = checkClosed(feature, new Set(['type', 'id', 'properties', 'geometry']), featurePath);
    if (featureClosed) return featureClosed;
    if (feature.type !== 'Feature') return { path: [...featurePath, 'type'] };
    if (feature.id !== undefined && !(typeof feature.id === 'string' || isFiniteNumber(feature.id))) return { path: [...featurePath, 'id'] };
...
const validateGeometry = (geometry, path) => {
  ...
  if (geometry.type === 'LineString') {
    if (!Array.isArray(geometry.coordinates) || geometry.coordinates.length < 2) return { path: [...path, 'coordinates'] };
    for (let index = 0; index < geometry.coordinates.length; index += 1) {
      if (!isPosition(geometry.coordinates[index])) return { path: [...path, 'coordinates', index] };
    }
    return null;
  }
  if (geometry.type === 'Polygon') {
    if (!Array.isArray(geometry.coordinates) || geometry.coordinates.length < 1) return { path: [...path, 'coordinates'] };
    for (let ringIndex = 0; ringIndex < geometry.coordinates.length; ringIndex += 1) {
      const ring = geometry.coordinates[ringIndex];
      if (!Array.isArray(ring) || ring.length < 4) return { path: [...path, 'coordinates', ringIndex] };
...
const validateDrawingStyle = (style, path) => {
  ...
  if (style.strokeWidth !== undefined && (!isFiniteNumber(style.strokeWidth) || style.strokeWidth <= 0)) return { path: [...path, 'strokeWidth'] };
  if (style.opacity !== undefined && (!isFiniteNumber(style.opacity) || style.opacity < 0 || style.opacity > 1)) return { path: [...path, 'opacity'] };
  return null;
};
...
const validateDrawingStructure = (node, path) => {
  ...
  const viewBoxClosed = checkClosed(node.viewBox, new Set(['x', 'y', 'width', 'height']), [...path, 'viewBox']);
  ...
  if (!isFiniteNumber(node.viewBox.width) || node.viewBox.width <= 0) return { path: [...path, 'viewBox', 'width'] };
  if (!isFiniteNumber(node.viewBox.height) || node.viewBox.height <= 0) return { path: [...path, 'viewBox', 'height'] };
  if (!Array.isArray(node.elements) || node.elements.length > LIMITS_V1.maxDrawingElements) return { path: [...path, 'elements'] };
...
const validateDrawingElement = (element, path) => {
  if (element.type === 'path') {
    ...
    if (!Array.isArray(element.points) || element.points.length < 2) return { path: [...path, 'points'] };
  ...
  if (element.type === 'rect') {
    const closed = checkClosed(element, new Set(['id', 'type', 'x', 'y', 'width', 'height', 'style']), path);
    if (closed) return closed;
    if (!isLocalFieldId(element.id)) return { path: [...path, 'id'] };
    ...
    if (!isFiniteNumber(element.height) || element.height <= 0) return { path: [...path, 'height'] };
    return validateDrawingStyle(element.style, [...path, 'style']);
  }
  if (element.type === 'ellipse') {
    const closed = checkClosed(element, new Set(['id', 'type', 'cx', 'cy', 'rx', 'ry', 'style']), path);
    if (closed) return closed;
    if (!isLocalFieldId(element.id)) return { path: [...path, 'id'] };
...
const validateStatusStructure = (node, path) => {
  const closed = checkClosed(node, new Set(['id', 'title', 'type', 'status', 'label', 'detail']), path);
  if (closed) return closed;
  if (node.status !== 'neutral' && node.status !== 'info' && node.status !== 'success' && node.status !== 'warning' && node.status !== 'error') return { path: [...path, 'status'] };
  if (!isShortText(node.label)) return { path: [...path, 'label'] };
  if (node.detail !== undefined && !isMarkdownText(node.detail)) return { path: [...path, 'detail'] };
  return null;
};
...
const validateImageStructure = (node, path) => {
  const closed = checkClosed(node, new Set(['id', 'title', 'type', 'source', 'alt', 'caption', 'fit']), path);
  if (closed) return closed;
  if (!isPlainObject(node.source)) return { path: [...path, 'source'] };
  const sourceClosed = checkClosed(node.source, new Set(['type', 'artifact', 'path', 'sha256']), [...path, 'source']);
  if (sourceClosed) return sourceClosed;
  if (node.source.type !== 'artifact.resource') return { path: [...path, 'source', 'type'] };
  const artifactErr = validateArtifactReference(node.source.artifact, [...path, 'source', 'artifact']);
  ...
  if (node.caption !== undefined && !isShortText(node.caption)) return { path: [...path, 'caption'] };
  if (node.fit !== 'contain' && node.fit !== 'cover' && node.fit !== 'fill' && node.fit !== 'none') return { path: [...path, 'fit'] };
  return null;
};

const validateProgressStructure = (node, path) => {
  const closed = checkClosed(node, new Set(['id', 'title', 'type', 'state', 'value', 'label', 'detail']), path);
  if (closed) return closed;
  if (node.state !== 'active' && node.state !== 'paused' && node.state !== 'complete' && node.state !== 'failed') return { path: [...path, 'state'] };
  if (node.value !== null && !isFiniteNumber(node.value)) return { path: [...path, 'value'] };
  if (node.value !== null && (node.value < 0 || node.value > 1)) return { path: [...path, 'value'] };
  if (!isShortText(node.label)) return { path: [...path, 'label'] };
...
const validateArtifactNodeStructure = (node, path) => {
  const closed = checkClosed(node, new Set(['id', 'title', 'type', 'artifact', 'fallbackText']), path);
  if (closed) return closed;
...
export const validateNodeSelfRelations = (node, path) => {
  if (node.type === 'layout.tabs') {
    ...
    if (new Set(tabIds).size !== tabIds.length) return { code: 'INVALID_LAYOUT', path: [...path, 'tabs'] };
    if (!tabs.some((tab) => tab.tabId === node.activeTabId)) return { code: 'INVALID_LAYOUT', path: [...path, 'activeTabId'] };
    return null;
  }
  if (node.type === 'layout.grid') {
    ...
    for (let index = 0; index < node.children.length; index += 1) {
      ...
      const childPath = [...path, 'children', index];
      if (placement.column + placement.columnSpan - 1 > node.columns || placement.row + placement.rowSpan - 1 > node.rows) return { code: 'INVALID_LAYOUT', path: childPath };
  ...
  if (node.type === 'layout.canvas') {
    for (let index = 0; index < node.children.length; index += 1) {
      const placement = node.children[index];
      if (placement.x + placement.width > node.width || placement.y + placement.height > node.height) return { code: 'INVALID_LAYOUT', path: [...path, 'children', index] };
    }
    return null;
  }
  if (node.type === 'content.table') {
    ...
    if (new Set(keys).size !== keys.length) return { code: 'INVALID_LAYOUT', path: [...path, 'columns'] };
    const rowIds = rows.map((row) => row.id);
    if (new Set(rowIds).size !== rowIds.length) return { code: 'INVALID_LAYOUT', path: [...path, 'rows'] };
    if (columns.length * rows.length > LIMITS_V1.maxTableCells) return { code: 'LIMIT_EXCEEDED', path: [...path, 'rows'], limit: 'maxTableCells' };
    for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
      const row = rows[rowIndex];
      const cellsPath = [...path, 'rows', rowIndex, 'cells'];
      if (Object.keys(row.cells).length !== keys.length || keys.some((key) => !Object.hasOwn(row.cells, key))) return { code: 'INVALID_LAYOUT', path: cellsPath };
      for (const column of columns) {
        const value = row.cells[column.key];
        if (value === null) continue;
        if (!validateTableCellValue(value, column.valueType)) return { code: 'INVALID_LAYOUT', path: [...cellsPath, column.key] };
      }
    }
    return null;
  }
  if (node.type === 'content.chart') {
    if (node.yAxis.min !== undefined && node.yAxis.max !== undefined && node.yAxis.min > node.yAxis.max) return { code: 'INVALID_LAYOUT', path: [...path, 'yAxis', 'max'] };
    const totalPoints = node.series.reduce((total, series) => total + series.points.length, 0);
    if (totalPoints > LIMITS_V1.maxChartPoints) return { code: 'LIMIT_EXCEEDED', path: [...path, 'series'], limit: 'maxChartPoints' };
    if (node.chartType === 'pie') {
      const first = node.series[0];
      const invalid = node.series.length !== 1 || node.xAxis.valueType !== 'category' || (first && first.points.some((point) => point.y === null || point.y < 0));
      if (invalid) return { code: 'INVALID_LAYOUT', path: [...path, 'series'] };
    }
    if (node.chartType === 'scatter' && node.xAxis.valueType !== 'number') return { code: 'INVALID_LAYOUT', path: [...path, 'xAxis', 'valueType'] };
    for (let seriesIndex = 0; seriesIndex < node.series.length; seriesIndex += 1) {
      const series = node.series[seriesIndex];
      for (let pointIndex = 0; pointIndex < series.points.length; pointIndex += 1) {
        const point = series.points[pointIndex];
        const validX = node.xAxis.valueType === 'number' ? typeof point.x === 'number' : node.xAxis.valueType === 'datetime' ? typeof point.x === 'string' && isTimestamp(point.x) : typeof point.x === 'string';
        if (!validX) return { code: 'INVALID_LAYOUT', path: [...path, 'series', seriesIndex, 'points', pointIndex, 'x'] };
      }
    }
    return null;
  }
    ...
    for (let featureIndex = 0; featureIndex < features.length; featureIndex += 1) {
      ...
      if (feature.geometry.type === 'Polygon') {
        for (let ringIndex = 0; ringIndex < feature.geometry.coordinates.length; ringIndex += 1) {
          ...
          if (!first || !last || first[0] !== last[0] || first[1] !== last[1]) return { code: 'INVALID_LAYOUT', path: [...path, 'data', 'features', featureIndex, 'geometry', 'coordinates', ringIndex] };
        }
      }
    }
    return null;
  ...
  if (node.type === 'content.progress') {
    ...
    const invalid = state === 'complete' ? value !== 1 : (state !== 'active' && state !== 'paused') && value === null;
