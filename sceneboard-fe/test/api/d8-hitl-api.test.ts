...
const success = (requestIdValue: string, result: Record<string, unknown>, status = 200): Response =>
  new Response(
    JSON.stringify({
      protocolVersion: 1,
...
test('respond selector preserves exact retry identity, CSRF, and typed response', async () => {
  const value = setup((_url, _init) =>
    success(requestId, fixture('mutation-result-hitl-respond.v1.json')),
  );
...
test('read selector encodes exact bounded wait cursor and correlates interaction ID', async () => {
  const value = setup((url) =>
    success(url.searchParams.get('requestId') ?? '', fixture('operation-result-hitl-read.v1.json')),
  );
  await value.coordinator.reconcileSessionGeneration();
...
test('lifecycle adapter rejects duplicate response members and extra request members', async () => {
  ...
  const value = setup(
    () =>
      new Response(
        `{"protocolVersion":1,"type":"hitl.lifecycle.result","requestId":"request_1","boardId":"board_1","action":"cancel","action":"cancel","replayed":false,"eventIds":["event_1"],"hitl":${cancelled}}`,
        {
          status: 200,
          headers: {
            'Content-Type': 'application/json; charset=utf-8',
  ...
  await assert.rejects(
    value.api.cancelInteraction(boardId, hitlRequestId, {
      protocolVersion: 1,
      requestId,
      expectedRevisionId: revisionId,
      expectedStateUpdatedAt: stateUpdatedAt,
