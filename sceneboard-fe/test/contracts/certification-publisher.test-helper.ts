...
export const readPublisher = (name: string): CertificationPublisherV1 =>
  JSON.parse(
    readFileSync(new URL(`test/contracts/certification-handoffs/${name}`, root), 'utf8'),
...
export const assertPublisher = (input: {
  ...
  assert.equal(publisher.resourcePath, 'sceneboard-fe/lib/api/board-api.ts');
  assert.equal(publisher.publisherTestPath, input.publisherTestPath);
  assert.deepEqual(publisher.contractIds, input.contractIds);
  assert.deepEqual(
    publisher.selectors.map(({ memberName }) => memberName),
    input.memberNames,
  );
  assert.equal(new Set(publisher.contractIds).size, publisher.contractIds.length);
  assert.equal(
    new Set(publisher.selectors.map(({ projectionId }) => projectionId)).size,
    publisher.selectors.length,
  );
