...
test('restores the matching code only inside its bounded tab session lifetime', () => {
  ...
  assert.deepEqual(
    readCreatedPairingSession(memory.storage, Date.parse('2026-07-17T13:09:59.000Z')),
    pairing,
  );
  assert.equal(
    readCreatedPairingSession(memory.storage, Date.parse('2026-07-17T13:10:00.000Z')),
    null,
  );
