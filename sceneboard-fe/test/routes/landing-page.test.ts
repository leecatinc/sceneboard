...
test('the landing page leads with graph engineering then presentation and HITL', () => {
  ...
  assert.match(graphHero, /data-landing-workflow-interaction="static"/);
  ...
  assert.doesNotMatch(graphHero, /<button[^>]*data-landing-workflow-node/u);
  assert.doesNotMatch(graphHero, /aria-haspopup="dialog"|<dialog|openDetail|showModal\(\)/u);
