...
export const normalizeLocale = (value: string | null | undefined): Locale | null => {
  ...
  const matched = SUPPORTED_LOCALES.find(
    (locale) => locale.toLowerCase().split('-')[0] === normalized.split('-')[0],
  );
  return matched ?? null;
};
...
): Locale => normalizeLocale(cookieLocale) ?? localeFromAcceptLanguage(acceptLanguage);

export const resolveClientLocale = (
  cookieHeader: string,
  ...
  const savedLocale = normalizeLocale(
    cookieLocale === undefined ? undefined : decodeURIComponent(cookieLocale),
  );
