import type { CreatedPairing } from '../api/board-api';
...
function parseStoredCreatedPairing(value: unknown, now: number): StoredCreatedPairing | null {
  if (
    !isObject(value) ||
    !hasExactKeys(value, ['version', 'expiresAt', 'pairing']) ||
    value.version !== 1
  )
    return null;
  if (
  ...
  if (
    ...
    !Number.isFinite(Date.parse(pairing.codeExpiresAt))
  )
    return null;
  return { version: 1, expiresAt: value.expiresAt, pairing: pairing as unknown as CreatedPairing };
}

export function readCreatedPairingSession(
...
export function writeCreatedPairingSession(
  ...
  try {
    storage.setItem(
      CREATED_PAIRING_SESSION_KEY,
      JSON.stringify({ version: 1, expiresAt, pairing }),
    );
  } catch {}
}
