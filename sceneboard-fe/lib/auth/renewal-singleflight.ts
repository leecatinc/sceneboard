...
export class SessionRequestCoordinator implements BoardStreamDispatchPortV1 {
  ...
  constructor(
    ...
    if (new URL(apiOrigin).origin !== apiOrigin)
      throw new TypeError('API URL must be a canonical origin');
  }
  ...
  > {
    ...
    return this.withApplicationLease('exclusive', async (lease) => {
      ...
      const submitted = await this.fetchExclusive(lease, kind, {
        credentials,
      ...
      if (submitted.response.status === 403 || submitted.response.status === 409) {
        try {
          ...
          if (error.error.code === 'AUTH_EMAIL_VERIFICATION_REQUIRED')
            return { kind: 'verification_required' as const };
        } catch {
          return { kind: 'reconciliation_required' as const };
        }
      }
  ...
  async withBoardStream<T>(
    ...
    const result = await this.withApplicationLease('shared', async (lease) => {
      ...
      if (
        (response.status === 401 && error.code === 'UNAUTHENTICATED') ||
    ...
    if (result.kind === 'reconciliation_required') {
      if (!Object.hasOwn(result, 'sourceStatus'))
        return { kind: 'protocol_error', sourceStatus: null, error: null };
  ...
  private async renewWithinOneLease(
  ...
  private async fetchExclusive(
    ...
    if (
      ...
      !['admitted', 'committed'].includes(lease.phase)
    ) {
      throw new TypeError('invalid exclusive session lease');
    }
    if (lease.kind === 'reconciliation' && kind !== 'session')
      throw new TypeError('reconciliation can only probe the session');
    if (
      lease.kind === 'application' &&
      this.readAuthoritativeGeneration() !== lease.expectedGeneration
    ) {
  ...
  private commitGeneration(
    ...
    if (generation === null || !GENERATION_PATTERN.test(generation))
      throw new TypeError('missing auth generation proof');
  ...
  private ensureSupported(): boolean {
    ...
    try {
      ...
      if (this.dependencies.storage.getItem(GENERATION_KEY) !== probe)
...
const validateBoardStreamInput = (input: BoardStreamOpenInputV1, apiOrigin: string): void => {
  if (input.apiOrigin !== apiOrigin)
    throw new TypeError('board stream origin does not match the coordinator');
  if (!BoardIdParserV1.parse(input.boardId).ok)
    throw new TypeError('board stream board ID is invalid');
  ...
  if (input.cursor !== null && !STREAM_CURSOR.test(input.cursor))
    throw new TypeError('board stream cursor is invalid');
  if (
...
export const sessionCoordinationConstants = Object.freeze({
  LOCK_NAME,
  GENERATION_KEY,
  CHANNEL_NAME,
  UNKNOWN_PATTERN,
