...
import type { BoardSummaryV1, PageCursorV1 } from '@sceneboard/board-schema';

import { useI18n } from '../../components/i18n/I18nProvider';
import { BoardApiClient, createBoardRequestIdentity } from '../../lib/api/board-api';
...
export function BoardsClient() {
  ...
  const load = useCallback(
    async (cursor: PageCursorV1 | null = null) => {
      ...
      if (result.kind === 'ok') {
        setBoards((current) =>
          cursor === null ? result.value.boards : [...current, ...result.value.boards],
        );
        setNextCursor(result.value.nextCursor);
        setError(null);
      } else
        setError(
          result.kind === 'board_error' && result.error.code === 'FORBIDDEN'
            ? t('boards.forbidden')
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    const identity =
      attempt.current?.title === title
        ? attempt.current
        : { title, ...createBoardRequestIdentity() };
  ...
  return (
    <section className="boards-page" aria-labelledby="boards-title">
      <div className="page-heading">
        <div>
          <p className="eyebrow">{t('boards.workspace')}</p>
          <h2 id="boards-title">{t('boards.yours')}</h2>
          <p>{t('boards.description')}</p>
        </div>
      </div>
      <form className="create-board" onSubmit={create}>
        <label htmlFor="board-title">{t('boards.new')}</label>
        <div>
          <input
            id="board-title"
            name="title"
            maxLength={200}
            required
            placeholder={t('boards.placeholder')}
          />
          <button className="button" disabled={phase === 'creating'}>
            {phase === 'creating' ? t('boards.creating') : t('boards.create')}
          </button>
        </div>
      </form>
      {error && (
      ...
      {phase === 'loading' ? (
        <div className="board-grid" aria-busy="true">
          {[1, 2, 3].map((item) => (
            <div className="board-card skeleton" key={item} />
          ))}
        </div>
      ) : boards.length === 0 ? (
        ...
        <div className="board-grid">
          {boards.map((board) => (
            <Link
              ...
              href={`/boards/${encodeURIComponent(board.boardId)}`}
              key={board.boardId}
            >
              <div>
                <span className="live-dot" />
                {board.archivedAt === null ? t('boards.available') : t('boards.archived')}
              </div>
              <h3>{board.title}</h3>
              <p>{t('boards.revision', { number: board.headRevision.revisionNumber })}</p>
              <time dateTime={board.updatedAt}>
                {t('boards.updated', { date: formatDateTime(board.updatedAt) })}
              </time>
            </Link>
          ))}
        </div>
      ...
      {nextCursor !== null && (
        ...
        >
          {phase === 'loading-more' ? t('common.loading') : t('boards.loadMore')}
        </button>
      )}
    </section>
  );
