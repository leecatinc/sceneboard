...
export function BoardTitleEditor({
  ...
  return (
    <form className="board-title-form" onSubmit={submit}>
      ...
      />
      <button type="submit" className="board-title-save" disabled={saving}>
        {saving ? t('board.savingName') : t('board.saveName')}
      </button>
      <button type="button" className="board-title-cancel" disabled={saving} onClick={cancel}>
        {t('board.cancelRename')}
      </button>
      {error && (
        <span className="board-title-error" role="alert">
