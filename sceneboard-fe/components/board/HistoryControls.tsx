...
export function HistoryControls({
  ...
  const previousDisabled =
    ...
    (state.mode.kind === 'history'
      ? state.mode.navigation.previousRevisionId === null
      : state.liveSnapshot.revision.previousRevisionId === null);
  const nextDisabled =
    state.pendingNavigation ||
    state.mode.kind !== 'history' ||
    state.mode.navigation.nextRevisionId === null;
  return (
    <nav className="history-controls" aria-label={t('board.historyLabel')}>
      ...
      <button
        ...
        onClick={onLatest}
        disabled={state.pendingNavigation || state.mode.kind === 'live'}
      >
        {t('board.latest')}
      </button>
      {state.pendingNavigation && <span role="status">{t('board.loadingRevision')}</span>}
      {liveUpdated && (
        <span className="live-updated" role="status">
          {t('board.liveUpdated')}
        </span>
      )}
    </nav>
