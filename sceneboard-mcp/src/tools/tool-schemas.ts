...
const PlacementSchema = z.discriminatedUnion('parentType', [
  z
    .object({ parentType: z.literal('layout.split'), weight: z.number().finite().positive() })
    .strict(),
  z
    .object({
      parentType: z.literal('layout.grid'),
  ...
  z
    .object({
      parentType: z.literal('layout.tabs'),
      tabId: LocalIdSchema,
  ...
  z
    .object({
      parentType: z.literal('layout.canvas'),
...
export const SceneTransformOperationSchemaV1 = z.discriminatedUnion('type', [
  ...
  z
    .object({
      type: z.literal('move_child'),
      sourceParentNodeId: LocalIdSchema,
      destinationParentNodeId: LocalIdSchema,
      nodeId: LocalIdSchema,
      destinationIndex: z.number().int().safe().nonnegative(),
      placement: PlacementSchema,
    })
    .strict(),
  z
    .object({
      type: z.literal('set_split_weight'),
      splitNodeId: LocalIdSchema,
      childNodeId: LocalIdSchema,
      weight: z.number().finite().positive(),
    })
    .strict(),
  z
    .object({
      type: z.literal('set_grid_placement'),
      gridNodeId: LocalIdSchema,
      childNodeId: LocalIdSchema,
      column: z.number().int().positive(),
      row: z.number().int().positive(),
      columnSpan: z.number().int().positive(),
  ...
  z
    .object({
      type: z.literal('set_canvas_rect'),
      canvasNodeId: LocalIdSchema,
      childNodeId: LocalIdSchema,
      x: z.number().finite().nonnegative(),
      y: z.number().finite().nonnegative(),
      width: z.number().finite().positive(),
    ...
    .strict(),
  z
    .object({ type: z.literal('set_active_tab'), tabsNodeId: LocalIdSchema, tabId: LocalIdSchema })
    .strict(),
  z
    .object({
      type: z.literal('upsert_drawing_element'),
      drawingNodeId: LocalIdSchema,
      element: z.unknown(),
    })
    .strict(),
  z
    .object({
      type: z.literal('remove_drawing_element'),
      drawingNodeId: LocalIdSchema,
      elementId: LocalIdSchema,
    })
    .strict(),
]);
