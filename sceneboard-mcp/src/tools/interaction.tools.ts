...
const ContentSchemaV1 = z
  ...
  .refine(
    (value) => [...value].length <= BOARD_LIMITS_V1.maxMarkdownChars,
    'HITL body is too long',
  );
const ResponseTextSchemaV1 = z
  .string()
  .refine((value) => !/[\uD800-\uDFFF]/u.test(value), 'lone surrogate is not allowed')
  .refine(
    (value) => [...value].length <= BOARD_LIMITS_V1.maxHitlTextChars,
    'HITL text is too long',
  );

const HitlOptionSchemaV1 = z
  .object({
...
const HitlFieldSchemaV1 = z.discriminatedUnion('type', [
  z
    .object({
      ...
      label: ShortTextSchemaV1,
      required: z.boolean(),
      defaultValue: ResponseTextSchemaV1.nullable(),
...
const HitlDefinitionSchemaV1 = z.discriminatedUnion('kind', [
  ...
  z
    .object({
      ...
      title: ShortTextSchemaV1,
      body: ContentSchemaV1.optional(),
      multiple: z.boolean(),
      minSelections: z.number().int().safe().min(1).max(BOARD_LIMITS_V1.maxHitlOptions),
  ...
  z
    .object({
      ...
      values: z.record(
        ...
        z.union([ResponseTextSchemaV1, z.number().finite(), z.boolean(), z.null()]),
      ),
    })
    .strict(),
...
export const InteractionRequestInputSchemaV1 = z
  .object({
...
export const InteractionStatusInputSchemaV1 = z
  .object({
    boardId: GlobalIdSchemaV1,
    hitlRequestId: GlobalIdSchemaV1,
    wait: z
...
export const InteractionRespondInputSchemaV1 = z
  .object({
    boardId: GlobalIdSchemaV1,
    expectedRevisionId: GlobalIdSchemaV1,
    idempotencyKey: IdempotencyKeySchemaV1,
...
export class InteractionToolHandlersV1 {
  ...
  async request(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    if (!parsed.success)
      return validationFailureV1('board_interaction_request', requestId, parsed.error);
  ...
  async respond(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    if (!parsed.success)
      return validationFailureV1('board_interaction_respond', requestId, parsed.error);
