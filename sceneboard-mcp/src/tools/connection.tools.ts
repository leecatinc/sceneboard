...
export const ConnectionStatusInputSchemaV1 = z
  ...
  .strict();
export const PairRequestInputSchemaV1 = z
  .object({
...
export const PairStatusInputSchemaV1 = z
  .object({
    ...
    waitTimeoutMs: z.number().int().safe().min(0).max(120_000),
  })
  .strict();
...
const pairLocalError = (error: PairingCoordinatorLocalErrorV1): BoardMcpLocalErrorV1 => {
  if (error.code === 'PAIRING_SINK_READ_ONLY' || error.code === 'PAIRING_SINK_UNAVAILABLE')
    return {
      ...
      details: {
        reason:
          error.code === 'PAIRING_SINK_READ_ONLY'
            ? 'pairing_sink_read_only'
            : 'pairing_sink_unavailable',
  ...
  if (error.code === 'PAIRING_STATE_LOST')
    return {
  ...
  if (error.code === 'PAIRING_CLAIM_OUTCOME_UNKNOWN')
    return {
  ...
  if (error.code === 'PAIRING_CREDENTIAL_UNRECOVERABLE')
    return {
  ...
  return {
    ...
    retryable: true,
    details: { phase: error.code === 'TRANSPORT_OUTCOME_UNKNOWN' ? error.phase : 'request' },
  };
};
...
export class ConnectionToolHandlersV1 {
  ...
  async status(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    if (!parsed.success)
      return validationFailureV1('board_connection_status', requestId, parsed.error);
    const result = await this.connections.status(parsed.data.boardId, requestId, signal);
    if (!result.ok)
      return toolFailureV1('board_connection_status', requestId, result.source, result.value);
    if (result.value.state === 'connected') this.onConnectionState(true);
    else if (
      result.value.state === 'not_configured' ||
      result.value.state === 'credential_missing' ||
      result.value.state === 'credential_invalid'
    )
  ...
  async pairRequest(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    return result.source === 'pairing'
      ...
      : toolFailureV1(
          ...
          requestId,
          'mcp',
          pairLocalError(result.error) as unknown as Record<string, unknown>,
        );
  }
  ...
  async pairStatus(raw: unknown, signal?: AbortSignal): Promise<CallToolResult> {
    ...
    const result = await this.pairing.status(
      parsed.data.pairingId,
      parsed.data.waitTimeoutMs,
      signal,
    );
    if (result.ok) {
