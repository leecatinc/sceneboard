...
const StatusResponseSchema = z
  ...
  .superRefine((value, context) => {
    if (value.decisionExpiresAt === null)
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['decisionExpiresAt'],
        message: 'decision deadline is required',
      });
    if (
      value.state === 'pending' &&
      (value.retryAfterSeconds === null || value.redeemExpiresAt !== null)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['state'],
        message: 'pending status is inconsistent',
      });
    }
    if (value.state !== 'pending' && value.retryAfterSeconds !== null) {
      context.addIssue({
        ...
        message: 'terminal delay is not allowed',
      });
    }
    if (
      (value.state === 'approved' || value.state === 'redeemed') &&
      value.redeemExpiresAt === null
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['redeemExpiresAt'],
        message: 'redeem deadline is required',
      });
    }
    if (value.state === 'denied' && value.redeemExpiresAt !== null) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['redeemExpiresAt'],
        message: 'denied status has no redeem deadline',
      });
    }
  });
...
const GrantSchema = z
  ...
  .superRefine((value, context) => {
    if (
      ...
      (!value.scopes.includes('board.write') ||
        !value.lifecyclePermissions.includes('board.create'))
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['boardIds'],
...
export type PairingUpstreamErrorV1 = z.infer<typeof ErrorSchema>['error'] & {
...
const expectedStatus: Readonly<Record<RouteKind, number>> = {
  claim: 202,
  status: 200,
  redeem: 200,
};
...
export class PairingHttpClientV1 {
  ...
  constructor(options: PairingHttpClientOptionsV1) {
    ...
    if (url.origin !== options.baseUrl)
      throw new TypeError('pairing base URL must be a canonical origin');
    if (
  ...
  claim(
    request: PairingClaimRequestV1,
    signal?: AbortSignal,
  ): Promise<PairingHttpResultV1<PairingClaimResponseV1>> {
    return this.#call(
      'claim',
  ...
  clientStatus(
    pairingId: string,
    signal?: AbortSignal,
  ): Promise<PairingHttpResultV1<PairingClientStatusV1>> {
    return this.#call(
      'status',
      `/api/v1/pairings/${encodeURIComponent(pairingId)}/client-status`,
      'GET',
      null,
  ...
  redeem(
    pairingId: string,
    signal?: AbortSignal,
  ): Promise<PairingHttpResultV1<PairingRedeemResponseV1>> {
    return this.#call(
      'redeem',
      `/api/v1/pairings/${encodeURIComponent(pairingId)}/redeem`,
      'POST',
      {},
  ...
  async #call<T>(
    ...
    if (route !== 'claim') {
      ...
      if (!/^[A-Za-z0-9_-]{43}$/.test(proof)) {
        ...
        return {
          ok: false,
          source: 'local',
          error: { code: 'RESPONSE_INVALID', reason: 'schema' },
        };
      }
    ...
    try {
      ...
      if (timeoutSignal.aborted)
        return {
          ok: false,
          source: 'local',
          error: { code: 'TIMEOUT', timeoutMs: this.#timeoutMs },
        };
      return route === 'status'
        ...
        : {
            ok: false,
            source: 'local',
            error: { code: 'TRANSPORT_OUTCOME_UNKNOWN', phase: route },
          };
    } finally {
      proof = '';
    ...
    if (response.headers.get('content-type')?.toLowerCase() !== 'application/json; charset=utf-8') {
      ...
      return {
        ok: false,
        source: 'local',
        error: { code: 'RESPONSE_INVALID', reason: 'content_type' },
      };
    }
    ...
    if (typeof bytes === 'string') {
      return {
        ok: false,
        source: 'local',
        error: { code: 'RESPONSE_INVALID', reason: bytes === 'body_too_large' ? bytes : 'schema' },
      };
    }
    ...
    if (!parsed.ok)
      return {
        ok: false,
        source: 'local',
        error: { code: 'RESPONSE_INVALID', reason: parsed.reason },
      };
    ...
    const requiresRetry =
      failure.data.error.code === 'PAIRING_NOT_READY' || failure.data.error.code === 'RATE_LIMITED';
    ...
    return {
      ok: false,
      source: 'pairing',
      error: { ...failure.data.error, retryAfterSeconds: retryAfter },
    };
  }
}
