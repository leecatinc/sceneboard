...
```bash
node /absolute/path/to/sceneboard/scripts/sceneboard-api.mjs describe
printf '%s' '<exact tool input JSON>' | node /absolute/path/to/sceneboard/scripts/sceneboard-api.mjs invoke board_list
```
...
```powershell
$adapter = 'C:\absolute\path\to\sceneboard\scripts\sceneboard-api.ps1'
...
```bash
node /absolute/path/to/sceneboard/scripts/sceneboard-api.mjs pair
```
