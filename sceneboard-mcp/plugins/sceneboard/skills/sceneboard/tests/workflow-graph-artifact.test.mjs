...
test("workflow graph renders the closed WorkflowSpec v1 contract deterministically", () => {
  ...
  assert.doesNotMatch(
    first.source.javascript,
    /!event\.target\.closest\('\.sb-graph-scroll'\)/u,
  );
  assert.doesNotMatch(
    first.source.javascript,
    /if\(event\.target\.closest\('button,a,input,/u,
  );
  assert.doesNotMatch(
...
test("canonical byte limits select interactive rendering without rejecting valid export", () => {
  ...
  assert.ok(
    Buffer.byteLength(canonicalizeWorkflowSpec(below), "utf8") <= 32_768,
  );
  assert.ok(
    Buffer.byteLength(canonicalizeWorkflowSpec(above), "utf8") > 32_768,
  );
  const rendered = compileSceneArtifactDraft(
    recipe("manual", below),
    descriptor,
  );
  const exported = compileSceneArtifactDraft(
    recipe("manual", above),
    descriptor,
  );
