...
export const protectedSpec = (operation, input, requestId) => {
  ...
  if (operation === "board_get" || operation === "board_capabilities_get") {
    ...
    return {
      ...
      body: null,
      expectedType:
        operation === "board_get" ? "board.get" : "capabilities.get",
      retryKind: "read",
      correlation: { boardId },
    };
  }
  ...
  if (operation === "board_scene_clear") {
    ...
    return mutationSpec(
      baseMutation(input, requestId, { type: "scene.clear" }),
      "scene.clear",
    );
  }
  ...
  if (operation === "board_interaction_status") {
    ...
    return {
      ...
      minimumTimeoutMs:
        input.wait === null
          ? undefined
          : Math.max(30_000, input.wait.timeoutMs + 5_000),
    };
  }
...
export const assertSortedCatalog = (
  value,
  catalog,
  field,
  allowEmpty = false,
