...
const readHandle = async (handle) => {
  ...
  if (!stat.isFile())
    throw new CliError('UNSAFE_PRESET', 'Preset entry is not a safe regular file.', ['preset']);
...
const parseFlags = (args, positionalMax) => {
  ...
  const bindingCount = ['boardId', 'expectedRevisionId', 'idempotencyKey'].filter(
...
const main = async (args) => {
  ...
  if (command === 'validate' || command === 'compile') {
    ...
    if (command === 'validate' && (values.output !== 'scene' || Object.keys(values).length !== 1))
      usage();
