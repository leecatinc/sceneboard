...
export const hasNoLoneSurrogate = (value) =>
...
const validateSplitStructure = (node, path, collected) => {
  ...
  if (
    !isFiniteNumber(node.gap) ||
    node.gap < 0 ||
    node.gap > LIMITS_V1.maxCanvasExtent
  )
  ...
  for (let index = 0; index < node.children.length; index += 1) {
    ...
    const inner = checkNodeStructure(
      child.node,
      [...childPath, "node"],
      collected,
    );
...
const validateGridStructure = (node, path, collected) => {
  ...
  if (closed) return closed;
  if (
    !isInteger(node.columns) ||
    node.columns < 1 ||
    node.columns > LIMITS_V1.maxGridColumns
  )
    return { path: [...path, "columns"] };
  if (
    !isInteger(node.rows) ||
    node.rows < 1 ||
    node.rows > LIMITS_V1.maxGridRows
  )
  ...
  for (let index = 0; index < node.children.length; index += 1) {
    ...
    if (childClosed) return childClosed;
    if (!isInteger(child.column) || child.column < 1)
      return { path: [...childPath, "column"] };
    if (!isInteger(child.row) || child.row < 1)
      return { path: [...childPath, "row"] };
    if (!isInteger(child.columnSpan) || child.columnSpan < 1)
      return { path: [...childPath, "columnSpan"] };
    if (!isInteger(child.rowSpan) || child.rowSpan < 1)
      return { path: [...childPath, "rowSpan"] };
    const inner = checkNodeStructure(
      child.node,
      [...childPath, "node"],
      collected,
...
const validateTabsStructure = (node, path, collected) => {
  const closed = checkClosed(
    node,
    new Set(["id", "title", "type", "activeTabId", "tabs"]),
    path,
  );
  if (closed) return closed;
  if (!isLocalFieldId(node.activeTabId))
    return { path: [...path, "activeTabId"] };
  if (
    !Array.isArray(node.tabs) ||
    node.tabs.length < 1 ||
    node.tabs.length > LIMITS_V1.maxTabs
  )
  ...
  for (let index = 0; index < node.tabs.length; index += 1) {
    ...
    const tabClosed = checkClosed(
      tab,
      new Set(["tabId", "label", "node"]),
      tabPath,
    );
...
const validateCanvasStructure = (node, path, collected) => {
  ...
  if (
    ...
    node.width <= 0 ||
    node.width > LIMITS_V1.maxCanvasExtent
  )
    return { path: [...path, "width"] };
  if (
    !isFiniteNumber(node.height) ||
    node.height <= 0 ||
    node.height > LIMITS_V1.maxCanvasExtent
  )
  ...
  for (let index = 0; index < node.children.length; index += 1) {
    ...
    if (childClosed) return childClosed;
    if (!isFiniteNumber(child.x) || child.x < 0)
      return { path: [...childPath, "x"] };
    if (!isFiniteNumber(child.y) || child.y < 0)
      return { path: [...childPath, "y"] };
    if (!isFiniteNumber(child.width) || child.width <= 0)
      return { path: [...childPath, "width"] };
...
const validateMarkdownStructure = (node, path) => {
  const closed = checkClosed(
    node,
    new Set(["id", "title", "type", "markdown"]),
    path,
  );
  if (closed) return closed;
...
const validateCodeStructure = (node, path) => {
  ...
  if (!isBoolean(node.showLineNumbers))
    return { path: [...path, "showLineNumbers"] };
...
const validateTableStructure = (node, path) => {
  const closed = checkClosed(
    node,
    new Set(["id", "title", "type", "columns", "rows"]),
    path,
  );
  if (closed) return closed;
  if (
  ...
  for (let index = 0; index < node.columns.length; index += 1) {
    const colErr = validateTableColumn(node.columns[index], [
      ...path,
      "columns",
      index,
    ]);
  ...
  for (let index = 0; index < node.rows.length; index += 1) {
    ...
    for (const cellKey of Object.keys(row.cells)) {
      if (!isLocalFieldId(cellKey))
        return { path: [...rowPath, "cells", cellKey] };
    }
  }
  return null;
};
...
const validateChartStructure = (node, path) => {
  ...
  const xAxisClosed = checkClosed(node.xAxis, new Set(["valueType", "label"]), [
    ...path,
    "xAxis",
  ]);
  ...
  const yAxisClosed = checkClosed(
    node.yAxis,
    new Set(["label", "min", "max"]),
    [...path, "yAxis"],
  );
  ...
  for (let index = 0; index < node.series.length; index += 1) {
    ...
    const seriesClosed = checkClosed(
      series,
      new Set(["id", "label", "points"]),
      seriesPath,
    );
    ...
    if (
      ...
      series.points.length > LIMITS_V1.maxChartPoints
    )
    ...
    for (
      ...
      if (point.y !== null && !isFiniteNumber(point.y))
        return { path: [...pointPath, "y"] };
    }
  }
  return null;
};
...
const validateMapStructure = (node, path) => {
  const closed = checkClosed(
    node,
    new Set(["id", "title", "type", "viewport", "data"]),
    path,
  );
  if (closed) return closed;
  if (!isPlainObject(node.viewport)) return { path: [...path, "viewport"] };
  const viewportClosed = checkClosed(
    node.viewport,
    new Set(["longitude", "latitude", "zoom"]),
    [...path, "viewport"],
  );
  ...
  if (
    !isFiniteNumber(node.viewport.zoom) ||
    node.viewport.zoom < 0 ||
    node.viewport.zoom > 24
  )
    return { path: [...path, "viewport", "zoom"] };
  if (!isPlainObject(node.data)) return { path: [...path, "data"] };
  const dataClosed = checkClosed(node.data, new Set(["type", "features"]), [
    ...path,
    "data",
  ]);
  if (dataClosed) return dataClosed;
  if (node.data.type !== "FeatureCollection")
    return { path: [...path, "data", "type"] };
  if (
    !Array.isArray(node.data.features) ||
    node.data.features.length > LIMITS_V1.maxMapFeatures
  )
  ...
  for (let index = 0; index < node.data.features.length; index += 1) {
    ...
    if (
      ...
      !(typeof feature.id === "string" || isFiniteNumber(feature.id))
    )
      return { path: [...featurePath, "id"] };
    if (!isProperties(feature.properties))
      return { path: [...featurePath, "properties"] };
    const geometryErr = validateGeometry(feature.geometry, [
      ...featurePath,
      "geometry",
    ]);
...
const validateGeometry = (geometry, path) => {
  ...
  if (geometry.type === "Polygon") {
    ...
    for (
      let ringIndex = 0;
      ringIndex < geometry.coordinates.length;
      ringIndex += 1
    ) {
      ...
      for (let index = 0; index < ring.length; index += 1) {
        if (!isPosition(ring[index]))
          return { path: [...path, "coordinates", ringIndex, index] };
      }
    }
    return null;
  }
...
const validateDrawingStyle = (style, path) => {
  ...
  const closed = checkClosed(
    style,
    new Set(["stroke", "fill", "strokeWidth", "opacity"]),
    path,
  );
  if (closed) return closed;
  if (style.stroke !== undefined && !isColor(style.stroke))
    return { path: [...path, "stroke"] };
  if (style.fill !== undefined && !isColor(style.fill))
...
const validateDrawingStructure = (node, path) => {
  const closed = checkClosed(
    node,
    new Set(["id", "title", "type", "viewBox", "elements"]),
    path,
  );
  if (closed) return closed;
  if (!isPlainObject(node.viewBox)) return { path: [...path, "viewBox"] };
  const viewBoxClosed = checkClosed(
    node.viewBox,
    new Set(["x", "y", "width", "height"]),
    [...path, "viewBox"],
  );
  if (viewBoxClosed) return viewBoxClosed;
  if (!isFiniteNumber(node.viewBox.x))
    return { path: [...path, "viewBox", "x"] };
  if (!isFiniteNumber(node.viewBox.y))
    return { path: [...path, "viewBox", "y"] };
  ...
  if (
    ...
    node.elements.length > LIMITS_V1.maxDrawingElements
  )
...
const validateDrawingElement = (element, path) => {
  if (element.type === "path") {
    const closed = checkClosed(
      element,
      new Set(["id", "type", "points", "closed", "style"]),
      path,
    );
    if (closed) return closed;
    ...
    for (let index = 0; index < element.points.length; index += 1) {
      const pointErr = validatePoint(element.points[index], [
        ...path,
        "points",
        index,
      ]);
  ...
  if (element.type === "ellipse") {
    ...
    if (!isFiniteNumber(element.rx) || element.rx <= 0)
      return { path: [...path, "rx"] };
    if (!isFiniteNumber(element.ry) || element.ry <= 0)
      return { path: [...path, "ry"] };
    return validateDrawingStyle(element.style, [...path, "style"]);
  }
  ...
  if (element.type === "text") {
    const closed = checkClosed(
      element,
      new Set(["id", "type", "x", "y", "text", "style"]),
      path,
    );
    if (closed) return closed;
    if (!isLocalFieldId(element.id)) return { path: [...path, "id"] };
...
const validateImageStructure = (node, path) => {
  ...
  const sourceClosed = checkClosed(
    node.source,
    new Set(["type", "artifact", "path", "sha256"]),
    [...path, "source"],
  );
  if (sourceClosed) return sourceClosed;
  if (node.source.type !== "artifact.resource")
    return { path: [...path, "source", "type"] };
  ...
  if (!isNormalizedArtifactPath(node.source.path))
    return { path: [...path, "source", "path"] };
  if (!isDigest(node.source.sha256))
    return { path: [...path, "source", "sha256"] };
...
const validateProgressStructure = (node, path) => {
  ...
  if (node.value !== null && !isFiniteNumber(node.value))
    return { path: [...path, "value"] };
  if (node.value !== null && (node.value < 0 || node.value > 1))
    return { path: [...path, "value"] };
  if (!isShortText(node.label)) return { path: [...path, "label"] };
  if (node.detail !== undefined && !isShortText(node.detail))
    return { path: [...path, "detail"] };
  return null;
};
...
const validateHitlStructure = (node, path) => {
  const closed = checkClosed(
    node,
    new Set(["id", "title", "type", "hitlRequestId"]),
    path,
  );
  if (closed) return closed;
  if (!isGlobalId(node.hitlRequestId))
    return { path: [...path, "hitlRequestId"] };
  return null;
};
...
const validateArtifactNodeStructure = (node, path) => {
  ...
  if (closed) return closed;
  const artifactErr = validateArtifactReference(node.artifact, [
    ...path,
    "artifact",
  ]);
  if (artifactErr) return artifactErr;
  if (!isShortText(node.fallbackText))
    return { path: [...path, "fallbackText"] };
  return null;
};
...
export const NODE_TYPES_V1 = Object.freeze(
  Object.keys(NODE_STRUCTURE_VALIDATORS),
);
...
export const checkNodeStructure = (node, path, collected) => {
  ...
  if (node.title !== undefined && !isShortText(node.title))
...
export const validateNodeSelfRelations = (node, path) => {
  ...
  if (node.type === "content.chart") {
    ...
    const totalPoints = node.series.reduce(
    ...
    if (node.chartType === "pie") {
      ...
      const invalid =
        ...
        node.xAxis.valueType !== "category" ||
        (first &&
          first.points.some((point) => point.y === null || point.y < 0));
    ...
    for (
      let seriesIndex = 0;
      seriesIndex < node.series.length;
      seriesIndex += 1
    ) {
  ...
  if (node.type === "content.map") {
    ...
    for (
      ...
      if (feature.geometry.type === "Polygon") {
        for (
          let ringIndex = 0;
          ringIndex < feature.geometry.coordinates.length;
          ringIndex += 1
        ) {
