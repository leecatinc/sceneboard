...
const CONDITION_LANGUAGES = new Set([
  "natural",
  "cel",
  "javascript",
  "python",
...
const exactKeys = (value, keys, path) => {
  ...
  for (const key of keys)
    if (!Object.hasOwn(value, key)) fail("INVALID_TYPE", pointer(path, key));
};
...
const validateStructureLimits = (value) => {
  ...
  const visit = (item, depth) => {
    ...
    if (entries > WORKFLOW_SPEC_LIMITS_V1.containerEntries)
      fail("LIMIT_EXCEEDED", "");
...
const validateSourceRef = (value, sourceIds, path) => {
  ...
  if (!sourceIds.has(value.sourceId))
    fail("DANGLING_REFERENCE", pointer(path, "sourceId"));
...
const validateEvidence = (value, sourceIds, path) => {
  ...
  refs.forEach((ref, index) =>
    validateSourceRef(
      ref,
      sourceIds,
      pointer(pointer(path, "sourceRefs"), index),
    ),
  );
  unique(
    refs,
    (ref) =>
      `${ref.sourceId}\0${ref.startLine}\0${ref.endLine}\0${ref.locator}`,
    pointer(path, "sourceRefs"),
  );
};
...
const validateStateItem = (value, path) => {
  ...
  if (typeof value.required !== "boolean")
    fail("INVALID_TYPE", pointer(path, "required"));
...
const validateRetryPolicy = (value, path) => {
  ...
  exactKeys(
    value,
    ["maxAttempts", "backoff", "initialDelayMs", "maxDelayMs", "retryOn"],
    path,
  );
  ...
  if (value.maxDelayMs < value.initialDelayMs)
    fail("INVALID_VALUE", pointer(path, "maxDelayMs"));
...
const validateErrorPolicy = (value, path) => {
  ...
  const keys = array(
    value.emitsStateKeys,
    0,
    32,
    pointer(path, "emitsStateKeys"),
  );
  keys.forEach((item, index) =>
    token(item, pointer(pointer(path, "emitsStateKeys"), index)),
  );
...
const validateControl = (value, kind, path) => {
  if (value === null) {
    if (["parallel", "join", "human"].includes(kind))
  ...
  if (kind === "parallel") {
    ...
    const ids = array(
      ...
      2,
      32,
      pointer(path, "branchEdgeIds"),
    );
    ids.forEach((item, index) =>
      id(item, pointer(pointer(path, "branchEdgeIds"), index)),
    );
  ...
  if (kind === "join") {
    ...
    enumeration(value.mode, new Set(["all", "any"]), pointer(path, "mode"));
    const ids = array(
      value.incomingEdgeIds,
      2,
      32,
      pointer(path, "incomingEdgeIds"),
    );
    ids.forEach((item, index) =>
      id(item, pointer(pointer(path, "incomingEdgeIds"), index)),
    );
  ...
  if (kind === "human") {
    ...
    if (typeof value.blocking !== "boolean")
      fail("INVALID_TYPE", pointer(path, "blocking"));
    return;
  }
  fail("INVALID_KIND_COMBINATION", path);
};
...
const validateNode = (value, sourceIds, path) => {
  ...
  for (const key of ["stateInputs", "stateOutputs"]) {
    ...
    items.forEach((item, index) =>
      validateStateItem(item, pointer(pointer(path, key), index)),
    );
  ...
  for (const [key, maximum] of [
    ...
    items.forEach((item, index) =>
      token(item, pointer(pointer(path, key), index)),
    );
  ...
  else if (value.subflowId !== null)
    fail("INVALID_KIND_COMBINATION", pointer(path, "subflowId"));
...
const validateEdge = (value, sourceIds, path) => {
  ...
  if (value.condition !== null) {
    exactKeys(
      value.condition,
      ["text", "language"],
      pointer(path, "condition"),
    );
    text(
      value.condition.text,
      1,
      2_000,
      pointer(pointer(path, "condition"), "text"),
    );
    enumeration(
  ...
  keys.forEach((item, index) =>
    token(item, pointer(pointer(path, "stateKeys"), index)),
  );
...
const validateFlow = (flow, sourceIds, path, isRoot) => {
  ...
  entries.forEach((item, index) =>
    id(item, pointer(pointer(path, "entryNodeIds"), index)),
  );
  exits.forEach((item, index) =>
    id(item, pointer(pointer(path, "exitNodeIds"), index)),
  );
  ...
  const declaredStateKeys = new Set(
    nodes.flatMap((node) =>
      [...node.stateInputs, ...node.stateOutputs].map(({ key }) => key),
    ),
  );
  for (const entry of entries)
    if (
      !nodeIds.has(entry) ||
      nodes.find(({ id: nodeId }) => nodeId === entry)?.kind !== "start"
    )
      fail("DANGLING_REFERENCE", pointer(path, "entryNodeIds"));
  for (const exit of exits)
    if (
      !nodeIds.has(exit) ||
      nodes.find(({ id: nodeId }) => nodeId === exit)?.kind !== "end"
    )
  ...
  for (const edge of edges) {
    ...
    if (
      edge.kind === "fallback" &&
      !["action", "decision", "human", "subflow"].includes(from.kind)
    )
      fail("INVALID_KIND_COMBINATION", pointer(path, "edges"));
  }
  for (const [nodeIndex, node] of nodes.entries()) {
    ...
    if (node.errorPolicy !== null) {
      if (
        ...
        fail(
          "DANGLING_REFERENCE",
          pointer(pointer(nodePath, "errorPolicy"), "onExhaustedEdgeId"),
        );
      if (node.errorPolicy.emitsStateKeys.some((key) => !stateKeys.has(key)))
        fail(
          "DANGLING_REFERENCE",
          pointer(pointer(nodePath, "errorPolicy"), "emitsStateKeys"),
        );
    }
    const controlIds =
      node.kind === "parallel"
    ...
    if (node.kind === "parallel") {
      const outbound = edges
        .filter(
          (edge) => edge.fromNodeId === node.id && edge.kind === "parallel",
        )
        .map((edge) => edge.id)
        .sort();
      if (JSON.stringify(outbound) !== JSON.stringify([...controlIds].sort()))
        fail(
          "INVALID_KIND_COMBINATION",
          pointer(pointer(nodePath, "control"), "branchEdgeIds"),
        );
    }
    if (node.kind === "join") {
      ...
      if (JSON.stringify(incoming) !== JSON.stringify([...controlIds].sort()))
        fail(
          "INVALID_KIND_COMBINATION",
          pointer(pointer(nodePath, "control"), "incomingEdgeIds"),
        );
    }
  }
...
export const validateWorkflowSpec = (input) => {
  ...
  if (value.schemaVersion !== WORKFLOW_SPEC_VERSION)
  ...
  subflows.forEach((flow, index) =>
    validateFlow(flow, sourceIds, `/subflows/${index}`, false),
  );
  ...
  subflows.forEach((flow, flowIndex) =>
    admitElementId(flow.id, `/subflows/${flowIndex}/id`),
  );
  ...
  const subflowIndexById = new Map(
    subflows.map((flow, index) => [flow.id, index]),
  );
  const visiting = new Set();
  const visited = new Set();
  const visit = (
    flowId,
    referencePath = `/subflows/${subflowIndexById.get(flowId)}`,
  ) => {
    ...
    for (const [nodeIndex, node] of subflowById.get(flowId).nodes.entries())
      if (node.kind === "subflow")
        visit(
          node.subflowId,
          `/subflows/${flowIndex}/nodes/${nodeIndex}/subflowId`,
        );
  ...
  questions.forEach((question, index) => {
    ...
    const refs = array(
      question.relatedElements,
      1,
      32,
      `${path}/relatedElements`,
    );
    refs.forEach((ref, refIndex) => {
      ...
      enumeration(
        ref.kind,
        new Set(["node", "edge", "subflow"]),
        `${refPath}/kind`,
      );
      id(ref.id, `${refPath}/id`);
      if (!elementIds[ref.kind].has(ref.id))
        fail("DANGLING_REFERENCE", refPath);
    });
  ...
  warnings.forEach((warning, index) => {
    ...
    exactKeys(
      warning,
      ["id", "code", "elementType", "elementId", "message", "evidence"],
      path,
    );
    id(warning.id, `${path}/id`);
    enumeration(
      warning.code,
      new Set(WORKFLOW_SPEC_WARNING_CODES_V1),
      `${path}/code`,
    );
    enumeration(
      warning.elementType,
    ...
    if (warning.elementType === "workflow") {
      if (warning.elementId !== null)
        fail("INVALID_VALUE", `${path}/elementId`);
    } else {
  ...
  const warnedUnknownConditions = new Set(
    warnings
      .filter(
        (warning) =>
          warning.code === "UNKNOWN_CONDITION" &&
          warning.elementType === "edge",
      )
  ...
  if (Array.isArray(value)) {
    ...
    return items.sort((left, right) => {
      ...
      if (key === "sourceRefs") {
        ...
        return endOrder !== 0
          ? endOrder
          : compareNullable(left.locator, right.locator);
      }
  ...
  if (value !== null && typeof value === "object")
    return Object.fromEntries(
      Object.keys(value)
        ...
        .map((nestedKey) => [
          nestedKey,
          normalize(value[nestedKey], nestedKey),
        ]),
    );
  return value;
};
...
export const analyzeWorkflowSpec = (input) => {
  ...
  const messages = Object.freeze({
    ...
    NON_RETRY_CYCLE:
      "The flow contains a cycle that is not expressed with retry edges.",
    UNKNOWN_CONDITION: "The conditional route has no known condition.",
    DECISION_ROUTE_MISSING:
      "The decision does not declare at least two conditional routes.",
    PARALLEL_JOIN_MISMATCH:
      "The parallel branches do not converge on one compatible join.",
    UNCERTAIN_ROUTE:
      "The conditional route is based on unknown evidence or language.",
  });
  const add = (code, elementType, element) =>
    findings.push({
      ...
      evidence:
        element?.evidence ??
        Object.freeze({ basis: "inferred", confidence: 1, sourceRefs: [] }),
    });
  const flows = [
    {
  ...
  for (const flow of flows) {
    const nodes = [...flow.nodes].sort((left, right) =>
      compare(left.id, right.id),
    );
    const edges = [...flow.edges].sort((left, right) =>
      compare(left.id, right.id),
    );
    ...
    for (const node of nodes)
      if (!reachable.has(node.id)) add("UNREACHABLE_NODE", "node", node);
    ...
    const closure = new Map(
      nodes.map((node) => [node.id, nonRetryReachable(node.id)]),
    );
    ...
    for (const node of nodes) {
      ...
      const members = nodes
        .filter(
          (candidate) =>
            closure.get(node.id).has(candidate.id) &&
            closure.get(candidate.id).has(node.id),
        )
    ...
    for (const node of nodes) {
      if (node.kind === "decision") {
        const routes = (outbound.get(node.id) ?? []).filter(
      ...
      if (node.kind === "parallel") {
        const branches = node.control.branchEdgeIds
          .map((edgeId) =>
            edges.find(({ id: candidateId }) => candidateId === edgeId),
          )
          .filter(Boolean);
        const joinSets = branches.map((branch) => {
          const reachableNodes = new Set([
            branch.toNodeId,
            ...closure.get(branch.toNodeId),
          ]);
          return new Set(
            nodes
              .filter(
                (candidate) =>
                  candidate.kind === "join" && reachableNodes.has(candidate.id),
              )
              .map(({ id: nodeId }) => nodeId),
          );
        });
        ...
        if (
          ...
          commonJoins.every(
            (joinId) => nodeById.get(joinId).control.mode !== node.control.mode,
          )
        )
    ...
    for (const edge of edges) {
      ...
      if (edge.condition === null) add("UNKNOWN_CONDITION", "edge", edge);
        ...
        edge.condition.language === "unknown" ||
        edge.evidence.basis === "unknown"
      )
        add("UNCERTAIN_ROUTE", "edge", edge);
    }
  }
  const codeOrder = new Map(
    WORKFLOW_SPEC_WARNING_CODES_V1.map((code, index) => [code, index]),
  );
  findings.sort(
    (left, right) =>
