...
const renderFlow = (flow, index, flows, hasMultipleFlows, exportControls) => {
  ...
  const breadcrumb = hasMultipleFlows
    ...
    : "";
  const minimapNodes = flow.nodes
    .map((node) => {
      const position = layout.positions.get(node.id);
      return `<rect data-minimap-node-id="${escapeHtml(node.id)}" x="${position.x}" y="${position.y}" width="${GRAPH_LAYOUT.nodeWidth}" height="${GRAPH_LAYOUT.nodeHeight}" rx="8"/>`;
    })
    .join("");
  return `<section class="sb-graph-flow" data-workflow-flow="${index}" data-flow-id="${escapeHtml(flow.id)}" data-flow-title="${escapeHtml(flow.title)}"${index === 0 ? "" : " hidden"} aria-labelledby="workflow-flow-${index}"><header>${breadcrumb}<div class="sb-graph-title"><div><p>Flow ${index + 1}</p><h2 id="workflow-flow-${index}" tabindex="-1">${escapeHtml(flow.title)}</h2></div><div class="sb-graph-view-actions" aria-label="Graph viewport controls"><button type="button" data-zoom-out aria-label="Zoom out" aria-keyshortcuts="-">−</button><output data-zoom-output>100%</output><button type="button" data-zoom-in aria-label="Zoom in" aria-keyshortcuts="+">+</button><button type="button" data-reset-zoom aria-keyshortcuts="0">100%</button><button type="button" data-fit aria-keyshortcuts="F">Fit</button><button type="button" data-focus-selected aria-keyshortcuts="S">Selected</button>${exportControls}</div></div><p class="sb-graph-shortcuts">Shortcuts: +/− zoom · 0 reset · F fit · S selected</p></header><div class="sb-graph-workspace"><div class="sb-graph-stage-wrap"><div class="sb-graph-scroll" tabindex="0" aria-label="Scrollable workflow graph; use both axes or drag with Space or the middle button"><div class="sb-graph-scale-box" style="width:${layout.width}px;height:${layout.height}px" data-base-width="${layout.width}" data-base-height="${layout.height}"><div class="sb-graph-canvas" style="width:${layout.width}px;height:${layout.height}px"><svg viewBox="0 0 ${layout.width} ${layout.height}" aria-hidden="true"><defs><marker id="workflow-arrow-${index}" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z"/></marker></defs>${edgeLines.replaceAll("url(#workflow-arrow)", `url(#workflow-arrow-${index})`)}</svg><span class="sb-graph-port sb-graph-port-entry" data-entry-port>${escapeHtml(entryContext)} · ${escapeHtml(flow.entryNodeIds.join(", "))}</span><span class="sb-graph-port sb-graph-port-exit" data-exit-port>${escapeHtml(exitContext)} · ${escapeHtml(flow.exitNodeIds.join(", "))}</span>${edgeButtons}${nodeButtons}${subflowLinks}</div></div></div><button type="button" class="sb-graph-minimap" data-minimap aria-label="Move graph viewport"><svg viewBox="0 0 ${layout.width} ${layout.height}" preserveAspectRatio="none" aria-hidden="true">${minimapNodes}</svg><span data-minimap-viewport></span></button></div><button type="button" class="sb-graph-inspector-backdrop" data-detail-backdrop hidden aria-label="Close details"></button><aside class="sb-graph-inspector" data-detail-panel hidden role="region" aria-labelledby="workflow-inspector-title-${index}"><button type="button" class="sb-graph-sheet-handle" data-sheet-handle aria-label="Drag down or activate to close details"><span></span></button><header><h2 id="workflow-inspector-title-${index}">Details</h2><button type="button" data-detail-close aria-label="Close details">Close</button></header><div data-detail-body></div></aside></div>${details}</section>`;
};
...
export const WORKFLOW_GRAPH_PROGRAM_V2 = `(()=>{
  ...
  const manual=[...root.querySelectorAll('[data-copy-manual]')];
  ...
  const select=message=>{source.focus();source.select();if(status)status.textContent=message};
  const copyFallback=message=>{
    if(source.hidden){
      source.hidden=false;
      source.classList.remove('sb-graph-export-source');
      source.classList.add('sb-graph-export-fallback')
    }
    select(message)
  };
  ...
  const closePanel=(panel,backdrop,returnFocus=true)=>{
    ...
    flow?.querySelectorAll('[data-minimap-node-id][data-selected="true"]').forEach(element=>delete element.dataset.selected);
  ...
  root.querySelectorAll('[data-workflow-flow]').forEach(flow=>{
    ...
    const minimap=flow.querySelector('[data-minimap]');
    const viewport=flow.querySelector('[data-minimap-viewport]');
    const shortcutHelp=flow.querySelector('.sb-graph-shortcuts');
    if(!panel||!body||!close||!backdrop||!scroll||!box||!canvas||!output)return;
    if(shortcutHelp)shortcutHelp.textContent='Ctrl/⌘ + wheel zoom · Touch, middle drag, or Space + drag pan · Shift+1 fit · Shift+2 selected';
    ...
    let scale=1,selected=null,spacePan=false;
    const width=Number(box.dataset.baseWidth),height=Number(box.dataset.baseHeight);
    const supportsLayoutZoom=typeof CSS!=='undefined'&&CSS.supports?.('zoom','2');
    const syncGrid=()=>{
      const step=24*scale;
      scroll.style.backgroundSize=step+'px '+step+'px';
      scroll.style.backgroundPosition=(-scroll.scrollLeft%step)+'px '+(-scroll.scrollTop%step)+'px'
    };
    const syncMini=()=>{
      if(!viewport)return;
      const x=scroll.scrollWidth<=scroll.clientWidth?0:scroll.scrollLeft/scroll.scrollWidth*100;
      const y=scroll.scrollHeight<=scroll.clientHeight?0:scroll.scrollTop/scroll.scrollHeight*100;
      Object.assign(viewport.style,{left:x+'%',top:y+'%',width:Math.min(100,scroll.clientWidth/scroll.scrollWidth*100)+'%',height:Math.min(100,scroll.clientHeight/scroll.scrollHeight*100)+'%'})
    };
    const renderTransform=()=>{
      box.style.transform='none';box.style.width=Math.round(width*scale)+'px';box.style.height=Math.round(height*scale)+'px';
      canvas.style.zoom=supportsLayoutZoom?String(scale):'';
      canvas.style.transform=supportsLayoutZoom?'none':'scale('+scale+')';
      output.textContent=Math.round(scale*100)+'%';syncGrid();syncMini()
    };
    const apply=(next,anchor=null)=>{
      const previous=scale;
      const bounded=Math.min(2,Math.max(.5,next));
      const contentX=anchor?(scroll.scrollLeft+anchor.x)/previous:0;
      const contentY=anchor?(scroll.scrollTop+anchor.y)/previous:0;
      scale=bounded;
      renderTransform();
      if(anchor)scroll.scrollTo({left:contentX*scale-anchor.x,top:contentY*scale-anchor.y})
    };
    const focusSelected=()=>{if(!selected)return;scroll.scrollTo({left:(selected.offsetLeft+selected.offsetWidth/2)*scale-scroll.clientWidth/2,top:(selected.offsetTop+selected.offsetHeight/2)*scale-scroll.clientHeight/2,behavior:'smooth'})};
    const fit=()=>{scale=Math.min(2,Math.max(.5,Math.min(scroll.clientWidth/width,scroll.clientHeight/height)));renderTransform();scroll.scrollTo({left:Math.max(0,(width*scale-scroll.clientWidth)/2),top:Math.max(0,(height*scale-scroll.clientHeight)/2)})};
    ...
    scroll.addEventListener('wheel',event=>{
      ...
      apply(scale*Math.exp(-event.deltaY*.002),{x:event.clientX-rect.left,y:event.clientY-rect.top})
    },{passive:false});
    scroll.addEventListener('scroll',()=>{syncGrid();syncMini()},{passive:true});
    minimap?.addEventListener('click',event=>{const rect=minimap.getBoundingClientRect();scroll.scrollTo({left:(event.clientX-rect.left)/rect.width*scroll.scrollWidth-scroll.clientWidth/2,top:(event.clientY-rect.top)/rect.height*scroll.scrollHeight-scroll.clientHeight/2,behavior:'smooth'})});
    let pan=null;
    ...
    scroll.addEventListener('pointermove',event=>{if(!pan||pan.id!==event.pointerId)return;scroll.scrollLeft=pan.left-(event.clientX-pan.x);scroll.scrollTop=pan.top-(event.clientY-pan.y)});
    ...
    const openDetail=button=>{
      ...
      if(elementId)flow.querySelector('[data-minimap-node-id="'+CSS.escape(elementId)+'"]')?.setAttribute('data-selected','true');
    ...
    controllers.set(index,{
      fit,
      startInitialFit,
      snapshot(){viewState.set(key,{scale,scrollLeft:scroll.scrollLeft,scrollTop:scroll.scrollTop,selectedId:selected?.getAttribute('data-element-id')??null,panelOpen:!panel.hidden})},
      restore(){const saved=viewState.get(key);if(!saved){startInitialFit();return}stopInitialFit();scale=saved.scale;renderTransform();scroll.scrollTo({left:saved.scrollLeft,top:saved.scrollTop});requestAnimationFrame(()=>{if(saved.selectedId){selected=flow.querySelector('[data-element-id="'+CSS.escape(saved.selectedId)+'"]');if(saved.panelOpen&&selected)openDetail(selected)}})}
    });
    apply(1)
  });
  if(overview)showOverview();
  else controllers.get('0')?.startInitialFit();
  manual.forEach(button=>button.addEventListener('click',()=>select('Canonical WorkflowSpec JSON selected.')));
  hostCopy.forEach(button=>button.addEventListener('click',()=>{
    const api=window.SceneBoardArtifact;
    if(!api||!api.userAction||!api.requestCapability||!api.onHostMessage){copyFallback('Clipboard copy is unavailable.');return}
    ...
    unsubscribe=api.onHostMessage(message=>{
      ...
      if(message.ok){if(status)status.textContent='Canonical WorkflowSpec JSON copied.'}else copyFallback('Clipboard copy was denied or unavailable.')
    });
    api.userAction(id,'clipboard.write');api.requestCapability(id,'clipboard.write',{text:source.value});
    timer=setTimeout(()=>{if(pending===id){pending=null;if(unsubscribe)unsubscribe();unsubscribe=null;copyFallback('No clipboard result arrived.')}},5500)
  }));
...
export const renderWorkflowGraph = (content, title, fallbackText) => {
  ...
  const copyButton =
    content.copyMode === "manual"
      ? '<button type="button" data-copy-manual>클립보드 복사</button>'
      : '<button type="button" data-copy-host>클립보드 복사</button>';
  const exportControls = copyButton;
  const exportSource =
    content.copyMode === "manual"
      ? `<section class="sb-graph-export"><h2>WorkflowSpec export</h2><textarea readonly data-workflow-json aria-label="Canonical WorkflowSpec JSON">${escapeHtml(canonical)}</textarea></section>`
      : `<textarea readonly hidden class="sb-graph-export-source" data-workflow-json aria-label="Canonical WorkflowSpec JSON">${escapeHtml(canonical)}</textarea>`;
  return {
    artifactId: null,
