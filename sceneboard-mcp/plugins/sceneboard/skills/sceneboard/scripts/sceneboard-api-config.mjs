...
const PROJECT_ENV_KEYS_WITH_MODE = [
  ...PROJECT_ENV_KEYS,
...
export const resolveApiConfig = async ({
  ...
  try {
    ...
    const server = isRecord(parsed?.mcpServers)
      ? parsed.mcpServers.sceneboard
      : undefined;
    ...
    if (selectedEnvironment) {
      ...
      const hasCredentialModeTuple = hasExactKeys(
        server.env,
        PROJECT_ENV_KEYS_WITH_MODE,
      );
      ...
      credentialMode = hasCredentialModeTuple
        ? server.env.BOARD_CREDENTIAL_MODE
        : "pairing";
      if (
      ...
      if (
        server !== undefined &&
        (!isRecord(server) || !isRecord(server.env))
      ) {
        throw new SceneBoardApiError(
          "BOARD_API_CONFIG_INVALID",
          "Project SceneBoard configuration is invalid",
...
export const readCredential = async (config) => {
  ...
  try {
    ...
    if (isWindows(config.platform)) {
      ...
      let accessToken;
      try {
        accessToken = await config.windowsDataProtection.unprotect(
...
export const writeCredential = async (config, accessToken) => {
  ...
  if (isWindows(config.platform)) {
    ...
    try {
      protectedAccessToken =
        await config.windowsDataProtection.protect(accessToken);
    } catch (error) {
      if (error instanceof SceneBoardApiError) throw error;
      throw new SceneBoardApiError(
        "BOARD_API_CREDENTIAL_UNAVAILABLE",
