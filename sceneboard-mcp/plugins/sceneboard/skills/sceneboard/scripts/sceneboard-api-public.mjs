...
export const sanitizePublicValue = (
  ...
  if (Array.isArray(value))
    return value
      .slice(0, 100)
      .map((item) => sanitizePublicValue(item, depth + 1, budget));
...
export const validGlobalId = (value) =>
  typeof value === "string" && GLOBAL_ID_PATTERN.test(value);
export const validLocalId = (value) =>
  typeof value === "string" && LOCAL_ID_PATTERN.test(value);
const validPublicString = (value) =>
  typeof value === "string" && !containsSecretValue(value);
const validErrorPath = (value) =>
  Array.isArray(value) &&
  value.every(
    (part) => validPublicString(part) || (Number.isInteger(part) && part >= 0),
  );
...
const projectBoardErrorDetails = (code, value) => {
  ...
  if (code === "REVISION_NOT_FOUND")
    return hasExactKeys(value, ["revisionId"]) &&
      validGlobalId(value.revisionId)
      ? { revisionId: value.revisionId }
      : undefined;
  if (code === "ARTIFACT_NOT_FOUND")
    return hasExactKeys(value, ["artifact"]) &&
      validArtifactReference(value.artifact)
  ...
  if (
    ...
    return hasExactKeys(value, ["hitlRequestId"]) &&
  ...
  if (code === "IDEMPOTENCY_KEY_REUSED") {
    if (!hasExactKeys(value, ["scope", "boardId", "operationType", "reason"]))
      return undefined;
    const variants = {
      "board.mutation": {
        boardRequired: true,
...
export const errorFromResponse = (
  ...
  if (pairing) {
    ...
    const requiresRetryAfter = ["PAIRING_NOT_READY", "RATE_LIMITED"].includes(
      error.code,
    );
    if (
    ...
    return new SceneBoardApiError(
      error.code,
      `SceneBoard pairing failed: ${error.code}`,
      {
  ...
  return new SceneBoardApiError(
    error.code,
    `SceneBoard request failed: ${error.code}`,
    {
...
export const parseCapabilities = (value) => {
  if (
    ...
    fullCatalog(
      value.supported.artifactRequestCapabilities,
      ARTIFACT_CAPABILITIES,
    ) === null ||
    !hasExactKeys(value.limits, Object.keys(BOARD_LIMITS)) ||
    Object.entries(BOARD_LIMITS).some(
      ([key, expected]) => value.limits[key] !== expected,
    )
  )
    return null;
  const grantedCapabilities = exactCatalog(
  ...
  if (
    ...
    allowedArtifactRequestCapabilities === null
  )
    return null;
  return {
    protocolVersion: 1,
...
const parsePairingConnection = (value, boardId) => {
  if (
    ...
    !hasExactKeys(value.grant.client, [
      "clientId",
      "clientName",
  ...
  if (boardId !== null) {
    ...
    if (
      ...
      !hasExactKeys(selected.board.headRevision, [
        "revisionId",
        "revisionNumber",
        "createdAt",
      ]) ||
      ...
      (selected.board.archivedAt !== null &&
        !validTimestamp(selected.board.archivedAt)) ||
