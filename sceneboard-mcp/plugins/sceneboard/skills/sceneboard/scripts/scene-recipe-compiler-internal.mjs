...
const copy = (value) => {
  ...
  if (object(value))
    return Object.fromEntries(
      Object.keys(value).map((key) => [key, copy(value[key])]),
    );
  return value;
};
...
export const createSceneRecipeCompiler = ({
  digest,
  slugify,
  canonicalPath,
  ErrorType,
  ...
  const fail = (code, path = []) => {
    throw new ErrorType(code, path);
  };

  const requireValue = (condition, path) => {
    if (!condition) fail("INVALID_VALUE", path);
  };
  ...
  const common = (block, fields, path) => {
    ...
    if (block.key !== undefined)
      requireValue(isKeyScalar(block.key), [...path, "key"]);
    if (block.title !== undefined)
      requireValue(isShortText(block.title), [...path, "title"]);
  };

  const validateFriendly = (block, path, keys) => {
    if (!object(block)) fail("INVALID_RECIPE", path);
    if (!KINDS.includes(block.kind))
      fail("UNKNOWN_BLOCK_KIND", [...path, "kind"]);
    ...
    const registerKey = () => {
      ...
      if (keys.has(block.key))
        fail("DUPLICATE_SEMANTIC_IDENTITY", [...path, "key"]);
      keys.add(block.key);
    };
    if (block.kind === "presentation") {
      common(block, ["activePageKey", "pages"], path);
      registerKey();
      requireValue(isKeyScalar(block.activePageKey), [
        ...path,
        "activePageKey",
      ]);
      ...
      block.pages.forEach((page, index) => {
        ...
        if (pageKeys.has(page.key))
          fail("DUPLICATE_SEMANTIC_IDENTITY", [...pagePath, "key"]);
      ...
      if (!pageKeys.has(block.activePageKey))
        fail("INVALID_LAYOUT", [...path, "activePageKey"]);
      return;
    }
    if (block.kind === "dashboard") {
      common(block, ["columns", "rows", "gap", "items"], path);
      registerKey();
      requireValue(
        isInteger(block.columns) &&
          block.columns >= 1 &&
          block.columns <= LIMITS_V1.maxGridColumns,
        [...path, "columns"],
      );
      requireValue(
        isInteger(block.rows) &&
          block.rows >= 1 &&
          block.rows <= LIMITS_V1.maxGridRows,
        [...path, "rows"],
      );
      requireValue(isFiniteNumber(block.gap) && block.gap >= 0, [
        ...path,
        "gap",
      ]);
      ...
      block.items.forEach((item, index) => {
        ...
        closed(
          item,
          ["content", "column", "row", "columnSpan", "rowSpan"],
          fail,
          itemPath,
        );
        for (const field of ["column", "row", "columnSpan", "rowSpan"])
          requireValue(isInteger(item[field]) && item[field] >= 1, [
            ...itemPath,
            field,
          ]);
    ...
    if (block.kind === "architecture") {
      common(block, ["width", "height", "items"], path);
      registerKey();
      requireValue(
        isFiniteNumber(block.width) &&
          block.width > 0 &&
          block.width <= LIMITS_V1.maxCanvasExtent,
        [...path, "width"],
      );
      requireValue(
        isFiniteNumber(block.height) &&
          block.height > 0 &&
          block.height <= LIMITS_V1.maxCanvasExtent,
        [...path, "height"],
      );
      requireArray(block.items, 1, LIMITS_V1.maxCanvasItems, [
        ...path,
        "items",
      ]);
      block.items.forEach((item, index) => {
        ...
        closed(
          item,
          ["content", "x", "y", "width", "height", "zIndex"],
          fail,
          itemPath,
        );
        requireValue(isFiniteNumber(item.x) && item.x >= 0, [...itemPath, "x"]);
        requireValue(isFiniteNumber(item.y) && item.y >= 0, [...itemPath, "y"]);
        requireValue(isFiniteNumber(item.width) && item.width > 0, [
          ...itemPath,
          "width",
        ]);
        requireValue(isFiniteNumber(item.height) && item.height > 0, [
          ...itemPath,
          "height",
        ]);
        requireValue(Number.isSafeInteger(item.zIndex), [
          ...itemPath,
          "zIndex",
        ]);
        validateFriendly(item.content, [...itemPath, "content"], keys);
    ...
    const fields = {
      markdown: ["markdown"],
      code: ["language", "code", "showLineNumbers", "wrap"],
      table: ["columns", "rows"],
      chart: ["chartType", "xAxis", "yAxis", "series"],
      map: ["viewport", "data"],
      drawing: ["viewBox", "elements"],
      status: ["status", "label", "detail"],
      progress: ["state", "value", "label", "detail"],
    }[block.kind];
    common(block, fields, path);
    registerKey();
    const probe = { id: "n_probe_000000000000", type: nodeTypeFor(block.kind) };
    if (block.title !== undefined) probe.title = block.title;
    for (const field of fields)
      if (own(block, field)) probe[field] = copy(block[field]);
  ...
  const validate = (input) => {
    ...
    if (input.recipeVersion !== 1)
      fail("UNSUPPORTED_RECIPE_VERSION", ["recipeVersion"]);
  ...
  const compile = (input) => {
    ...
    const registerNode = (id, path, identity = null) => {
      if (ids.has(id))
        fail(
          identity && generated.has(id) && generated.get(id) !== identity
            ? "NODE_ID_COLLISION"
            : "DUPLICATE_NODE_ID",
          [...path, "id"],
        );
      ids.set(id, true);
      if (identity) generated.set(id, identity);
    };
    const derive = (path, type, key) => {
      const identity = `${canonicalPath(path)}\n${type}\n${key ?? ""}`;
      const id = `n_${slugify(key ?? type)}_${digest(`recipe-v1\n${identity}`).slice(0, 12)}`;
      registerNode(id, path, identity);
      return id;
    };
    const tabId = (path, key) =>
      `t_${slugify(key)}_${digest(`recipe-v1\n${canonicalPath(path)}\ntab\n${key}`).slice(0, 12)}`;
    const registerExact = (node, path) => {
      ...
      if (
        ...
        node.children.forEach((child, index) =>
          registerExact(child.node, [...path, "children", index, "node"]),
        );
      if (node.type === "layout.tabs")
        node.tabs.forEach((tab, index) =>
          registerExact(tab.node, [...path, "tabs", index, "node"]),
        );
    };
    const walk = (block, path) => {
      if (block.kind === "exact-node") {
        const node = copy(block.node);
        registerExact(node, [...path, "node"]);
        return node;
      }
      ...
      if (block.kind === "presentation") {
        node.tabs = block.pages.map((page, index) => ({
          tabId: tabId([...path, "pages", index], page.key),
          label: page.label,
          node: walk(page.content, [...path, "pages", index, "content"]),
        }));
        const activeIndex = block.pages.findIndex(
          (page) => page.key === block.activePageKey,
        );
        ...
        Object.assign(node, {
          ...
          children: block.items.map((item, index) => ({
            node: walk(item.content, [...path, "items", index, "content"]),
            column: item.column,
            row: item.row,
        ...
        for (const key of Object.keys(block))
          if (!skip.has(key)) node[key] = copy(block[key]);
      }
      return node;
    };
  ...
  const replaceInput = (input, binding) => {
    closed(
      binding,
      ["boardId", "expectedRevisionId", "idempotencyKey"],
      fail,
      [],
    );
    requireValue(isGlobalId(binding.boardId), ["boardId"]);
    requireValue(isGlobalId(binding.expectedRevisionId), [
      "expectedRevisionId",
    ]);
