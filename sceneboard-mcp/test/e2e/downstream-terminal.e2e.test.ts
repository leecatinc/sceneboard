...
test(
  'terminal MCP drives artifact and blocking-first HITL flows through the real SDK HTTP boundary',
  { timeout: 5_000 },
  async () => {
    const definition = {
      kind: 'info' as const,
    ...
    const answerRecorded = new Promise<void>((resolve) => {
      answerRecordedResolve = resolve;
    });
    const requests: Array<{ method: string; path: string }> = [];

    const success = (requestId: string, result: Record<string, unknown>): Response =>
      new Response(
        JSON.stringify({
          protocolVersion: 1,
    ...
    const fetchFixture: typeof fetch = async (input, init) => {
      ...
      const body =
        init?.body === undefined
          ? null
          : (JSON.parse(new TextDecoder().decode(init.body as Uint8Array)) as Record<
              string,
              unknown
            >);
      const requestId =
      ...
      if (
        method === 'GET' &&
        url.pathname === '/api/v1/boards/board_1/artifacts/artifact_1/versions/version_1'
      ) {
        return success(requestId, {
          protocolVersion: 1,
    ...
    try {
      ...
      assert.equal(
        (structured(artifactStop).result.result.artifact as { status: string }).status,
        'stopped',
      );
      ...
      assert.equal(
        serializedOutputs.includes('<main>SceneBoard terminal acceptance</main>'),
        false,
      );
