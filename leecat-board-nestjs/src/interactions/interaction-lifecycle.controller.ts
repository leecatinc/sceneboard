...
import type { BoardId, HitlRequestId } from '@leecat-board/board-schema';
...
export class InteractionLifecycleController {
  ...
  async cancel(
    ...
    const requestId = admitBoardRequestId(
      request, typeof candidate?.requestId === 'string' ? candidate.requestId : undefined,
    );
    const parsed = parseHitlLifecycleBody({
      body, requestId, boardId, hitlRequestId, action: 'cancel',
    });
    return this.lifecycle.cancel({
      principal: principal(request, this.actors), boardId, hitlRequestId, request: parsed,
    });
  }
  ...
  async supersede(
    ...
    const parsed = parseHitlLifecycleBody({
      body, requestId, boardId, hitlRequestId, action: 'supersede',
