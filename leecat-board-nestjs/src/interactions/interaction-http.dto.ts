...
export const parseHitlReadQuery = (input: {
  ...
  const timeoutMs = hasTimeout && /^[0-9]{1,5}$/u.test(query.timeoutMs as string)
  ...
  if (!parsed.ok || parsed.data.value.type !== 'hitl.read') {
    throw new BoardContractError(parsed.ok ? invalidBoardPayload('invalid hitl.read query') : parsed.error);
  }
...
export const parseHitlLifecycleBody = (input: {
  ...
  const allowed = input.action === 'cancel'
    ...
    : [
      'protocolVersion', 'requestId', 'expectedRevisionId',
      'expectedStateUpdatedAt', 'successorHitlRequestId',
    ];
  ...
  if (!cursor.ok || cursor.data.value.type !== 'hitl.read' || cursor.data.value.wait === null) {
    throw new BoardContractError(cursor.ok ? invalidBoardPayload('invalid lifecycle cursor') : cursor.error);
  }
  ...
  if (!successor.ok || successor.data.value.type !== 'hitl.read') {
    throw new BoardContractError(successor.ok ? invalidBoardPayload('invalid successor') : successor.error);
  }
