...
const definitionKind = (kind: HitlRequestDefinitionV1['kind']): string => ({
  info: 'I', choice: 'H', form: 'F', confirmation: 'C',
})[kind];
...
const actorShape = (kind: string | null, principalId: string | null, grantId: string | null): boolean => (
...
export const mapInteractionRowV1 = (row: InteractionRowV1): StoredInteractionV1 => {
  ...
  if (row.definitionCanonicalBytes !== row.definitionPayload.byteLength
  ...
  if (!parsedDefinition.ok || definitionKind(parsedDefinition.data.value.kind) !== row.definitionKind) {
    throw new BoardPersistenceError('row_integrity');
  }
  let response: HitlResponseV1 | null = null;
  if (row.responsePayload === null) {
    if (row.responseKind !== null || row.responseCanonicalBytes !== null || row.responseSha256 !== null) {
      throw new BoardPersistenceError('row_integrity');
    }
  } else {
    if (row.responseCanonicalBytes !== row.responsePayload.byteLength || row.responseSha256 === null
      || row.responseSha256.byteLength !== 32 || !equal(row.responseSha256, digest(row.responsePayload))) {
      throw new BoardPersistenceError('row_integrity');
    }
  ...
  if (stateEventSequence < createdEventSequence || !actorShape(
    row.createdByKind, row.createdByPrincipalId, row.createdByGrantId,
  )) throw new BoardPersistenceError('row_integrity');
  if (currentState === 'open') {
    if (stateEventSequence !== createdEventSequence || row.answeredByKind !== null
      || row.answeredByPrincipalId !== null || row.answeredByGrantId !== null
      || row.terminalByKind !== null || row.terminalByPrincipalId !== null
      || row.terminalByGrantId !== null || row.answeredRequestId !== null) {
      throw new BoardPersistenceError('row_integrity');
    }
  } else if (currentState === 'answered') {
    if (!actorShape(row.answeredByKind, row.answeredByPrincipalId, row.answeredByGrantId)
      || row.terminalByKind !== null || row.terminalByPrincipalId !== null || row.terminalByGrantId !== null
      || row.answeredRequestId === null || stateEventSequence <= createdEventSequence) {
      throw new BoardPersistenceError('row_integrity');
    }
  } else if (currentState === 'expired') {
    if (row.terminalByKind !== 'S' || row.terminalByPrincipalId !== 'hitl-expiry-v1'
      || row.terminalByGrantId !== null || row.answeredByKind !== null
      || row.answeredByPrincipalId !== null || row.answeredByGrantId !== null
      || row.answeredRequestId !== null || stateEventSequence <= createdEventSequence
    ...
    if (row.answeredByKind !== null || row.answeredByPrincipalId !== null
      || row.answeredByGrantId !== null || row.answeredRequestId !== null
      || stateEventSequence <= createdEventSequence) throw new BoardPersistenceError('row_integrity');
  }
