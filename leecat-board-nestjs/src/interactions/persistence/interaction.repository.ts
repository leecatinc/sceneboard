...
export class InteractionRepository {
  ...
  async create(connection: PoolConnection, input: {
  ...
  async readByPublicId(
    ...
    if (rows.length !== 1 || rows[0] === undefined) throw new BoardPersistenceError('row_integrity');
  ...
  async answer(connection: PoolConnection, input: {
  ...
  async expire(connection: PoolConnection, input: {
    stored: StoredInteractionV1;
    sequence: number;
  }): Promise<void> {
    const [result] = await connection.execute<ResultSetHeader>(`
  ...
  async cancel(connection: PoolConnection, input: {
    stored: StoredInteractionV1;
    context: AuthorizedBoardContextV1;
    sequence: number;
    updatedAtSql: string;
  ...
  async supersede(connection: PoolConnection, input: {
    stored: StoredInteractionV1;
