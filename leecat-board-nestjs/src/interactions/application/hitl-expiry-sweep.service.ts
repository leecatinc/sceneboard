...
export class HitlExpirySweepService implements OnApplicationBootstrap, OnModuleDestroy {
  ...
  async runOnce(): Promise<number> {
    ...
    for (const candidate of candidates) {
      ...
      const changed = await this.mysql.withConnection((connection) => withTransaction(
        connection,
        'READ COMMITTED',
        async () => {
          ...
          const stored = await this.interactions.lockByBoardPk(
            connection, head.boardPk, candidate.hitlRequestId,
          );
          if (stored === null || stored.interaction.state !== 'open'
            || Date.parse(stored.interaction.expiresAt as TimestampV1) > now.valueOf()) return false;
