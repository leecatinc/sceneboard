...
export class InteractionQueryService extends HitlQueryApplicationPortV1 {
  ...
  async read(
    ...
    if (current.interaction.stateUpdatedAt > cursor) return result(request, current.interaction, true);
    ...
    while (true) {
      ...
      current = await this.expireIfDue(principal, request, current);
      if (current.interaction.stateUpdatedAt > cursor) return result(request, current.interaction, true);
      const expiryDelay = current.interaction.state === 'open'
  ...
  private async probe(
    ...
    return this.accessPolicy.withAuthorizedBoardTransaction({
      ...
      const stored = await this.interactions.readByPublicId(
        connection, request.boardId, request.hitlRequestId,
      );
  ...
  private async expireIfDue(
    ...
    if (stored.interaction.state !== 'open'
      || this.clock() < Date.parse(stored.interaction.expiresAt as TimestampV1)) return stored;
