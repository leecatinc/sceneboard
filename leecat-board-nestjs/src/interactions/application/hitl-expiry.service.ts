...
export class HitlExpiryService {
  ...
  async expireForAuthorizedRead(input: {
    ...
    const interaction = await this.accessPolicy.withAuthorizedBoardTransaction({
      ...
      const stored = await this.interactions.lockByBoardPk(connection, head.boardPk, input.hitlRequestId);
      if (stored === null) throw hitlNotFound(input.hitlRequestId);
      if (stored.interaction.state !== 'open'
  ...
  async expireLocked(
    ...
    if (input.stored.interaction.state !== 'open'
      || !Number.isFinite(input.now.valueOf())
      || input.now.valueOf() < expiresAt
      || Date.parse(input.occurredAt) < expiresAt) throw internalHitlFailure();
    const sequence = await this.mutations.allocateSequenceAt(
      connection, input.head, formatMysqlTimestampUtc(input.now),
    );
    ...
    await this.mutations.appendEvent(connection, input.head, hitlUpdatedEvent({
      boardId: input.boardId,
