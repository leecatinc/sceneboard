...
export const formatPublicUuidV4 = (bytes: Uint8Array): string => {
  if (bytes.byteLength !== 16) throw new TypeError('UUID storage value must contain exactly 16 bytes');
  const hex = Buffer.from(bytes).toString('hex');
  const value = `${hex.slice(0, 8)}-${hex.slice(8, 12)}-${hex.slice(12, 16)}-${hex.slice(16, 20)}-${hex.slice(20)}`;
  if (!UUID_V4_PATTERN.test(value)) throw new TypeError('UUID storage value is not a canonical UUIDv4');
  return value;
};
