...
export type ForwardingState = 'direct' | 'ignored_untrusted_peer' | 'trusted_chain' | 'malformed_fallback';
...
const parseIp = (address: string): Uint8Array | null => parseIpv4(normalizeAddress(address) ?? '') ?? parseIpv6(address);
...
const cidrContains = (cidr: string, address: string): boolean => {
  ...
  if (!network || !candidate || network.byteLength !== candidate.byteLength || !/^(?:0|[1-9][0-9]*)$/.test(prefixSource)) return false;
  const prefix = Number(prefixSource);
  if (prefix > network.byteLength * 8) return false;
  const fullBytes = Math.floor(prefix / 8);
  const remainder = prefix % 8;
  for (let index = 0; index < fullBytes; index += 1) if (network[index] !== candidate[index]) return false;
  if (remainder === 0) return true;
  const mask = (0xff << (8 - remainder)) & 0xff;
  return (network[fullBytes]! & mask) === (candidate[fullBytes]! & mask);
};

const isTrusted = (address: string, cidrs: readonly string[]): boolean => cidrs.some((cidr) => cidrContains(cidr, address));
