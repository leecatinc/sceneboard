...
export const RequireCsrf = (kind: RequiredCsrfKind): MethodDecorator => SetMetadata(CSRF_KIND, kind);
...
export const assertOriginAndCsrf = (input: OriginCsrfInput): void => {
  ...
  const valid = input.requiredKind === 'anonymous'
    ? input.csrf.verify(input.csrfCookie, { kind: 'anonymous', now: input.now })
    : input.familyPublicId !== undefined && input.csrf.verify(input.csrfCookie, {
      kind: 'session',
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    try {
      ...
      throw new BoardContractError({
        protocolVersion: 1, type: 'board.error', code: 'FORBIDDEN', message: 'Forbidden',
        category: 'auth', retryable: false, httpStatusHint: 403, details: null,
      });
    }
    return true;
  }
