...
interface ProfiledRequest extends AsyncIterable<Uint8Array | string>, BoardRequestCorrelationCarrier {
...
export class StrictJsonBodyMiddleware implements NestMiddleware {
  async use(request: ProfiledRequest, _response: unknown, next: Next): Promise<void> {
    try {
      ...
      const declaredLength = contentLength !== undefined && /^(?:0|[1-9][0-9]*)$/.test(contentLength)
        ? Number(contentLength)
        : null;
      const maximumBytes = maximumReadBytes(profile);
      let body: Buffer;
      try {
        body = declaredLength !== null && declaredLength >= maximumBytes
          ? Buffer.alloc(0)
          : await readBody(request, maximumBytes);
      } catch (error) {
        if (profile.kind === 'd7-artifact-network-body'
          && error instanceof AppError && error.code === 'INVALID_PAYLOAD') {
      ...
      else if (parsed.kind === 'd1-contract-body' || parsed.kind === 'd1-adapter-body'
        || parsed.kind === 'd7-artifact-source-body' || parsed.kind === 'd7-artifact-network-body') {
