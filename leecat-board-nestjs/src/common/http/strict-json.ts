...
class StrictJsonParser {
  ...
  private parseLiteral<Value>(literal: string, value: Value): Value {
    if (!this.source.startsWith(literal, this.index)) throw new StrictJsonError('invalid_json', this.index);
    this.index += literal.length;
    return value;
  }

  private skipWhitespace(): void {
    while (this.index < this.source.length && JSON_WHITESPACE.has(this.source[this.index] ?? '')) this.index += 1;
  }
}
...
export const parseStrictJsonBytes = (bytes: Uint8Array, options: StrictJsonOptions = {}): unknown => {
  ...
  if (!Number.isSafeInteger(maximumDepth) || maximumDepth < 1) throw new TypeError('maximumDepth must be positive');
