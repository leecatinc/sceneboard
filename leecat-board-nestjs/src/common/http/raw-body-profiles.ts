...
const d7ArtifactSourceRoutes = [
  '/api/v1/boards/:boardId/artifacts',
] as const;
...
export const RAW_BODY_PROFILES: readonly RawBodyProfile[] = [
  ...d2BodyRoutes.map((pathTemplate): RawBodyProfile => ({
    kind: 'd2-rest-json-body',
    method: 'POST',
    pathTemplate,
  })),
  ...d2NoBodyRoutes.map(([method, pathTemplate]): RawBodyProfile => ({
    kind: 'd2-no-body',
    method,
    pathTemplate,
  })),
  ...d1Routes.map(([method, pathTemplate, d1Parser]): RawBodyProfile => ({
    kind: 'd1-contract-body',
    method,
    pathTemplate,
    d1Parser,
  })),
  ...d1AdapterBodyRoutes.map((pathTemplate): RawBodyProfile => ({
    kind: 'd1-adapter-body',
    method: 'POST',
    pathTemplate,
  })),
  ...d7ArtifactSourceRoutes.map((pathTemplate): RawBodyProfile => ({
    kind: 'd7-artifact-source-body',
    method: 'POST',
    pathTemplate,
  })),
  ...d7ArtifactNetworkRoutes.map((pathTemplate): RawBodyProfile => ({
    kind: 'd7-artifact-network-body',
    method: 'POST',
    pathTemplate,
  })),
  ...d1NoBodyRoutes.map(([method, pathTemplate]): RawBodyProfile => ({
    kind: 'd1-no-body',
    method,
    pathTemplate,
  })),
...
const templateMatches = (template: string, pathname: string): boolean => {
  ...
  return expected.every((segment, index) => segment.startsWith(':')
...
export const matchRawBodyProfile = (method: string, pathname: string): RawBodyProfile | null => {
  ...
  return RAW_BODY_PROFILES.find((profile) => profile.method === normalizedMethod && templateMatches(profile.pathTemplate, pathname)) ?? null;
};
...
const assertJsonContentType = (profile: RawBodyProfile, value: string | undefined): void => {
  if (value !== undefined && value.toLowerCase() === 'application/json' && value === value.trim()) return;
  if (profile.kind === 'd7-artifact-network-body') {
    throw new ArtifactBrokerError('INVALID_REQUEST', randomBytes(16).toString('base64url'));
  }
  if (profile.kind === 'd1-contract-body' || profile.kind === 'd1-adapter-body'
...
export const parseProfiledBody = (profile: RawBodyProfile, input: ProfiledBodyInput): ProfiledBodyResult => {
  ...
  if (profile.kind === 'd2-no-body' || profile.kind === 'd1-no-body') {
    if ((declaredLength ?? input.body.byteLength) !== 0 || input.body.byteLength !== 0) {
      if (profile.kind === 'd1-no-body') throw new BoardContractError(invalidBoardPayload('request body is not allowed'));
      throw new AppError('INVALID_PAYLOAD');
    }
  ...
  const maximumBytes = profile.kind === 'd7-artifact-source-body'
    ? 11_534_336
    : profile.kind === 'd7-artifact-network-body'
      ? 8_192
    : isD1Body ? BOARD_LIMITS_V1.maxEnvelopeBytes : 65_536;
  if (declaredLength !== null && declaredLength > maximumBytes) {
    if (profile.kind === 'd7-artifact-network-body') {
      throw new ArtifactBrokerError('INVALID_REQUEST', randomBytes(16).toString('base64url'));
    }
    if (isD1Body || profile.kind === 'd7-artifact-source-body') {
      throw new BoardContractError(profile.kind === 'd7-artifact-source-body'
  ...
  if (profile.kind === 'd1-adapter-body' || profile.kind === 'd7-artifact-source-body'
    || profile.kind === 'd7-artifact-network-body') {
    try {
      return {
        kind: profile.kind,
      ...
      throw new BoardContractError(invalidBoardPayload(
...
for (const profile of RAW_BODY_PROFILES) {
  if (profile.kind === 'd1-contract-body' && !profile.d1Parser) throw new Error(`D1 profile ${routeKey(profile)} has no parser`);
}
