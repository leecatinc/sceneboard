...
import type { PageCursorV1 } from '@leecat-board/board-schema';
...
import {
  type CursorMacKeyV1,
  cursorHmacSha256V1,
} from '../security/cursor-mac-key.js';
...
export const encodeSignedCursorV1 = (
  ...
  const cursor = encodeBase64Url(Buffer.concat([
    Buffer.from(canonicalPayload),
    cursorHmacSha256V1(key, CURSOR_DOMAIN, canonicalPayload),
  ]));
  if (cursor.length < 1 || cursor.length > MAX_CURSOR_CHARS) throw new Error('cursor exceeds its wire limit');
  return cursor as PageCursorV1;
};

export const decodeSignedCursorV1 = (key: CursorMacKeyV1, cursor: string): {
  payload: Buffer;
  decoded: Record<string, unknown>;
} => {
  try {
    if (cursor.length < 1 || cursor.length > MAX_CURSOR_CHARS) throw new Error('invalid cursor length');
