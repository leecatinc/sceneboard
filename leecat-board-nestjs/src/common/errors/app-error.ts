...
export const D2_ERROR_CATALOG = {
  ...
  AUTH_PASSWORD_UNCHANGED: { status: 409, message: 'New password must differ from the current password' },
  AUTH_EMAIL_IN_USE: { status: 409, message: 'Email is already in use' },
  AUTH_EMAIL_VERIFICATION_INVALID: { status: 400, message: 'Email verification code is invalid or expired' },
...
export class AppError extends Error {
  ...
  constructor(code: D2ErrorCode, options: { retryAfterSeconds?: number | null; cause?: unknown } = {}) {
