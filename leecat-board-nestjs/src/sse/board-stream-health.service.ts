...
@Injectable()
export class BoardStreamHealthService implements BoardStreamOperationalHealthPortV1, OnApplicationBootstrap, OnModuleDestroy {
  #health = INITIAL;
  #degradeSamples = 0;
  #recoverSamples = 0;
  ...
  constructor(
    @Inject(BOARD_EVENT_OUTBOX_HEALTH_PORT_V1) private readonly outbox: BoardEventOutboxHealthPortV1,
  ...
  async sampleOnce(): Promise<BoardStreamOperationalHealthV1> {
    try {
      ...
      const dependenciesHealthy = commandReady && subscriberReady && !outbox.quarantinedCorruptPending;
