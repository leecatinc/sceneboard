...
import type { BoardEventDeliveryPortV1, DeliverableBoardEventV1 } from '../events/ports/board-event-delivery.port.js';
...
export class BoardStreamCutService {
  ...
  async prepare(
    ...
    if (cursor.b !== boardId || !this.cursors.isTimeUsable(cursor) || cursor.s > watermark
      || watermark - cursor.s > 1_000) return this.#snapshotCut(snapshot);
    if (cursor.k === 'event') {
      const correlated = await this.events.getEvent(boardId, cursor.s);
      if (correlated === null || correlated.eventId !== cursor.e) return this.#snapshotCut(snapshot);
    }
  ...
  async rangeAfter(boardId: BoardId, afterSequence: number, headSequence: number): Promise<PreparedBoardStreamFrameV1[] | null> {
  ...
  eventFrame(event: DeliverableBoardEventV1): PreparedBoardStreamFrameV1 {
    ...
    return {
      ...
      cursor: this.cursors.encode({
        v: 1, k: 'event', b: event.boardId, s: event.sequence, e: event.eventId, t: issuedAt,
      }),
    };
  }

  async #authorizedSnapshot(principal: ResolvedBoardPrincipalV1, boardId: BoardId): Promise<BoardSnapshotV1> {
  ...
  #snapshotCut(snapshot: BoardSnapshotV1): PreparedBoardStreamCutV1 {
    ...
    return {
      ...
      frames: [{
        ...
        cursor: this.cursors.encode({
          v: 1, k: 'snapshot', b: snapshot.boardId, s: snapshot.lastEventSequence, e: eventId, t: occurredAt,
        }),
      }],
    };
  }

  async #range(boardId: BoardId, after: number, head: number): Promise<DeliverableBoardEventV1[] | null> {
    const output: DeliverableBoardEventV1[] = [];
    let sequence = after;
    while (sequence < head) {
      const rows = await this.events.listContiguousEvents(boardId, sequence, Math.min(100, head - sequence));
