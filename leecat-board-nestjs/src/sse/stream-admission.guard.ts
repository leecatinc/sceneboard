...
import { BoardIdParserV1, type BoardId, type TabId } from '@leecat-board/board-schema';
...
export const RequireBrowserBoardStream = (): MethodDecorator => SetMetadata(BROWSER_BOARD_STREAM, true);
...
const scalarHeader = (headers: BrowserBoardStreamRequestV1['headers'], name: string): string | undefined => {
  const value = headers[name];
  if (Array.isArray(value)) throw invalid(`${name} must be a singleton header`);
  return value;
};

const includesEventStream = (accept: string): boolean => accept.split(',').some((part) => (
  part.trim().split(';', 1)[0]?.trim().toLowerCase() === 'text/event-stream'
));
...
export class StreamAdmissionGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (accept === undefined || !includesEventStream(accept)) throw invalid('Accept must include text/event-stream');
    if (request.headers.authorization !== undefined) throw invalid('Authorization is forbidden for browser streams');
    const contentLength = scalarHeader(request.headers, 'content-length');
    if (contentLength !== undefined && contentLength !== '0') throw invalid('board stream body is forbidden');
    if (request.headers['transfer-encoding'] !== undefined) throw invalid('board stream transfer body is forbidden');
    ...
    if (keys.join(',') !== 'presenceState,tabId') throw invalid('query must contain only tabId and presenceState');
    if (typeof query.tabId !== 'string' || !TAB_ID.test(query.tabId)) throw invalid('invalid tabId', ['tabId']);
    ...
    if (Array.isArray(cursorValue) || (cursorValue !== undefined && !VISIBLE_CURSOR.test(cursorValue))) {
