...
import { RedisPresenceRepository, type RedisPresenceHandleV1 } from '../presence/redis-presence.repository.js';
...
export class BoardSseService {
  ...
  async stream(input: {
    ...
    const onRequestClose = (): void => { stopped = true; signalWake(); };
    ...
    const drain = async (): Promise<void> => {
      ...
      for (const frame of frames) {
        if (stopped) return;
        await this.writer.write(input.response, this.writer.encodeEvent(frame.canonicalBytes, frame.cursor));
    ...
    const writePresence = async (): Promise<void> => {
      ...
      await this.writer.write(input.response, this.writer.encodeEvent(parsed.data.canonicalBytes, null));
    ...
    try {
      ...
      if (postCutHead > sequence) {
        const catchUp = postCutHead - sequence <= 1_000
      ...
      if (input.principal.kind !== 'user') throw new Error('browser presence requires a user principal');
      ...
      if (input.response.httpVersionMajor === 1) input.response.setHeader('Connection', 'keep-alive');
      ...
      while (!stopped && Date.now() - startedAt < 30_000) {
        ...
        if (presenceHandle !== null && now - lastPresenceTouchAt >= 10_000) {
          if (!await this.presence.touch(presenceHandle)) throw new Error('presence lease was lost');
