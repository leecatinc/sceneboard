...
export class GrantPrincipalRepository implements GrantPrincipalPersistence {
  ...
  async resolve(input: { locator: Buffer; tokenHash: Buffer; now: number }): Promise<GrantPrincipalRecord | null> {
    return this.mysql.withConnection((connection) => withTransaction(connection, 'READ COMMITTED', async () => {
      ...
      if (row.ownerUserStatus !== 1 || row.grantStatus !== 2 || row.credentialStatus !== 1) return null;
      ...
      if (deadlineExpired || familyEnded) {
        ...
        if (credentials.affectedRows < 1) throw new Error('active grant has no active credential');
      ...
      const [boardRows] = await connection.execute<Array<RowDataPacket & { boardPublicId: string }>>(`
