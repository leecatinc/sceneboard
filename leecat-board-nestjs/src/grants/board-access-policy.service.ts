...
const BOARD_ERROR_DEFINITIONS = {
  UNAUTHENTICATED: { message: 'Authentication is required', category: 'auth', retryable: false, httpStatusHint: 401, details: null },
  FORBIDDEN: { message: 'Forbidden', category: 'auth', retryable: false, httpStatusHint: 403, details: null },
  BOARD_NOT_FOUND: { message: 'Board not found', category: 'not_found', retryable: false, httpStatusHint: 404, details: null },
  SERVICE_UNAVAILABLE: { message: 'Service unavailable', category: 'availability', retryable: true, httpStatusHint: 503, details: { retryAfterSeconds: null } },
  INTERNAL_ERROR: { message: 'Internal server error', category: 'internal', retryable: false, httpStatusHint: 500, details: null },
} as const satisfies Partial<Record<BoardErrorV1['code'], Omit<BoardErrorV1, 'protocolVersion' | 'type' | 'code'>>>;

const boardFailure = (code: keyof typeof BOARD_ERROR_DEFINITIONS): BoardContractError => new BoardContractError({
  protocolVersion: 1,
  type: 'board.error',
  code,
...
const isRetryableTransactionError = (error: unknown): boolean => {
  ...
  return candidate.errno === 1205 || candidate.errno === 1213
    || candidate.code === 'ER_LOCK_WAIT_TIMEOUT' || candidate.code === 'ER_LOCK_DEADLOCK';
};
...
const assertPrincipalShape = (principal: ResolvedBoardPrincipalV1): void => {
  if (principal.kind === 'user') {
    if (principal.actor.principalKind !== 'user' || principal.actor.grantId !== null) throw boardFailure('FORBIDDEN');
    return;
  }
  if (principal.kind === 'mcp') {
    if (principal.actor.principalKind !== 'mcp_client' || principal.actor.grantId !== principal.grantId) {
...
export class MysqlBoardAccessPolicy implements BoardAccessPolicy {
  ...
  constructor(
    ...
    this.runtime = {
      ...
      sleep: runtime.sleep ?? ((milliseconds) => new Promise((resolve) => setTimeout(resolve, milliseconds))),
    };
  }
  ...
  async withAuthorizedBoardTransaction<T>(
    ...
    for (let attempt = 0; attempt <= retryDelays.length; attempt += 1) {
      try {
        return await this.mysql.withConnection((connection) => withTransaction(
          connection,
        ...
        if (!Number.isInteger(jitter) || jitter < 0 || jitter > 25) throw boardFailure('INTERNAL_ERROR');
  ...
  private async authorizeAndApply<T>(
    ...
    if (input.principal.kind === 'user') {
      context = await this.authorizeUser(
        connection, input, input.principal, boardId, transactionNow, transactionNowSql,
      );
    } else {
    ...
    const guardedContext = context.createBinding === null ? context : {
      ...context,
      createBinding: {
    ...
    try {
      return await apply(connection, guardedContext);
    } finally {
  ...
  private async lockUser(connection: PoolConnection, userPk: bigint): Promise<UserRow> {
    ...
    `, [userPk.toString()]);
    const row = rows[0];
    if (rows.length !== 1 || row === undefined || parseDatabasePk(row.userPk) !== userPk || row.status !== 1) {
  ...
  private async authorizeUser(
    ...
    if (current === undefined || !isCurrentSession(current, transactionNow)) throw boardFailure('UNAUTHENTICATED');
    const board = await this.authorizeBoardTarget(
      connection, input, boardId, principal.userPk, null,
    );
    const policy = board === null
      ? DEFAULT_DENY_UNTARGETED_POLICY
      : await this.readArtifactPolicy(connection, board, principal.userPk, transactionNowSql);
    return {
  ...
  private async authorizeMcp(
    ...
    const family = principal.sourceFamilyPublicId === null
      ? []
    ...
    if (grantRows.length !== 1 || grant === undefined
    ...
    const rule = authorizationRuleFor(input.operation);
    if (!rule.requiredCapabilities.every((capability) => scopes.includes(capability))) throw boardFailure('FORBIDDEN');
    const lifecycle = lifecycleValuesFromMask(grant.lifecycleMask);
    if (rule.requiredLifecyclePermission !== null && !lifecycle.includes(rule.requiredLifecyclePermission)) {
      throw boardFailure('FORBIDDEN');
    }
    if (grant.lifetime === 1) {
      ...
      if (!family.some((row) => parseDatabasePk(row.sessionPk) === sourceSessionPk && isCurrentSession(row, transactionNow))) {
        throw boardFailure('UNAUTHENTICATED');
      }
    } else if (grant.lifetime !== 2 || principal.sourceFamilyPublicId !== null || grant.sourceSessionPk !== null) {
      throw boardFailure('UNAUTHENTICATED');
    }
    ...
    if (credentialRows.length !== 1 || credential === undefined
    ...
    if (boardId !== null) {
      ...
      if (bindings.length !== 1 || parseDatabasePk(bindings[0]?.grantPk ?? '') !== principal.grantPk) {
        throw boardFailure('FORBIDDEN');
      }
    }
    const board = await this.authorizeBoardTarget(
      connection, input, boardId, principal.ownerUserPk, principal.grantPk,
    );
    const policy = board === null
      ? DEFAULT_DENY_UNTARGETED_POLICY
      : await this.readArtifactPolicy(connection, board, principal.ownerUserPk, transactionNowSql);
    const createBinding = input.operation === 'board.create'
  ...
  private async authorizeBoardTarget(
    ...
    ownerUserPk: bigint,
    grantPk: bigint | null,
  ): Promise<{ boardPk: bigint; archivedAt: string | null } | null> {
    ...
    const board = rows[0];
    if (board === undefined) {
      if (grantPk !== null) throw boardFailure('FORBIDDEN');
      throw boardFailure('BOARD_NOT_FOUND');
    }
    if (parseDatabasePk(board.ownerUserPk) !== ownerUserPk) throw boardFailure('FORBIDDEN');
    ...
    return { boardPk: parseDatabasePk(board.boardPk), archivedAt: board.archivedAt };
  }
  ...
  private async readArtifactPolicy(
    ...
    if (epochs.length !== 1 || epoch === undefined
      ...
      || !Buffer.isBuffer(epoch.policyEpoch) || epoch.policyEpoch.byteLength !== 16) {
      throw boardFailure('INTERNAL_ERROR');
    }
    ...
    for (const row of policyRows) {
      if (!known.has(row.capability) || capabilities.includes(row.capability as ArtifactRequestCapabilityV1)) {
        throw boardFailure('INTERNAL_ERROR');
      }
