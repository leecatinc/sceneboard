...
import type { ResolvedBoardPrincipalV1, ResolvedMcpGrantProjectionV1 } from './board-access.policy.js';
...
export interface GrantPrincipalPersistence {
  resolve(input: { locator: Buffer; tokenHash: Buffer; now: number }): Promise<GrantPrincipalRecord | null>;
}

const hasSameScopes = (
  left: readonly string[],
  right: readonly string[],
): boolean => left.length === right.length && left.every((scope) => right.includes(scope));
...
export class ActorContextService {
  ...
  async resolveMcp(
    ...
    if (resolved.connectionGrant !== undefined && (
    ...
    return {
      ...
      ...(resolved.connectionGrant === undefined ? {} : { connectionGrant: resolved.connectionGrant }),
    };
  }
...
const databaseId = (value: string): bigint => {
  ...
  if (parsed < 1n || parsed > 18_446_744_073_709_551_615n) throw new AppError('SERVICE_UNAVAILABLE');
  return parsed;
};
