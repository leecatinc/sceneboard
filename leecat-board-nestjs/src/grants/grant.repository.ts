...
export class GrantRepository {
  ...
  async list(input: GrantListPersistenceInput): Promise<GrantListPersistenceResult> {
    return this.mysql.withConnection((connection) => withTransaction(connection, 'REPEATABLE READ', async () => {
      const cursorClause = input.cursor === null
        ? ''
        : ' AND (g.created_at < ? OR (g.created_at = ? AND g.id < ?))';
      const cursorValues = input.cursor === null
        ? []
        : [mysqlDate(input.cursor.createdAt), mysqlDate(input.cursor.createdAt), input.cursor.id];
      ...
      const boardMap = await this.readBoardMap(connection, rows.map((row) => row.id));
      const grants = rows.map((row) => this.summaryFromRow(
        row,
        boardMap.get(row.id) ?? [],
      ...
      return {
        grants,
        nextTuple: hasNext && last !== undefined
          ? { createdAt: iso(last.createdAt), id: last.id }
          : null,
      };
    }));
  }
  ...
  async revoke(input: GrantMutationPersistenceInput): Promise<RevokeGrantPersistenceResult> {
    try {
      return await this.mysql.withConnection((connection) => withTransaction(connection, 'READ COMMITTED', async () => {
        ...
        const expired = millis(grant.expiresAt) <= input.now
          || (pairing !== null && millis(pairing.redeemExpiresAt) <= input.now)
          || !familyLive;

        if ((grant.status === 1 || grant.status === 2) && expired) {
          await this.expireLockedGrant(connection, grant, pairing, input, familyLive ? 'deadline' : 'session_ended');
          grant.status = 4;
        } else if (grant.status === 1) {
          if (pairing === null || pairing.state !== 3) throw new Error('pending grant has no approved pairing');
          ...
          if (pairingResult.affectedRows !== 1) throw new Error('pending pairing revoke compare-and-set failed');
  ...
  async rotate(input: GrantRotationPersistenceInput): Promise<RotateGrantPersistenceResult> {
    try {
      return await this.mysql.withConnection((connection) => withTransaction(connection, 'READ COMMITTED', async () => {
        ...
        if (credentialIds.length !== 1) throw new Error('active grant must have exactly one active credential');
        ...
        if (rotated.affectedRows !== 1) throw new Error('credential rotation compare-and-set failed');
        try {
          await connection.execute(`
            INSERT INTO mcp_grant_credentials (
              grant_id, locator, token_hash, status, rotated_from_id, created_at
            ) VALUES (?, ?, ?, 1, ?, ?)
          `, [grant.id, input.credentialLocator, input.credentialHash, credentialIds[0], new Date(input.now)]);
        } catch (error) {
          if (isDuplicateKey(error)) throw new GrantCredentialCollisionError('grant credential locator collision');
          throw error;
        }
        await this.audit.writeMandatory({ connection }, {
          event: 'grant_rotate',
  ...
  private async prepareMutation(
    ...
    const [pairings] = await connection.execute<Array<RowDataPacket & {
      id: string;
      publicId: string;
      state: number;
    ...
    const [credentials] = await connection.execute<Array<RowDataPacket & { id: string }>>(`
      ...
      FOR UPDATE
    `, [grant.id]);
    return { grant, pairing: pairings[0] ?? null, familyRows, credentialIds: credentials.map((row) => row.id) };
  }
  ...
  private async expireLockedGrant(
    ...
    await this.revokeCredentials(connection, grant.id, input.now, 3);
    await this.updateGrantTerminal(connection, grant.id, 4, reason === 'session_ended' ? 2 : 3, input.now, grant.status);
    if (pairing?.state === 3) {
      const [result] = await connection.execute<ResultSetHeader>(`
        UPDATE pairing_requests SET state = 7, updated_at = ? WHERE id = ? AND state = 3
      `, [new Date(input.now), pairing.id]);
      if (result.affectedRows !== 1) throw new Error('pending pairing expiry compare-and-set failed');
      await this.audit.writeMandatory({ connection }, {
  ...
  private async readBoardMap(connection: PoolConnection, grantIds: string[]): Promise<Map<string, BoardId[]>> {
    const placeholders = grantIds.map(() => '?').join(', ');
    const [rows] = await connection.execute<Array<RowDataPacket & { grantId: string; boardPublicId: string }>>(`
...
const nullableMillis = (value: string | null): number | null => value === null ? null : millis(value);
...
const hasLiveFamily = (
  ...
  now: number,
): boolean => rows.some((row) => row.status === 1 && millis(row.idleExpiresAt) > now && millis(row.absoluteExpiresAt) > now);
