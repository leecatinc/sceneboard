...
const assertCatalogOrder = <Value extends string>(
  ...
  for (const value of values) {
    ...
    if (index < 0 || index <= prior) throw new TypeError('values must be a unique catalog-ordered subset');
    prior = index;
  }
};
...
export const scopeValuesFromMask = (mask: number): ClientGrantCapabilityV1[] => {
  if (!Number.isInteger(mask) || mask < 0 || mask > 127) throw new TypeError('scope mask contains unknown bits');
...
export const lifecycleValuesFromMask = (mask: number): LifecyclePermission[] => {
  if (!Number.isInteger(mask) || mask < 0 || mask > 3) throw new TypeError('lifecycle mask contains unknown bits');
