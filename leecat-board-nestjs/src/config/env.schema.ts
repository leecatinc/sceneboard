...
const APP_ENVIRONMENTS = new Set<AppEnvironmentName>(['development', 'test', 'staging', 'production']);
...
const ASCII_HOST_PATTERN = /^(?=.{1,253}$)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)(?:\.(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?))*$/;
const DECIMAL_PATTERN = /^(?:0|[1-9][0-9]*)$/;

const required = (input: EnvironmentInput, key: string): string => {
  const value = input[key];
  if (value === undefined || value.length === 0) throw new EnvironmentValidationError(key, 'is required');
  if (PLACEHOLDER_PATTERN.test(value)) throw new EnvironmentValidationError(key, 'placeholder values are forbidden');
  return value;
};
...
const parseEnum = <Value extends string>(
  ...
  if (!allowed.has(value as Value)) throw new EnvironmentValidationError(key, 'is outside the allowed set');
  return value as Value;
};

const parseInteger = (input: EnvironmentInput, key: string, minimum: number, maximum: number): number => {
  const value = required(input, key);
  if (!DECIMAL_PATTERN.test(value)) throw new EnvironmentValidationError(key, 'must be a canonical decimal integer');
...
const parseBoundedText = (
  ...
  if (CONTROL_PATTERN.test(value) || bytes < minimumBytes || bytes > maximumBytes) {
    throw new EnvironmentValidationError(key, `must contain ${minimumBytes}-${maximumBytes} non-control UTF-8 bytes`);
  }
  return value;
};
...
const parseOrigin = (input: EnvironmentInput, key: string): ParsedOrigin => {
  ...
  if (
    ...
    throw new EnvironmentValidationError(key, 'must use its canonical bare http(s) origin serialization');
  }
...
const parseKey = (input: EnvironmentInput, key: string): Buffer => {
  ...
  try {
    ...
    throw new EnvironmentValidationError(key, 'must be canonical unpadded base64url encoding at least 32 bytes');
  }
};
...
const parseStreamKey = (input: EnvironmentInput): Buffer => {
  ...
  if (!/^[A-Za-z0-9+/]{43}=$/u.test(value)) {
    throw new EnvironmentValidationError('BOARD_STREAM_KEY_B64', 'must be canonical padded RFC 4648 base64');
  }
...
const parseTrustedProxyCidrs = (input: EnvironmentInput): string[] => {
  ...
  if (entries.length > 16 || entries.some((entry) => entry === '' || entry !== entry.trim())) {
    throw new EnvironmentValidationError('TRUSTED_PROXY_CIDRS', 'must contain 1-16 comma-separated canonical CIDRs');
  }
  const seen = new Set<string>();
  for (const entry of entries) {
    const separator = entry.lastIndexOf('/');
    if (separator <= 0) throw new EnvironmentValidationError('TRUSTED_PROXY_CIDRS', 'CIDR prefix is required');
    ...
    if (prefix > (family === 4 ? 32 : 128) || (family === 4 && !canonicalIpv4(address)) || address !== address.toLowerCase()) {
      throw new EnvironmentValidationError('TRUSTED_PROXY_CIDRS', 'contains a non-canonical CIDR');
    }
    if (seen.has(entry)) throw new EnvironmentValidationError('TRUSTED_PROXY_CIDRS', 'contains a duplicate CIDR');
    seen.add(entry);
  }
  return entries;
};
...
export const parsePersistenceEnvironment = (input: EnvironmentInput): PersistenceEnvironment => {
  ...
  if (mysqlDatabase !== 'leecat_board') throw new EnvironmentValidationError('MYSQL_DATABASE', 'must be leecat_board');
  const redisKeyPrefix = required(input, 'REDIS_KEY_PREFIX');
  if (redisKeyPrefix !== 'leecat_board:') throw new EnvironmentValidationError('REDIS_KEY_PREFIX', 'must be leecat_board:');
  return {
    mysql: {
...
export const parseEnvironment = (input: EnvironmentInput): AppEnvironment => {
  ...
  if ((appEnv === 'staging' || appEnv === 'production') && nodeEnv !== 'production') {
    throw new EnvironmentValidationError('NODE_ENV', 'must be production for staging or production');
  }
  ...
  if (browser.protocol !== api.protocol || browser.hostname !== api.hostname) {
    throw new EnvironmentValidationError('BOARD_PUBLIC_API_ORIGIN', 'must use the browser origin scheme and hostname');
  }
  if (appEnv === 'staging' || appEnv === 'production') {
    if (browser.protocol !== 'https:') throw new EnvironmentValidationError('BOARD_ALLOWED_ORIGINS', 'must use https');
  } else if (browser.protocol === 'http:' && !LOOPBACK_HOSTS.has(browser.hostname)) {
    throw new EnvironmentValidationError('BOARD_ALLOWED_ORIGINS', 'http is limited to a shared loopback host');
  }
