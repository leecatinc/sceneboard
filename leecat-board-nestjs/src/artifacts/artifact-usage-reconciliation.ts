...
export class ArtifactUsageReconciliation {
  ...
  async inspectBoard(
    ...
    if (usageRows.length !== 1 || usage === undefined) throw new BoardPersistenceError('row_integrity');
    const stored = {
    ...
    while (true) {
      ...
      for (const row of rows) {
        ...
        if (!Number.isSafeInteger(row.manifestCanonicalBytes) || row.manifestCanonicalBytes < 1
          || !Number.isSafeInteger(row.resourceCount) || row.resourceCount < 1
          || !Number.isSafeInteger(row.resourceTotalBytes) || row.resourceTotalBytes < 0) {
    ...
    const aggregateColumnsMatchResources = versionResourceCount === resourceCount
      && versionResourceBytes === resourceBytes;
    const drift = Object.keys(computed).some((key) => (
      computed[key as keyof typeof computed] !== stored[key as keyof typeof stored]
