...
export class CurrentArtifactRuntimeSummaryProvider extends CurrentArtifactRuntimeSummaryPort {
  async readAuthorizedAtCut(
    ...
    for (let offset = 0; offset < pairs.length; offset += 100) {
      ...
      const binds = batch.flatMap((pair, index) => [offset + index, pair.artifactId, pair.artifactVersionId]);
      ...
      for (let index = 0; index < rows.length; index += 1) {
        ...
        if (row === undefined || pair === undefined || row.inputOrdinal !== offset + index
          || row.artifactId !== pair.artifactId || row.versionId !== pair.artifactVersionId
