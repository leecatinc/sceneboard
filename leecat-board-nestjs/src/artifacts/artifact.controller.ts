...
import { boardHttpSuccess, type BoardHttpSuccessEnvelopeV1 } from '../boards/board-http-envelope.js';
...
import { admitBoardRequestId, type BoardRequestCorrelationCarrier } from '../common/http/board-request-correlation.js';
...
const singletonQuery = (query: unknown): Record<string, string> => {
  if (query === null || typeof query !== 'object' || Array.isArray(query)) throw invalid('invalid query');
  const source = query as Record<string, unknown>;
  const result: Record<string, string> = Object.create(null) as Record<string, string>;
  for (const [key, value] of Object.entries(source)) {
    if (typeof value !== 'string' || value.length === 0) throw invalid('invalid query field', [key]);
    result[key] = value;
  }
  return result;
};
...
const artifactGetRequest = (
  ...
  if (!parsed.ok || parsed.data.value.type !== 'artifact.get') {
    throw new BoardContractError(parsed.ok ? invalidBoardPayload('expected artifact.get') : parsed.error);
  }
...
export class ArtifactController {
  constructor(
    ...
    @Inject(ArtifactCapabilityBrokerService) private readonly broker: ArtifactCapabilityBrokerService,
  ...
  async publish(
    ...
    if (source.boardId !== pathBoardId) throw invalid('path and body board IDs differ', ['boardId']);
  ...
  async get(
    ...
    const operation = artifactGetRequest(
      request, pathBoardId, pathArtifactId, pathVersionId, query.requestId,
    );
  ...
  async getPackage(
    ...
    const operation = artifactGetRequest(
      request, pathBoardId, pathArtifactId, pathVersionId, generatePublicUuidV4(),
    );
