...
const runCohort = async (
  ...
  const worker = async (): Promise<void> => {
    while (cursor < attempts) {
      ...
      if (!Number.isFinite(sample.lookupLockHmacMs) || sample.lookupLockHmacMs < 0
        || !Number.isFinite(sample.totalResponseMs) || sample.totalResponseMs < sample.lookupLockHmacMs) {
...
const validateBudget = (budget: PairingClaimCapacityBudget): void => {
  if (!CONCURRENCY_LEVELS.includes(budget.maxConcurrency)) throw new TypeError('pairing capacity maxConcurrency must be 1, 4, or 8');
  for (const [key, value] of Object.entries(budget)) {
    if (key === 'maxConcurrency') continue;
    if (!Number.isFinite(value) || value <= 0) throw new TypeError(`pairing capacity budget ${key} must be positive`);
  }
};
...
export const calibratePairingClaims = async (
  ...
  if (!Number.isInteger(attempts) || attempts < 500) throw new TypeError('pairing calibration requires at least 500 attempts per cohort');
  ...
  if (!Number.isInteger(input.configuredFailureJitterMs)
    || input.configuredFailureJitterMs < 10 || input.configuredFailureJitterMs > 25) {
  ...
  const slowestRawP99 = Math.max(...measurements.map((measurement) => measurement.lookupLockHmac.p99Ms));
  ...
  const timingDistributionPassed = input.configuredFailureMinimumMs >= recommendedFailureMinimumMs
    && maxPairwiseMedianDeltaMs <= 10 && maxPairwiseP95DeltaMs <= 25;
  const capacityMeasurements = measurements.filter(
    (measurement) => measurement.concurrency === input.capacityBudget.maxConcurrency,
  );
  const capacityGatePassed = capacityMeasurements.length === PAIRING_CLAIM_CALIBRATION_COHORTS.length
    && capacityMeasurements.every((measurement) => (
