import type { BoardId, ClientGrantCapabilityV1 } from '@leecat-board/board-schema';
...
const stateFromNumber = (value: number): PairingOwnerState => {
  const states: Record<number, PairingOwnerState> = {
    1: 'created', 2: 'pending', 3: 'approved', 4: 'redeemed',
    5: 'denied', 6: 'cancelled', 7: 'expired', 8: 'locked',
  };
...
export const buildPairingOwnerStatus = (
  ...
  approvedScopes: source.approvedScopeMask === null ? null : scopeValuesFromMask(source.approvedScopeMask),
  approvedLifecyclePermissions: source.approvedLifecycleMask === null
    ? null
