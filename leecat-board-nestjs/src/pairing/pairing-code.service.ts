...
export class PairingCodeService {
  ...
  verify(parsed: ParsedPairingCode, locatorHash: Uint8Array, verifierHash: Uint8Array): boolean {
    return this.crypto.constantTimeEqual(this.crypto.hmac('pairing-locator/v1', parsed.locator), locatorHash)
      && this.crypto.constantTimeEqual(this.crypto.hmac('pairing-verifier/v1', parsed.verifier), verifierHash);
  }
}
