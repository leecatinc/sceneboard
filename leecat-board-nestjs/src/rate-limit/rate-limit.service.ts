...
export interface RateLimitInput {
  surface: string;
  purpose: Extract<SecurityPurpose,
    'rate-limit-ip/v1'
    | 'rate-limit-email/v1'
    | 'rate-limit-user/v1'
...
export class RateLimitService {
  ...
  async consume(input: RateLimitInput): Promise<void> {
    ...
    if (!Number.isSafeInteger(input.limit) || input.limit < 1) throw new TypeError('rate-limit count is invalid');
    if (!Number.isSafeInteger(input.windowMs) || input.windowMs < 1_000) throw new TypeError('rate-limit window is invalid');
    ...
    try {
      result = await this.redis.consume(RATE_LIMIT_LUA, key, [String(input.limit), String(input.windowMs)]);
    } catch (error) {
    ...
    if (!Number.isSafeInteger(count) || !Number.isFinite(ttlMs)) throw new AppError('SERVICE_UNAVAILABLE');
    if (count > input.limit) {
      throw new AppError('RATE_LIMITED', { retryAfterSeconds: Math.max(1, Math.ceil(ttlMs / 1_000)) });
    }
  }
}
