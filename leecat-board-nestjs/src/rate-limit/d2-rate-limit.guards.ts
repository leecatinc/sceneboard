...
const surfaceOf = (reflector: Reflector, context: ExecutionContext): D2RateLimitedSurface | undefined => (
...
export class D2PreAuthRateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    } else if (surface === 'session-renewal' || surface === 'pairing-create') parseEmptyObject(request.body);
    ...
    if (emailIdentity !== null) {
      await this.limiter.consume({
        ...
        windowMs: surface === 'signup' || surface === 'email-verification-request'
          ? 60 * 60 * 1_000
          : 15 * 60 * 1_000,
      });
    }
