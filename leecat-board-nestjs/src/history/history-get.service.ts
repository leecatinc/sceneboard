...
const notFound = (revisionId: RevisionId): BoardContractError => new BoardContractError({
  protocolVersion: 1,
  type: 'board.error',
...
export class HistoryGetService {
  ...
  async getWithMetadata(input: {
    ...
    return this.accessPolicy.withAuthorizedBoardTransaction({
      ...
      if (selectedRevisionId !== input.request.revisionId) throw new BoardPersistenceError('row_integrity');
      ...
      const revision = {
        ...
        previousRevisionId: row.previousRevisionId === null ? null : storedRevisionId(row.previousRevisionId),
        originType: originType(row.originCode),
        sourceRevisionId: row.sourceRevisionId === null ? null : storedRevisionId(row.sourceRevisionId),
        actor: {
      ...
      return {
        ...
        metadata: historyGetMetadata({
          entry,
          label: row.label,
          nextRevisionId: row.nextRevisionId === null ? null : storedRevisionId(row.nextRevisionId),
