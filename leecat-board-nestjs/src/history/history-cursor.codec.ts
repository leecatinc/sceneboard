import type { BoardId, PageCursorV1 } from '@leecat-board/board-schema';
...
export class HistoryCursorCodec {
  ...
  issue(boardId: BoardId, beforeRevisionNumber: number): PageCursorV1 {
    ...
    return encodeSignedCursorV1(this.key, Buffer.from(JSON.stringify({
      v: 1,
      k: 'history',
      b: boardId,
  ...
  parse(cursor: string, boardId: BoardId): number {
    ...
    if (Object.keys(decoded).join(',') !== 'v,k,b,n'
      || decoded.v !== 1 || decoded.k !== 'history' || decoded.b !== boardId
      || typeof decoded.n !== 'number') {
      throw invalidCursorV1();
    }
    this.assertRevisionNumber(decoded.n);
    const canonical = Buffer.from(JSON.stringify({ v: 1, k: 'history', b: boardId, n: decoded.n }), 'utf8');
