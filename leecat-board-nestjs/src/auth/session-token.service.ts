...
export class SessionTokenService {
  ...
  parse(token: string): ParsedSessionToken {
    ...
    if (parts.length !== 3 || parts[0] !== 'lcbs_v1' || parts[1] === undefined || parts[2] === undefined) {
  ...
  verify(token: string, expectedHash: Uint8Array): boolean {
    try {
      this.parse(token);
      return this.crypto.constantTimeEqual(this.crypto.hmac('session-token/v1', token), expectedHash);
    } catch {
      return false;
    }
  }
