...
import { parseSessionId, parseUserId, type SessionId, type UserId } from '../common/ids/public-id.js';
...
export class AuthService {
  ...
  async login(credentials: AuthCredentials, now: number, signal?: AbortSignal): Promise<IssuedAuthSession> {
  ...
  private issueResponse(input: {
    ...
    const issuedCsrf = this.csrf.issueSession(input.session.familyPublicId, input.now, input.session.idleExpiresAt);
    ...
    return {
      ...
      sessionMaxAgeSeconds: Math.max(0, Math.floor((input.session.absoluteExpiresAt - input.now) / 1_000)),
      csrfMaxAgeSeconds: Math.max(0, Math.floor((issuedCsrf.expiresAt - input.now) / 1_000)),
      authGeneration: this.csrf.authGeneration('s', input.session.sessionPublicId, issuedCsrf.token),
    };
  }
