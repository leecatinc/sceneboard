...
const cookieNames = (environment: AppEnvironmentName): CookieNames => {
  if (environment === 'development') return { session: 'lcb_session', csrf: 'lcb_csrf', secure: false };
  if (environment === 'test') return { session: 'lcb_test_session', csrf: 'lcb_test_csrf', secure: false };
  return { session: '__Host-lcb_session', csrf: '__Host-lcb_csrf', secure: true };
};

const assertCookieValue = (value: string): void => {
  if (value === '' || /[\u0000-\u0020;,\u007f]/.test(value)) throw new TypeError('cookie value is unsafe');
};
...
export class CookieService {
  ...
  private serialize(name: string, value: string, maximumAgeSeconds: number, httpOnly: boolean): string {
    if (!Number.isSafeInteger(maximumAgeSeconds) || maximumAgeSeconds < 0) throw new TypeError('cookie Max-Age is invalid');
