...
import type { PersistenceTransaction, UserInsertResult, UserWriterPort } from './auth.persistence.js';
...
const duplicateConstraint = (error: unknown): string | null => {
  if (typeof error !== 'object' || error === null || !('code' in error) || error.code !== 'ER_DUP_ENTRY') return null;
...
export class UsersRepository implements UserWriterPort {
  ...
  async insert(transaction: PersistenceTransaction, input: CreateUserWithSessionInput): Promise<UserInsertResult> {
    try {
  ...
  async lockForLogin(transaction: PersistenceTransaction, databaseId: string): Promise<{
    status: 'active' | 'disabled';
    passwordHash: string;
  } | null> {
    const [rows] = await connectionOf(transaction).execute<Array<RowDataPacket & { status: number; passwordHash: string }>>(`
  ...
  async updateLogin(transaction: PersistenceTransaction, input: {
    databaseId: string;
    ...
    if (input.replacementPasswordHash === null) {
      await connection.execute('UPDATE users SET last_login_at = ? WHERE id = ?', [new Date(input.now), input.databaseId]);
      return;
    }
    ...
    if (result.affectedRows !== 1) throw new Error('locked user password hash changed unexpectedly');
  }
