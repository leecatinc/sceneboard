...
export interface AuthControllerService {
  ...
  login(credentials: AuthCredentials, now: number, signal?: AbortSignal): Promise<IssuedAuthSession>;
}
...
export class AuthController {
  ...
  csrf(
    ...
    if (request.cookies?.[this.cookies.names.session] !== undefined) throw new AppError('AUTH_SESSION_PRESENT');
    ...
    response.setHeader('X-Auth-Generation', this.csrfTokens.authGeneration('a', null, issued.token));
  ...
  async session(
    ...
    try {
      ...
      if (resolved.csrfWasReissued) {
        response.appendHeader('Set-Cookie', this.cookies.csrf(resolved.response.csrfToken, resolved.csrfMaxAgeSeconds));
      }
  ...
  async logout(
    ...
    try {
      ...
      if (error instanceof LogoutClearableError || error instanceof SessionResolutionError && error.clearCookies) {
        this.writeClearedSession(response);
      }
      throw error;
    }
  ...
  private writeIssuedSession(response: AuthHttpResponse, issued: IssuedAuthSession): void {
    response.appendHeader('Set-Cookie', this.cookies.session(issued.sessionCredential, issued.sessionMaxAgeSeconds));
    response.appendHeader('Set-Cookie', this.cookies.csrf(issued.response.csrfToken, issued.csrfMaxAgeSeconds));
