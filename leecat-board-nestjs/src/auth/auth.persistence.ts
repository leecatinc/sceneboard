...
export interface UserWriterPort {
  insert(transaction: PersistenceTransaction, input: CreateUserWithSessionInput): Promise<UserInsertResult>;
  findLoginCandidate(emailNormalized: string): Promise<LoginCandidate | null>;
  lockForLogin(transaction: PersistenceTransaction, databaseId: string): Promise<{
    status: 'active' | 'disabled';
    passwordHash: string;
  } | null>;
  updateLogin(transaction: PersistenceTransaction, input: {
    databaseId: string;
...
export interface SessionWriterPort {
  insert(transaction: PersistenceTransaction, input: {
...
export interface AuditWriterPort {
  writeMandatory(transaction: PersistenceTransaction, input: {
...
export class AuthPersistenceService implements AuthPersistence {
  ...
  async createUserWithSession(input: CreateUserWithSessionInput): Promise<
  ...
  async commitLogin(input: CommitLoginInput): Promise<
    { kind: 'created' }
    | { kind: 'disabled' | 'stale_hash' | 'public_id_collision' }
  > {
    return this.transactions.run(async (transaction) => {
      const locked = await this.users.lockForLogin(transaction, input.userDatabaseId);
      if (!locked || locked.passwordHash !== input.expectedPasswordHash) return { kind: 'stale_hash' };
