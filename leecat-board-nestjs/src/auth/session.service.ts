...
export interface SessionPersistence {
  ...
  rotate(record: SessionRecord, replacement: {
    publicId: SessionId;
    familyPublicId: string;
    locator: Buffer;
    tokenHash: Buffer;
...
export class SessionResolutionError extends AppError {
  ...
  constructor(code: Extract<D2ErrorCode,
...
export class SessionService implements SessionControllerService {
  ...
  async renew(credential: string | undefined, currentCsrfToken: string | undefined, now: number): Promise<IssuedAuthSession> {
    const active = await this.resolveExclusive(credential, currentCsrfToken, now);
    if (currentCsrfToken === undefined || active.csrfWasReissued) throw new AppError('CSRF_INVALID');
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      ...
      const idleExpiresAt = Math.min(now + SESSION_IDLE_LIFETIME_MS, active.record.absoluteExpiresAt);
      ...
      return {
        ...
        sessionMaxAgeSeconds: Math.max(0, Math.floor((active.record.absoluteExpiresAt - now) / 1_000)),
  ...
  private async resolve(
    ...
    if (record.status === 'revoked') throw new SessionResolutionError('AUTH_SESSION_REVOKED', clearCookies);
    if (record.status === 'expired') throw new SessionResolutionError('AUTH_SESSION_EXPIRED', clearCookies);
    ...
    const csrfValid = currentCsrfToken !== undefined && this.csrf.verify(currentCsrfToken, {
      kind: 'session',
    ...
    const issued = csrfValid
      ...
      : this.csrf.issueSession(record.familyPublicId, now, Math.min(record.idleExpiresAt, record.absoluteExpiresAt));
  ...
  private async commitTerminal(record: SessionRecord, reason: SessionTerminalReason, now: number): Promise<void> {
