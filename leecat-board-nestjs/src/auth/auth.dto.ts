...
export const parseAuthCredentials = (input: unknown): AuthCredentials => {
  if (!isRecord(input) || !hasExactKeys(input, ['email', 'password'])) throw new AppError('INVALID_PAYLOAD');
  if (typeof input.email !== 'string' || typeof input.password !== 'string') throw new AppError('INVALID_PAYLOAD');
...
export const parseEmailVerificationRequest = (input: unknown): EmailVerificationRequest => {
  if (!isRecord(input) || !hasExactKeys(input, ['email', 'locale'])) throw new AppError('INVALID_PAYLOAD');
  if (typeof input.locale !== 'string' || !EMAIL_VERIFICATION_LOCALES.includes(input.locale as EmailVerificationLocale)) {
    throw new AppError('INVALID_PAYLOAD');
  }
  return { ...parseEmail(input.email), locale: input.locale as EmailVerificationLocale };
};

export const parseEmailVerificationConfirmation = (input: unknown): EmailVerificationConfirmation => {
  if (!isRecord(input) || !hasExactKeys(input, ['email', 'code'])) throw new AppError('INVALID_PAYLOAD');
  if (typeof input.code !== 'string' || !/^[0-9]{6}$/.test(input.code)) throw new AppError('INVALID_PAYLOAD');
...
export const parsePasswordChangeRequest = (input: unknown): PasswordChangeRequest => {
  ...
  if (currentPasswordBytes === 0 || currentPasswordBytes > 72) throw new AppError('INVALID_PAYLOAD');
