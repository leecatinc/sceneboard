...
export const prepareControlMutationV1 = (input: {
  ...
  const fingerprint = buildMutationFingerprintV1({ ...input.request, actor: input.principal.actor });
...
export class ControlMutationRepository {
  ...
  async allocateSequenceAt(
    ...
    if (head.lastEventSequence >= MAX_SAFE_SEQUENCE) throw new BoardPersistenceError('capacity_exhausted');
  ...
  private async replay(
    ...
    if (row.statusCode !== 'C' || row.operationType !== request.command.type
      || row.resultPayload === null || row.resultCanonicalBytes !== row.resultPayload.byteLength
      || row.resultSha256 === null || !equalDigest(row.resultSha256, digest(row.resultPayload))
      || row.resultBoardPk === null || row.resultRevisionPk !== null) {
      throw internalFailure();
    }
    const stored = MutationResultParserV1.parseBytes(row.resultPayload);
    if (!stored.ok || stored.data.value.boardId !== request.boardId
      || stored.data.value.result.type !== request.command.type) throw internalFailure();
