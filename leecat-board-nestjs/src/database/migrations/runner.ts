...
import { MIGRATION_REGISTRY, MIGRATION_REGISTRY_VERSION, type MigrationRegistryEntry } from './registry.js';
...
export const assessMigrationLedger = (
  ...
  if (applied.length > expected.length) throw new MigrationStateError('ledger contains unknown versions');
  for (let index = 0; index < applied.length; index += 1) {
    const actual = applied[index]!;
    const wanted = expected[index];
    if (!wanted || actual.version !== wanted.version) throw new MigrationStateError('ledger has a hole, unknown version, or order drift');
  ...
  return {
    state: applied.length === 0 ? 'empty' : applied.length === expected.length ? 'complete' : 'partial',
...
const expectedChecksums = async (): Promise<Array<MigrationChecksumEntry & { entry: MigrationRegistryEntry; bytes: Buffer }>> => Promise.all(
...
export class MigrationRunner {
  ...
  async status(): Promise<MigrationCertificationStateV1> {
    return this.mysql.withConnection((connection) => this.withMigrationLock(connection, async () => {
      ...
      const assessment = assessMigrationLedger(expected, await this.readLedger(connection, expected));
      if (assessment.state !== 'complete') throw new MigrationStateError(`pending migrations: ${assessment.pendingVersions.join(', ')}`);
  ...
  async up(): Promise<MigrationCertificationStateV1> {
    return this.mysql.withConnection((connection) => this.withMigrationLock(connection, async () => {
      ...
      for (let index = applied.length; index < expected.length; index += 1) {
        ...
        const executionMs = Math.min(3_600_000, Math.max(0, Math.ceil(performance.now() - startedAt)));
      ...
      const finalAssessment = assessMigrationLedger(expected, await this.readLedger(connection, expected));
      if (finalAssessment.state !== 'complete') throw new MigrationStateError('up did not reach the complete registry');
      await this.verifyAllPostconditions(connection);
      return {
  ...
  async adopt(version: string): Promise<MigrationCertificationStateV1> {
    if (version !== MIGRATION_REGISTRY_VERSION) throw new MigrationStateError('adopt version is not the exact registry version');
    return this.mysql.withConnection((connection) => this.withMigrationLock(connection, async () => {
      const connectionProfile = await this.mysql.certifyConnection(connection);
      await this.bootstrapLedger(connection);
      const expected = await expectedChecksums();
      const applied = await this.readLedger(connection, expected);
      if (applied.length !== 0) throw new MigrationStateError('adoption requires an empty ledger');
      await this.verifyAllPostconditions(connection);
  ...
  private async readLedger(
    ...
    for (const version of byVersion.keys()) {
      if (!expected.some((entry) => entry.version === version)) throw new MigrationStateError(`unknown ledger version: ${version}`);
    }
  ...
  private async verifyAllPostconditions(connection: PoolConnection): Promise<void> {
    for (const entry of MIGRATION_REGISTRY) await this.verifyPostcondition(connection, entry.postcondition);
  }

  private async verifyPostcondition(connection: PoolConnection, postcondition: string): Promise<void> {
    const postconditions: Readonly<Record<string, readonly string[]>> = {
      ...
      d3_boards_v1: ['boards'],
      d2_pairing_grants_v1: ['mcp_clients', 'mcp_grants', 'pairing_requests', 'mcp_grant_credentials', 'mcp_grant_boards'],
    ...
    if (expectedTables === null) throw new MigrationStateError(`unknown postcondition: ${postcondition}`);
    ...
    for (const table of expectedTables) {
      if (!actual.has(table)) throw new MigrationStateError(`postcondition is missing table ${table}`);
    }
  }
  ...
  private async withMigrationLock<Value>(
    ...
    const [rows] = await connection.query<LockRow[]>("SELECT GET_LOCK('leecat-board:migrations:v1', 60) AS acquired");
    if (!isMysqlLockAcquired(rows[0]?.acquired)) throw new MigrationStateError('migration lock acquisition failed');
    try {
