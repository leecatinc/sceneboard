...
export class AuthorizedBrowserPresenceService implements AuthorizedBrowserPresencePortV1 {
  ...
  captureAuthorizedSubject(
    ...
    if (!parsed.ok || context.ownerUserPk < 1n
      || (context.access.kind === 'owner' && context.access.ownerUserPk !== context.ownerUserPk)) return null;
  ...
  async getStatus(
    ...
    if (subject === null || typeof subject !== 'object' || this.#consumed.has(subject)) return 'unknown';
