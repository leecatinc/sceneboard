...
export const encodeDurableBoardEventHintV1 = (
  ...
  if (!Number.isSafeInteger(sequence) || sequence < 1) throw new TypeError('event sequence is invalid');
  ...
  if (Buffer.byteLength(encoded, 'utf8') > MAX_HINT_BYTES) throw new TypeError('event hint is too large');
  return encoded;
};
...
export const parseDurableBoardEventHintV1 = (
  ...
  if (!FINGERPRINT.test(expectedBoardFp) || Buffer.byteLength(source, 'utf8') > MAX_HINT_BYTES) return null;
  let value: unknown;
  try {
  ...
  if (Object.keys(record).length !== 5
    ...
    || (record.sequence as number) < 1) return null;
  const canonical = canonicalizeJsonV1(value);
  if (!canonical.ok || Buffer.from(canonical.data.canonicalBytes).toString('utf8') !== source) return null;
...
export const encodePresenceBoardEventHintV1 = (boardFp: string, version: number): string => {
  if (!FINGERPRINT.test(boardFp)) throw new TypeError('board fingerprint is invalid');
  if (!Number.isSafeInteger(version) || version < 1) throw new TypeError('presence version is invalid');
...
export const parseBoardEventHintV1 = (source: string, expectedBoardFp: string): BoardEventHintV1 | null => {
  ...
  if (value === null || typeof value !== 'object' || Array.isArray(value)) return null;
  const record = value as Record<string, unknown>;
  if (Object.keys(record).length !== 4 || record.v !== 1 || record.kind !== 'presence'
    || record.boardFp !== expectedBoardFp || !Number.isSafeInteger(record.version)
    || Number(record.version) < 1) return null;
  const canonical = canonicalizeJsonV1(value);
  if (!canonical.ok || Buffer.from(canonical.data.canonicalBytes).toString('utf8') !== source) return null;
