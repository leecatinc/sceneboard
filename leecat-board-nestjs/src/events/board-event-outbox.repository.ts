...
@Injectable()
export class BoardEventOutboxRepository implements BoardEventDeliveryPortV1, BoardEventOutboxHealthPortV1 {
  ...
  async listPendingCandidates(limit = 100): Promise<readonly PendingBoardEventCandidateV1[]> {
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 100) throw new TypeError('candidate limit must be 1..100');
    return this.mysql.withConnection(async (connection) => {
      const [rows] = await connection.query<CandidateRow[]>(`
        ...
        LIMIT ?
      `, [limit]);
      return rows.map((row) => ({ eventPk: safeBigInt(row.eventPk), eventId: asEventId(row.eventId) }));
    });
  }

  async loadPendingEvent(candidate: PendingBoardEventCandidateV1): Promise<DeliverableBoardEventV1 | null> {
    return this.mysql.withConnection(async (connection) => {
      ...
      try {
        ...
        if (event.eventPk !== candidate.eventPk || event.eventId !== candidate.eventId) throw new Error('candidate drift');
        return event;
      } catch (error) {
  ...
  async listContiguousEvents(
    ...
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 1_000) throw new TypeError('range limit must be 1..1000');
    const rows = await this.#readRange(boardId, afterSequence, limit);
    for (let index = 0; index < rows.length; index += 1) {
      if (rows[index]?.sequence !== afterSequence + index + 1) throw new Error('event range is not contiguous');
    }
    return rows;
  }
  ...
  async #readRange(boardId: BoardId, afterSequence: number, limit: number): Promise<DeliverableBoardEventV1[]> {
    return this.mysql.withConnection(async (connection) => {
      const [rows] = await connection.query<EventRow[]>(`
        SELECT CAST(o.event_pk AS CHAR) AS eventPk, o.event_id AS eventId,
          b.public_id AS boardId, r.revision_id AS revisionId,
  ...
  #mapEvent(row: EventRow): DeliverableBoardEventV1 {
    if (row.eventPayload.byteLength !== row.eventCanonicalBytes || !equalDigest(row.eventPayload, row.eventSha256)) {
    ...
    if (envelope.eventId !== eventId || envelope.boardId !== boardId || envelope.revisionId !== revisionId
      || envelope.sequence !== sequence || envelope.data.type !== row.eventType) {
