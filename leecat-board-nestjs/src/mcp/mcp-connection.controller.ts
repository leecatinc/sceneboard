...
const parseQuery = (request: McpConnectionRequest, value: unknown): { requestId: ReturnType<typeof admitBoardRequestId>; boardId: BoardId | null } => {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) throw new BoardContractError(invalidBoardPayload('invalid query'));
  ...
  if (typeof query.boardId !== 'string') throw new BoardContractError(invalidBoardPayload('boardId must be a scalar'));
...
export class McpConnectionController {
  ...
  async get(
    ...
    return this.connections.get({ principal: request.boardPrincipal, ...parseQuery(request, query) });
  }
}
