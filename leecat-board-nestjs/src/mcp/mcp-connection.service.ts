...
const failure = (code: 'UNAUTHENTICATED' | 'INTERNAL_ERROR'): BoardContractError => new BoardContractError(code === 'UNAUTHENTICATED' ? {
  protocolVersion: 1,
  type: 'board.error',
  code,
...
export class McpConnectionService {
  ...
  async get(input: {
    ...
    if (input.principal.kind !== 'mcp' || input.principal.connectionGrant === undefined
      || input.principal.actor.grantId !== input.principal.grantId) throw failure('UNAUTHENTICATED');
    const base = {
      principal: {
    ...
    const browserPresence = subject === null
      ? 'unknown'
  ...
  private async readBoardSummary(
    ...
    `, [boardId]);
    const row = rows[0];
    if (rows.length !== 1 || row === undefined || row.boardId !== boardId || !Buffer.isBuffer(row.revisionId)) {
      throw failure('INTERNAL_ERROR');
    }
    ...
    if (!result.ok || result.data.value.result.type !== 'board.list' || result.data.value.result.boards.length !== 1) {
      throw failure('INTERNAL_ERROR');
    }
