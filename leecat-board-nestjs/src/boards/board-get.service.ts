...
const boardNotFound = (): BoardContractError => new BoardContractError({
  protocolVersion: 1,
  type: 'board.error',
...
const timestamp = (value: string): TimestampV1 => parseMysqlTimestampUtc(value).toISOString() as TimestampV1;
  ...
  async get(input: {
    ...
    return this.accessPolicy.withAuthorizedBoardTransaction({
      ...
      const revision = {
        ...
        previousRevisionId: row.previousRevisionId === null ? null : parsedRevisionId(row.previousRevisionId),
        originType: originType(row.originCode),
        sourceRevisionId: row.sourceRevisionId === null ? null : parsedRevisionId(row.sourceRevisionId),
        actor: {
