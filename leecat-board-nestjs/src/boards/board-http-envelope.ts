...
export const boardHttpSuccess = (
  ...
  if (history !== null) {
    ...
    if (resultType === 'history.list') {
      const ids = result.result.entries.map((entry: { revision: { revisionId: string } }) => entry.revision.revisionId);
      if (history.navigation !== null
      ...
      if (history.entries.length !== 1 || history.entries[0]?.revisionId !== revisionId
