...
const isNamedDuplicate = (error: unknown, constraint: string): boolean => {
  ...
  const message = typeof candidate.sqlMessage === 'string'
...
const idempotencyReuse = (reason: 'grant_changed' | 'scopes_changed' | 'title_changed'): BoardContractError => (
...
const actorKindCode = (kind: AuthorizedBoardContextV1['actor']['principalKind']): 'U' | 'M' | 'S' => {
...
export class BoardCreateService {
  ...
  private regenerateCollision(prepared: PreparedCreateV1, kind: CreateCollisionKind): PreparedCreateV1 {
    if (kind === 'board') return { ...prepared, boardId: asBoardId(this.crypto.generatePublicIdV1()) };
    const uuid = this.runtime.generateUuid();
    const bytes = Buffer.from(parsePublicUuidV4(uuid));
    if (kind === 'revision') return { ...prepared, revisionId: asRevisionId(uuid), revisionIdBytes: bytes };
  ...
  private async prepare(input: {
    ...
    return {
      ...
      checkpoint: await this.checkpointCodec.encode({ protocolVersion: 1, type: 'scene', root: null }),
    };
  }
