...
const internalFailure = (): BoardContractError => new BoardContractError({
  protocolVersion: 1, type: 'board.error', code: 'INTERNAL_ERROR', message: 'Internal error',
  category: 'internal', retryable: false, httpStatusHint: 500, details: null,
});
const archivedFailure = (boardId: BoardId, archivedAt: TimestampV1): BoardContractError => new BoardContractError({
  protocolVersion: 1, type: 'board.error', code: 'BOARD_ALREADY_ARCHIVED',
  message: 'Board is already archived', category: 'conflict', retryable: false,
  httpStatusHint: 409, details: { boardId, archivedAt },
});
const idempotencyReuse = (
  ...
  protocolVersion: 1, type: 'board.error', code: 'IDEMPOTENCY_KEY_REUSED',
  message: 'Idempotency key was reused for a different operation', category: 'conflict',
  retryable: false, httpStatusHint: 409,
  details: { scope: 'board.archive', boardId: request.boardId, operationType: 'board.archive', reason },
});
...
export class BoardArchiveService {
  ...
  private async applyNewOrReplay(
    ...
    `, [
      prepared.recordIdBytes, actorCode(context.actor.principalKind), context.actor.principalId,
      request.boardId, request.idempotencyKey, prepared.fingerprintPayload,
      prepared.fingerprintPayload.byteLength, prepared.fingerprintSha256,
      context.actor.grantId, prepared.actorScopesPayload, prepared.actorScopesSha256,
      prepared.commandPayloadSha256, request.requestId, prepared.occurredAtSql,
    ]);
    if (pending.affectedRows !== 1) return this.replayOrReject(connection, context, request, prepared);
    const recordPk = insertedPk(pending);
    const head = await this.lockHead(connection, request.boardId);
    if (head.archivedAt !== null) throw archivedFailure(request.boardId, timestamp(head.archivedAt));
    ...
    `, [
      prepared.occurredAtSql, prepared.occurredAtSql,
      actorCode(context.actor.principalKind), context.actor.principalId, context.actor.grantId,
      head.boardPk,
    ]);
    ...
    `, [
      payload, payload.byteLength, digest(payload), head.boardPk, head.headRevisionPk,
      prepared.occurredAtSql, prepared.expiresAtSql, recordPk.toString(),
    ]);
    if (complete.affectedRows !== 1) throw internalFailure();
    return result.data.value;
  }

  private async lockHead(connection: PoolConnection, expectedBoardId: BoardId): Promise<ArchiveHeadRow> {
  ...
  private async replayOrReject(
    ...
    const [rows] = await connection.execute<ArchiveIdempotencyRow[]>(`
      ...
      FOR UPDATE
    `, [actorCode(context.actor.principalKind), context.actor.principalId, request.boardId, request.idempotencyKey]);
    const row = rows[0];
    if (rows.length === 0) throw new ArchiveRecordCollisionError();
    if (rows.length !== 1 || row === undefined || row.operationType !== 'board.archive') throw internalFailure();
    ...
    if (row.statusCode !== 'C' || row.resultPayload === null
      ...
      || !digestEquals(digest(row.resultPayload), row.resultSha256)) throw internalFailure();
    ...
    if (!stored.ok || !Buffer.from(stored.data.canonicalBytes).equals(row.resultPayload)
      || stored.data.value.replayed || stored.data.value.result.type !== 'board.archive'
      || stored.data.value.result.board.boardId !== request.boardId) throw internalFailure();
