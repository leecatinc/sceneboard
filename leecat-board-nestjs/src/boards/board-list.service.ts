...
export class BoardListService {
  ...
  async list(input: {
    ...
    return this.accessPolicy.withAuthorizedBoardTransaction({
      ...
      const cursor = input.request.cursor === null ? null : this.cursors.parse({
  ...
  private async readPage(
    ...
    const cursorPredicate = cursor === null
      ? ''
      : 'AND (b.created_at < ? OR (b.created_at = ? AND b.board_pk < ?))';
    const cursorBinds = cursor === null ? [] : [
      formatMysqlTimestampUtc(new Date(cursor.createdAt)),
      formatMysqlTimestampUtc(new Date(cursor.createdAt)),
      cursor.boardPk,
    ];
    const from = context.access.kind === 'owner'
      ? 'FROM boards b'
      : 'FROM mcp_grant_boards gb JOIN boards b ON b.public_id = gb.board_public_id';
    const accessPredicate = context.access.kind === 'owner'
      ? 'b.owner_user_id = ?'
      : 'gb.grant_id = ?';
    const accessBind = context.access.kind === 'owner'
