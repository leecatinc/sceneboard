...
import { BoardRenameService, type BoardRenameRequestV1, type BoardRenameResultV1 } from './board-rename.service.js';
...
const readRenameRequest = (request: BoardHttpRequest, pathBoardId: string): BoardRenameRequestV1 => {
  ...
  if (typeof value.title !== 'string'
    ...
    || /[\uD800-\uDFFF]/u.test(value.title)) throw invalid('invalid board title', ['title']);
...
const readArchiveRequest = (request: BoardHttpRequest, pathBoardId: string): BoardArchiveRequestV1 => {
  const parsed = BoardOperationRequestParserV1.parse(body(request));
  if (!parsed.ok) throw new BoardContractError(parsed.error);
  if (parsed.data.value.type !== 'board.archive') throw invalid('expected board.archive', ['type']);
  if (parsed.data.value.boardId !== pathBoardId) throw invalid('path and body board IDs differ', ['boardId']);
...
const record = (value: unknown, allowed: readonly string[]): Record<string, unknown> => {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) throw invalid('invalid body');
  const source = value as Record<string, unknown>;
  if (Object.keys(source).some((key) => !allowed.includes(key))) throw invalid('unknown body field');
  return source;
};
...
const queryRecord = (value: unknown, allowed: readonly string[]): Record<string, string> => {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) throw invalid('invalid query');
  const source = value as Record<string, unknown>;
  ...
  for (const key of keys) {
    ...
    if (typeof item !== 'string' || item.length === 0) throw invalid('query fields must be non-empty scalars', [key]);
    result[key] = item;
  }
  return result;
};
...
const canonicalLimit = (value: string | undefined): number => {
  if (value === undefined || !/^[1-9][0-9]{0,2}$/.test(value)) throw invalid('limit is required', ['limit']);
...
const parseListQuery = (request: BoardHttpRequest, value: unknown): BoardListRequestV1 => {
  ...
  if (!parsed.ok || parsed.data.value.type !== 'board.list') {
    throw new BoardContractError(parsed.ok ? invalidBoardPayload('invalid board.list query') : parsed.error);
  }
...
const parseGetQuery = (
  ...
  const requestId = admitBoardRequestId(request, query.requestId);
  const parsed = BoardOperationRequestParserV1.parse({
    protocolVersion: 1, requestId, type: 'board.get', boardId: pathBoardId,
  });
  if (!parsed.ok || parsed.data.value.type !== 'board.get') {
    throw new BoardContractError(parsed.ok ? invalidBoardPayload('invalid board.get query') : parsed.error);
  }
  return {
...
const parseCapabilitiesQuery = (
  ...
  if (!parsed.ok || parsed.data.value.type !== 'capabilities.get') {
    throw new BoardContractError(parsed.ok ? invalidBoardPayload('invalid capabilities.get query') : parsed.error);
  }
...
const parseHistoryListQuery = (
  ...
  const requestId = admitBoardRequestId(request, query.requestId);
  const parsed = BoardOperationRequestParserV1.parse({
    protocolVersion: 1, requestId, type: 'history.list',
    boardId: pathBoardId, cursor: query.cursor ?? null, limit: canonicalLimit(query.limit),
  });
  if (!parsed.ok || parsed.data.value.type !== 'history.list') {
    throw new BoardContractError(parsed.ok ? invalidBoardPayload('invalid history.list query') : parsed.error);
  }
...
const parseHistoryGetQuery = (
  ...
  const requestId = admitBoardRequestId(request, query.requestId);
  const parsed = BoardOperationRequestParserV1.parse({
    protocolVersion: 1, requestId, type: 'history.get',
    boardId: pathBoardId, revisionId: pathRevisionId,
  });
  if (!parsed.ok || parsed.data.value.type !== 'history.get') {
    throw new BoardContractError(parsed.ok ? invalidBoardPayload('invalid history.get query') : parsed.error);
  }
...
export class BoardController {
  ...
  async list(
    ...
    const result = await this.lists.list({
      principal: principal(request, this.actors), request: parseListQuery(request, query),
    });
  ...
  async mutate(
    ...
    const result = parsedRequest.command.type === 'artifact.stop'
      ? await this.artifacts.stop({ principal: actor, request: parsedRequest as ArtifactStopRequestV1 })
      : parsedRequest.command.type === 'hitl.request' || parsedRequest.command.type === 'hitl.respond'
