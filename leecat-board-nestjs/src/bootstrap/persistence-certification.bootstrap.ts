...
export const buildCertificationDispatch = (
  ...
  switch (caller) {
    ...
    case 'db:migrate:adopt':
      if (state.mode !== 'adopt') throw new TypeError('migration adoption certification requires adopt state');
      return {
        ...stateFields,
        stateMode: state.mode,
        caller,
    ...
    case 'quarantine.restore.promote':
      if (state.mode !== 'restart') throw new TypeError('restore promotion certification requires restart state');
      return {
        ...stateFields,
        stateMode: state.mode,
        caller,
    ...
    case 'db:migrate:status':
      if (state.mode !== 'restart') throw new TypeError('migration status certification requires restart state');
      return {
        ...stateFields,
        stateMode: state.mode,
        caller,
    ...
    case 'http-mcp.bootstrap':
      if (state.mode !== 'restart') throw new TypeError('application bootstrap certification requires restart state');
      return {
        ...stateFields,
        stateMode: state.mode,
        caller,
    ...
    case 'db:persistence:scan':
      if (state.mode !== 'restart') throw new TypeError('persistence scan certification requires restart state');
      return {
        ...stateFields,
        stateMode: state.mode,
        caller,
...
const isCorrelatedResult = (
  ...
  && (result.status === 'failed'
    || (result.successAction === dispatch.successAction
      && result.connectionProfile.databaseIdentitySha256 === dispatch.connectionProfile.databaseIdentitySha256
...
export const createMigrationCertificationCallback = (
  service: PersistenceCertificationServiceV1,
): MigrationCliCertificationCallbackV1 => async (
...
if (entry !== undefined && import.meta.url === pathToFileURL(entry).href) {
  runMigrationCertificationProcess(process.argv.slice(2)).then((result) => {
    process.exitCode = result.exitCode;
    const destination = result.status === 'succeeded' ? process.stdout : process.stderr;
    destination.write(result.status === 'succeeded'
    ...
    process.exitCode = 1;
    process.stderr.write('SceneBoard persistence certification failed: MIGRATION_COMMAND_FAILED\n');
  });
}
