...
const failureCategory = (error: unknown): { category: PersistenceCertificationFailureCategoryV1; retryable: boolean } => (
...
export class PersistenceCertificationService implements PersistenceCertificationServiceV1 {
  ...
  constructor(
    ...
    if (this.#probes.size !== PERSISTENCE_PROBE_ORDER_V1.length
  ...
  async certify<Dispatch extends CertificationDispatchV1>(
    ...
    const deadline = dispatch.certificationMode === 'BOUNDED_RESTART'
      ...
      for (const probeId of PERSISTENCE_PROBE_ORDER_V1) {
        ...
        do {
          ...
          const result = await probe.run({
            ...
            scope: dispatch.certificationMode === 'BOUNDED_RESTART' ? 'bounded-canary' : 'complete-keyset',
            cursor,
          ...
          if (performance.now() - startedAt > 15_000) throw new PersistenceProbeFailure('DEADLINE', true);
          if (![result.scannedRows, result.scannedBytes, result.deferredRows].every(safeCount)
            || result.scannedRows > (dispatch.certificationMode === 'BOUNDED_RESTART' ? 200 : rowsFor(probeId))
