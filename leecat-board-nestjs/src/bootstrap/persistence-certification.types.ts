...
export type CertificationDispatchV1 =
  | (CertificationStateFieldsV1<'fresh' | 'restart'> & Readonly<{
    caller: 'db:migrate:up'; certificationMode: 'FULL_OFFLINE';
    successAction: 'CLI_EXIT_0_LISTENERS_STOPPED'; authorizesListener: false;
  }>)
  | (CertificationStateFieldsV1<'adopt'> & Readonly<{
    caller: 'db:migrate:adopt'; certificationMode: 'FULL_OFFLINE';
    successAction: 'CLI_EXIT_0_LISTENERS_STOPPED'; authorizesListener: false;
  }>)
  | (CertificationStateFieldsV1<'restart'> & Readonly<{
    caller: 'quarantine.restore.promote'; certificationMode: 'FULL_OFFLINE';
    successAction: 'WRITE_PROMOTION_EVIDENCE_LISTENERS_STOPPED'; authorizesListener: false;
  }>)
  | (CertificationStateFieldsV1<'restart'> & Readonly<{
    caller: 'db:migrate:status'; certificationMode: 'BOUNDED_RESTART';
    successAction: 'CLI_EXIT_0_BOUNDED_REPORT_ONLY'; authorizesListener: false;
  }>)
  | (CertificationStateFieldsV1<'restart'> & Readonly<{
    caller: 'http-mcp.bootstrap'; certificationMode: 'BOUNDED_RESTART';
    successAction: 'START_LISTENER'; authorizesListener: true;
  }>)
  | (CertificationStateFieldsV1<'restart'> & Readonly<{
    caller: 'db:persistence:scan'; certificationMode: 'RESUMABLE_AUDIT';
    successAction: 'CLI_EXIT_0_OPERATOR_EVIDENCE_ONLY'; authorizesListener: false;
  }>);
...
export type PersistenceCertificationSuccessV1<Dispatch extends CertificationDispatchV1 = CertificationDispatchV1> = Readonly<{
  status: 'succeeded';
...
export type PersistenceCertificationFailureV1<Dispatch extends CertificationDispatchV1 = CertificationDispatchV1> = Readonly<{
  status: 'failed';
...
export type PersistenceCertificationResultV1<Dispatch extends CertificationDispatchV1 = CertificationDispatchV1> =
  | PersistenceCertificationSuccessV1<Dispatch>
  | PersistenceCertificationFailureV1<Dispatch>;
