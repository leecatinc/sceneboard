...
const probes = (observed: string[], failAt?: string): PersistenceCertificationProbeV1[] => (
  PERSISTENCE_PROBE_ORDER_V1.map((probeId) => ({
    ...
    run: async (input) => {
      ...
      return { complete: true, nextCursor: null, scannedRows: 1, scannedBytes: 100, deferredRows: 0 };
    },
  }))
);
...
test('runs the exact bounded probe graph and grants listener authority only to bootstrap', async () => {
  ...
  const service = new PersistenceCertificationService(probes(observed), () => new Date('2026-07-17T00:00:00.000Z'));
  ...
  assert.equal(status.status === 'succeeded' && status.successAction, 'CLI_EXIT_0_BOUNDED_REPORT_ONLY');
});
...
test('denies exposure when an injected service returns an uncorrelated result', async () => {
  ...
  } as never, 'http-mcp.bootstrap', state);
...
test('full scans advance only through valid unique cursors and exceed one batch without a global restart deadline', async () => {
  ...
  const completeProbes = PERSISTENCE_PROBE_ORDER_V1.map((probeId): PersistenceCertificationProbeV1 => ({
    probeId,
    run: async ({ cursor, maxRows, scope }) => {
      ...
      return count === 1
        ? { complete: false, nextCursor: cursor === null ? 'verified-page-1' : `${cursor}-next`, scannedRows: 1, scannedBytes: 1, deferredRows: 0 }
  ...
  const result = await service.certify(buildCertificationDispatch('db:migrate:up', { ...state, mode: 'restart' }));
