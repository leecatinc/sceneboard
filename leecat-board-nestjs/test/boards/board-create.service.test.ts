...
const parseCreate = (requestId: string, title = 'First scene'): CreateRequest => {
  ...
  if (!result.ok || result.data.value.type !== 'board.create') throw new Error('invalid request fixture');
...
const mcpPrincipal = (): Extract<ResolvedBoardPrincipalV1, { kind: 'mcp' }> => ({
  kind: 'mcp',
  actor: parseActor({
    principalKind: 'mcp_client', principalId: 'client_1', grantId: 'grant_1',
    scopes: ['board.read', 'board.write'],
  }),
  ownerUserPk: 20n,
  grantPk: 30n,
...
test('creates an initial empty head and canonical result with zero user binding calls', async () => {
  const value = setup();
  const result = await value.service.create({ principal: userPrincipal(), request: parseCreate('request_1') });
...
test('binds a newly inserted MCP board exactly once before revision 1', async () => {
  ...
  const revisionInsert = value.calls.findIndex((call) => call.startsWith('INSERT INTO board_revisions'));
...
test('replays one validated stored create with a fresh request ID and no domain write', async () => {
  const value = setup();
  const first = await value.service.create({ principal: userPrincipal(), request: parseCreate('request_1') });
  const replay = await value.service.create({ principal: userPrincipal(), request: parseCreate('request_2') });
  ...
  assert.equal(value.calls.filter((call) => call.startsWith('INSERT INTO board_revisions')).length, 1);
  assert.equal(value.calls.filter((call) => call.startsWith('INSERT INTO board_event_outbox')).length, 1);
});
...
test('classifies a changed create title without touching board state', async () => {
  ...
  await assert.rejects(
    value.service.create({ principal: userPrincipal(), request: parseCreate('request_2', 'Different title') }),
