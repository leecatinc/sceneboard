...
import type { BoardId, GrantId, TimestampV1 } from '@leecat-board/board-schema';
...
test('board cursor binds archive filter and opaque owner/grant access context', () => {
  ...
  assert.throws(() => codec.parse({ cursor: changed, includeArchived: false, access: owner }), invalidCursor);
});
