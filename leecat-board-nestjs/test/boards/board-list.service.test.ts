...
const request = (): BoardListRequestV1 => {
  ...
  assert.equal(parsed.ok, true);
  if (!parsed.ok || parsed.data.value.type !== 'board.list') throw new Error('invalid request fixture');
...
const row = (boardPk: string, createdSecond: number, uuidByte: string) => ({
  ...
  archivedAt: null,
  headRevisionId: Buffer.from(`${uuidByte.repeat(12)}4${uuidByte.repeat(3)}8${uuidByte.repeat(15)}`, 'hex'),
...
const setup = (kind: 'owner' | 'grant') => {
  ...
  const principal: ResolvedBoardPrincipalV1 = kind === 'owner' ? {
    kind: 'user',
    actor: actor('user'),
    userPk: 20n,
    sessionPk: 21n,
...
test('owner board page uses one narrow limit+1 projection and cursors from the last returned row', async () => {
  ...
  assert.deepEqual(value.cursors.parse({
