...
test('commits request creation with one event and a service-owned fifteen-minute expiry', async () => {
  ...
  let expiresAt = '';
  const access = {
    withAuthorizedBoardTransaction: async (_input: unknown, apply: (connection: never, value: never) => Promise<unknown>) => {
  ...
  const mutations = {
    begin: async () => { calls.push('idempotency'); return { kind: 'new', recordPk: 1n }; },
    lockHead: async () => {
      calls.push('head');
      return {
        boardPk: '1', headRevisionPk: '2', headRevisionId: 'revision_1',
        headRevisionNumber: 1, lastEventSequence: 0,
      };
    },
    allocateSequence: async () => { calls.push('sequence'); return 1; },
    appendEvent: async () => { calls.push('event'); },
    complete: async (_connection: unknown, _pk: unknown, _head: unknown, _prepared: unknown, value: unknown) => {
      calls.push('complete');
      return value;
    },
  };
  const service = new InteractionCommandService(
    ...
    mutations as never,
    { expireLocked: async () => { throw new Error('unexpected expiry'); } } as never,
    new HitlWaitCoordinator(),
    { writeSuccess: async () => { calls.push('audit'); } } as never,
  ...
  assert.deepEqual(calls, [
    'authorized', 'idempotency', 'head', 'sequence', 'interaction', 'event', 'complete', 'audit',
  ]);
...
test('rejects destructive positive confirmation from MCP before sequence or response persistence', async () => {
  ...
  const stored = {
    ...
    interaction: {
      hitlRequestId: 'hitl_1',
      definition: {
        kind: 'confirmation', title: 'Delete', body: 'Irreversible', impact: 'destructive',
  ...
  const interactions = {
    ...
    answer: async () => { answerCalls += 1; },
  };
  const mutations = {
    begin: async () => ({ kind: 'new', recordPk: 1n }),
    lockHead: async () => ({
      boardPk: '1', headRevisionPk: '2', headRevisionId: 'revision_1',
      headRevisionNumber: 1, lastEventSequence: 1,
    }),
    allocateSequence: async () => { sequenceCalls += 1; return 2; },
  };
  const service = new InteractionCommandService(
    access as never,
    interactions as never,
...
test('commits authoritative expiry and deletes pending respond idempotency before returning 410', async () => {
  ...
  const mutations = {
    begin: async () => { calls.push('idempotency'); return { kind: 'new', recordPk: 9n }; },
    lockHead: async () => {
      calls.push('head');
      return {
        boardPk: '1', headRevisionPk: '2', headRevisionId: 'revision_1',
  ...
  const expiry = {
    expireLocked: async (_connection: unknown, input: { stored: typeof stored }) => {
      ...
      calls.push('expiry');
      return { ...stored.interaction, state: 'expired', stateUpdatedAt: stored.interaction.expiresAt };
    },
  };
  const service = new InteractionCommandService(
    access as never,
