...
import { parseHitlLifecycleBody, parseHitlReadQuery } from '../../src/interactions/interaction-http.dto.js';
...
test('admits only the exact HITL read wait query pair and bounded timeout', () => {
  ...
  assert.throws(() => parseHitlReadQuery({
    query: { requestId: 'request_1', timeoutMs: '1' },
    requestId: 'request_1', boardId: 'board_1', hitlRequestId: 'hitl_1',
  }));
  assert.throws(() => parseHitlReadQuery({
...
test('keeps lifecycle adapters strict and validates the successor through D1 identifiers', () => {
  ...
  assert.throws(() => parseHitlLifecycleBody({
    body: {
      protocolVersion: 1,
      requestId: 'request_1',
      expectedRevisionId: 'revision_1',
