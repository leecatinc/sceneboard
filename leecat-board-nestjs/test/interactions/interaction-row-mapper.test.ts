...
import { mapInteractionRowV1, type InteractionRowV1 } from '../../src/interactions/persistence/interaction-row.mapper.js';
...
test('fails closed on digest, actor, sequence, and expired-clock drift', () => {
  ...
  assert.throws(() => mapInteractionRowV1({ ...row(), createdByKind: 'M', createdByGrantId: null }));
  assert.throws(() => mapInteractionRowV1({ ...row(), stateEventSequence: '5' }));
  assert.throws(() => mapInteractionRowV1({
    ...row(),
    stateCode: 'E',
