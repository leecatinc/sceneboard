...
const setup = (row: Record<string, unknown>, boardIds = ['board_1']) => {
  ...
  const mysql = { async withConnection<T>(work: (value: typeof connection) => Promise<T>) { return work(connection); } } as unknown as MysqlService;
  const audits: Array<Record<string, unknown>> = [];
  const audit = {
    async writeMandatory(_transaction: unknown, input: Record<string, unknown>) { audits.push(input); },
...
test('lazily expires a verified overdue credential and grant with mandatory audit', async () => {
  ...
  assert.equal(value.calls.some((call) => call.startsWith('UPDATE mcp_grant_credentials')), true);
  assert.equal(value.calls.some((call) => call.startsWith('UPDATE mcp_grants')), true);
