...
const setup = (options: SetupOptions = {}) => {
  ...
  const connection = {
    ...
    async execute(sql: string) {
      ...
      if (normalized.includes('FROM auth_sessions') && normalized.includes('FOR UPDATE')) {
        return [[{
          sessionPk: '21', userPk: '20', familyPublicId: 'family_1', status: 1,
          idleExpiresAt: '2026-07-17 00:00:00.000', absoluteExpiresAt: '2026-07-18 00:00:00.000',
        }], []] as QueryResult;
      }
      if (normalized.includes('FROM mcp_grants') && normalized.includes('FOR UPDATE')) {
        return [[{
          grantPk: '30', publicId: 'grant_1', ownerUserPk: '20', sourceSessionPk: null,
          scopeMask: options.grantScopeMask ?? 3, lifecycleMask: options.grantLifecycleMask ?? 1,
          lifetime: 2, status: 2, expiresAt: '2026-07-17 00:00:00.000',
        }], []] as QueryResult;
      }
  ...
  return {
    policy: new MysqlBoardAccessPolicy(mysql, crypto, { retryJitter: () => 0, sleep: async () => undefined }),
    calls,
    connectionCount: () => connectionCount,
  };
};

const isBoardError = (code: string) => (error: unknown): boolean => (
...
test('rejects operation target and isolation drift before acquiring a connection', async () => {
  const value = setup();
  await assert.rejects(value.policy.withAuthorizedBoardTransaction({
    principal: userPrincipal(), operation: 'board.list', boardId: null,
    isolation: 'READ_COMMITTED_WRITE',
  }, async () => 'unreachable'), isBoardError('FORBIDDEN'));
...
test('revalidates the current user family and commits before exposing board work', async () => {
  ...
  const result = await value.policy.withAuthorizedBoardTransaction({
    principal: userPrincipal(), operation: 'board.get', boardId: boardId('board_1'),
  ...
  assert.ok(value.calls.indexOf('COMMIT') > value.calls.findIndex((call) => call.includes('FROM boards b')));
...
test('gives MCP board.create one same-connection binding call and closes it after apply', async () => {
  ...
  await value.policy.withAuthorizedBoardTransaction({
    principal: mcpPrincipal(), operation: 'board.create', boardId: null,
    isolation: 'READ_COMMITTED_WRITE',
  }, async (_connection, context) => {
  ...
  assert.equal(value.calls.filter((call) => call.startsWith('INSERT INTO mcp_grant_boards')).length, 1);
  assert.ok(captured);
...
test('denies stale MCP scope before board binding or protected apply work', async () => {
  ...
  let applied = false;
  await assert.rejects(value.policy.withAuthorizedBoardTransaction({
    principal: mcpPrincipal(['board.read']), operation: 'board.create', boardId: null,
    isolation: 'READ_COMMITTED_WRITE',
  }, async () => { applied = true; }), isBoardError('FORBIDDEN'));
  assert.equal(applied, false);
  assert.equal(value.calls.some((call) => call.includes('FROM mcp_grant_boards')), false);
});
...
test('returns the archived conflict only after current owner authorization', async () => {
  ...
  await assert.rejects(value.policy.withAuthorizedBoardTransaction({
    ...
    isolation: 'READ_COMMITTED_WRITE',
  }, async () => { applied = true; }), (error: unknown) => (
  ...
  assert.equal(applied, false);
  assert.equal(value.calls.some((call) => call.includes('FROM boards b')), true);
});
