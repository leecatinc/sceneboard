...
test('grant list uses one page query plus one batched binding query and projects crossed expiry without mutation', async () => {
  ...
  const mysql = {
    async withConnection<Value>(operation: (value: PoolConnection) => Promise<Value>) { return operation(connection); },
  } as MysqlService;
  ...
  assert.equal(calls.some((call) => call.startsWith('UPDATE') || call.startsWith('INSERT')), false);
