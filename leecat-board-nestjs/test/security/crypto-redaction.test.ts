...
test('redacts every case-insensitive audit key class from the D2 contract', () => {
  const input = Object.fromEntries([
    'password', 'authorization', 'cookie', 'set-cookie', 'token', 'secret',
    'credential', 'otp', 'code', 'proof', 'challenge', 'csrf', 'hash',
  ].map((key) => [key.toUpperCase(), 'canary']));
  assert.deepEqual(redactSecrets(input), Object.fromEntries(Object.keys(input).map((key) => [key, '[REDACTED]'])));
});
