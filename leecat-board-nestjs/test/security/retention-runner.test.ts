...
test('pins every retention cursor to a bounded supporting index', () => {
  ...
  assert.equal(SECURITY_RETENTION_TARGETS.every((target) => target.batchLimit === 500), true);
  assert.equal(SECURITY_RETENTION_TARGETS.every((target) => target.indexName.startsWith('ix_')), true);
  assert.equal(SECURITY_RETENTION_TARGETS.some((target) => target.name === 'pairing-approved-expiry'), true);
  assert.equal(SECURITY_RETENTION_TARGETS.some((target) => target.name === 'security-audit-delete'), true);
  const expiredSession = SECURITY_RETENTION_TARGETS.find((target) => target.name === 'session-expired-delete');
  ...
  const expiredGrant = SECURITY_RETENTION_TARGETS.find((target) => target.name === 'grant-expired-delete');
...
test('status reports exact due counts and oldest UTC timestamps without a bounded selector', async () => {
  ...
  const mysql = {
    async withConnection<T>(work: (value: typeof connection) => Promise<T>) { return work(connection); },
  ...
  assert.equal(calls.every((sql) => /COUNT\(\*\) AS dueCount/.test(sql)), true);
  assert.equal(calls.every((sql) => /MIN\(/.test(sql) && !/\bLIMIT\b/i.test(sql)), true);
});
...
test('dry-run uses the same target selectors and performs no update or delete', async () => {
  ...
  assert.equal(calls.some((sql) => /^\s*(?:UPDATE|DELETE)\b/i.test(sql)), false);
});
...
test('terminal deletion rechecks state and cutoff inside the committed batch', async () => {
  ...
  const connection = {
    ...
    async execute(sql: string, parameters: readonly unknown[] = []) {
      ...
      if (sql.includes('FROM pairing_requests FORCE INDEX (ix_pairing_state_updated)') && !selected) {
        selected = true;
        return [[{ id: '41' }], []];
      }
  ...
  const report = await new SecurityRetentionService(mysql, audit).execute('run', Date.parse('2026-07-16T12:00:00.000Z'));
...
test('approved pairing expiry preserves family-before-pair-before-grant order and clears secrets', async () => {
  ...
  const connection = {
    ...
    async execute(sql: string) {
      calls.push(sql);
      if (sql.includes('FROM pairing_requests FORCE INDEX (ix_pairing_state_redeem_expiry)') && !selected) {
        selected = true;
        return [[{ id: '42' }], []];
      }
      if (sql.includes('SELECT s.family_public_id AS familyPublicId')) return [[{ familyPublicId: 'family-1' }], []];
      if (sql.includes('SELECT CAST(p.id AS CHAR) AS id')) return [[{
        id: '42', state: 3, publicId: 'pairing-1', ownerUserPublicId: 'user-1',
        sourceSessionPublicId: 'session-1', lockedFamilyPublicId: 'family-1',
        clientPublicId: 'client-1', grantDatabaseId: '43', due: 1,
      }], []];
  ...
  const events: string[] = [];
  const audit = {
    async writeMandatory(_transaction: unknown, input: { event: string }) { events.push(input.event); },
  } as unknown as AuditRepository;
  const report = await new SecurityRetentionService(mysql, audit).execute('run', Date.parse('2026-07-16T12:00:00.000Z'));
  const familyLock = calls.findIndex((sql) => sql.includes('FROM auth_sessions WHERE family_public_id'));
  const pairingLock = calls.findIndex((sql) => sql.includes('SELECT CAST(p.id AS CHAR) AS id'));
  const grantLock = calls.findIndex((sql) => sql.includes('SELECT public_id AS publicId, status FROM mcp_grants'));
