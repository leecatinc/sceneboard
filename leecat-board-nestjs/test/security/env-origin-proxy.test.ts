...
test('parses the exact development environment and canonical same-host origins', () => {
  ...
  assert.equal(environment.mysql.database, 'leecat_board');
  assert.equal(environment.redis.keyPrefix, 'leecat_board:');
...
test('parses persistence settings without requiring unrelated application secrets', () => {
  ...
  const persistence = parsePersistenceEnvironment(Object.fromEntries(
    Object.entries(input).filter(([keyName]) => keyName.startsWith('MYSQL_') || keyName.startsWith('REDIS_')),
  ));
  assert.equal(persistence.mysql.database, 'leecat_board');
  assert.equal(persistence.redis.keyPrefix, 'leecat_board:');
});
