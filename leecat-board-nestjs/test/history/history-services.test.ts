...
import { HistoryGetService, type HistoryGetRequestV1 } from '../../src/history/history-get.service.js';
import { HistoryListService, type HistoryListRequestV1 } from '../../src/history/history-list.service.js';
...
const parseList = (): HistoryListRequestV1 => {
  const parsed = BoardOperationRequestParserV1.parse({
    protocolVersion: 1, requestId: 'request_history_list_1', type: 'history.list',
    boardId, cursor: null, limit: 2,
  });
  assert.equal(parsed.ok, true);
  if (!parsed.ok || parsed.data.value.type !== 'history.list') throw new Error('invalid list fixture');
  return parsed.data.value as HistoryListRequestV1;
};

const parseGet = (): HistoryGetRequestV1 => {
  const parsed = BoardOperationRequestParserV1.parse({
    protocolVersion: 1, requestId: 'request_history_get_1', type: 'history.get',
    boardId, revisionId: revisions[0],
  });
  assert.equal(parsed.ok, true);
  if (!parsed.ok || parsed.data.value.type !== 'history.get') throw new Error('invalid get fixture');
...
test('history list uses one narrow newest-first page and cursors from the last returned revision', async () => {
  ...
  const connection = {
    async execute(sql: string, binds: unknown[]): Promise<[unknown, unknown]> {
      ...
      return [[
        {
          revisionId: bytes(revisions[2]), revisionNumber: '3',
          revisionCreatedAt: '2026-07-16 12:00:03.000',
          previousRevisionId: bytes(revisions[1]), sourceRevisionId: null,
          originCode: 'L', actorKind: 'U', actorPrincipalId: 'user_1', label: 'Cleared',
        },
        {
          revisionId: bytes(revisions[1]), revisionNumber: '2',
          revisionCreatedAt: '2026-07-16 12:00:02.000',
          previousRevisionId: bytes(revisions[0]), sourceRevisionId: null,
          originCode: 'R', actorKind: 'U', actorPrincipalId: 'user_1', label: 'Updated',
        },
        {
          revisionId: bytes(revisions[0]), revisionNumber: '1',
          revisionCreatedAt: '2026-07-16 12:00:01.000',
          previousRevisionId: null, sourceRevisionId: null,
          originCode: 'C', actorKind: 'U', actorPrincipalId: 'user_1', label: 'Created',
        },
      ], []];
    },
  ...
  const response = await new HistoryListService(policy, cursors).listWithMetadata({
    principal: principal(), request: parseList(),
  });
  ...
  assert.deepEqual(
    result.result.entries.map((entry: { revision: { revisionNumber: number } }) => entry.revision.revisionNumber),
    [3, 2],
  );
...
test('history get composes an immutable selected scene with current response-cut capabilities and watermark', async () => {
  ...
  const connection = {
    async execute(sql: string): Promise<[unknown, unknown]> {
      ...
      if (normalized.includes('JOIN board_revisions latest')) {
        return [[{
          revisionPk: '65', revisionId: bytes(revisions[0]), revisionNumber: '1',
          revisionCreatedAt: '2026-07-16 12:00:01.000',
          previousRevisionId: null, sourceRevisionId: null,
          originCode: 'C', actorKind: 'U', actorPrincipalId: 'user_1',
          label: 'Created', nextRevisionId: bytes(revisions[1]), latestRevisionId: bytes(revisions[2]),
          sceneSchemaVersion: checkpoint.schemaVersion, sceneCodec: checkpoint.codec,
          scenePayload: checkpoint.payload, sceneCanonicalBytes: checkpoint.canonicalBytes,
          sceneStoredBytes: checkpoint.storedBytes, sceneSha256: checkpoint.sha256,
          lastEventSequence: '3',
        }], []];
      }
  ...
  const response = await new HistoryGetService(
    policy, new SceneCheckpointCodec(), snapshots,
