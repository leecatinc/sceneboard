...
test('stream admission accepts only the exact browser fetch shape', () => {
  ...
  assert.deepEqual((valid as typeof valid & { boardStreamAdmission: unknown }).boardStreamAdmission, {
    boardId: 'board_1',
  ...
  for (const mutate of [
    (value: ReturnType<typeof request>) => { value.headers.accept = 'application/json'; },
    (value: ReturnType<typeof request>) => { Object.assign(value.headers, { authorization: 'Bearer secret' }); },
    (value: ReturnType<typeof request>) => { value.query = { ...value.query, extra: 'x' } as never; },
    (value: ReturnType<typeof request>) => { value.query = { ...value.query, tabId: ['a', 'b'] } as never; },
    (value: ReturnType<typeof request>) => { value.headers['last-event-id'] = 'bad\nvalue'; },
    (value: ReturnType<typeof request>) => { value.headers['content-length'] = '1'; },
  ]) {
