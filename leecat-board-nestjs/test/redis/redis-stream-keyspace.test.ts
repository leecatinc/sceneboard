...
test('derives deterministic purpose-separated fingerprints and exact physical names', () => {
  ...
  assert.equal(keyspace.boardHintChannel(boardId), `leecat_board:stream:events:v1:${board}`);
  assert.equal(keyspace.eventLeaseKey(eventId), `leecat_board:stream:dispatch:v1:${event}`);
  assert.equal(
    keyspace.presenceConnectionKey(boardId, 1n, 'AAAAAAAAAAAAAAAAAAAAAA' as never),
    `leecat_board:stream:presence:v1:${board}:${principal}:AAAAAAAAAAAAAAAAAAAAAA`,
  );
  assert.equal(keyspace.presenceIndexKey(boardId), `leecat_board:stream:presence-index:v1:${board}`);
  assert.equal(keyspace.presenceConcurrencyKey(boardId, 1n), `leecat_board:stream:concurrency:v1:${board}:${principal}`);
});
