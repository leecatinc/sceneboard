...
test('family terminalization locks sessions, pairings, grants, and credentials before one audited cascade', async () => {
  ...
  const connection = {
    ...
    async execute(sql: string) {
      ...
      if (statement.startsWith('SELECT CAST(p.id AS CHAR)')) return [[{ id: '20', grantId: '30' } as RowDataPacket], []];
  ...
  const audit = {
    async writeMandatory() { calls.push('AUDIT'); },
  ...
  assert.deepEqual(await repository.terminalizeFamily(record, 'reuse', 1_800_000_000_000), { kind: 'committed' });
  const indexes = {
    sessions: calls.findIndex((call) => call.includes('FROM auth_sessions') && call.includes('FOR UPDATE')),
    pairings: calls.findIndex((call) => call.includes('FROM pairing_requests p') && call.includes('FOR UPDATE')),
    grants: calls.findIndex((call) => call.includes('FROM mcp_grants g') && call.includes('FOR UPDATE')),
  ...
  assert.ok(indexes.sessions < indexes.pairings && indexes.pairings < indexes.grants && indexes.grants < indexes.credentials);
