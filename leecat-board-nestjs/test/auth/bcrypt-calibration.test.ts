...
test('requires exact warmup/sample floors and withholds cost selection without a capacity budget', async () => {
  ...
  let compares = 0;
  const adapter = {
    async hash(_password: string, cost: number) { hashes += 1; return `hash-${cost}`; },
    async compare(_password: string, hash: string) { compares += 1; return hash === 'hash-10'; },
  };
  ...
  await assert.rejects(() => calibrateBcrypt(adapter, { costs: [10], warmupsPerCost: 2 }), /three warmups/);
  await assert.rejects(() => calibrateBcrypt(adapter, { costs: [10], samplesPerKind: 19 }), /twenty samples/);
});
