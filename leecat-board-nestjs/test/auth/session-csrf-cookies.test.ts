...
test('issues and verifies one opaque session credential without persisting raw material', () => {
  ...
  for (const value of ['', 'lcbs_v1.bad.bad', `${issued.token}.extra`]) assert.throws(() => service.parse(value));
});
...
test('separates anonymous/session CSRF domains and binds sessions to one family', () => {
  ...
  assert.match(anonymous.token, /^lcbcsrf_v1\.a\.-\.[A-Za-z0-9_-]{22}\.[0-9]{13}\.[A-Za-z0-9_-]{43}$/);
  assert.equal(service.verify(anonymous.token, { kind: 'anonymous', now }), true);
  assert.equal(service.verify(anonymous.token, { kind: 'session', familyPublicId: 'family_1', now }), false);

  const session = service.issueSession('family_1', now, now + 60_000);
  assert.match(session.token, /^lcbcsrf_v1\.s\.[A-Za-z0-9_-]{22}\.[A-Za-z0-9_-]{22}\.[0-9]{13}\.[A-Za-z0-9_-]{43}$/);
  assert.equal(service.verify(session.token, { kind: 'session', familyPublicId: 'family_1', now }), true);
  assert.equal(service.verify(session.token, { kind: 'session', familyPublicId: 'family_2', now }), false);
  assert.equal(service.verify(session.token, { kind: 'session', familyPublicId: 'family_1', now: now + 60_001 }), false);
...
test('serializes environment-specific host-only cookie contracts exactly', () => {
  ...
  assert.equal(
    development.csrf('csrf', 60),
    'lcb_csrf=csrf; Max-Age=60; Path=/; SameSite=Lax',
  );
