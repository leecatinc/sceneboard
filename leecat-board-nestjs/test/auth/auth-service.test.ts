...
import { AuthService, type AuthPersistence, type PasswordPort } from '../../src/auth/auth.service.js';
...
test('signup commits only hashes and returns the session credential out-of-band from JSON', async () => {
  ...
  const persistence: AuthPersistence = {
    ...
    async commitLogin() { throw new Error('unexpected login'); },
  };
...
test('signup maps only normalized-email conflicts to AUTH_EMAIL_IN_USE', async () => {
  const crypto = makeCrypto();
  const persistence: AuthPersistence = {
    async createUserWithSession() { return { kind: 'email_conflict' }; },
    async findLoginCandidate() { return null; },
    async commitLogin() { throw new Error('unexpected login'); },
  };
  const service = new AuthService(persistence, new FakePasswords(), new SessionTokenService(crypto), new CsrfService(crypto), crypto);
  await assert.rejects(() => service.signup(credentials, 1_800_000_000_000), (error) => error instanceof AppError && error.code === 'AUTH_EMAIL_IN_USE');
});
...
test('unknown and disabled login use dummy/real hashes but converge to one padded error', async () => {
  ...
  const persistence: AuthPersistence = {
    async createUserWithSession() { throw new Error('unexpected signup'); },
    async findLoginCandidate() { return candidates.shift() ?? null; },
    async commitLogin() { throw new Error('must not commit invalid login'); },
  };
  const service = new AuthService(persistence, passwords, new SessionTokenService(crypto), new CsrfService(crypto), crypto);

  for (let index = 0; index < 2; index += 1) {
    await assert.rejects(() => service.login(credentials, 1_800_000_000_000 + index), (error) => error instanceof AppError && error.code === 'AUTH_INVALID_CREDENTIALS');
  }
  assert.deepEqual(passwords.verified.map(({ hash }) => hash), ['dummy-hash', `hash:${credentials.password}`]);
