...
const setup = (status: SessionRecord['status'] = 'active', overrides: Partial<SessionRecord> = {}) => {
  let random = 1;
  ...
  const persistence: SessionPersistence = {
    async findByLocator() { return record; },
    async terminalizeFamily(_record, reason) { cascades.push(reason); return { kind: 'committed' }; },
    async rotate() { throw new Error('unexpected rotation'); },
    async observeLogout() { throw new Error('unexpected logout observation'); },
  };
...
test('resolves an active credential from MySQL and reissues only a family-bound CSRF token', async () => {
  const value = setup();
  const service = new SessionService(value.persistence, value.tokens, new CsrfService(value.crypto), value.crypto);
...
test('never promotes malformed, unknown, or bad-HMAC credentials to destructive reuse', async () => {
  ...
  const persistence: SessionPersistence = {
    ...value.persistence,
    async findByLocator(locator) { return locator.equals(value.issued.locator) ? value.record : null; },
  };
  const service = new SessionService(persistence, value.tokens, new CsrfService(value.crypto), value.crypto);
  for (const token of ['malformed', value.issued.token.replace(/.$/, 'A')]) {
    await assert.rejects(() => service.resolveExclusive(token, undefined, 1_800_000_000_000), (error) => error instanceof SessionResolutionError && error.code === 'UNAUTHENTICATED' && error.clearCookies);
  }
  assert.deepEqual(value.cascades, []);
});
...
test('verified rotated, expired, and disabled rows commit the mandatory family cascade first', async () => {
  const cases = [
    { status: 'rotated' as const, override: {}, reason: 'reuse' as const, code: 'AUTH_SESSION_REUSED' },
    { status: 'active' as const, override: { idleExpiresAt: 1_799_999_999_999 }, reason: 'expired' as const, code: 'AUTH_SESSION_EXPIRED' },
    { status: 'active' as const, override: { user: { databaseId: '2', publicId: 'user_1', email: 'User@Example.dev', status: 'disabled' as const, createdAt: '2026-07-16T00:00:00.000Z' } }, reason: 'disabled' as const, code: 'AUTH_SESSION_REVOKED' },
  ];
  for (const item of cases) {
    const value = setup(item.status, item.override);
    const service = new SessionService(value.persistence, value.tokens, new CsrfService(value.crypto), value.crypto);
    await assert.rejects(() => service.resolveExclusive(value.issued.token, undefined, 1_800_000_000_000), (error) => error instanceof SessionResolutionError && error.code === item.code && error.clearCookies);
...
test('mandatory cascade failure returns retryable 503 and preserves cookies', async () => {
  ...
  const service = new SessionService(persistence, value.tokens, new CsrfService(value.crypto), value.crypto);
  await assert.rejects(() => service.resolveExclusive(value.issued.token, undefined, 1_800_000_000_000), (error) => error instanceof AppError && error.code === 'SERVICE_UNAVAILABLE' && (!(error instanceof SessionResolutionError) || !error.clearCookies));
});
...
test('renews by rotating the verified row and returns a different opaque credential and CSRF generation', async () => {
  ...
  const currentCsrf = csrf.issueSession(value.record.familyPublicId, 1_800_000_000_000, value.record.idleExpiresAt).token;
...
test('renewal consumes the resolved session bucket after valid CSRF and before rotation', async () => {
  ...
  const persistence: SessionPersistence = {
    ...value.persistence,
    async rotate() { order.push('rotate'); return { kind: 'created' }; },
  };
  const limiter = {
    async consume(input: { surface: string; purpose: string; identity: string; limit: number; windowMs: number }) {
      order.push('limit');
...
test('renewal does not consume the resolved-session bucket for invalid CSRF', async () => {
  ...
  let consumed = false;
  const limiter = {
    async consume(_input: Parameters<RateLimitService['consume']>[0]) { consumed = true; },
  } as RateLimitService;
...
test('a renewal loser terminalizes verified old-token reuse before returning AUTH_SESSION_REUSED', async () => {
  ...
  const persistence: SessionPersistence = {
    ...value.persistence,
    async rotate() { return { kind: 'already_rotated' }; },
  };
  const csrf = new CsrfService(value.crypto);
  const currentCsrf = csrf.issueSession(value.record.familyPublicId, 1_800_000_000_000, value.record.idleExpiresAt).token;
  const service = new SessionService(persistence, value.tokens, csrf, value.crypto);
  await assert.rejects(() => service.renew(value.issued.token, currentCsrf, 1_800_000_000_000), (error) => error instanceof SessionResolutionError && error.code === 'AUTH_SESSION_REUSED' && error.clearCookies);
