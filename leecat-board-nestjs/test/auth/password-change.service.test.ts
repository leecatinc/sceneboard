...
class FakePasswords implements PasswordPort {
  ...
  dummyHash(): string { return 'hash:dummy-password'; }
  async padFailure(startedAt: number): Promise<void> { this.padded.push(startedAt); }
}
...
test('changes a verified password and binds the commit to the current session family', async () => {
  ...
  const persistence: PasswordChangePersistence = {
    async findCandidate() { return { passwordHash: 'hash:current-password', status: 'active' }; },
    async commit(input) {
      commits.push(input);
...
test('rejects and pads an incorrect current password without writing', async () => {
  ...
  const persistence: PasswordChangePersistence = {
    async findCandidate() { return { passwordHash: 'hash:current-password', status: 'active' }; },
    async commit() { commits += 1; return { kind: 'changed', otherSessionFamiliesRevoked: 0 }; },
  };
  const service = new PasswordChangeService(persistence, passwords);

  await assert.rejects(
    () => service.change(session, { currentPassword: 'wrong-password', newPassword: 'replacement-password' }, 1_800_000_000_000),
...
test('requires a different policy-compliant new password', async () => {
  ...
  await assert.rejects(
    () => service.change(session, { currentPassword: 'current-password', newPassword: 'current-password' }, 1_800_000_000_000),
    (error) => error instanceof AppError && error.code === 'AUTH_PASSWORD_UNCHANGED',
  );
  await assert.rejects(
    () => service.change(session, { currentPassword: 'current-password', newPassword: 'short' }, 1_800_000_000_000),
