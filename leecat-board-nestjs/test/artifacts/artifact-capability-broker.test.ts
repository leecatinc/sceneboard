...
test('keeps network fetch default-denied after current authorization and exact-pair certification', async () => {
  const calls: string[] = [];
  const policy = {
    withAuthorizedBoardTransaction: async (_input: unknown, apply: (connection: never, context: never) => Promise<unknown>) => {
  ...
  const broker = new ArtifactCapabilityBrokerService(
    policy as never, artifacts as never, { write: async () => undefined } as never,
  );
  await assert.rejects(broker.networkFetch({
    principal: {} as never,
    boardId: 'board_1' as never,
    artifact: { artifactId: 'artifact_1', versionId: 'version_1' } as never,
    request,
  }), (error: unknown) => (
    error instanceof ArtifactBrokerError && error.code === 'POLICY_DENIED'
  ));
...
test('rejects malformed, credentialed, non-HTTPS, and fragmented broker requests without I/O', async () => {
  assert.throws(() => parseArtifactNetworkFetchRequestV1({
    ...request, method: 'POST',
  }), ArtifactBrokerError);
  let authorizationCalls = 0;
  const broker = new ArtifactCapabilityBrokerService({
    withAuthorizedBoardTransaction: async () => { authorizationCalls += 1; },
  } as never, {} as never, { write: async () => undefined } as never);
  for (const url of [
...
test('package transport rejects an MCP principal before reading package bytes', async () => {
  ...
  const controller = new ArtifactController({
    getPackage: async () => { packageReads += 1; return Buffer.alloc(0); },
  } as never, {} as never, {
    resolveUser: () => { throw new Error('not a browser session'); },
  } as never);
  await assert.rejects(controller.getPackage({
    headers: { authorization: 'Bearer hidden' },
    boardPrincipal: { kind: 'mcp' },
  } as never, 'board_1', 'artifact_1', 'version_1', {}, {} as never), (
    error: unknown,
