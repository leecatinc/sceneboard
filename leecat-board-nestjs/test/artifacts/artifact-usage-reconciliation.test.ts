...
test('reconciles immutable artifact aggregates with bounded keyset reads and performs no repair', async () => {
  ...
  const responses = [
    [[{
      artifactCount: '1', versionCount: '1', resourceCount: '2',
      manifestCanonicalBytes: '100', resourceBytes: '30',
    }], []],
    [[{ artifactPk: '10' }], []],
    [[{ versionPk: '20', manifestCanonicalBytes: 100, resourceCount: 2, resourceTotalBytes: 30 }], []],
    [[{ resourcePk: '30', resourceBytes: 10 }, { resourcePk: '31', resourceBytes: 20 }], []],
  ];
  const connection = {
  ...
  const report = await new ArtifactUsageReconciliation().inspectBoard(connection as never, '1', 500);
  ...
  assert.equal(sql.every((statement) => !/\b(?:UPDATE|DELETE|INSERT)\b/iu.test(statement)), true);
  assert.equal(sql.slice(1).every((statement) => /ORDER BY/iu.test(statement) && /LIMIT 500/u.test(statement)), true);
});
