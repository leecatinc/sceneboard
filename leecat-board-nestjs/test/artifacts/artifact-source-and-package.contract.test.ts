...
test('requires the exact eight source keys, explicit nulls, and sorted unique capabilities', () => {
  ...
  assert.equal(BoardArtifactPutSourceV1Parser.parse({
    ...source(), requestedCapabilities: ['fullscreen', 'clipboard.write'],
  }).ok, false);
  assert.equal(BoardArtifactPutSourceV1Parser.parse({
    ...source(), requestedCapabilities: ['fullscreen', 'fullscreen'],
  }).ok, false);
});
...
test('builds deterministic one-to-three resource manifests and LCARTV1 packages', () => {
  ...
  assert.deepEqual(first.manifest.resources.map((item) => [item.path, item.mediaType]), [
  ...
  const minimal = normalizer.normalize({
    principal,
    source: BoardArtifactPutSourceV1Parser.parseOrThrow({ ...source(), css: null, javascript: null }),
  });
