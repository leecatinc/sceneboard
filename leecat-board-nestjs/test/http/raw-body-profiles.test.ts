...
import {
  matchRawBodyProfile,
  parseProfiledBody,
} from '../../src/common/http/raw-body-profiles.js';
...
test('selects static D2 and D1 profiles from method plus canonical pathname only', () => {
  ...
  assert.equal(matchRawBodyProfile('POST', '/api/v1/boards/board_1/mutations')?.kind, 'd1-contract-body');
  assert.equal(
  ...
  assert.equal(matchRawBodyProfile('POST', '/api/v1/boards/board_1/title')?.kind, 'd2-rest-json-body');
  assert.equal(matchRawBodyProfile('POST', '/api/v1/boards/board_1/artifacts')?.kind, 'd7-artifact-source-body');
  assert.equal(
    matchRawBodyProfile('GET', '/api/v1/boards/board_1/artifacts/artifact_1/versions/version_1/package')?.kind,
    'd1-no-body',
  );
  assert.equal(
    matchRawBodyProfile('POST', '/api/v1/boards/board_1/artifacts/artifact_1/versions/version_1/capability-requests/network-fetch')?.kind,
...
test('retains the strict D7 network body and rejects its 8193-byte one-over sentinel', () => {
  ...
  assert.ok(profile);
  const source = bytes(JSON.stringify({
    protocolVersion: 1,
  ...
  assert.throws(() => parseProfiledBody(profile, {
    contentType: 'application/json',
    contentLength: '8193',
    body: new Uint8Array(),
  }), ArtifactBrokerError);
...
test('retains the bounded D7 source body and maps its one-over sentinel to artifact total', () => {
  ...
  assert.ok(publish);
  const source = bytes('{"boardId":"board_1","expectedRevisionId":"revision_1","idempotencyKey":"0123456789abcdef","artifactId":null,"html":"","css":null,"javascript":null,"requestedCapabilities":[]}');
...
test('retains a strict restore adapter body for controller translation', () => {
  const restore = matchRawBodyProfile('POST', '/api/v1/boards/board_1/revisions/revision_1/restore');
  assert.ok(restore);
  const source = bytes('{"protocolVersion":1,"requestId":"request_01","idempotencyKey":"0123456789abcdef","expectedRevisionId":"revision_2","confirm":true}');
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof BoardContractError
      && error.boardError.code === 'INVALID_PAYLOAD',
  );
});
...
test('retains D1 bytes and maps D1 framing failures to exact board errors', () => {
  ...
  assert.ok(create);
  const source = bytes('{"protocolVersion":1,"requestId":"request_01","type":"board.create","title":"SceneBoard","idempotencyKey":"0123456789abcdef"}');
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof BoardContractError && error.boardError.code === 'PAYLOAD_TOO_LARGE',
  );
  assert.throws(
    () => parseProfiledBody(create, { contentType: 'text/plain', body: source }),
    (error: unknown) => error instanceof BoardContractError && error.boardError.code === 'INVALID_PAYLOAD',
  );
});
