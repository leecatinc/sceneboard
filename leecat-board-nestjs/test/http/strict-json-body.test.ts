...
test('materializes strict JSON objects with null prototypes', () => {
  const value = parseStrictJsonBytes(encode('{"email":"dev@sceneboard.dev","nested":{"ok":true}}')) as Record<string, unknown>;
...
test('rejects decoded duplicate names and D2 special keys at every depth', () => {
  ...
  for (const source of invalid) assert.throws(() => parseStrictJsonBytes(encode(source)), StrictJsonError, source);
});
