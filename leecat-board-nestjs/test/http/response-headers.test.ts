...
test('sets one safe request ID and auth no-store headers before handler execution', async () => {
  ...
  await new Promise<void>((resolve, reject) => {
    new ResponseHeadersInterceptor(crypto).intercept(context, next).subscribe({ complete: resolve, error: reject });
  });
...
test('proof-authenticated pairing responses vary on Authorization only', async () => {
  for (const url of ['/api/v1/pairings/pairing_1/client-status', '/api/v1/pairings/pairing_1/redeem']) {
    const headers = new Map<string, string>();
    const context = {
      switchToHttp: () => ({
        getRequest: () => ({ url }),
...
test('cookie-authenticated pairing and grant responses vary on Origin and Cookie', async () => {
  for (const url of ['/api/v1/pairings/active', '/api/v1/pairings/pairing_1', '/api/v1/grants?limit=25']) {
    const headers = new Map<string, string>();
    const context = {
      switchToHttp: () => ({
        getRequest: () => ({ url }),
