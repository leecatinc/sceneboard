...
const request = (method: string, url: string, source: string, headers: Record<string, string> = {}) => {
...
test('retains D1 raw bytes and parsed contract on private symbols', async () => {
  const source = '{"protocolVersion":1,"requestId":"request_01","type":"board.create","title":"SceneBoard","idempotencyKey":"0123456789abcdef"}';
