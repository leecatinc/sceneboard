...
test('accepts only the explicitly confirmed SceneBoard development schema', () => {
  ...
  assert.throws(() => assertLightweightDatabasePreparationAllowed({ ...valid, APP_ENV: 'production' }));
  assert.throws(() => assertLightweightDatabasePreparationAllowed({ ...valid, NODE_ENV: 'production' }));
  assert.throws(() => assertLightweightDatabasePreparationAllowed({ ...valid, MYSQL_DATABASE: 'other' }));
  assert.throws(() => assertLightweightDatabasePreparationAllowed({ ...valid, CONFIRM_LIGHTWEIGHT_DB_PREPARE: 'no' }));
});
