...
test('serializes D7 007-011 and D8 012 as six forward-only single-table migrations', async () => {
  ...
  assert.equal(terminal.every((entry) => !entry.reversible && entry.downAsset === null), true);
...
test('keeps D7 immutable aggregates and D8 interaction chronology fail closed', async () => {
  ...
  const runtime = await readFile(new URL('010_d7_artifact_runtime_states.up.sql', directory), 'utf8');
  assert.match(runtime, /status_code IN \('R','S','F','B'\)/u);
  const interactions = await readFile(new URL('012_d8_board_hitl_interactions.up.sql', directory), 'utf8');
