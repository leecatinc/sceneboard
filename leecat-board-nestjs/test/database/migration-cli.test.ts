...
import {
  ...
  runMigrationCli,
} from '../../src/database/migrations/cli.js';
...
test('parses only exact status, up, and ordered adoption grammar', () => {
  ...
  assert.deepEqual(parseMigrationCliArguments([
    'adopt', '--version', state.registryVersion, '--incident-ref', 'INC-2026-07-17',
  ]), {
    command: 'adopt', version: state.registryVersion, incidentRef: 'INC-2026-07-17',
  });
  for (const argv of [
    [], ['status', '--extra'], ['up', '--force'],
...
test('does not report migration success before the injected certification callback succeeds', async () => {
  const calls: string[] = [];
  const runner = {
    status: async () => { calls.push('runner.status'); return state; },
    up: async () => { calls.push('runner.up'); return state; },
    adopt: async () => { calls.push('runner.adopt'); return { ...state, mode: 'adopt' as const }; },
  };
  ...
  const invalid = await runMigrationCli(['status', '--force'], runner, async () => ({ status: 'succeeded' }));
