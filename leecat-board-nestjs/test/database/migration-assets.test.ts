...
test('interleaves the irreversible D3 board owner before D2 grant bindings', async () => {
  assert.deepEqual(MIGRATION_REGISTRY.slice(0, 9).map((entry) => entry.version), [
  ...
  const source = await readFile(new URL(
    '../../src/database/migrations/sql/001_d3_boards.up.sql',
    import.meta.url,
  ), 'utf8');
...
test('materializes the exact terminal fifteen-entry and eighteen-asset checkpoint', async () => {
  ...
  assert.match(revisions, /FOREIGN KEY \(board_pk, previous_revision_pk\)\s+REFERENCES board_revisions \(board_pk, revision_pk\)/);
  const references = await readFile(new URL('005_d3_board_revision_artifact_refs.up.sql', directory), 'utf8');
...
test('the live runner verifies every terminal D7 and D8 migration postcondition', async () => {
  const source = await readFile(new URL('../../src/database/migrations/runner.ts', import.meta.url), 'utf8');
...
test('binds D2 pairing and grant tables to the exact D3 public board key', async () => {
  const source = await readFile(new URL(
    '../../src/database/migrations/sql/002_d2_pairing_grants.up.sql',
    import.meta.url,
  ), 'utf8');
  assert.equal(splitSqlStatements(source).length, 5);
  for (const table of ['mcp_clients', 'mcp_grants', 'pairing_requests', 'mcp_grant_credentials', 'mcp_grant_boards']) {
    assert.match(source, new RegExp(`CREATE TABLE ${table} \\(`));
  }
  assert.match(source, /CONSTRAINT ck_pairing_state_fields CHECK/);
  assert.match(source, /CONSTRAINT fk_mcp_grant_boards_board_public_id FOREIGN KEY \(board_public_id\)\s+REFERENCES boards \(public_id\)/);
  assert.match(source, /CONSTRAINT ck_mcp_grants_masks CHECK \(scope_mask BETWEEN 1 AND 127 AND lifecycle_mask BETWEEN 0 AND 3\)/);
...
test('uses the exact compile-then-copy build contract', async () => {
  ...
  assert.equal(packageJson.scripts?.build, 'tsc -p tsconfig.build.json && tsx scripts/copy-sql-assets.ts');
  const copier = await readFile(new URL('../../scripts/copy-sql-assets.ts', import.meta.url), 'utf8');
