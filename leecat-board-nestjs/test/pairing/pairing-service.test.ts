...
test('pairing create returns the raw short code once while persisting only its two digests', async () => {
  ...
  const repository = {
    async create(input: Parameters<PairingRepository['create']>[0]) { captured = input; return { kind: 'created' as const }; },
...
test('pairing create maps the durable five-active-owner cap to exact rate limiting', async () => {
  const repository = {
    async create() { return { kind: 'quota' as const, retryAfterSeconds: 42 }; },
  } as unknown as PairingRepository;
  const service = new PairingService(repository, new PairingCodeService(crypto), crypto);
  await assert.rejects(
    () => service.create(session, 1_800_000_000_000),
    (error) => error instanceof AppError && error.code === 'RATE_LIMITED' && error.retryAfterSeconds === 42,
  );
});
...
test('pairing claim consumes the code into a secret-free pending response', async () => {
  ...
  const service = new PairingService(repository, new PairingCodeService(crypto), crypto, 50, 10, async () => {});
...
test('pairing claim pads every non-enumerating unavailable result', async () => {
  const delays: number[] = [];
  const repository = {
    async claim() { return { kind: 'unavailable' as const }; },
  } as unknown as PairingRepository;
  ...
  await assert.rejects(() => service.claim({
    ...
    clientProofChallenge: Buffer.alloc(32, 1),
  }, 1_800_000_000_000), (error) => error instanceof AppError && error.code === 'PAIRING_UNAVAILABLE');
...
test('redemption preserves pending retry and terminal distinctions without issuing a credential', async () => {
  const pendingRepository = {
    async redeem() { return { kind: 'not_ready' as const, retryAfterSeconds: 10 }; },
  } as unknown as PairingRepository;
  ...
  await assert.rejects(
    ...
    (error) => error instanceof AppError && error.code === 'PAIRING_NOT_READY' && error.retryAfterSeconds === 10,
  );

  const terminalRepository = {
    async redeem() { return { kind: 'terminal' as const }; },
  } as unknown as PairingRepository;
...
test('owner pairing reads, active list, and cancellation stay owner-scoped through session identity', async () => {
  ...
  const captured: Array<{ operation: string; ownerUserDatabaseId: string; sessionPublicId: string }> = [];
  const repository = {
  ...
  assert.deepEqual(captured.map(({ operation, ownerUserDatabaseId, sessionPublicId }) => ({
    operation,
    ownerUserDatabaseId,
    sessionPublicId,
  })), [
...
test('owner pairing routes preserve not-found and state-conflict classifications', async () => {
  const repository = {
    async ownerStatus() { return { kind: 'not_found' as const }; },
    async cancel() { return { kind: 'conflict' as const }; },
  } as unknown as PairingRepository;
  const service = new PairingService(repository, new PairingCodeService(crypto), crypto);
  await assert.rejects(
