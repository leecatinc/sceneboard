...
test('accepts only the exact PairingProof authorization credential and derives SHA-256 challenge bytes', () => {
  ...
  assert.equal(parsed.challenge.toString('hex'), '4bb06f8e4e3a7715d201d573d0aa423762e55dabd61a2c02278fa56cc6d294e0');
  assert.equal(parsed.rateLimitFingerprint.length, 22);
  for (const invalid of [undefined, `Bearer ${encoded}`, `PairingProof  ${encoded}`, `PairingProof ${encoded}=`, 'PairingProof AA']) {
    assert.throws(
