...
import { D2PostAuthRateLimitGuard, D2PreAuthRateLimitGuard } from '../../src/rate-limit/d2-rate-limit.guards.js';
...
const surfaceReflector = (surface: string) => ({ getAllAndOverride: () => surface }) as unknown as Reflector;
...
test('pairing create limiter consumes parsed IP before DB-resolved owner identity', async () => {
  const calls: string[] = [];
  const limiter = {
    async consume(input: { surface: string; identity: string }) { calls.push(`${input.surface}:${input.identity}`); },
  } as RateLimitService;
  const request = {
    body: {},
    headers: {},
...
test('proof routes consume IP first and a proof-fingerprinted pairing bucket second', async () => {
  const inputs: Array<{ surface: string; purpose: string; identity: string; limit: number; windowMs: number }> = [];
  const limiter = {
    async consume(input: { surface: string; purpose: string; identity: string; limit: number; windowMs: number }) { inputs.push(input); },
  } as RateLimitService;
  const proofReflector = { getAllAndOverride: () => 'pairing-client-status' } as unknown as Reflector;
  const request = {
    headers: {},
  ...
  await new D2PreAuthRateLimitGuard(proofReflector, limiter, environment).canActivate(context(request));
...
test('signup consumes the masked IP bucket before the normalized email bucket', async () => {
  ...
  await new D2PreAuthRateLimitGuard(surfaceReflector('signup'), limiter, environment).canActivate(context(request));
  assert.deepEqual(inputs, [
    {
      surface: 'signup',
      purpose: 'rate-limit-ip/v1',
...
test('password change consumes the IP bucket before the authenticated owner bucket', async () => {
  ...
  await new D2PreAuthRateLimitGuard(passwordReflector, limiter, environment).canActivate(context(request));
...
test('CSRF bootstrap uses the fixed Redis-unavailable retry contract', async () => {
  ...
  const limiter = {
    async consume(input: (typeof inputs)[number]) { inputs.push(input); },
  } as RateLimitService;
  const request = { headers: {}, socket: { remoteAddress: '192.0.2.19' } };
  await new D2PreAuthRateLimitGuard(surfaceReflector('csrf-bootstrap'), limiter, environment).canActivate(context(request));
  assert.deepEqual(inputs, [{
...
test('grant rotation consumes the IP bucket before the owner-and-grant bucket', async () => {
  ...
  await new D2PreAuthRateLimitGuard(grantReflector, limiter, environment).canActivate(context(request));
