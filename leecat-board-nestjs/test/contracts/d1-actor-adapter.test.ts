...
test('constructs user and MCP actors only from resolved principals with D1 scope order', async () => {
  ...
  const mcp = await service.resolveMcp('Bearer lcbg_v1.locator.secret', Date.parse('2026-07-16T12:00:00.000Z'));
...
test('rejects non-exact bearer transport before persistence and has one normalizer call site', async () => {
  ...
  const persistence: GrantPrincipalPersistence = {
    async resolve() { persistenceCalls += 1; return null; },
  };
  const tokens = { parseAndHash() { throw new Error('must not parse malformed transport'); } } as unknown as GrantTokenService;
  const service = new ActorContextService(persistence, tokens);
  for (const value of ['', 'bearer token', 'Bearer  token', 'Bearer token ', 'PairingProof token']) {
    await assert.rejects(() => service.resolveMcp(value, Date.now()));
  }
  assert.equal(persistenceCalls, 0);

  const source = await readFile(new URL('../../src/grants/actor-context.service.ts', import.meta.url), 'utf8');
...
test('owns the exact sixteen-operation authorization matrix', () => {
  const expected: readonly AuthorizedBoardOperationV1[] = [
    'board.list', 'board.get', 'capabilities.get', 'board.create', 'board.archive',
    'scene.replace', 'scene.clear', 'scene.restore', 'history.list', 'history.get',
    'hitl.request', 'hitl.respond', 'hitl.read', 'artifact.get', 'artifact.publish', 'artifact.stop',
  ];
  assert.deepEqual(AUTHORIZED_BOARD_OPERATIONS_V1, expected);
  assert.equal(new Set(AUTHORIZED_BOARD_OPERATIONS_V1).size, 16);
  assert.deepEqual(authorizationRuleFor('scene.restore').requiredCapabilities, ['board.write', 'board.history.read']);
  ...
  for (const operation of AUTHORIZED_BOARD_OPERATIONS_V1) {
    ...
    assert.equal(rule.isolation, ['board.list', 'board.get', 'capabilities.get', 'history.list', 'history.get', 'hitl.read', 'artifact.get'].includes(operation)
      ? 'REPEATABLE_READ_CUT'
      : 'READ_COMMITTED_WRITE');
    if (operation !== 'board.list' && operation !== 'board.create') assert.equal(rule.target, 'board');
  }
});
