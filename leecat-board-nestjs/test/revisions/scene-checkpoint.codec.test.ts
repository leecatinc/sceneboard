...
test('fails closed on corrupt payload, digest, length, or storage bounds', async () => {
  const codec = new SceneCheckpointCodec();
  const checkpoint = await codec.encode({ protocolVersion: 1, type: 'scene', root: null });
  await assert.rejects(() => codec.decode({ ...checkpoint, payload: Buffer.from('not-brotli') }), isCheckpointIntegrityError);
  await assert.rejects(() => codec.decode({ ...checkpoint, sha256: Buffer.alloc(32) }), isCheckpointIntegrityError);
  await assert.rejects(() => codec.decode({ ...checkpoint, canonicalBytes: checkpoint.canonicalBytes + 1 }), isCheckpointIntegrityError);
  await assert.rejects(() => codec.decode({ ...checkpoint, storedBytes: 800_001 }), isCheckpointIntegrityError);
});
