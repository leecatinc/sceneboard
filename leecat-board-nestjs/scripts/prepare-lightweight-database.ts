...
export const assertLightweightDatabasePreparationAllowed = (
  ...
  if (environment.APP_ENV !== 'development'
    ...
    || environment.MYSQL_DATABASE !== 'leecat_board'
    || environment.CONFIRM_LIGHTWEIGHT_DB_PREPARE !== CONFIRMATION) {
    throw new TypeError('lightweight database preparation is limited to the confirmed SceneBoard development schema');
  }
};
...
export const prepareLightweightDatabase = async (): Promise<void> => {
  ...
  try {
    await administrator.query(
      'CREATE DATABASE IF NOT EXISTS `leecat_board` CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci',
    );
  } finally {
