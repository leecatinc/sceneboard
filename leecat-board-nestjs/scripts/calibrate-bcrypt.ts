...
import {
  calibrateBcrypt,
  type BcryptCapacityBudget,
} from '../src/auth/bcrypt-calibration.js';
...
const parseBudget = (encoded: string | undefined): BcryptCapacityBudget | null => {
  ...
  const keys = [
    'maxConcurrency', 'minHashThroughputPerSecond', 'minCompareThroughputPerSecond',
    'maxEventLoopDelayP95Ms', 'maxRssDeltaBytes', 'maxBatchDurationMs',
  ] as const;
...
const deploymentClassConfirmed = process.env.CONFIRM_DEPLOYMENT_CLASS_CPU === 'I_CONFIRM_THIS_IS_THE_DEPLOYMENT_CPU';
if (cpuClass !== undefined && (!/^[-A-Za-z0-9_. ]{1,100}$/.test(cpuClass) || cpuClass.trim() !== cpuClass)) {
...
process.stdout.write(`${JSON.stringify({
  ...
  cpu: { class: cpuClass ?? null, model: cpus()[0]?.model ?? 'unknown', logicalCount: cpus().length },
